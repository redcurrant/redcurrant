...
static GError *
_delete_chunks_on_rawx(gboolean dryrun, GSList * chunks, guint32 * count,
	gint64 * del_size)
{
	GError *error = NULL;
	...
	for (; chunks; chunks = chunks->next) {
		...
		if (!rawx_delete_v2(chunks->data, &error)) {
			char *cid =
				CHUNKS_get_id((struct bean_CHUNKS_s *) chunks->data)->str;
			if (!error)
				error =
					NEWERROR(1, "Failed to create HTTP session RAWX [%s]", cid);
			else
...
gboolean
polix_action_drop_chunks(gboolean dryrun, GSList * del_chunks_list,
	polix_action_purge_result_t * result, GError ** error)
{
	...
	*error =
		_delete_chunks_on_rawx(dryrun, del_chunks_list, &nb_del, &del_size);
 ...
 */
gboolean
polix_action_purge(char *namespace, char *hexid, const char *meta2_url,
	...
	polix_action_purge_result_t * result, GError ** error)
{
	...
	if (!hexid || strlen(hexid) != 64) {
		if (error)
			*error =
				NEWERROR(1, "Invalid source path (%s/%s)", namespace, hexid);
		return FALSE;
	}
	...
	gboolean rc =
		polix_action_purge_byurl(url, meta2_url, timeout_request, dryrun,
		result, error);
...
gboolean
polix_action_purge_byurl(struct hc_url_s * url, const char *meta2_url,
	gdouble timeout_request, gboolean dryrun,
	polix_action_purge_result_t * result, GError ** error)
{
