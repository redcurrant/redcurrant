...
_prepare_tags(void)
{
	...
	service_tag_set_value_macro(service_info_ensure_tag(result,
			NAME_MACRO_CPU_NAME), NAME_MACRO_CPU_TYPE, NULL);
...
stats_copy_counters(GHashTable * src)
{
	...
	while (g_hash_table_iter_next(&iter, &k, &v))
		g_hash_table_insert(result, g_strdup((gchar *) k), g_memdup(v,
				sizeof(guint64)));
...
stats_count_event(const gchar * type)
{
	...
	if (counter)
		*counter = *counter + 1LLU;
		...
		u64 = 1LLU;
		g_hash_table_insert(ht_type_counters, g_strdup(type), g_memdup(&u64,
				sizeof(u64)));
	}
...
stats_update(gpointer d)
{
	...
	__srvstat_set_const_u64(MODULE_NAME ".counter.request.total",
		local_stats.counters.total);
	__srvstat_set_const_u64(MODULE_NAME ".counter.request.push",
		local_stats.counters.push);
	__srvstat_set_const_u64(MODULE_NAME ".counter.request.status",
		local_stats.counters.status);
	__srvstat_set_const_u64(MODULE_NAME ".counter.times.total",
		local_stats.times.total);
	__srvstat_set_const_u64(MODULE_NAME ".counter.times.push",
		local_stats.times.push);
	__srvstat_set_const_u64(MODULE_NAME ".counter.times.status",
		local_stats.times.status);

	/* Event types counters */
	g_hash_table_iter_init(&iter, ht_copy);
	while (g_hash_table_iter_next(&iter, &k, &v)) {
		g_snprintf(name, sizeof(name), MODULE_NAME ".counter.type:%s",
			(gchar *) k);
...
thread_func_event_manager(gpointer p, gpointer user_data)
{
	...
	if (!pe || !pe->event)
		GRID_ERROR("Event loading error UEID[%s] : %s", pe->ueid,
			gerror_get_message(err));
	else {
		if (err)
			g_clear_error(&err);

		if (polix_event_manager(polix, pe->ueid, pe->event, &allow_retry,
				FALSE /*dryrun */ , &err))
			status = ES_DONE;
		else if (err)
			GRID_ERROR("Failed to manage event UEID[%s: %s", pe->ueid,
				gerror_get_message(err));
	...
	if (!pes_set_status(pe, status)) {
		GRID_ERROR("Event set status failed UEID[%s]",
			pe->ueid /*, gerror_get_message(err) */ );
		delete_ueid(pe->ueid);
	}
	...
	gint value)
{
	...
	g_byte_array_append(gba_code, (const guint8 *) wrk_buf,
		strlen(wrk_buf) + 1);
	...
	switch (*status) {
		case ES_ERROR_DEF:
			...
			repctx_add_string_header(ctx, MSG_HEADER_EVENT_MESSAGE,
				"management failed");
			reply_context_set_message(ctx, 200,
				"event cannot be managed definitely");
			return TRUE;
		case ES_ERROR_TMP:
			repctx_add_int_header(ctx, MSG_HEADER_EVENT_STATUS,
				CODE_EVT_ERROR_TMP);
			repctx_add_string_header(ctx, MSG_HEADER_EVENT_MESSAGE,
				"management failed");
			reply_context_set_message(ctx, 200,
				"event cannot be managed temporarily");
			return TRUE;
		case ES_NOTFOUND:
			repctx_add_int_header(ctx, MSG_HEADER_EVENT_STATUS,
				CODE_EVT_NOTFOUND);
...
handler_push_event(struct request_context_s *req_ctx, GError ** err)
{
	...
	if (!(pe->event =
			extract_event_from_body(req_ctx->request, &(ctx.warning)))) {
	...
	if (!reply_set_status_code(&ctx, str_ueid, &status, &(ctx.warning))) {
		GSETERROR(&(ctx.warning), "Could not fetch the status of UEID=[%s]",
			str_ueid);
...
handler_status_event(struct request_context_s *req_ctx, GError ** err)
{
	...
	switch (status) {
		...
		case ES_ERROR_DEF:
			...
			break;
		default:
			GRID_DEBUG("Event still pending so not destroyed UEID=[%s]",
				str_ueid);
			break;
	}
...
module_find_handler(gchar * n, gsize l)
{
	static struct cmd_s CMD[] = {
		{REQ_EVT_PUSH, handler_push_event, &(module_stats.counters.push),
				&(module_stats.times.push)},
		{REQ_EVT_STATUS, handler_status_event, &(module_stats.counters.status),
				&(module_stats.times.status)},
		{NULL, NULL, NULL, NULL}
	};
...
static gboolean
plugin_configure(GHashTable * params, GError ** error)
{
	guint max_workers;
	gchar *ns = NULL;
...
plugin_init(GHashTable * params, GError ** err)
{
	...
	if (!message_handler_add_v2(MODULE_NAME, plugin_matcher, plugin_handler,
			service_tags, err)) {
		GSETCODE(err, POLIX_ERRCODE_OTHER, "Module init error");
		return 0;
	}

	srvtimer_register_regular(MODULE_NAME " stats", stats_update, NULL, NULL,
		1LLU);
