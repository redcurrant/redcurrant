...
struct event_manager_s
{
	const gchar *type_ref;
		...
		gboolean(*manage) (grid_polix_t * polix,
		const gchar * ueid,
		gridcluster_event_t * e,
		gboolean * retry, gboolean dryrun, GError ** error);
};
...
static GSList *
polix_event_get_unmarshall_list_field(gridcluster_event_t * event,
	const gchar * name)
{
	GByteArray *gba = NULL;
	GSList *list = NULL;
...
polix_event_content_delete(grid_polix_t * polix, const gchar * ueid,
	gridcluster_event_t * event, gboolean * allow_retry, gboolean dryrun,
	GError ** error)
{
	gboolean rc = FALSE;
	...
	if (!str_m2addr) {
		...
		if (!m2_url) {
			GSETERROR(error, "Content purge error ID[%s] CPATH[%s]", str_cid,
				str_cpath);
			goto label_exit;
		}
	}
	...
	if (!rc) {
		if (error && allow_retry
			&& CODE_CONTAINER_NOTFOUND == gerror_get_code(*error)) {
			GSList *list = NULL;
			...
			*error = NULL;
			...
			list =
				polix_event_get_unmarshall_list_field(event,
				META2_EVTFIELD_CHUNKS);
			if (list != NULL) {
				GError *err = NULL;
	...
	if (!rc) {
		...
		if (0 < result.nb_del) {
			GRID_DEBUG("%s%" G_GUINT32_FORMAT " chunks deleted ("
				"%" G_GINT64_FORMAT " bytes deleted) from %s/%s",
				(dryrun == TRUE) ? "(DRYRUN)" : "", result.nb_del,
				result.del_size, str_ns, str_cid);
		}
		else {
			GRID_DEBUG("No chunks deleted from %" G_GINT64_FORMAT
				" sized chunk's list of %s/%s", result.del_size, str_ns,
				str_cid);
		}
	}
...
void
polix_event_free(grid_polix_t * polix)
{
	if (polix)
		g_free(polix);
}
...
polix_event_manager(grid_polix_t * polix, const gchar * ueid,
	gridcluster_event_t * event, gboolean * flag_retry, gboolean flag_dryrun,
	GError ** err)
{
	...
	for (manager = MANAGERS; manager->type_ref; manager++) {
		...
		GRID_ERROR("Event management failed for UEID[%s] type[%s] : %s", ueid,
			type, err ? gerror_get_message(*err) : "unknown error");
