...
meta1_url_dup(struct meta1_service_url_s *u)
{
	...
	result =
		g_malloc0(sizeof(struct meta1_service_url_s) + 1 + strlen(u->args));
	result->seq = u->seq;
...
convert_url_to_serviceinfo(struct meta1_service_url_s *u,
	const gchar * excludeurl)
{
	GError *err = NULL;
	...
	for (lst = extracted; *lst; ++lst) {
		...
		if (!l4_address_init_with_url(&(srv->addr), (*lst)->host, &err)) {
			GRID_DEBUG("failed to build addr with url [%s], %s", (*lst)->host,
				err->message);
			g_clear_error(&err);
			continue;
		}
	...
	const container_id_t cid, const gchar * srvtype)
{
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"DELETE FROM properties WHERE cid = ? AND name LIKE ?", -1, &stmt,
		NULL);
	if (rc != SQLITE_OK && rc != SQLITE_DONE)
		...
		if (tmp_name) {
			...
			(void) sqlite3_bind_blob(stmt, 1, cid, sizeof(container_id_t),
				NULL);
	...
	const container_id_t cid, const gchar * srvtype)
{
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"DELETE FROM services WHERE cid = ? AND srvtype = ?", -1, &stmt, NULL);
	if (rc != SQLITE_OK)
		err = M1_SQLITE_GERROR(sq3->db, rc);
	else {
...
__del_container_one_service(struct sqlx_sqlite3_s *sq3,
	const container_id_t cid, const gchar * srvtype, gint64 seq)
{
	static const gchar *sql =
		"DELETE FROM services WHERE cid = ? AND srvtype = ? AND seq = ?";
	sqlite3_stmt *stmt = NULL;
	GError *err = NULL;
	int rc;
	...
	struct meta1_service_url_s *url)
{
	...
	if (rc != SQLITE_OK)
		...
		(void) sqlite3_bind_text(stmt, 4, url->srvtype, strlen(url->srvtype),
			NULL);
		do {
			rc = sqlite3_step(stmt);
		} while (rc == SQLITE_ROW);
	...
	struct meta1_service_url_s *url)
{
	...
	if (rc != SQLITE_OK)
		...
		(void) sqlite3_bind_blob(stmt, 1, cid, sizeof(container_id_t), NULL);
		(void) sqlite3_bind_text(stmt, 2, url->srvtype, strlen(url->srvtype),
			NULL);
		(void) sqlite3_bind_int64(stmt, 3, url->seq);
	...
	struct meta1_service_url_s ***result)
{
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"SELECT DISTINCT srvtype,url FROM services order by srvtype,url", -1,
		&stmt, NULL);
	if (rc != SQLITE_OK)
	...
	while (SQLITE_ROW == (rc = sqlite3_step(stmt))) {
		...
		u = g_malloc0(sizeof(struct meta1_service_url_s) + 1 +
...
static GError *
__get_container_all_services(struct sqlx_sqlite3_s *sq3,
	const container_id_t cid, const gchar * srvtype,
	struct meta1_service_url_s ***result)
{
	GError *err = NULL;
	...
	if (srvtype && *srvtype) {
		sqlite3_prepare_debug(rc, sq3->db,
			"SELECT seq,srvtype,url,args FROM services WHERE cid = ? AND srvtype = ?",
			-1, &stmt, NULL);
		if (rc != SQLITE_OK)
			return M1_SQLITE_GERROR(sq3->db, rc);
		(void) sqlite3_bind_blob(stmt, 1, cid, sizeof(container_id_t), NULL);
		...
		sqlite3_prepare_debug(rc, sq3->db,
			"SELECT seq,srvtype,url,args FROM services WHERE cid = ?", -1,
			&stmt, NULL);
		if (rc != SQLITE_OK)
			return M1_SQLITE_GERROR(sq3->db, rc);
		(void) sqlite3_bind_blob(stmt, 1, cid, sizeof(container_id_t), NULL);
	...
	while (SQLITE_ROW == (rc = sqlite3_step(stmt))) {
		...
		g_strlcpy(u->srvtype, (gchar *) sqlite3_column_text(stmt, 1),
			sizeof(u->srvtype));
		g_strlcpy(u->host, (gchar *) sqlite3_column_text(stmt, 2),
			sizeof(u->host) - 1);
		memcpy(u->args, (gchar *) sqlite3_column_text(stmt, 3),
			sqlite3_column_bytes(stmt, 3));
		g_ptr_array_add(gpa, u);
	}
	...
	struct meta1_service_url_s **used, GError ** err)
{
	...
	if (!(*err = _get_iterator(m1, ct, &iter))) {
		...
		if (!grid_lb_iterator_next_set2(iter, &siv, &opt)) {
			g_assert(siv == NULL);
			*err =
				NEWERROR(CODE_POLICY_NOT_SATISFIABLE, "No service available");
		}

		grid_lb_iterator_clean(iter);
		iter = NULL;
		g_slist_free_full(opt.srv_forbidden,
			(GDestroyNotify) service_info_clean);
	}
...
__get_container_service2(struct sqlx_sqlite3_s *sq3,
	...
	struct meta1_backend_s *m1, gchar *** result, enum m1v2_getsrv_e mode)
{
	GError *err = NULL;
	struct meta1_service_url_s **used = NULL;
	...
	err =
		__get_container_all_services(sq3, hc_url_get_id(url), ct->type, &used);
	if (NULL != err) {
		g_prefix_error(&err, "Preliminary lookup error : ");
		return err;
	}
			...
			if (!(mode & M1V2_GETSRV_DRYRUN)) {
				...
				if (NULL == err) {
					if (policy == SVCUPD_REPLACE)
						err =
							__delete_service(sq3, hc_url_get_id(url), ct->type);
					if (NULL == err)
	...
	struct meta1_backend_s *m1, gboolean dryrun, gchar *** result)
{
	enum m1v2_getsrv_e mode =
		M1V2_GETSRV_REUSE | (dryrun ? M1V2_GETSRV_DRYRUN : 0);
...
{
	enum m1v2_getsrv_e mode =
		M1V2_GETSRV_RENEW | (dryrun ? M1V2_GETSRV_DRYRUN : 0);
	return __get_container_service(sq3, url, srvtype, m1, result, mode);
}
	...
	gchar *** result)
{
	...
	err =
		_open_and_lock(m1, hc_url_get_id(url), M1V2_OPENBASE_MASTERONLY, &sq3);
	if (!err) {
...
static GError *
__get_services(struct sqlx_sqlite3_s *sq3, const gchar * excludeurl,
	const container_id_t cid, const gchar * srvtype,
	struct meta1_full_service_url_s ***result)
{
	GError *err = NULL;
	sqlite3_stmt *stmt = NULL;
	GPtrArray *gpa;
	...
	GString *sql =
		g_string_new
		("SELECT cid,seq,srvtype,url,args FROM services WHERE srvtype = ?");
	...
	sqlite3_prepare_debug(rc, sq3->db, sql->str, -1, &stmt, NULL);
	...
	if (cid) {
		(void) sqlite3_bind_blob(stmt, cidindex, cid, sizeof(container_id_t),
			NULL);
	}
	...
	while (SQLITE_ROW == (rc = sqlite3_step(stmt))) {
		...
		urls =
			g_malloc0(sizeof(struct meta1_service_url_s) + 1 +
			sqlite3_column_bytes(stmt, 3));
		memcpy(u->cid, (guint8 *) sqlite3_column_blob(stmt, 0),
			sqlite3_column_bytes(stmt, 0));
		urls->seq = sqlite3_column_int(stmt, 1);
		g_strlcpy(urls->srvtype, (gchar *) sqlite3_column_text(stmt, 2),
			sizeof(urls->srvtype));
		g_strlcpy(urls->host, (gchar *) sqlite3_column_text(stmt, 3),
			sizeof(urls->host) - 1);
		memcpy(urls->args, (gchar *) sqlite3_column_text(stmt, 4),
	...
	const gchar * tag_name, const gchar * tag_value, GError ** err)
{
	...
	if (list_srv) {
		hash_srv2 =
			g_hash_table_new_full(g_str_hash, g_str_equal, g_free, clean_hash);
		GSList *lst = NULL;
		...
		for (lst = list_srv; lst; lst = lst->next) {
			...
			if (tag_name && tag_name[0]) {
				...
				if (NULL != (tag = service_info_get_tag(si->tags, tag_name))) {
					switch (tag->type) {
						case STVT_BUF:
							serviceavailable =
								!fnmatch(tag_value, tag->value.buf, 0);
							break;
						case STVT_STR:
							serviceavailable =
								!fnmatch(tag_value, tag->value.s, 0);
							break;
						default:
			...
			g_hash_table_insert(hash_srv2, g_strdup(target),
				(serviceavailable ? si : NULL));
		}
...
_update_m1_policy_by_prefix(struct meta1_backend_s *m1,
	...
	GHashTable * hash_srv, gboolean checkonly, gchar ** result)
{
	GError *err = NULL;
	...
	for (; items && *items; items++) {
		...
		if (NULL == repctx) {
			...
			if (NULL != localerr) {
				GRID_ERROR("M1 update policy failure (%d): %s", localerr->code,
					localerr->message);
				failedRefCount++;
				continue;
			}
		...
		localerr =
			_update_m1_policy(m1, *items, ct, replicas, reqdist, tag_name,
		...
		if (update_m1_srv_urls) {
			...
			if (!checkonly) {
				err =
					__save_service(sq3, (*items)->cid,
					update_m1_srv_urls->urls);
			}
		...
		if ((checkedRefCount % 100 == 0) || (currentCount % 10 == 0)
			|| err != NULL) {
...
meta1_backend_update_m1_policy(struct meta1_backend_s * m1, const gchar * ns,
	const container_id_t prefix, const container_id_t cid,
	...
	gboolean checkonly, gchar ** result)
{
	...
	if (ct.req.k) {
		hash_srv =
			_update_m1_policy_set_services_hash(ns, srvtype, ct.req.k, ct.req.v,
			&err);
		if (err)
			goto failedend;
	}
	...
	if (cid) {
		err =
			_update_m1_policy_by_prefix(m1, FALSE, cid, &ct, replicas, reqdist,
			tag_name, excludesrv, excludeaddr, hash_srv, checkonly, result);
	}
	else if (prefix) {
		err =
			_update_m1_policy_by_prefix(m1, TRUE, prefix, &ct, replicas,
			reqdist, tag_name, excludesrv, excludeaddr, hash_srv, checkonly,
			result);
	}
	else {
