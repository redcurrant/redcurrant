...
_cache_load_from_m0(struct meta1_prefixes_set_s *m1ps,
	...
	struct addr_info_s *m0_addr, GArray ** updated_prefixes)
{
	GError *err = NULL;
	...
	if (m1ps->by_prefix) {
		...
		*updated_prefixes = g_array_new(FALSE, FALSE, sizeof(guint16));
		for (prefix = 0; prefix < 65536; prefix++) {
			if (_cache_is_managed(m1ps->cache,
					(guint8 *) & prefix) != _cache_is_managed(cache,
					(guint8 *) & prefix)) {
	...
	const gchar * ns_name, const gchar * local_url, GArray ** updated_prefixes)
{
	...
	if (!ns_name || !local_url) {
		GRID_TRACE
			("META1 prefix set not configured to be reloaded from a namespace");
		return NULL;
	}
