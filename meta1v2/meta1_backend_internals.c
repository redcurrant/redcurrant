	...
	enum m1v2_open_type_e how, struct sqlx_sqlite3_s ** handle)
{
	...
	if (how != M1V2_OPENBASE_LOCAL) {
		err =
			sqlx_repository_status_base(m1->repository, META1_TYPE_NAME, base);
		if (!err) {				/* MASTER */
			...
			if (err->code == CODE_REDIRECT) {	/* SLAVE */
				...
				GRID_TRACE("STATUS error [%s][%s]: (%d) %s",
					base, META1_TYPE_NAME, err->code, err->message);
				return err;
			}
		}
	}
	...
	gchar *** result)
{
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"SELECT vns,cname FROM containers WHERE cid = ?", -1, &stmt, NULL);
	if (rc != SQLITE_OK)
