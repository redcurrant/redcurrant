...
_reload_prefixes(struct sqlx_service_s *ss, gboolean init)
{
	...
	err =
		meta1_prefixes_load(m1ps, ss->ns_name, ss->url->str, &updated_prefixes);
	if (err) {
	...
	if (updated_prefixes && !init) {
		if (updated_prefixes->len)
			GRID_INFO("RELOAD prefix, nb updated prefixes %d",
				updated_prefixes->len);
		guint i, max;
		guint16 prefix;
		...
		for (i = 0; i < max; i++) {
			...
			g_snprintf(name, sizeof(name), "%02X%02X", ((guint8 *) & prefix)[0],
				((guint8 *) & prefix)[1]);
			if (meta1_prefixes_is_managed(m1ps, (guint8 *) & prefix)) {
				...
				err = election_exit(ss->election_manager, type, name);
				if (err) {
					GRID_WARN("SQLX error : (%d) %s", err->code, err->message);
					g_clear_error(&err);
				}
...
_task_reload_policies(gpointer p)
{
	...
	void _update_each(gpointer k, gpointer v, gpointer ignored)
	{
		...
		if (!err)
			err =
				service_update_reconfigure(meta1_backend_get_svcupdate(m1,
					(char *) k), (char *) v);
	}

	GHashTable *ht =
		gridcluster_get_service_update_policy(&(PSRV(p)->nsinfo), "meta1");
	if (!ht)
	...
	gchar *** result)
{
	...
	if (!n || !t || !result)
		return NEWERROR(500, "BUG [%s:%s:%d]", __FUNCTION__, __FILE__,
			__LINE__);
	...
	gchar **peers =
		meta1_prefixes_get_peers(meta1_backend_get_prefixes(m1), cid);
...
_post_config(struct sqlx_service_s *ss)
{
	...
	for (gboolean done = FALSE; !done;) {
		...
		if (NULL == err) {
			...
			GRID_WARN("PREFIXES reload failure : (%d) %s", err->code,
				err->message);
			g_clear_error(&err);
	...
	grid_task_queue_register(ss->gtq_reload, 11, _task_reload_lb, NULL, ss);
