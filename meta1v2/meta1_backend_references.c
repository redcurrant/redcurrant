	...
	const gchar * cname, const container_id_t cid)
{
	...
	if (err)
		GRID_DEBUG("Container creation failed : (%d) %s", err->code,
			err->message);
...
static GError *
__count_services(struct sqlx_sqlite3_s *sq3, const container_id_t cid,
	guint * count)
{
	gint rc;
	guint _count = 0;
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"SELECT COUNT(*) FROM services WHERE cid = ?", -1, &stmt, NULL);
	if (rc != SQLITE_OK)
	...
	m1b_ref_hook ref_hook, gpointer ref_hook_data)
{
	...
	sqlite3_prepare_debug(rc, sq3->db, "SELECT c.vns, c.cname "
		...
		"WHERE c.cid = s.cid "
		"AND s.srvtype = ? " "AND s.url LIKE ?", -1, &stmt, NULL);
	if (rc != SQLITE_OK) {
		err = M1_SQLITE_GERROR(sq3->db, rc);
		g_prefix_error(&err, "SQLITE error: ");
	}
