	...
	guint8 * body, gsize bodySize)
{
	...
	list =
		meta2_maintenance_sized_arrays_unmarshall_buffer(body, bodySize,
...
static gboolean
extract_raw_container(GError ** err, gpointer udata, gint code, guint8 * body,
	gsize bodySize)
{
	(void) code;
	...
	guint8 * body, gsize bodySize)
{
	...
	if (TRACE_ENABLED()) {
		TRACE("Arrays sequence unserialized, %d elements!",
			g_slist_length(list));
	}
...
static gint
__get_request_name1(meta1_operation_t mo, gchar ** str, gsize * len)
{
#define SETSTR(S) do { *str=S; *len=sizeof(S)-1; } while (0)
	switch (mo) {
		case M1_OP_CREATE:
			SETSTR(NAME_MSGNAME_M1_CREATE);
			return 1;
		case M1_OP_DESTROY:
			SETSTR(NAME_MSGNAME_M1_DESTROY);
			return 1;
		case M1_OP_GET:
			SETSTR(NAME_MSGNAME_M1_GET);
			return 1;
		case M1_OP_GETFLAGS:
			SETSTR(NAME_MSGNAME_M1_GETFLAGS);
			return 1;
		case M1_OP_SETFLAGS:
			SETSTR(NAME_MSGNAME_M1_SETFLAGS);
			return 1;

		case M1_OP_RANGE_GET:
			SETSTR(NAME_MSGNAME_M1_RANGE_GET);
			return 1;
		case M1_OP_RANGE_DEL:
			SETSTR(NAME_MSGNAME_M1_RANGE_DEL);
			return 1;
		case M1_OP_RANGE_ADD:
			SETSTR(NAME_MSGNAME_M1_RANGE_ADD);
			return 1;
		case M1_OP_RANGE_SET:
			SETSTR(NAME_MSGNAME_M1_RANGE_SET);
			return 1;
		case M1_OP_RANGE_LIST:
			SETSTR(NAME_MSGNAME_M1_RANGE_LIST);
			return 1;

		case M1_OP_GETALLONM2:
			SETSTR(NAME_MSGNAME_M1_GETALLONM2);
			return 1;
		case M1_OP_FORCECREATE:
			SETSTR(NAME_MSGNAME_M1_FORCECREATE);
			return 1;
		case M1_OP_GETCONTBYID:
			SETSTR(NAME_MSGNAME_M1_CONT_BY_ID);
			return 1;
		case M1_OP_GETCONTBYNAME:
			SETSTR(NAME_MSGNAME_M1_CONT_BY_NAME);
			return 1;
		case M1_OP_GETCONTMATCHING:
			SETSTR(NAME_MSGNAME_M1_GETMATCHES);
			return 1;
		case M1_OP_UPDCONT:
			SETSTR(NAME_MSGNAME_M1_UPDATE_CONTAINERS);
			return 1;
		case M1_OP_GETVNSSTATE:
			SETSTR(NAME_MSGNAME_M1_GET_VNS_STATE);
			return 1;
		...
		case M1V2_OP_CREATE:
			SETSTR("M1V2_CREATE");
			return 1;
		case M1V2_OP_DESTROY:
			SETSTR("M1V2_DESTROY");
			return 1;
		case M1V2_OP_HAS:
			SETSTR("M1V2_HAS");
			return 1;
		case M1V2_OP_SRVAVAIL:
			SETSTR("M1V2_SRVAVAIL");
			return 1;
		case M1V2_OP_SRVALL:
			SETSTR("M1V2_SRVALL");
			return 1;
		case M1V2_OP_SRVDEL:
			SETSTR("M1V2_SRVDEL");
			return 1;
		case M1V2_OP_SETPROP:
			SETSTR("M1V2_CID_PROPSET");
			return 1;
		case M1V2_OP_GETPROP:
			SETSTR("M1V2_CID_PROPGET");
			return 1;
		case M1V2_OP_DELPROP:
			SETSTR("M1V2_CID_PROPDEL");
			return 1;
	}
	return 0;
#undef SETSTR
...
static gint
meta1_range_request_common(MESSAGE m, meta1_operation_t mop, prefix_t * prefix,
	prefix_data_t * pData, GError ** err)
{
	void *body = NULL;
	gsize bodySize = 0;
	gint retCode;
	...
	if (prefix) {
		...
		if (pData) {
			...
			if (pData->addr) {
				retCode =
					addr_info_marshall(pData->addr, &body, &bodySize, err);
				if (!retCode) {
...
static gint
meta1_container_request_common_v2(MESSAGE m, meta1_operation_t mop,
	const container_id_t id, const gchar * name,
	const gchar * virtual_namespace, GError ** err)
{
	gsize nameSize;
	gsize virtualNsSize;
	gint retCode;
	...
	if (id != NULL || name != NULL) {
		...
		retCode =
			...
			sizeof(NAME_MSGKEY_CONTAINERID) - 1, usedID, sizeof(container_id_t),
			err);
	}
	...
	if (name && nameSize > 0) {
		retCode =
			message_add_field(m, NAME_MSGKEY_CONTAINERNAME,
			sizeof(NAME_MSGKEY_CONTAINERNAME) - 1, name, nameSize, err);
		if (!retCode) {
			GSETERROR(err, "Cannot set the container name in the request");
			goto errorLabel;
		}
	...
	if (virtual_namespace && virtualNsSize > 0) {
		retCode =
			...
			sizeof(NAME_MSGKEY_VIRTUALNAMESPACE) - 1, virtual_namespace,
			virtualNsSize, err);
		if (!retCode) {
...
static gint
meta1_container_request_common(MESSAGE m, meta1_operation_t mop,
	const container_id_t id, const gchar * name, GError ** err)
{
	...
	gdouble to_step, gdouble to_overall, gchar ** master)
{
	...
	if (!meta1_container_request_common_v2(request, M1_OP_CREATE, cID, cName,
			virtualNs, err)) {
...
gboolean
meta1_remote_create_container(addr_info_t * meta1, gint ms, GError ** err,
	const char *cName, container_id_t cID)
{
	return meta1_remote_create_container_v2(meta1, ms, err, cName, NULL, cID, 0,
		0, NULL);
}

gint
meta1_remote_destroy_container_by_name(addr_info_t * meta1, gint ms,
	...
	char **master)
{
	...
	if (!meta1_container_request_common(request, M1_OP_DESTROY, NULL, cName,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto end_label;
	}
...
GSList *
meta1_remote_get_meta2_by_container_name(addr_info_t * meta1, gint ms,
	GError ** err, const char *cName)
{
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the GET_META2 request");
		goto errorLabel;
	}
...
gboolean
meta1_remote_destroy_container_with_flags(addr_info_t * meta1, gint ms,
	...
	gdouble to_overall, char **master)
{
	...
	gboolean add_flag(const gchar * str_flag)
	{
		return (0 < message_add_field(request,
				str_flag, strlen(str_flag), "true", sizeof("true") - 1, err))
			? TRUE : FALSE;
	}
...
gint
meta1_remote_destroy_container_by_id(addr_info_t * meta1, gint ms,
	GError ** err, const container_id_t cID)
{
	return meta1_remote_destroy_container_with_flags(meta1, ms, err, cID,
		0x0000000, 0, 0, NULL);
}
...
GSList *
meta1_remote_get_meta2_by_container_id(addr_info_t * meta1, gint ms,
	...
	const container_id_t cID, guint32 flags)
{
	...
	if (!meta1_container_request_common(request, M1_OP_SETFLAGS, cID, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
...
meta1_remote_get_container_flag(addr_info_t * meta1, gint ms, GError ** err,
	const container_id_t cID, guint32 * flags)
{
	gboolean extract_flags(GError ** e1, gpointer udata, gint code,
		guint8 * body, gsize bodySize)
	{
		(void) udata;
		(void) code;
	...
	if (!meta1_container_request_common(request, M1_OP_GETFLAGS, cID, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the GET_CONTAINER request");
		goto errorLabel;
	}
	message_destroy(request, NULL);
	return TRUE;
	...
	prefix_t prefix, prefix_data_t * pData)
{
	...
	if (!meta1_range_request_common(request, M1_OP_RANGE_ADD, &prefix, pData,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the RANGE_ADD request");
		goto errorLabel;
	}
	...
	prefix_t prefix)
{
	...
	if (!meta1_range_request_common(request, M1_OP_RANGE_DEL, &prefix, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the RANGE_DEL request");
		goto errorLabel;
	}
	...
	prefix_t prefix, prefix_data_t * pData)
{
	...
	if (!meta1_range_request_common(request, M1_OP_RANGE_SET, &prefix, pData,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the RANGE_SET request");
		goto errorLabel;
	}
	...
	prefix_t prefix, prefix_data_t * pData)
{
	...
	gboolean content_handler_flag(GError ** e1, gpointer udata, gint code,
		guint8 * body, gsize bodySize)
	{
		(void) udata;
		(void) code;
		...
		if (bodySize != sizeof(pData->flags)) {
			GSETERROR(e1,
				"<%s> invalid content (bad flag size, %u instead of %u)", pProc,
				bodySize, sizeof(pData->flags));
			return FALSE;
		}
	...
	gboolean content_handler_addr(GError ** e1, gpointer udata, gint code,
		guint8 * body, gsize bodySize)
	{
		(void) udata;
		(void) code;
		...
		if (!addr_info_unmarshall(&(pData->addr), body, &bodySize, e1)) {
			GSETERROR(e1,
				"<%s> inavlid content (cannot unserialize the addr_info list)",
				pProc);
			return FALSE;
		}
		return FALSE;
	...
	if (!meta1_range_request_common(request, M1_OP_RANGE_GET, &prefix, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the RANGE_GET request");
		goto errorLabel;
	}
...
meta1_remote_range_list(addr_info_t * meta1, gint ms, GError ** err)
{
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the RANGE_LIST request");
		goto errorLabel;
	}
	...
	addr_info_t * m2_addr, GError ** err)
{
	...
	/*name the request */
	if (!message_set_NAME(request, NAME_MSGNAME_M1_GETALLONM2,
			sizeof(NAME_MSGNAME_M1_GETALLONM2) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_context(err, ctx, request, &data)) {
		GSETERROR(err,
			"An error occured while executing the " NAME_MSGNAME_M1_GETALLONM2
			" request");
		goto errorLabel;
	}
	...
	const container_id_t cID, const gchar * cName, GSList * addr_list)
{
	...
	if (!meta1_container_request_common(request, M1_OP_FORCECREATE, cID, cName,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
...
meta1_remote_get_container_by_id(struct metacnx_ctx_s *ctx,
	container_id_t container_id, GError ** err, gdouble to_step,
	gdouble to_overall)
{
	struct meta1_raw_container_s *raw_container = NULL;
	struct client_s *client = NULL;
	...
	if (!meta1_container_request_common(request, M1_OP_GETCONTBYID,
			container_id, NULL, err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto end_label;
	}
...
struct meta1_raw_container_s *
meta1_remote_get_container_by_name(struct metacnx_ctx_s *ctx,
	gchar * container_name, GError ** err)
{
	MESSAGE request = NULL;
	struct meta1_raw_container_s *raw_container = NULL;
	...
	if (!meta1_container_request_common(request, M1_OP_GETCONTBYNAME, NULL,
			container_name, err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_context(err, ctx, request, &data)) {
		GSETERROR(err,
			"An error occured while executing the GET_CONTAINER_BY_NAME request");
		goto errorLabel;
	}
...
gboolean
meta1_remote_get_container_names_matching(struct metacnx_ctx_s * ctx,
	GSList * list_of_patterns, GSList ** result, GError ** err)
{
	...
	if (!ctx || !list_of_patterns || !result) {
		GSETCODE(err, EINVAL, "Invalid parameter (%p %p %p)", ctx,
			list_of_patterns, result);
		return FALSE;
	}
	...
	if (!meta1_container_request_common(request, M1_OP_GETCONTMATCHING, NULL,
			NULL, err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
...
gboolean
meta1_remote_change_container_reference(struct metacnx_ctx_s * cnx,
	const container_id_t cid, const addr_info_t * old_m2,
	const addr_info_t * new_m2, GSList ** new_set, GError ** err)
{
	static struct code_handler_s codes[] = {
		{200, REPSEQ_FINAL, &addr_info_concat, NULL},
		{0, 0, NULL, NULL},
	...
	if (!(request =
			meta1_create_message(NAME_MSGNAME_M1_MIGRATE_CONTAINER, cid,
				err))) {
	...
	GSList * list_of_containers, gint ms, GError ** err)
{
	...
	if (!meta1_addr_str || !list_of_containers) {
		GSETCODE(err, EINVAL, "Invalid parameter (%p %p)", meta1_addr_str,
			list_of_containers);
		return FALSE;
	}
	...
	if (!meta1_container_request_common(request, M1_OP_UPDCONT, NULL, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
...
meta1_remote_get_virtual_ns_state(addr_info_t * meta1, gint ms, GError ** err)
{
	...
	if (!meta1_container_request_common(request, M1_OP_GETVNSSTATE, NULL, NULL,
			err)) {
		GSETERROR(err, "Cannot init the message as a simple metaX request");
		goto errorLabel;
	}
	...
	if (!metaXClient_reply_sequence_run_from_addrinfo(err, request, meta1, ms,
			&data)) {
		GSETERROR(err,
			"An error occured while executing the VIRTUAL_NS_STATE request");
		goto errorLabel;
	}
	...
	gdouble to_step, gdouble to_overall, gchar ** master)
{
	...
	MESSAGE req = message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_CREATE,
		...
		"CONTAINER_NAME", gba_poolify(&pool,
			metautils_gba_from_string(refname)),
		NULL);
...
meta1v2_remote_has_reference(const addr_info_t * meta1, GError ** err,
	const char *ns, const container_id_t refid, gdouble to_step,
	gdouble to_overall)
{
	EXTRA_ASSERT(meta1 != NULL);
	EXTRA_ASSERT(ns != NULL);
...
gboolean
meta1v2_remote_unlink_one_service(const addr_info_t * meta1,
	...
	char **master, gint64 seqid)
{
	...
	if (seqid <= 0) {
		if (err)
			*err =
				NEWERROR(400, "Invalid sequence number [%" G_GINT64_FORMAT "]",
				seqid);
		GBA_POOL_CLEAN(pool);
		return FALSE;
	}
	...
	gdouble to_overall)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_UPDATEM1POLICY,
		NULL,
		"NAMESPACE", gba_poolify(&pool, metautils_gba_from_string(ns)),
		"SRVTYPE", gba_poolify(&pool, metautils_gba_from_string(srvtype)),
	...
	if (prefix) {
		message_add_fields_gba(req, "PREFIX", gba_poolify(&pool,
				metautils_gba_from_cid(prefix)), NULL);
	}
	if (refid) {
		message_add_fields_gba(req, "CONTAINER_ID", gba_poolify(&pool,
				metautils_gba_from_cid(refid)), NULL);
	}
	if (checkonly)
		message_add_field(req, "CHECKONLY", strlen("CHECKONLY"),
			"true", sizeof("true") - 1, NULL);
	if (excludeurl)
		message_add_fields_gba(req, "EXCLUDEURL", gba_poolify(&pool,
				metautils_gba_from_string(excludeurl)), NULL);
	...
	const gchar * url, gdouble to_step, gdouble to_overall, char **master)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_SRVSETARG,
		gba_poolify(&pool, metautils_gba_from_string(url)),
		"NAMESPACE", gba_poolify(&pool, metautils_gba_from_string(ns)),
		"CONTAINER_ID", gba_poolify(&pool, metautils_gba_from_cid(refid)),
		NULL);
	...
	gdouble to_step, gdouble to_overall, char **master)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_CID_PROPSET,
	...
	gchar ** keys, gchar *** result, gdouble to_step, gdouble to_overall)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_CID_PROPGET,
	...
	gchar ** keys, gdouble to_step, gdouble to_overall, char **master)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_CID_PROPDEL,
		gba_poolify(&pool, metautils_encode_lines(keys)),
		"NAMESPACE", gba_poolify(&pool, metautils_gba_from_string(ns)),
		"CONTAINER_ID", gba_poolify(&pool, metautils_gba_from_cid(refid)),
		NULL);
	...
	gchar *** result)
{
	...
	GSList *pool = NULL;
	MESSAGE req =
		message_create_request(NULL, NULL, NAME_MSGNAME_M1V2_GETPREFIX,
		NULL,
		NULL);
