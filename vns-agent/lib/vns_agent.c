...
aggregate_vns_space_used(gpointer key, gpointer value, gpointer udata)
{
	...
	gint64 to_add = 0;
	GByteArray *tmp =
		g_hash_table_lookup(vns_agent_handle->vns_space_used, (gchar *) key);
	if (tmp && tmp->data)
	...
	g_snprintf(ns_space_str, sizeof(ns_space_str), "%" G_GINT64_FORMAT,
		ns_space);
	g_hash_table_replace(vns_agent_handle->vns_space_used, g_strdup(key),
		g_byte_array_append(g_byte_array_new(), (guint8 *) ns_space_str,
			strlen(ns_space_str) + 1));
	DEBUG("%" G_GINT64_FORMAT " total space used of VNS %s", ns_space,
		(gchar *) key);
}
...
collect_vns_space_used(gpointer data, gpointer udata)
{
	...
	response =
		meta1_remote_get_virtual_ns_state(&(((service_info_t *) data)->addr),
		4000, &error);
	if (!response) {
...
vns_agent_space_used_refresh(gpointer d)
{
	...
	/* get all ns_m1 */
	m1_list =
		list_namespace_services(vns_agent_handle->ns_info.name, "meta1",
		&error);
	if (!m1_list) {
	...
	gcluster_push_virtual_ns_space_used(&ns_addr, 4000,
		vns_agent_handle->vns_space_used, &error);
	/* unlock the table */
...
vns_agent_init(GHashTable * params, GError ** error)
{
	...
	if (!get_ns_info) {
		GSETCODE(error, VNS_AGENT_ERRCODE_CONFIG,
			"Failed to get namespace info function from hash table");
		return (0);
	}
