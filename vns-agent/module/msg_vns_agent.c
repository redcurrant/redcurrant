...
handler_info(struct request_context_s *req_ctx, GError ** err)
{
	...
	reply_context_set_body(&ctx, ns_name, strlen(ns_name),
		REPLYCTX_COPY | REPLYCTX_DESTROY_ON_CLEAN);
...
static struct cmd_s CMD[] = {
	{NAME_MSGNAME_VNSA_INFO, handler_info, &(vnsastats.info),
			&(vnsatimes.info)},
	{NULL, NULL, NULL, NULL}
};
...
plugin_init(GHashTable * params, GError ** err)
{
	...
	if (!get_ns_info) {
		GSETCODE(err, VNS_AGENT_ERRCODE_CONFIG, "no [%s] key in the config",
			KEY_NS_INFO_FUNC);
		return 0;
	}
	...
	if (!message_handler_add_v2("vns_agent", plugin_matcher, plugin_handler,
			service_tags, err)) {
	...
	if (!tmp_str)
		...
		if (vns_agent_space_used_refresh_rate <
			LIMIT_MIN_SPACE_USED_REFRESH_RATE
			...
			WARN("[space_used_refresh_rate] parameter out of range [%u,%u]",
				...
				LIMIT_MAX_SPACE_USED_REFRESH_RATE);
			vns_agent_space_used_refresh_rate =
				CLAMP(vns_agent_space_used_refresh_rate,
				LIMIT_MIN_SPACE_USED_REFRESH_RATE,
				LIMIT_MAX_SPACE_USED_REFRESH_RATE);
		}
	}

	NOTICE("space_used refresh rate set to [%lu] s",
		vns_agent_space_used_refresh_rate);
