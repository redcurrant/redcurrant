...
static GError *
_resolve_m1_through_one_m0(const gchar * m0, const guint8 * prefix,
	gchar *** result)
{
	GError *err = NULL;
...
static GError *
_resolve_m1_through_many_m0(gchar ** urlv, const guint8 * prefix,
	gchar *** result)
{
	GError *err;
	guint i, last;
...
_resolve_meta1(struct hc_resolver_s *r, struct hc_url_s *u, gchar *** result)
{
	...
	hk = hashstr_printf("meta1|%s|%.4s",
		hc_url_get(u, HCURL_NSPHYS), hc_url_get(u, HCURL_HEXID));
	...
	const gchar * s, gchar *** result)
{
	...
	GRID_TRACE2("%s(%s,%s,%s)", __FUNCTION__, m1, hc_url_get(u, HCURL_WHOLE),
		s);
	...
	const gchar * srvtype, gchar *** result)
{
	...
	GRID_TRACE2("%s(%s,%s)", __FUNCTION__, hc_url_get(url, HCURL_WHOLE),
		srvtype);
	g_assert(r != NULL);
	g_assert(url != NULL);
	...
	if (!hc_url_get_id(url) || !hc_url_has(url, HCURL_NS))
		return NEWERROR(400, "Incomplete URL [%s]", hc_url_get(url,
				HCURL_WHOLE));

	hk = hashstr_printf("%s|%s|%s", srvtype,
		hc_url_get(url, HCURL_NSPHYS), hc_url_get(url, HCURL_HEXID));
...
static inline void
_LRU_set_max(struct lru_ext_s *l, guint v)
{
	if (l)
		l->max = v;
}
...
static inline void
_LRU_set_ttl(struct lru_ext_s *l, time_t v)
{
	if (l)
		l->ttl = v;
}
