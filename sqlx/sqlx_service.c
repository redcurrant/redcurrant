...
_configure_backend(struct sqlx_service_s *ss)
{
	...
	err = sqlx_repository_configure_type(ss->repository,
		ss->service_config->srvtype, NULL, ss->service_config->schema);

	if (err) {
		GRID_ERROR("SQLX schema init failure : (%d) %s", err->code,
			err->message);
		g_clear_error(&err);
		return FALSE;
	}
...
_configure_registration(struct sqlx_service_s *ss)
{
	...
	metautils_strlcpy_physical_ns(si->ns_name, ss->ns_name,
		sizeof(si->ns_name));
	g_strlcpy(si->type, ss->service_config->srvtype, sizeof(si->type) - 1);
	grid_string_to_addrinfo(ss->url->str, NULL, &(si->addr));

	service_tag_set_value_string(service_info_ensure_tag(si->tags, "tag.type"),
		ss->service_config->srvtag);

	_add_custom_tags(si, ss->custom_tags);

	service_tag_set_value_string(service_info_ensure_tag(si->tags, "tag.vol"),
		ss->volume);

	service_tag_set_value_float(service_info_ensure_tag(si->tags,
			"stat.req_idle"), 100.0);
	service_tag_set_value_macro(service_info_ensure_tag(si->tags, "stat.cpu"),
		"cpu", NULL);
	service_tag_set_value_macro(service_info_ensure_tag(si->tags, "stat.space"),
		"space", ss->volume);
	service_tag_set_value_macro(service_info_ensure_tag(si->tags, "stat.io"),
		"io", ss->volume);
	return TRUE;
}
...
sqlx_service_get_options(void)
{
	static struct grid_main_option_s sqlx_options[] = {
		...
		{"MaxBases", OT_INT64, {.u = &SRV.cfg_max_bases},
			"Limits the number of concurrent open bases"},
		{"MaxPassive", OT_INT64, {.u = &SRV.cfg_max_passive},
			"Limits the number of concurrent passive connections"},
		{"MaxActive", OT_INT64, {.u = &SRV.cfg_max_active},
			"Limits the number of concurrent passive connections"},
...
_task_register(gpointer p)
{
	...
	grid_single_rrd_feed(network_server_get_stats(PSRV(p)->server),
		...
		INNER_STAT_NAME_REQ_TIME, PSRV(p)->gsr_reqtime, NULL);
	...
	if (!register_namespace_service(PSRV(p)->si, &err))
		g_message("Service registration failed: (%d) %s", err->code,
			err->message);
	if (err)
		g_clear_error(&err);
}
...
_task_retry_elections(gpointer p)
{
	...
	guint count =
		election_manager_retry_elections(PSRV(p)->election_manager, 100, &end);
	if (count)
