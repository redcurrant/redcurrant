...
{
	guint newval = 0, max = 0, available = 0, min = 0;
...
#define CONFIGURE_LIMIT(cfg,real) do { \
	...
	available = (limit.rlim_cur - 20);
	max = available * 49 / 100;
	min = available / 100;

	// max_bases cannot be changed at runtime, so we set it first and
	// clamp the other limits accordingly.
	CONFIGURE_LIMIT((max_bases > 0? max_bases : 30*min), ss->max_bases);
	CONFIGURE_LIMIT((max_passive > 0? max_passive : 40*min), ss->max_passive);
	CONFIGURE_LIMIT((max_active > 0? max_active : 30*min), ss->max_active);
