...
static service_info_t *get_rawx_srvinfo_from_addr(addr_info_t * addr);
...
struct grid_lb_s *load_balancer = NULL;
struct grid_lb_iterator_s *rawx_iter = NULL;
...
get_volume_usage(gdouble * result)
{
	...
	if (!last_update || now > (last_update + interval_update_statfs)) {
		...
		last_update = now;
		GRID_DEBUG
			("Volume usage computed: %.3f at time %ld (avail=%ld total=%ld)",
...
may_continue(int depth, struct mover_stats_s *stats)
{
	...
	if (max_chunks > 0LL) {
		...
		if (total_chunk >= max_chunks) {
			GRID_INFO("Limit matched: Chunks: total(%" G_GINT64_FORMAT
				") >= max(%" G_GINT64_FORMAT ")", total_chunk, max_chunks);
			return 0;
		}
	}
	return 1;
...
load_volumes(void)
{
	...
	GRID_DEBUG("Reloading the volumes list");

	new_services = list_namespace_services(ns_name, "rawx", &err);
	if (!new_services && err) {
		if (err) {
			GRID_ERROR("reload error: %s", gerror_get_message(err));
			g_error_free(err);
		}
		return FALSE;
	}
	...
	volumes_pointer = volumes = g_slist_sort(new_services,
		service_info_sort_by_score);
	volumes_last_update = time(NULL);
	GRID_DEBUG("Received %u volumes", g_slist_length(volumes));
	return TRUE;
}

/**
 * Iterator over the internal list of rawx volumes.
 *
 * @param dest A pointer to a service_info_t*, NULL to (re)initialize
 *   the iterator
 * @return TRUE when a result has been copied into *dest, FALSE otherwise
 */
static gboolean
_iterate_volumes(service_info_t ** dest)
{
	static GSList *cursor = NULL;

	if (cursor == NULL || dest == NULL) {
		cursor = volumes;
		return FALSE;
	}
	*dest = service_info_dup(cursor->data);
	cursor = cursor->next;
	return TRUE;
}
	...
	const gchar * meta2)
{
	...
	err =
		find_storage_policy_and_friend_chunks(meta2, url, check_info,
		&friend_chunk_ids);
	if (err) {
		GRID_ERROR(err->message);
		g_clear_error(&err);
		return NULL;
	}
	...
	if (policy != NULL) {
		...
		grid_lb_reload(load_balancer, _iterate_volumes);
		for (GSList * cursor = friend_chunk_ids; cursor; cursor = cursor->next) {
			GError *gerr = NULL;
			addr_info_t rawx_addr2;
			...
			GRID_TRACE("Current rawx address = %s", tokens[1]);
			l4_address_init_with_url(&rawx_addr2, tokens[1], &gerr);
			if (gerr != NULL) {
				GRID_WARN("%s", gerr->message);
				g_clear_error(&gerr);
				continue;
			}
			srvinfo = get_rawx_srvinfo_from_addr(&rawx_addr2);
			if (srvinfo == NULL) {
	...
	const gchar *reqdist_str =
		data_security_get_param(datasec, DS_KEY_DISTANCE);
	...
	if (!grid_lb_iterator_next_set2(rawx_iter, &new_rawx, &opt_ext)) {
	...
	/* g_slist_free_full(friend_chunk_srvinfo, g_free); */
...
static service_info_t *
get_rawx_srvinfo_from_addr(addr_info_t * addr)
{
	GSList *l;
	char addr_str[64];
	char si_addr_str[64];
	...
	addr_info_to_string(addr, addr_str, sizeof(addr_str));
	for (l = volumes; l; l = l->next) {
		struct service_info_s *si = l->data;

		memset(si_addr_str, '\0', sizeof(addr_str));
		addr_info_to_string(&(si->addr), si_addr_str, sizeof(si_addr_str));
		GRID_TRACE("current addr %s, service to check addr %s",
			addr_str, si_addr_str);
		if (0 == g_ascii_strcasecmp(addr_str, si_addr_str)) {
			GRID_TRACE("Match ok");
			return si;
		}
	}
	...
	struct meta2_raw_content_s *raw_old, struct meta2_raw_content_s *raw_new)
{
	...
	MY_DEBUG(*info, "About to validate the reference in meta2 at [%s]",
		m2_descr);
...
validate_chunk_in_all_meta2(struct upload_info_s *info)
{
	...
	for (url = info->location->m2_url; url && *url; url++) {
		if (!validate_chunk_in_one_meta2(info, *url, info->raw_old,
				info->raw_new))
			rc = FALSE;
	}
...
download_and_check_chunk(struct upload_info_s *info)
{
	...
	switch (ne_request_dispatch(request)) {
		case NE_OK:
			...
			break;
		case NE_AUTH:
			MY_WARN(*info, "Unexpected authentication: %s",
		...
		case NE_CONNECT:
			...
			break;
		case NE_TIMEOUT:
			MY_WARN(*info, "Connection/Read timeout: %s",
				ne_get_error(session));
			break;
		case NE_ERROR:
			MY_WARN(*info, "Connection error: %s", ne_get_error(session));
...
upload_chunk(struct upload_info_s *info)
{
	...
	ne_set_request_body_buffer(request, ((char *) info->chunk_buffer->data),
		bufsize);
...
load_chunk(const gchar * path, struct upload_info_s *info)
{
	...
	if (!get_rawx_info_in_attr(path, &err, &(info->content), &(info->chunk))) {
		...
		if (NULL != (str_err = check_attributes(&(info->chunk),
			...
			FINAL_ERROR(*info, "attributes conversion error (%s)"
				" src[%s] dst[%s]", str_err, info->src_descr, info->dst_descr);
			return 0;
		}
	}
	gserr = NULL;
	info->location = gs_locate_container_by_hexid(gs_client,
		info->content.container_id, &gserr);
	...
	g_strlcpy(check_info.ns_name, ns_name, sizeof(check_info.ns_name));
	g_strlcpy(check_info.rawx_str_addr, rawx_str_addr,
		sizeof(check_info.rawx_str_addr));
	...
	do {
		...
		if (!get_compression_info_in_attr(path, &err, &compress_opt)) {
			if (err) {
				GRID_WARN
					("Failed to load the compression information for [%s]: %s",
					path, err->message);
				g_clear_error(&err);
			}
		}
			...
			if (tmp && g_ascii_strcasecmp(tmp, NS_COMPRESSION_ON) == 0) {
				...
				tmp =
					g_hash_table_lookup(compress_opt, NS_COMPRESS_ALGO_OPTION);
				if (tmp) {
					g_free(info->algo);
				...
				tmp =
					g_hash_table_lookup(compress_opt,
					NS_COMPRESS_BLOCKSIZE_OPTION);
				if (tmp) {
	...
	if (download_old_chunk(path, info) == 0) {
		FINAL_ERROR(*info, "Failed to download old chunk [%s]", path);
		return 0;
	}
	...
	info->raw_old =
		load_raw_chunk("OLD", info, &check_info, &rawx_addr, rawx_vol);
	info->raw_new =
		load_raw_chunk("NEW", info, &check_info, &(info->service->addr),
		info->dst_volume);
	...
	do {
		...
		if (0 == g_ascii_strcasecmp(info->comp, "false")) {
#endif
			chunk_size =
				((struct meta2_raw_chunk_s *) info->raw_new->raw_chunks->data)->
				size;
			if (info->fd_stat.st_size != chunk_size) {
				FINAL_ERROR(*info, "Local/Meta2 sizes mismatch"
					" (local=%" G_GINT64_FORMAT " xattr=%" G_GINT64_FORMAT
					") src[%s] dst[%s]", info->fd_stat.st_size, chunk_size,
					info->src_descr, info->dst_descr);
			...
			if (!get_chunk_compressed_size_in_attr(path, &err, &attr_size)
				|| (size32 != attr_size)) {
				FINAL_ERROR(*info,
					"Local/Meta2 sizes mismatch" " (local=%" G_GINT64_FORMAT
					" xattr=%" G_GINT64_FORMAT ") src[%s] dst[%s]",
					info->fd_stat.st_size, chunk_size, info->src_descr,
					info->dst_descr);
				printf("Local/Meta2 sizes mismatch => compressed -_-\n");
				return 0;
			}
...
label_exit:
	if (rc)
		FINAL_INFO(info, "migration successful src[%s] dst[%s]",
			info.src_descr, info.dst_descr);
	free_chunk(&info);
	return rc;
}
...
run_directory(const gchar * basedir, int depth, struct mover_stats_s *stats)
{
	...
	while (NULL != (path = g_dir_read_name(gdir))) {
		...
		if ((path[0] == '.' && (path_len == 1 || (path_len == 2
						&& path[1] == '.')))
		...
		if (fullpath_len >= sizeof(fullpath)) {	/* Path too long */
			...
			if (!run_directory(fullpath, depth + 1, stats)
				&& flag_exit_on_error)
				goto label_error;
...
{
	GRID_NOTICE("Stopping rawx-mover!");
	flag_running = FALSE;
}
...
rawx_mover_init(void)
{
	...
	if (!*ns_name && (!forced_namespace || !forced_namespace->len)) {
		GRID_ERROR
			("No namespace found for [%s] (not in xattr, none forced with '-O Namespace=')",
			rawx_vol);
		return FALSE;
	}
	if (forced_namespace && forced_namespace->len) {
		if (*ns_name && 0 != strcmp(forced_namespace->str, ns_name)) {
			GRID_ERROR
				("Forced namespace [%s] does not match namespace in xattr [%s]",
	...
	if (!load_volumes()) {
		GRID_ERROR("No RAWX available in NS=[%s]", ns_name);
		return FALSE;
	}
	...
	if (!(str = rawx_get_real_volume_name(&rawx_addr))) {
		...
		if (0 != g_ascii_strcasecmp(str, rawx_vol)) {
			GRID_ERROR
				("RAWX volume mismatch for url [%s] in namespace [%s]: given(%s) != found(%s)",
	...
	/* Initialize a load balancer on rawx volumes */
	load_balancer = grid_lb_init(ns_name, "rawx");
	_iterate_volumes(NULL);		// initialize iterator
	grid_lb_reload(load_balancer, _iterate_volumes);	// load volumes into load balancer
	rawx_iter = grid_lb_iterator_init(load_balancer, NULL);	// initialize smart iterator with default lb policy
...
rawx_mover_fini(void)
{
	...
	if (rawx_iter) {
		grid_lb_iterator_clean(rawx_iter);
		rawx_iter = NULL;
	}
	if (load_balancer) {
		grid_lb_clean(load_balancer);
		load_balancer = NULL;
	}
