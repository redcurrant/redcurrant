	...
	for (;;) {					/* main loop */
		...
		if (flag_restart_children) {
			if (auto_restart_children) {
				...
				DEBUG
					("One of my children died, I will die too (auto_restart_children=%d)",
					auto_restart_children);
				break;
			}
		}
		...
		if (g_timer_elapsed(timer, NULL) >= 1.0) {
			...
			if (!register_namespace_service(si, &error)) {
				ERROR("Failed to register the service : %s",
...
main(int argc, char **argv)
{
	...
	if (argc <= 1 || strlen(svc_id) == 0 || strlen(svc_cmd) == 0) {
		g_printerr
			("Usage: %s --svc-id <NS|type|ip:port> --svc-cmd /service/cmd/to/launch\n"
	...
	if (0 != supervisor_children_set_limit(CHILD_KEY, SUPERV_LIMIT_THREAD_STACK,
			8192 * 1024))
		WARN("Limit on thread stack size cannot be set : %s", strerror(errno));
	if (0 != supervisor_children_set_limit(CHILD_KEY, SUPERV_LIMIT_MAX_FILES,
			32 * 1024))
		WARN("Limit on max opened files cannot be set : %s", strerror(errno));
	if (0 != supervisor_children_set_limit(CHILD_KEY, SUPERV_LIMIT_CORE_SIZE,
			-1))
