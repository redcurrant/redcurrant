...
_get_cs_addr(const char *ns_name, GError ** error)
{
	...
	if (!cs_addr) {
		GSETERROR(error, "No conscience addr found for namespace [%s]",
			ns_name);
	}
	else {
...
_get_namespace_info_from_agent(const char *ns_name, GError ** error)
{
	...
	g_snprintf(master_ns, 512, "%.*s:%s",
		virt_ns != NULL ? (gint) (virt_ns - ns_name) : (gint) strlen(ns_name),
		ns_name, SHORT_API_VERSION);
	...
	if (resp.status == STATUS_OK) {
		namespace_info_t *ns =
			namespace_info_unmarshall(resp.data, resp.data_size, error);
		clear_request_and_reply(&req, &resp);
		if (ns == NULL) {
...
static meta0_info_t *
get_meta0_info_from_conscience(const char *ns_name, long timeout_cnx,
	long timeout_req, GError ** error)
{
	...
	meta0 =
		gcluster_get_meta0_2timeouts(cs_addr, timeout_cnx, timeout_req, error);
	if (!meta0)
		GSETERROR(error,
			"Failed to retrieve meta0 infos from conscience of namespace [%s] timeout(%ld,%ld)",
...
meta0_info_t *
get_meta0_info2(const char *ns_name, long timeout_cnx, long timeout_req,
	GError ** error)
{
	if (!ns_name) {
		GSETERROR(error, "Invalid parameter");
	...
	if (!gridagent_available())
		return get_meta0_info_from_conscience(ns_name, timeout_cnx, timeout_req,
			error);
	else
...
_list_namespace_service_types_from_agent(const char *ns_name, GError ** error)
{
	...
	names =
		meta2_maintenance_names_unmarshall_buffer(resp.data, resp.data_size,
		error);
	if (!names)
		GSETERROR(error,
			"Invalid reply from agent : bad names payload (deserialization error)");
	clear_request_and_reply(&req, &resp);
	return names;
}

static GSList *
_list_namespace_service_types_from_conscience(const char *ns_name,
	GError ** error)
{
	addr_info_t *cs_addr;
...
list_namespace_services(const char *ns_name, const char *type, GError ** error)
{
	...
	if (resp.status == STATUS_OK) {
		...
		if (0 > service_info_unmarshall(&services, resp.data, &body_size,
				error)) {
...
get_one_namespace_service(const gchar * ns_name, const gchar * type,
	GError ** error)
{
	request_t req;
	response_t resp;
	...
	if (resp.status == STATUS_OK) {
		...
		GSETERROR(error, "No service found for type=[%s] in namespace [%s]",
			type, ns_name);
		return NULL;
	}
	...
	const gchar * cause)
{
	...
	DEBUG("Message sent to agent: (length=%" G_GSIZE_FORMAT "/%" G_GSIZE_FORMAT
		") [%s]", writen, strlen(wrkParam), wrkParam);
...
int
fixed_erroneous_content(const char *ns_name, const container_id_t cID,
	GError ** error, const gchar * path)
{
	request_t req;
	response_t resp;
	gsize writen;
	char wrkParam[LIMIT_LENGTH_NSNAME + 1 + STRLEN_CONTAINERID + 1 +
		LIMIT_LENGTH_CONTENTPATH + 1];
...
int
store_erroneous_meta1(const char *ns_name, const addr_info_t * m1_addr,
	GError ** error)
{
	gsize writen;
	request_t req;
...
int
store_erroneous_container(const char *ns_name, const container_id_t cID,
...
int
fixed_erroneous_meta1(const char *ns_name, GError ** error,
	addr_info_t * m1_addr)
{
	gsize writen;
	request_t req;
...
fetch_erroneous_containers(const char *ns_name, GError ** error)
{
	...
	if (resp.status == STATUS_OK) {
		...
		result = NULL;
		while (ptr
			&& ptr - ((gchar *) resp.data) < (signed_size = resp.data_size)) {
			if (*ptr) {
				int len = strlen(ptr);
...
list_tasks(GError ** error)
{
	...
	if (resp.status == STATUS_OK) {
		...
		while (size_read < resp.data_size) {
			...
			memcpy(&(task.next_schedule), resp.data + size_read,
				sizeof(task.next_schedule));
			...
			task_list =
				g_slist_prepend(task_list, g_memdup(&task,
...
event_get_configuration(const gchar * ns_name, GError ** error)
{
	...
	if (resp.status != STATUS_OK) {
		...
		result =
			g_byte_array_append(g_byte_array_new(), resp.data, resp.data_size);
		clear_request_and_reply(&req, &resp);
...
GByteArray *
get_srvtype_configuration(const gchar * ns_name, const gchar * type_name,
	GError ** error)
{
	request_t req;
	response_t resp;
...
gboolean
event_send_to_agent(const gchar * ns_name, gridcluster_event_t * event,
	GError ** error)
{
	request_t req;
	response_t resp;
...
namespace_in_worm_mode(namespace_info_t * ns_info)
{
	if (!ns_info || !ns_info->options)
		return FALSE;
	return _gba_to_bool(g_hash_table_lookup(ns_info->options, NS_WORM_OPT_NAME),
		FALSE);
}
...
namespace_container_max_size(namespace_info_t * ns_info)
{
	...
	return _gba_to_int64(g_hash_table_lookup(ns_info->options,
			NS_CONTAINER_MAX_SIZE_NAME), -1);
}

gchar *
namespace_storage_policy(const namespace_info_t * ns_info, const char *ns_name)
{
	if (!ns_info || !ns_info->options || !ns_name)
		return NULL;

	GByteArray *gba = NULL;

	/* First check if a policy was specified for the namespace */
	char *key = g_strdup_printf("%s_%s", ns_name, NS_STORAGE_POLICY_NAME);
	...
	if (!gba)
		gba = g_hash_table_lookup(ns_info->options, NS_STORAGE_POLICY_NAME);

	return !gba ? NULL : g_strndup((gchar *) gba->data, gba->len);
}

gchar *
namespace_storage_policy_value(const namespace_info_t * ns_info,
	const gchar * wanted_policy)
{
	...
	GByteArray *gba =
		g_hash_table_lookup(ns_info->storage_policy, policy_to_lookup);
	return !gba ? NULL : g_strndup((gchar *) gba->data, gba->len);
}
...
_get_token(const gchar * colon_separated_tokens, const guint token_rank)
{
	...
	if (g_strv_length(tokens) < token_rank) {
		ERROR("Cannot split string [%s] into %i ':'-separated tokens.",
			colon_separated_tokens, token_rank);
		goto end;
	}
...
gchar *
namespace_data_security_value(const namespace_info_t * ns_info,
	const gchar * wanted_policy)
{
	gchar *storage_policy_value =
		namespace_storage_policy_value(ns_info, wanted_policy);
	...
	if (!data_sec_val) {
		WARN("Cannot find data security with id [%s] (namespace [%s], wanted policy [%s])", data_sec_id, ns_info->name, wanted_policy);
	}
	...
	metautils_gba_data_to_string(data_sec_val, str_data_sec_val,
		LIMIT_LENGTH_STGPOLICY);
	return g_strdup(str_data_sec_val);
}

gboolean
namespace_is_storage_policy_valid(const namespace_info_t * ns_info,
...
namespace_in_compression_mode(namespace_info_t * ns_info)
{
	if (!ns_info || !ns_info->options)
		return FALSE;
	return _gba_to_bool(g_hash_table_lookup(ns_info->options,
			NS_COMPRESS_OPT_NAME), FALSE);
}
...
namespace_get_size(namespace_info_t * ns_info, const gchar * name, gsize def)
{
	...
	gint64 i64 =
		_gba_to_int64(g_hash_table_lookup(ns_info->options, name), def);
	gsize result = i64;
...
gboolean
namespace_get_rules_path(const gchar * ns, const gchar * typename,
	...
	if (max > 0) {
		for (;;) {
			...
			if (r == 0)
				break;
			if (r < 0) {
				GSETERROR(err, "read(%d) = %d (%s)", fd, errno,
					strerror(errno));
				return FALSE;
			}
			offset += r;
...
gba_read_path(GByteArray * gba, gchar * path, GError ** err)
{
	...
	if (fd < 0) {
		GSETERROR(err, "open(%s) error : errno=%d (%s)", path, errno,
			strerror(errno));
		return FALSE;
	}
...
_svcupd_preload_with_default(GHashTable * ht, gchar * ns, const gchar * srvtype)
{
	if (!g_ascii_strcasecmp(srvtype, "meta1"))
		g_hash_table_insert(ht, g_strdup(ns),
			g_strdup("meta2=NONE;solr=APPEND;redis=REPLACE"));
	else if (!g_ascii_strcasecmp(srvtype, "meta2"))
		g_hash_table_insert(ht, g_strdup(ns),
			g_strdup("solr=APPEND;rawx=NONE;tsmx=APPEND"));
	else
...
_evtcfg_preload_with_default(GHashTable * ht, gchar * ns, const gchar * srvtype)
{
	if (!g_ascii_strcasecmp(srvtype, "meta2"))
		g_hash_table_insert(ht, g_strdup(ns),
			g_strdup("enabled=false;dir=/GRID/common/spool;aggregate=false"));
	else
		g_hash_table_insert(ht, g_strdup(ns), g_strdup(""));
}
...
gridcluster_reload_lbpool(struct grid_lbpool_s * glp)
{
	gboolean _reload_srvtype(const gchar * ns, const gchar * srvtype)
	{
		...
		if (err) {
			GRID_WARN("Gridagent error : Failed to list the services "
				"of type [%s] : code=%d %s", srvtype, err->code, err->message);
			g_clear_error(&err);
			return FALSE;
		}
	...
	list_srvtypes =
		list_namespace_service_types(grid_lbpool_namespace(glp), &err);
	if (err)
