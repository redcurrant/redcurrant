...
int
read_response_from_message(response_t * response, message_t * message,
	GError ** error)
{
	guint32 status;
	...
	if (message->length > sizeof(status)) {
		...
		memcpy(response->data, message->data + sizeof(status),
...
int
build_message_from_request(message_t * message, request_t * request,
	GError ** error)
{
	GByteArray *req_gba;
	...
	g_byte_array_append(req_gba, (const guint8 *) request->cmd,
		strlen(request->cmd));
...
send_request(request_t * req, response_t * resp, GError ** error)
{
	...
	size_read =
		sock_to_read_size(fd, SOCKET_TIMEOUT, &(message.length),
		sizeof(message.length), error);
	...
	size_read =
		sock_to_read_size(fd, SOCKET_TIMEOUT, message.data, message.length,
		error);
