...
get_srvtype(const gchar * ns, const gchar * type, GError ** error)
{
	...
	return conscience_get_srvtype(ns_data->conscience, error, type,
		MODE_AUTOCREATE);
}
...
parse_names_list(worker_t * worker, GError ** error)
{
	...
	if (asn1_session->resp_body != NULL) {
		GSList *names =
			meta2_maintenance_names_unmarshall_buffer(asn1_session->resp_body,
			asn1_session->resp_body_size, error);
		if (!names) {
			GSETERROR(error,
				"Failed to unmarshall the service_types name list");
			return 0;
		}
		else {
...
asn1_error_handler(worker_t * worker, GError ** error)
{
	...
	sdata = asn1_worker_get_session_data(worker);
	if (sdata) {
		GSETERROR(error, "[task_id=%s] Failed to request volume list",
			sdata->task_id);
		task_done(sdata->task_id);
	}
	else
		GSETERROR(error,
			"[task_id=" TASK_ID ".?] Failed to request volume list");
	return (0);
}
...
task_worker(gpointer p, GError ** error)
{
	...
	asn1_worker_set_handlers(asn1_worker, parse_names_list, asn1_error_handler,
		asn1_final_handler);
	...
	if (!asn1_request_worker(asn1_worker, error)) {
		...
		GSETERROR(error, "[task_id=%s.%s] Failed to send asn1 request", TASK_ID,
			ns_data->name);
		return 0;
	}
