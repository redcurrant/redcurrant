...
int
write_message_worker(worker_t * worker, GError ** error)
{
	ssize_t wl = 0;
	...
	if (data->buffer == NULL) {
		gint message_length_size =
			data->size_64 ? sizeof(guint64) : sizeof(message->length);
		...
		memcpy(data->buffer + message_length_size, message->data,
			message->length);
	}
	...
	if (wl <= 0) {
		GSETERROR(error, "An error occured while writing message : %s",
			strerror(errno));
		return 0;
	}
