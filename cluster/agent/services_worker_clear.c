...
asn1_final_handler(worker_t * worker, GError ** error)
{
	...
	if (!original_worker) {
		GSETERROR(error,
			"Request successful, failed to reply due to invalid worker");
		return 0;
	}
...
static gboolean
create_sub_worker(struct namespace_data_s *ns_data, const gchar * type_name,
	worker_t * orig_worker, GError ** error)
{
	...
	asn1_worker =
		create_asn1_worker(&(ns_data->ns_info.addr), NAME_MSGNAME_CS_RM_SRV);
	...
	g_hash_table_insert(asn1_worker_get_session(asn1_worker)->req_headers,
		g_strdup("TYPENAME"), g_byte_array_append(g_byte_array_new(),
			(guint8 *) type_name, strlen_len((guint8 *) type_name,
...
services_worker_clear(worker_t * worker, GError ** error)
{
	...
	char **tokens, ns_name[LIMIT_LENGTH_NSNAME + 1],
		type_name[LIMIT_LENGTH_SRVTYPE + 1];
	...
	if (g_strv_length(tokens) != 2) {
		g_strfreev(tokens);
		return __respond_message(worker, 0,
			"Invalid request's argument format (not NS:TYPE)", error);
	}
	else {
	...
	if (!ns_data || !ns_data->configured)
		return __respond_message(worker, 0, "Namespace invalid or not ready",
			NULL);
	...
	if (ns_data->conscience) {
		...
		srvtype =
			conscience_get_srvtype(ns_data->conscience, NULL, type_name,
			MODE_STRICT);
		if (srvtype)
