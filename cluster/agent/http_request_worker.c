...
write_request(worker_t * worker, GError ** error)
{
	...
	if (data->buffer == NULL) {
		switch (http_session->method) {
			case E_GET:
				snprintf(request, HTTP_REQ_MAX_SIZE, "GET %s HTTP/1.0%s%s",
					http_session->url, EOL, EOL);
				break;
			case E_POST:
				break;
			default:
	...
	if (wl < 0) {
		GSETERROR(error, "Write on socket failed with error : %s",
			strerror(errno));
		goto error_write;
	}
...
int
read_http_status(worker_t * worker, GError ** error)
{
	ssize_t rl;
	worker_data_t *data = NULL;
	http_session_t *http_session = NULL;
	...
	if (data->buffer == NULL) {
		...
		if (data->buffer == NULL) {
			...
			memset(data->buffer + (data->buffer_size - DEFAULT_BUFFER_SIZE), 0,
	...
	rl = read(data->fd, data->buffer + data->done,
		data->buffer_size - data->done);
	if (rl < 0) {
		GSETERROR(error, "Read on socket failed with error : %s",
			strerror(errno));
		goto error_read;
	}
	...
	if (strstr((char *) data->buffer, EOL)) {
		if (sscanf((char *) data->buffer, HTTP_STATUS_PATTERN,
				&http_status) == 1) {
			...
			if (http_status == HTTP_STATUS_OK) {
				...
				GSETERROR(error, "HTTP request failed with status %d",
					http_status);
				goto error_status;
			}
...
int
read_content_length(worker_t * worker, GError ** error)
{
	ssize_t rl;
	worker_data_t *data = NULL;
	http_session_t *http_session = NULL;
	...
	if (content_length_line && header_end) {
		...
		if (sscanf(content_length_line, CONTENT_LENGTH_PATTERN,
				&content_length) == 1) {
			DEBUG("Found a Content-Length header with value %u",
				content_length);
...
int
read_response_body(worker_t * worker, GError ** error)
{
	ssize_t rl;
	worker_data_t *data = NULL;
	http_session_t *http_session = NULL;
