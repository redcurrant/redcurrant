...
enum process_type_e
{ PT_SUPERV, PT_EVT, PT_REQ };

/*One directory for each queue*/
struct event_queue_set_s
{
	gchar dir_incoming[SPOOL_DIRNAME_LENGTH + LIMIT_LENGTH_NSNAME +
		sizeof(SUFFIX_SPOOL_INCOMING)];
	gchar dir_pending[SPOOL_DIRNAME_LENGTH + LIMIT_LENGTH_NSNAME +
		sizeof(SUFFIX_SPOOL_PENDING)];
	gchar dir_trash[SPOOL_DIRNAME_LENGTH + LIMIT_LENGTH_NSNAME +
		sizeof(SUFFIX_SPOOL_TRASH)];
};
