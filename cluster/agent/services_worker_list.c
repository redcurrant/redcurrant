...
static guint
count_known_services_list(struct namespace_data_s *ns_data,
	const gchar * type_name, GError ** error)
{
	...
	srvtype =
		conscience_get_srvtype(ns_data->conscience, error, type_name,
		MODE_STRICT);
	if (!srvtype) {
...
static GSList *
build_known_services_list(struct namespace_data_s *ns_data,
	const gchar * type_name, GError ** error)
{
	gchar **array_types;
	struct request_data_s req_data;
	...
	if (!conscience_run_srvtypes(ns_data->conscience, error,
			SRVTYPE_FLAG_INCLUDE_EXPIRED, array_types, services_runner,
			&req_data)) {
...
services_worker_list(worker_t * worker, GError ** error)
{
	...
	gchar **tokens, ns_name[LIMIT_LENGTH_NSNAME],
		type_name[LIMIT_LENGTH_SRVTYPE];
	GByteArray *gba = NULL;
	request_t *req;
	...
	if (g_strv_length(tokens) != 2) {
		g_strfreev(tokens);
		return __respond_message(worker, 0, "Invalid format (not NS:TYPE)",
			error);
	}
	else {
	...
	do {
		GSList *services =
			build_known_services_list(ns_data, type_name, &error_local);
...
services_worker_count(worker_t * worker, GError ** error)
{
	struct namespace_data_s *ns_data;
	gchar **tokens, ns_name[LIMIT_LENGTH_NSNAME],
		type_name[LIMIT_LENGTH_SRVTYPE], str_count[12];
	...
	gba =
		g_byte_array_append(g_byte_array_new(), (guint8 *) str_count,
		strlen(str_count));
...
services_types_worker_list(worker_t * worker, GError ** error)
{
	...
	do {
		GSList *names =
			conscience_get_srvtype_names(ns_data->conscience, error);
