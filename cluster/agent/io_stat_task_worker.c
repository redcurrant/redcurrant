...
get_proc_stat_content(const char *path, GError ** error)
{
	...
	if (fd < 0) {
		GSETERROR(error, "Failed to open file [%s] : %s", path,
			strerror(errno));
		return NULL;
	}
	...
	if (rl < 0) {
		GSETERROR(error, "Read file [%s] failed with error : %s", PROC_DISKSTAT,
			strerror(errno));
...
io_stat_task_worker(gpointer p, GError ** error)
{
	...
	regex = g_regex_new("^\\s*([0-9]*)\\s*([0-9]*)"
		"\\s*\\w*"
		"\\s*([0-9]*)\\s*([0-9]*)\\s*([0-9]*)\\s*([0-9]*)"
		"\\s*([0-9]*)\\s*([0-9]*)\\s*([0-9]*)\\s*([0-9]*)"
		"\\s*([0-9]*)\\s*([0-9]*)\\s*([0-9]*)", 0, 0, error);
...
{
	if (!path_to_majmin) {
		path_to_majmin = g_hash_table_new_full(g_str_hash, g_str_equal,
			g_free, g_free);
	}

	if (!majmin_to_stats) {
		majmin_to_stats = g_hash_table_new_full(
			(GHashFunc) majmin_hash, (GEqualFunc) majmin_equal, g_free, g_free);
	}
...
get_io_idle(int major, int minor, int *idle, GError ** err)
{
	...
	result_d =
		100.0 - (percent_used_d / (time_spent_d > 0.0 ? time_spent_d : 1.0));
	...
	*pmin = 0;
	...
	if (!v->last_update || v->last_update > now || v->last_update < now - 30) {
		...
		if (0 > stat(path, &file_stat)) {
			GSETERROR(err, "stat(%s) : errno=%d %s", path, errno,
				strerror(errno));
			return 0;
		}
