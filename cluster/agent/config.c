...
parse_config(const char *config_file, GHashTable * params, GError ** error)
{
	...
	void copy_cfg_value(const gchar * g, const gchar * n)
	{
		if (g_key_file_has_key(key_file, g, n, error))
			g_hash_table_insert(params, g_strdup(n),
				g_key_file_get_value(key_file, g, n, error));
	}
	...
	if (g_key_file_has_group(key_file, NAME_SECTION_SERVER_INET)) {
		if (g_key_file_has_key(key_file, NAME_SECTION_SERVER_INET, BACKLOG_KEY,
				error)) {
			gchar *str;
			gint64 i64;

			str =
				g_key_file_get_value(key_file, NAME_SECTION_SERVER_INET,
				BACKLOG_KEY, error);
			i64 = g_ascii_strtoll(str, NULL, 10);
			g_free(str);
		...
		if (g_key_file_has_key(key_file, NAME_SECTION_SERVER_INET, PORT_KEY,
				error)) {
			gchar *str;
			gint64 i64;
			int i_port;
