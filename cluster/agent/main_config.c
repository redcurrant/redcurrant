...
_build_request(ne_session * http_session, const gchar * path_url)
{
	...
	ne_add_request_header(http_request, "User-Agent",
		"HoneyComb-gridagent-httpconf");
	return http_request;
}
	...
	switch (ne_request_dispatch(http_request)) {
		case NE_OK:
			if (ne_get_status(http_request)->klass != 2) {
				GSETERROR(error, "Failed to download '%s': %s", path_url,
		...
		case NE_ERROR:
		default:
			GSETERROR(error, "Failed download '%s': %s", path_url,
				ne_get_error(session));
			g_byte_array_free(gba, TRUE);
			gba = NULL;
			break;
...
download_file_services(ne_session * session, GError ** error)
{
	...
	g_snprintf(path_url, sizeof(path_url), "%s/%s/%s", gridconf_distant_basedir,
		hostname, gridconf_distant_listsrv_basename);
...
download_file_list(ne_session * session, GError ** error)
{
	...
	g_snprintf(path_url, sizeof(path_url), "%s/%s/%s", gridconf_distant_basedir,
		hostname, gridconf_distant_listfiles_basename);
	return _download_to_gba(session, path_url, error);
}

static gboolean
_download_to_file(ne_session * session, const gchar * path_url,
	const gchar * path_local, GError ** error)
{
	...
	g_snprintf(path_tmp, sizeof(path_tmp), "%s.%d.%ld", path_local, getpid(),
		time(0));
	...
	if (-1 == g_mkdir_with_parents(dirname, 0755)) {
		g_free(dirname);
		GSETERROR(error, "Failed to create the dirname of '%s' : %s", path_tmp,
			strerror(errno));
		return FALSE;
	}
	g_free(dirname);
	...
	if (!stream_out) {
		GSETERROR(error, "Failed to open '%s' in write mode : %s", path_local,
			strerror(errno));
		return FALSE;
	}

	http_request = _build_request(session, path_url);
	ne_add_response_body_reader(http_request, ne_accept_2xx, read_to_stream,
		stream_out);
	...
	switch (rc_dispatch = ne_request_dispatch(http_request)) {
		...
		case NE_TIMEOUT:
		case NE_ERROR:
			GSETERROR(error, "Failed download '%s' (rc=%d) : %s", path_url,
				rc_dispatch, ne_get_error(session));
			goto label_error;
	}

	if (-1 == g_rename(path_tmp, path_local)) {
		GSETERROR(error,
			"Failed to commit the temporary download file '%s' : %s", path_tmp,
			strerror(errno));
		goto label_error;
	}
...
static gboolean
download_each_configuration_file(ne_session * http_session, GByteArray * gba,
	GError ** error)
{
	gboolean rc = FALSE;
	...
	path_validator =
		g_regex_new("/GRID/([^/]+|[^/]+/[^/]+)/conf/[^/]+", 0, 0, error);
	if (!path_validator) {
	...
	for (pl = lines; is_running && pl && *pl; pl++) {
		...
		g_snprintf(path_uri, sizeof(path_uri), "%s/%s/%s",
			gridconf_distant_basedir, hostname, line);
		g_free(path_escaped);
		g_strlcpy(path_local, line, sizeof(path_local) - 1);
		if (!_download_to_file(http_session, path_uri, path_local, error)) {
			GSETERROR(error,
				"Failed to download '%s' in local '%s' (at line %d)", path_uri,
				path_local, line_nb);
			goto label_error;
		}
	}
...
main_http_config(const gchar * hn, int fd)
{
	...
	if (!gba_files) {
		ERROR("Failed to get the files list file : %s",
	...
	if (!gba_services) {
		ERROR("Failed to get the services definition file : %s",
			gerror_get_message(error));
		goto label_error;
	}
	...
	if (!download_each_configuration_file(http_session, gba_files, &error)) {
		ERROR("Failed to download a configuration file : %s",
			gerror_get_message(error));
		goto label_error;
	}
	...
	if (!dump_gba(gba_services, fd_out, &error)) {
		ERROR("Failed to dump the configuration to fd=%d : %s", fd_out,
			gerror_get_message(error));
		goto label_error;
	}
