...
static gboolean
expand_service_tags(struct namespace_data_s *ns_data, struct service_info_s *si,
	GError ** error)
{
	int i, max;
	...
	for (i = 0, max = si->tags->len; i < max; i++) {
		...
		if (tag->type == STVT_MACRO) {
			...
			if (!g_ascii_strcasecmp(tag->value.macro.type,
					NAME_MACRO_IOIDLE_TYPE)) {
			...
			else if (!g_ascii_strcasecmp(tag->value.macro.type,
					NAME_MACRO_CPU_TYPE)) {
				if (get_cpu_idle(&idle, error))
					...
					NAME_MACRO_SPACE_TYPE)) {
				long free_space;
				gint64 free_space_i64;

				free_space =
					get_free_space(tag->value.macro.param,
					ns_data->ns_info.chunk_size);
				free_space_i64 = free_space;
				service_tag_set_value_i64(tag, free_space_i64);
			}
			else {
				WARN("Service [NS=%s][SRVTYPE=%s][@=%s] : macro type not managed name=[%s] type=[%s]", si->ns_name, si->type, str_addr, tag->name, tag->value.macro.type);
			}
		}
	}
	return TRUE;
...
manage_service(struct service_info_s *si)
{
	...
	gsize key_size;
	gchar key[LIMIT_LENGTH_SRVTYPE + 1 + STRLEN_ADDRINFO + 1],
		str_addr[STRLEN_ADDRINFO];
	...
	if (error_local)
		g_error_free(error_local);
	if (!conscience_get_srvtype(ns_data->conscience, &error_local, si->type,
			MODE_STRICT)) {
		...
		if (!g_hash_table_lookup(ns_data->local_services, key)
			...
			INFO("New service type discovered [ns=%s type=%s addr=%s]",
				ns_data->name, si->type, str_addr);
		}
	}
	...
	if (old_si != NULL) {
		...
		if (tag_first != NULL)
			service_tag_set_value_boolean(service_info_ensure_tag(si->tags,
					NAME_TAGNAME_RAWX_FIRST), tag_first->value.b);
	}

	service_tag_set_value_boolean(service_info_ensure_tag(si->tags, "tag.up"),
		TRUE);
	g_hash_table_insert(ns_data->local_services, g_strndup(key, key_size), si);

	DEBUG("Service registration [ns=%s type=%s addr=%s]", ns_data->name,
		si->type, str_addr);
...
services_worker_push(worker_t * worker, GError ** error)
{
	...
	rc = service_info_unmarshall(&list_service_info, req->arg, &arg_size,
		error);
...
static void
fill_service_info_score(struct conscience_s *conscience,
