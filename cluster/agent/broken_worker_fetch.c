...
static gboolean
broken_content_runner(gpointer u, struct broken_meta2_s *bm2,
	struct broken_content_s *bc)
{
	...
	if (bm2) {
		str =
			bc ? broken_holder_write_content(bm2,
			bc) : broken_holder_write_meta2(bm2);
		data = u;
		if (str) {
...
agent_fetch_broken_all_elements(worker_t * worker, GError ** error)
{
	...
	broken_holder_run_elements(ns_data->conscience->broken_elements, 0, &data,
		broken_meta1_runner, broken_content_runner);
	DEBUG("BROKEN containers/contents of %s listed! nb=%" G_GINT64_FORMAT
		" buffer=%d", ns_name, data.counter, data.packed->len);
