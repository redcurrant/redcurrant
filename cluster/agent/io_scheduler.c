		...
		if (rc < 0) {
			...
			for (int i = 0; i < rc; i++) {
				...
				if (events[i].events & (EPOLLHUP | EPOLLERR)) {
					...
					ERROR
						("An error occured on fd [%d], closing connection : %s",
					...
					if (!(worker->func(worker, &local_error))) {
						if (local_error) {
							ERROR("Failed to execute worker : %s",
...
add_fd_to_io_scheduler(worker_t * worker, __uint32_t events, GError ** error)
{
	...
	if (0 > epoll_ctl(epfd, EPOLL_CTL_ADD, worker->data.fd, &ev)) {
		GSETERROR(error, "Failed to add fd to epoll pool : %s",
			strerror(errno));
		return (0);
	}
...
int
change_fd_events_in_io_scheduler(worker_t * worker, __uint32_t events,
	GError ** error)
{
	struct epoll_event ev;
