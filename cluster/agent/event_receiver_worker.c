...
static gchar *
path_get_from_event_aggrname(gridcluster_event_t * decoded_event,
	const gchar * basedir)
{
	...
	/* Decode and check there is an UEID in the event */
	if (!(gba =
			g_hash_table_lookup(decoded_event,
				GRIDCLUSTER_EVTFIELD_AGGRNAME))) {
	...
	for (s = aggr_name; '\0' != (c = *s); s++) {
		if (!g_ascii_isprint(c) || g_ascii_isspace(c) || c == G_DIR_SEPARATOR
			|| c == '.')
			*s = '_';
	}

	/* Replace the aggregation name with its canonized form */
	gridcluster_event_add_string(decoded_event, GRIDCLUSTER_EVTFIELD_AGGRNAME,
		aggr_name);
...
static gboolean
save_event(struct namespace_data_s *ns_data, gridcluster_event_t * e,
	GError ** error)
{
	...
	if (0 != g_mkdir_with_parents(ns_data->queues.dir_incoming,
			event_directory_mode)) {
	...
	if (!(rc =
			g_file_set_contents(path, (gchar *) encoded->data, encoded->len,
				error)))
		GSETERROR(error,
			"Failed to write the serialized event UEID[%s] at [%s]", ueid,
			path);
	else {
		gboolean xattr_rc;

		xattr_rc =
			event_set_timestamp(path, old_timestamp ? old_timestamp : time(0));
...
static gboolean
save_event_bytes(struct namespace_data_s *ns_data, const guint8 * buf,
	gsize buf_size, GError ** error)
{
...
agent_receive_event_worker(worker_t * worker, GError ** error)
{
	...
	DEBUG("[ns=%s] Received an event of %" G_GSIZE_FORMAT " bytes (from %d)",
		ns_name, encoded_event_size, req->arg_size);
	...
	if (!event_enable_receive)
		return __respond_message(worker, 0, "Events not managed on this host",
			error);

	if (!save_event_bytes(ns_data, (guint8 *) encoded_event, encoded_event_size,
			&error_local))
