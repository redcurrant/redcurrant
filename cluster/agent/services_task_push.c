...
task_worker(gpointer p, GError ** error)
{
	...
	if (!gba_body) {
		GSETERROR(error, "[ns=%s] services serialization failure",
			ns_data->name);
	...
	asn1_worker =
		create_asn1_worker(&(ns_data->ns_info.addr), NAME_MSGNAME_CS_PUSH_SRV);
	asn1_worker_set_handlers(asn1_worker, agent_asn1_default_response_handler,
		asn1_error_handler, asn1_final_handler);
	asn1_worker_set_session_data(asn1_worker, g_memdup(&sdata, sizeof(sdata)),
		asn1_cleaner);
