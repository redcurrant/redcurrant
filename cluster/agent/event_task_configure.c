...
static void
_prepend_implicit_rule_to_conscience(struct conscience_s *conscience,
	const gchar * pattern)
{
	guint i, old_length;
	...
	memcpy(&(action.parameter.address), &(conscience->ns_info.addr),
		sizeof(addr_info_t));
	...
	if (conscience->event_handler->ruleset) {
		...
		old_length =
			g_strv_length((gchar **) conscience->event_handler->ruleset);
	...
	gsize data_size, GError ** error)
{
	...
	rc = gridcluster_eventhandler_configure(conscience->event_handler, data,
		data_size, error);
	if (rc) {
...
agent_configure_events(struct conscience_s *conscience, GError ** error)
{
	...
	rc = agent_configure_events2(conscience, (gchar *) gba->data, gba->len - 1,
		error);
...
parse_names_response_handler(worker_t * worker, GError ** error)
{
	...
	if (!ns_data->configured) {
		GSETERROR(error, "Namespace %s not configured (corruption?)",
			sdata->ns);
		return 0;
	}
	...
	if (asn1_session->resp_body != NULL) {
		sdata->configuration_text =
			g_strndup(asn1_session->resp_body, asn1_session->resp_body_size);
		if (sdata->configuration_text)
...
asn1_final_handler(worker_t * worker, GError ** error)
{
	...
	if (!ns_data->configured || !ns_data->conscience) {
		GSETERROR(error, "Namespace %s configuration disappeared (corruption?)",
			sdata->ns);
		return 0;
	}

	if (!agent_configure_events2(ns_data->conscience, sdata->configuration_text,
			sdata->configuration_text_size, error)) {
...
task_worker(gpointer task_param, GError ** error)
{
	...
	asn1_worker_set_handlers(asn1_worker, parse_names_response_handler,
	...
	if (!asn1_request_worker(asn1_worker, error)) {
		GSETERROR(error, "Failed to send asn1 request for task [%s]",
			sdata->task_id);
...
NAMESPACE_TASK_CREATOR(task_starter, TASK_ID, task_worker,
	period_get_evtconfig);
...
agent_reply_event_configuration_worker(worker_t * worker, GError ** error)
{
	...
	gba_config =
		gridcluster_eventhandler_get_configuration(ns_data->conscience->
		event_handler, &e);
	if (!gba_config) {
...
agent_reply_event_managed_patterns_worker(worker_t * worker, GError ** error)
{
	...
	list_of_patterns =
		gridcluster_eventhandler_get_patterns(ns_data->conscience->
		event_handler, NULL);
	...
	if (!gba_config) {
		GSETERROR(error,
			"Failed to serialize the eventhandler patterns for ns='%s'",
			ns_name);
		return 0;
	}
...
worker_event_indirect_config(gpointer udata, GError ** error)
{
	...
	if (!IS_FORKED_AGENT) {
		GSETERROR(error,
			"CODE ERROR: a secondary task cannot be started within the main agent");
		goto label_end;
	}
	...
	if (!agent_configure_events(ns_data->conscience, error)) {
		GSETERROR(error,
			"NS[%s] Cannot the configure the events from the gridagent",
			ns_name);
		goto label_end;
	}
