...
void
asn1_worker_set_session_data(worker_t * asn1_worker, void *data,
...
asn1_worker_liberator(worker_t * worker)
{
	...
	if (worker->clean != &asn1_worker_liberator)
		ALERT
			("An ASN.1 worker has a cleaner (%p) that is not asn1_worker_liberator (%p)",
...
create_asn1_worker(addr_info_t * addr, const gchar * req_name)
{
	...
	asn1_session->resp_headers =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_clean);
	asn1_session->req_headers =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_clean);
...
void
asn1_worker_set_request_header(worker_t * asn1_worker, const char *key,
	const char *value)
{
	asn1_session_t *asn1_session;
...
void
asn1_worker_set_request_body_from_buffer(worker_t * asn1_worker, guint8 * body,
	gsize body_size)
{
	asn1_session_t *asn1_session;
...
asn1_request_worker(worker_t * worker, GError ** error)
{
	...
	DEBUG("ASN.1 request '%s' sent to %s (fd=%d)", asn1_session->req_name,
		str_addr, fd);
	return (1);
}
	...
	if (data->buffer == NULL) {
		...
		if (asn1_session->req_headers) {
			...
			for (key = keys; key && key->data; key = key->next) {
				GByteArray *value =
					(GByteArray *) g_hash_table_lookup(asn1_session->
					req_headers, key->data);
				if (!message_add_field(req, key->data,
						strlen((char *) key->data), value->data, value->len,
						error)) {
		...
		if (asn1_session->req_body
			&& !message_set_BODY(req, asn1_session->req_body,
				asn1_session->req_body_size, error)) {
			GSETERROR(error, "Failed to set asn1 message body");
			goto error_set_body;
		}

		if (!message_set_NAME(req, asn1_session->req_name,
				strlen(asn1_session->req_name), error)) {
		...
		wl = write(data->fd, data->buffer + data->done,
...
read_response(worker_t * worker, GError ** error)
{
	...
	if (data->done >= data->buffer_size) {
		...
		TRACE("Data available : (done=%d) >= (size=%d)", data->done,
			data->buffer_size);
		...
		if (message_has_BODY(resp, error)) {
			if (!message_get_BODY(resp, &(asn1_session->resp_body),
		...
		if (status == 200 || !rc) {
			...
			TRACE("Reply sequence not terminated (status=%d rc=%d)", status,
				rc);
			CLEAR_WORKER_DATA(data);
			worker->func = read_response_size;
			return 1;
