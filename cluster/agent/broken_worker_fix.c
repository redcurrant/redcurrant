	...
	GError ** error)
{
	...
	errno = 0;
	rc = broken_holder_check_element_format(conscience->broken_elements,
		element);
	if (rc)
		return TRUE;
	switch (errno) {
		case EINVAL:
			GSETERROR(error, "Invalid argument (%p %p)",
				conscience->broken_elements, element);
			return FALSE;
		case EBADMSG:
...
agent_fixed_erroneous_container(worker_t * worker, GError ** error)
{
	...
	if (!flag_manage_broken)
		return __respond_message(worker, 0, "Broken elements not managed",
			error);
	...
	g_snprintf(broken_element, sizeof(broken_element), "%s",
		req->arg + 1 + strlen(ns_name));
	g_strfreev(tokens);
