...
error_handler(worker_t * worker, GError ** error)
{
	...
	ns_name = asn1_worker_get_session_data(worker);
	if (ns_name)
		GSETERROR(error, "[task_id=%s.%s] Failed to request namespace info",
			TASK_ID, ns_name);
	else
		GSETERROR(error, "[task_id=%s.?] Failed to request namespace info",
			TASK_ID);
...
parse_namespace_info(worker_t * worker, GError ** error)
{
	...
	if (asn1_session->resp_body != NULL) {
		ns_info =
			namespace_info_unmarshall(asn1_session->resp_body,
			asn1_session->resp_body_size, error);
		if (ns_info == NULL) {
			GSETERROR(error,
				"Failed to unmarshall namespace_info from ASN1 data");
			return 0;
		}
		...
		if (strcmp(ns_info->name, ns_data->name)) {
			ERROR
				("Namespace name [%s] in /etc/gridstorage does not match name [%s] in conscience !",
		...
		DEBUG
			("Infos of namespace [%s] updated with value name [%s] / chunk_size[%"
			G_GINT64_FORMAT "]", ns_data->ns_info.name, ns_info->name,
			ns_info->chunk_size);
...
task_worker(gpointer p, GError ** error)
{
	...
	asn1_worker_set_session_data(asn1_worker, g_strdup(ns_data->name),
		session_data_cleaner);
	asn1_worker_set_request_header(asn1_worker, "VERSION", SHORT_API_VERSION);
	asn1_worker_set_handlers(asn1_worker, parse_namespace_info, error_handler,
		final_handler);
	...
	while (g_hash_table_iter_next(&ns_iterator, &ns_k, &ns_v)) {
		...
		if (!namespace_is_available(ns_data)) {
			if (!is_task_scheduled(ns_id)) {
				...
				task =
					set_task_callbacks(task, task_worker, g_free,
					g_strdup(ns_data->name));
				if (!add_task_to_schedule(task, error)) {
					ERROR("[task_id=" TASK_ID
						"] Failed to start a sub worker for namespace '%s'",
						(gchar *) ns_k);
					g_free(task);
				}
				else
...
get_namespace(const char *ns_name, GError ** error)
{
	...
	if (!ns_data->local_services
		|| !ns_data->down_services || !ns_data->conscience) {
