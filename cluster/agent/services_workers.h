...
gboolean agent_choose_best_service(const gchar * ns_name, const gchar * srvname,
	struct service_info_s *si, GError ** err);

gboolean agent_choose_best_service2(struct namespace_data_s *ns_data,
...
gsize agent_get_service_key(struct service_info_s *si, gchar * dst,
	gsize dst_size);
