...
static gboolean
poll_conscience_service(struct conscience_srvtype_s *srvtype,
...
gboolean
agent_choose_best_service2(struct namespace_data_s * ns_data,
	const gchar * srvname, struct service_info_s * si, GError ** err)
{
	struct conscience_srvtype_s *srvtype;

	srvtype =
		conscience_get_srvtype(ns_data->conscience, err, srvname, MODE_STRICT);
	if (!srvtype) {
...
gboolean
agent_choose_best_service(const gchar * ns_name, const gchar * srvname,
	struct service_info_s * si, GError ** err)
{
