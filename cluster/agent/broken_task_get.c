...
parse_names_response_handler(worker_t * worker, GError ** error)
{
	...
	if (!ns_data) {
		GSETERROR(error, "[task_id=%s] Namespace %s disappeared",
			sdata->task_id, sdata->ns);
		return 0;
	}
	if (!ns_data->configured) {
		GSETERROR(error,
			"[task_id=%s] Namespace %s not configured (corruption?)",
			sdata->task_id, sdata->ns);
		return 0;
	}
	...
	if (asn1_session->resp_body != NULL) {
		...
		if (!strings_unmarshall(&names, asn1_session->resp_body, &body_size,
				error)) {
			GSETERROR(error,
				"[task_id=%s] Failed to unmarshall broken elements list",
				sdata->task_id);
			return 0;
		}
		else {
			DEBUG("Received %d names", g_slist_length(names));
			sdata->broken_elements =
				!sdata->broken_elements ? names : g_slist_concat(names,
				sdata->broken_elements);
		}
	}
	else {
		DEBUG("[task_id=%s] Empty reply received from the conscience",
			sdata->task_id);
	}
