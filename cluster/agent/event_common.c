...
static gint
compare_path_data_time_ASC(const struct path_data_s *p1,
	const struct path_data_s *p2)
{
...
static gint
compare_path_data_sequence_ASC(const struct path_data_s *p1,
	const struct path_data_s *p2)
{
	register gint64 cmp, i1, i2;
...
static gint
compare_path_data_ASC(const struct path_data_s *p1,
	const struct path_data_s *p2)
{
	gint64 cmp;
	...
	return (cmp < 0) ? -1 : ((cmp > 0) ? 1 : compare_path_data_sequence_ASC(p1,
			p2));
}
...
keep_earliest_path_data(GHashTable * ht_current, const struct path_data_s *pd)
{
	...
	if (!previous) {
		TRACE("CID[%s] TIME[%ld] SEQ[%" G_GINT64_FORMAT
			"] saved because not found", pd->str_cid, pd->xattr_time,
			pd->xattr_seq);
	...
	if (compare_path_data_ASC(previous, pd) > 0) {
		TRACE("CID[%s] TIME[%ld] SEQ[%" G_GINT64_FORMAT
			"] saved because earlier", pd->str_cid, pd->xattr_time,
			pd->xattr_seq);
...
static guint
get_one_event_per_container2(GHashTable * ht, const gchar * dir,
	struct dir_run_info_s *info)
{
	...
	while (NULL != (subdir = g_dir_read_name(gdir))) {
		...
		if (-1 == stat(hashpath, &hashstat))
			...
			if (hashdir == NULL) {
				ERROR("g_dir_open(%s) error : %s", hashpath,
			...
			while (NULL != (path = g_dir_read_name(hashdir))) {
				...
				bzero(&pd, sizeof(pd));
				g_snprintf(pd.relpath, sizeof(pd.relpath), "%s%c%s", subdir,
					G_DIR_SEPARATOR, path);
				...
				if (-1 == stat(fullpath, &(pd.stat)))
					...
					if (0 != gridcluster_eventxattr_get_container_id(fullpath,
							&(pd.id), pd.str_cid, sizeof(pd.str_cid))) {
						DEBUG
							("Invalid event, missing XATTR (container) at [%s]",
							pd.relpath);
					}
					else if (0 !=
						gridcluster_eventxattr_get_incoming_time(fullpath,
							&(pd.xattr_time))) {
						DEBUG("Invalid event, missing XATTR (time) at [%s]",
							pd.relpath);
					}
					else if (0 != gridcluster_eventxattr_get_seq(fullpath,
							&(pd.xattr_seq))) {
						DEBUG("Invalid event, missing XATTR (sequence) at [%s]",
							pd.relpath);
					}
						...
						TRACE("CID[%s] TIME[%ld] SEQ[%" G_GINT64_FORMAT
							"] filtered at [%s]", pd.str_cid, pd.xattr_time,
...
GSList *
agent_list_earliest_events(const gchar * dir, guint max, time_t delay,
	gboolean(*filter) (path_data_t *))
{
	struct dir_run_info_s info;
