...
struct session_data_s
{
	...
	gchar task_id[sizeof(TASK_ID) + 1 + LIMIT_LENGTH_SRVTYPE + 1 +
		LIMIT_LENGTH_NSNAME + 1];
	GSList *services;
};
...
parse_srv_list(worker_t * worker, GError ** error)
{
	...
	if (asn1_session->resp_body != NULL) {
		...
		if (service_info_unmarshall(&services, asn1_session->resp_body,
				&(asn1_session->resp_body_size), error) <= 0) {
			GSETERROR(error,
				"[task_id=%s] Failed to unmarshall service_info list",
				sdata->task_id);
			return 0;
		}
		else {
			DEBUG("[task_id=%s] Received %d services", sdata->task_id,
				g_slist_length(services));
			sdata->services =
				sdata->services ? g_slist_concat(sdata->services, services)
				: services;
		}
	}
...
asn1_final_handler(worker_t * worker, GError ** error)
{
	...
	if (sdata->services)
		DEBUG
			("[task_id=%s] Saving the %d services received from the conscience",
			sdata->task_id, g_slist_length(sdata->services));
	else
		INFO("[task_id=%s] No service received, forgetting all currently saved",
			sdata->task_id);
	...
	for (l = sdata->services; l; l = g_slist_next(l)) {
		...
		srvtype =
			conscience_get_srvtype(ns_data->conscience, &error_local, si->type,
			MODE_STRICT);
		if (!srvtype) {
			ERROR
				("[task_id=%s] Orphan service : service type [%s] not found : %s",
				sdata->task_id, si->type, gerror_get_message(error_local));
			if (error_local)
		...
		if (conscience_srvtype_refresh(srvtype, &error_local, si, TRUE)) {
			DEBUG("[task_id=%s] service refreshed [%s/%s/%s]", sdata->task_id,
				sdata->ns, si->type, str_addr);
			counter++;
		}
		else
			WARN("[task_id=%s] Failed to refresh service [%s/%s/%s]",
				sdata->task_id, sdata->ns, si->type, str_addr);
	}

	DEBUG("[task_id=%s] Task terminated, %d services refreshed/registered",
		sdata->task_id, counter);
	return 1;
}
...
asn1_error_handler(worker_t * worker, GError ** error)
{
	...
	sdata = asn1_worker_get_session_data(worker);
	if (sdata) {
		GSETERROR(error, "[task_id=%s] services request failed",
			sdata->task_id);
		task_done(sdata->task_id);
	}
	else
...
task_worker(gpointer p, GError ** error)
{
	...
	if (!list_types) {
		...
		s = list_types->data;
		gba_types =
			g_byte_array_append(g_byte_array_new(), (guint8 *) s, strlen(s));
	...
	g_snprintf(sdata->task_id, sizeof(sdata->task_id), "%s.%s", TASK_ID,
		ns_data->name);

	asn1_worker =
		create_asn1_worker(&(ns_data->ns_info.addr), NAME_MSGNAME_CS_GET_SRV);
	asn1_worker_set_handlers(asn1_worker, parse_srv_list, asn1_error_handler,
		asn1_final_handler);
	asn1_worker_set_session_data(asn1_worker, sdata,
		(GDestroyNotify) sdata_cleaner);
	g_hash_table_insert(asn1_worker_get_session(asn1_worker)->req_headers,
		g_strdup("TYPENAME"), gba_types);

	/*create the worker */
	if (!asn1_request_worker(asn1_worker, error)) {
		GSETERROR(error, "[task_id=%s] Failed to send asn1 request",
			sdata->task_id);
		g_free(sdata);
