...
int
parse_cluster_conf(const char *file_path, namespace_data_t * ns_data,
	GError ** error)
{
	...
	if (g_key_file_has_group(key_file, NS_KEY)) {
		...
		if (g_key_file_has_key(key_file, NS_KEY, NS_NAME_KEY, error)) {
			...
			GSETERROR(error,
				"Namespace name is missing (add a %s= in %s group)",
				NS_NAME_KEY, NS_KEY);
			return (0);
		}
		...
		if (g_key_file_has_key(key_file, NS_KEY, NS_CHUNK_SIZE_KEY, error)) {
			ns_data->ns_info.chunk_size =
				g_key_file_get_integer(key_file, NS_KEY, NS_CHUNK_SIZE_KEY,
				error);
		}
		else {
			GSETERROR(error,
				"Namespace chunk_size is missing (add a %s= in %s group)",
				NS_CHUNK_SIZE_KEY, NS_KEY);
			return (0);
		}

		DEBUG("Set namespace info name(%s) chunk_size(%" G_GINT64_FORMAT
			") in conf", ns_data->ns_info.name, ns_data->ns_info.chunk_size);
	}
	else {
