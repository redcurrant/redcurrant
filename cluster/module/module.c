...
_alert_service_with_zeroed_score(struct conscience_srv_s *srv)
{
	...
	if (srv->time_last_alert < now - srv->srvtype->alert_frequency_limit) {
		str_id_size =
			g_snprintf(str_id, sizeof(str_id), "conscience.%.*s.score",
...
static void
init_reply_ctx_with_request(struct request_context_s *req,
	struct reply_context_s *rep)
{
	if (rep) {
...
timer_check_services(gpointer u)
{
	...
	for (l = list_type_names; l; l = g_slist_next(l)) {
		...
		rc = conscience_srvtype_run_all(srvtype, &error_local,
			SRVTYPE_FLAG_ADDITIONAL_CALL, service_checker, &now);
		conscience_release_locked_srvtype(srvtype);
		/* XXX end of critical section */

		if (!rc)
			ERROR
				("[NS=%s][SRVTYPE=%s] Failed to run the service check loop : %s",
				conscience_get_namespace(cs), str_name,
...
timer_expire_services(gpointer u)
{
	...
	for (l = list_type_names; l; l = g_slist_next(l)) {
		...
		rc = conscience_srvtype_remove_expired(srvtype, &error_local,
		...
		if (rc < 0)
			ERROR
				("[NS=%s][SRVTYPE=%s] Failed to remove the expired services : %s",
				conscience_get_namespace(cs), str_name,
				gerror_get_message(error_local));
		else if (rc > 0)
...
search_vns_in_tree(GNode * tree_root, const gchar * vns_name)
{
	...
	do {
		TRACE("Comparing [%s] || [%s]",
			((struct vns_info_s *) node->data)->name, vns_name);
		if (g_ascii_strcasecmp(((struct vns_info_s *) node->data)->name,
				vns_name) == 0) {
			TRACE("VNS [%s] found in tree", vns_name);
			break;
		}
		else {
			if (g_str_has_prefix(vns_name,
					((struct vns_info_s *) node->data)->name)) {
				if (node->children) {
					node = node->children;
				}
				else {
...
request_body_matches_namespace(struct request_context_s *req_ctx, GError ** err)
{
	...
	if (1 != g_slist_length(list_ns)) {
		GSETCODE(err, 400,
			"Too many namespaces, this conscience only manage exactly 1 NS");
		rc = FALSE;
	}
	else {
		bzero(local_ns, sizeof(local_ns));
		g_strlcpy(local_ns, conscience->ns_info.name,
			sizeof(conscience->ns_info.name) - 1);
		rc = (0 == g_ascii_strcasecmp(local_ns, (gchar *) list_ns->data));
		if (!rc) {
			GNode *vns_node =
				search_vns_in_tree(conscience->virtual_namespace_tree,
				(gchar *) list_ns->data);
			if (vns_node)
				rc = TRUE;
		}
...
handler_get_ns(struct request_context_s *req_ctx)
{
	...
	if (!reply_context_reply(&ctx, &(ctx.warning))) {
		...
		reply_context_log_access(&ctx, "NS=%s",
			conscience_get_namespace(conscience));
		reply_context_clear(&ctx, FALSE);
		return (0);
	}
...
handler_get_ns_info(struct request_context_s *req_ctx)
{
	...
	e = message_extract_string(req_ctx->request, "VERSION", version,
		sizeof(version));
	if (NULL != e)
		g_clear_error(&e);
	...
	gba =
		namespace_info_marshall(&(conscience->ns_info), version,
		&(ctx.warning));
	g_static_rec_mutex_unlock(&conscience_nsinfo_mutex);
	if (gba == NULL) {
...
handler_rm_broken_containers(struct request_context_s *req_ctx)
{
	...
	if (message_has_BODY(req_ctx->request, NULL)) {
		...
		if (0 >= message_get_BODY(req_ctx->request, &data, &data_size,
				&(ctx.warning))) {
			GSETCODE(&(ctx.warning), 400, "Bad request : no body");
			goto errorLabel;
		}
		if (!strings_unmarshall(&elements, data, &data_size, &(ctx.warning))) {
			GSETCODE(&(ctx.warning), 400,
				"Bad request : failed to unmarshall broken container list");
			goto errorLabel;
		}
		counter = 0;
		...
		for (list = elements; list && list->data; list = list->next) {
			if (list->data) {
				...
				broken_holder_remove_element(conscience->broken_elements,
					(gchar *) list->data);
		...
		NOTICE("[NS=%s] %u broken elements have been flushed",
			conscience_get_namespace(conscience), counter);
	}
	else {
		/* XXX start of critical section */
		conscience_lock_broken_elements(conscience, 'w');
		...
		NOTICE("[NS=%s] all the broken elements have been flushed",
			conscience_get_namespace(conscience));
	}
...
errorLabel:
	...
	reply_context_clear(&ctx, FALSE);
	reply_context_set_message(&ctx, ctx.warning->code,
		gerror_get_message(ctx.warning));
...
handler_fix_broken_containers(struct request_context_s *req_ctx)
{
	...
	containers =
		meta2_maintenance_names_unmarshall_buffer(data, data_size,
		&(ctx.warning));
	if (!containers) {
		GSETCODE(&(ctx.warning), 400,
			"Invalid request - invalid body, deserialization eror");
		goto errorLabel;
	}
	...
	for (list = containers; list && list->data; list = list->next) {
		if (list->data) {
			...
			if (errsav)
				INFO("Failed to fix [%s] : %s", (gchar *) list->data,
					strerror(errno));
...
handler_push_broken_containers(struct request_context_s *req_ctx)
{
	...
	for (list = elements; list && list->data; list = list->next) {
		if (list->data) {
			/* XXX start of critical section */
			conscience_lock_broken_elements(conscience, 'w');
			broken_holder_add_element(conscience->broken_elements,
				(gchar *) (list->data));
			conscience_unlock_broken_elements(conscience);
			/* XXX end of critical section */

			TRACE("Broken element successfuly managed : [%s]",
				(gchar *) (list->data));
			g_free(list->data);
			list->data = NULL;
		}
...
{
	DEBUG("Node info : name = %s, quota = %" G_GINT64_FORMAT ", tot_space = %"
		G_GINT64_FORMAT, ((struct vns_info_s *) node->data)->name,
		((struct vns_info_s *) node->data)->quota,
		((struct vns_info_s *) node->data)->total_space_used);
...
check_child(GNode * node, GSList ** writable_list)
{
	...
	while (child) {
		...
		if ((((struct vns_info_s *) child->data)->quota == -1) ||
			(((struct vns_info_s *) child->data)->total_space_used <
				((struct vns_info_s *) child->data)->quota)) {
			DEBUG("Node writable, add to list");
			/* writable, add in list and check its children */
			*writable_list =
				g_slist_prepend(*writable_list,
				g_strdup(((struct vns_info_s *) child->data)->name));
...
build_writable_vns_list(GNode * tree_root)
{
	...
	if ((((struct vns_info_s *) node->data)->quota == -1) ||
		...
		result =
			...
			g_strdup(((struct vns_info_s *) node->data)->name));
...
static void
namespace_info_set_writable_vns(namespace_info_t * ns_info,
	GSList * writable_vns)
{
	...
	for (l = writable_vns; l; l = l->next) {
		g_byte_array_append(vns_gba, (guint8 *) l->data,
			strlen((gchar *) l->data));
		if (l->next)
...
update_virtual_namespace_state(gpointer k, gpointer v, gpointer udata)
{
	...
	if (!v || !((GByteArray *) v)->data)
		...
		new_space_used =
			g_ascii_strtoll((gchar *) ((GByteArray *) v)->data, NULL, 10);

	DEBUG("new space used for vns [%s]: %" G_GINT64_FORMAT, (gchar *) k,
		new_space_used);

	/* search and update matching node */
	GNode *vns_node =
		search_vns_in_tree(conscience->virtual_namespace_tree, (gchar *) k);
	if (!vns_node)
		return;
...
handler_push_virtual_namespace_space_used(struct request_context_s *req_ctx)
{
	...
	if (0 >= key_value_pairs_unmarshall(&list, data, &data_size,
			&(ctx.warning))) {
		GSETCODE(&(ctx.warning), 400, "body unmarshalling error");
		goto errorLabel;
	}
	...
	if (!vns_space_used) {
		GSETERROR(&(ctx.warning),
			"Cannot unserialize the content of the reply");
		goto errorLabel;
	}
...
_brkget_reply_do(struct brkget_s *data, gboolean is_last)
{
	...
	if (data->lines) {
		...
		gba =
			meta2_maintenance_names_marshall(data->lines,
			&(data->reply_ctx.warning));
		if (!gba)
			return FALSE;
		reply_context_clear(&(data->reply_ctx), TRUE);
		reply_context_set_message(&(data->reply_ctx), (is_last ? 200 : 206),
			(is_last ? "OK" : "Partial content"));
		reply_context_set_body(&(data->reply_ctx), gba->data, gba->len,
			REPLYCTX_DESTROY_ON_CLEAN | REPLYCTX_COPY);
		g_byte_array_free(gba, TRUE);
		if (!reply_context_reply(&(data->reply_ctx),
				&(data->reply_ctx.warning)))
			return FALSE;
	}
...
static gboolean
brkget_manage_content(gpointer udata, struct broken_meta2_s *bm2,
...
handler_get_broken_containers(struct request_context_s *req_ctx)
{
	...
	rc = broken_holder_run_elements(conscience->broken_elements, 0,
		(gpointer) & data, brkget_manage_m1, brkget_manage_content);
	conscience_unlock_broken_elements(conscience);
	/* XXX end of critical section */
	...
	reply_context_log_access(&(data.reply_ctx), "NS=%s",
		conscience_get_namespace(conscience));
	reply_context_clear(&(data.reply_ctx), TRUE);
	return rc ? 1 : 0;
}
...
_conscience_srv_serialize(struct conscience_srv_s *srv)
{
	...
	if ((!flag_serialize_srvinfo_stats && !flag_serialize_srvinfo_tags)
		...
		for (u = 0; u < si->tags->len;) {
			...
			tag = si->tags->pdata[u];
			_t = !flag_serialize_srvinfo_tags
				&& g_str_has_prefix(tag->name, "tag.");
			_s = !flag_serialize_srvinfo_stats
				&& g_str_has_prefix(tag->name, "stat.");
			if (_t || _s) {
...
handler_get_service(struct request_context_s *req_ctx)
{
	...
	sg.full =
		(0 < message_get_field(req_ctx->request, "FULL", 4, &data, &data_size,
			NULL));

	/* get the service type's name */
	if (0 >= message_get_field(req_ctx->request, BUFLEN("TYPENAME"), &data,
			&data_size, &(reply_ctx.warning))) {
		GSETCODE(&(reply_ctx.warning), 400,
			"Bad request: no/invalid TYPENAME field");
	}
	else {
		gchar **array_types = buffer_split(data, data_size, ",", 0);

		g_strlcpy(sg.str_ns, conscience_get_namespace(conscience),
			sizeof(sg.str_ns));
	...
	reply_context_log_access(&(reply_ctx), "NS=%s",
		conscience_get_namespace(conscience));
	reply_context_clear(&(reply_ctx), TRUE);
	return rc ? 1 : 0;
}
	...
	gboolean lock_score)
{
	...
	/* XXX start of critical section */
	srvtype =
		conscience_get_locked_srvtype(cs, &error_local, si->type, MODE_STRICT,
		'w');
	if (!srvtype) {
		ERROR("Service type [%s/%s] not found : %s",
			conscience_get_namespace(cs), si->type,
			gerror_get_message(error_local));
		if (error_local)
			g_error_free(error_local);
		return;
	...
	srv =
		conscience_srvtype_get_srv(srvtype,
		(struct conscience_srvid_s *) &(si->addr));
	if (srv) {
	...
	if (conscience_srvtype_refresh(srvtype, &error_local, si, lock_score)) {
		if (srv) {				/* refresh */
			...
			if (lock_score) {
				...
				gboolean bval = FALSE;
				struct service_tag_s *tag =
					service_info_get_tag(si->tags, "tag.up");
		...
		if (srv)
			DEBUG("Service [%s] refreshed with score=%d", str_descr,
				srv->score.value);
		else {
			addr_info_to_string(&(si->addr), str_addr, sizeof(str_addr));
			NOTICE("Service [%s/%s/%s] registered",
				conscience_get_namespace(cs), si->type, str_addr);
		}
	}
	else {
		conscience_release_locked_srvtype(srvtype);
		...
		if (srv)
			WARN("Service [%s] refresh failed : %s", str_descr,
				gerror_get_message(error_local));
		else {
			addr_info_to_string(&(si->addr), str_addr, sizeof(str_addr));
			WARN("Service [%s/%s/%s] registration failed : %s",
				conscience_get_namespace(cs), si->type, str_addr,
				gerror_get_message(error_local));
		}
	}
...
handler_push_service(struct request_context_s *req_ctx)
{
	...
	lock_action =
		(0 < message_get_field(req_ctx->request, "LOCK", sizeof("LOCK") - 1,
			&data, &data_size, NULL));
	...
	if (0 >= service_info_unmarshall(&list_srvinfo, data, &data_size,
			&(ctx.warning))) {
		GSETCODE(&(ctx.warning), 400,
			"Bad request : failed to deserialize the body");
		goto errorLabel;
	}
	...
	reply_context_set_message(&ctx, 200, "OK");
	reply_context_reply(&ctx, NULL);
	reply_context_log_access(&ctx, "NS=%s %d service pushed",
		conscience_get_namespace(conscience), counter);
	reply_context_clear(&ctx, TRUE);
	return 1;
errorLabel:
...
handler_get_services_types(struct request_context_s *req_ctx)
{
	...
	if (!gba_names) {
		...
		ERROR("Failed to reply the service types : %s",
			gerror_get_message(ctx.warning));
		reply_context_reply(&ctx, NULL);
		reply_context_log_access(&ctx, "NS=%s %d names pushed",
			conscience_get_namespace(conscience), counter);
		reply_context_clear(&ctx, TRUE);
		return 0;
	}
...
{
	int str_desc_len;
	gchar str_desc[LIMIT_LENGTH_NSNAME + 1 + LIMIT_LENGTH_SRVTYPE + 1 +
		STRLEN_ADDRINFO + 1];
	GError *error_local;
	...
	if (INFO_ENABLED()) {
		str_desc_len =
			g_snprintf(str_desc, sizeof(str_desc), "%s/%s/",
			conscience_get_namespace(cs), si->type);
		addr_info_to_string(&(si->addr), str_desc + str_desc_len,
	...
	srvtype =
		...
		'w');
	if (!srvtype) {
		ERROR("Service type [%s] not found : %s", str_desc,
			gerror_get_message(error_local));
		if (error_local)
			g_error_free(error_local);
	}
...
handler_rm_service(struct request_context_s *req_ctx)
{
	...
	/*Get the body and unpack it as a list of services */
	if (0 < message_get_field(req_ctx->request, "TYPENAME",
			sizeof("TYPENAME") - 1, &data, &data_size, NULL)) {
		...
		/* XXX start of critical section */
		srvtype =
			conscience_get_locked_srvtype(conscience, &(ctx.warning), str_type,
			MODE_STRICT, 'w');
		if (!srvtype) {
		...
		NOTICE("[NS=%s][SRVTYPE=%s] flush done!",
			conscience_get_namespace(conscience), srvtype->type_name);
		reply_context_set_message(&ctx, 200, "OK");
	}
		...
		if (0 >= service_info_unmarshall(&list_srvinfo, data, &data_size,
				&(ctx.warning))) {
			GSETCODE(&(ctx.warning), 400,
				"Invalid body: deserialization error");
			goto errorLabel;
		}

		NOTICE("[NS=%s] [%d] services to be removed",
			conscience_get_namespace(conscience), g_slist_length(list_srvinfo));
		...
		counter = 0;
		GSETCODE(&(ctx.warning), 400,
			"Bad request : no service in the body, no service type in the fields");
		goto errorLabel;
	}

	reply_context_reply(&ctx, NULL);
	reply_context_log_access(&ctx, "NS=%s %d services removed",
		conscience_get_namespace(conscience), counter);
	reply_context_clear(&ctx, TRUE);
	return 1;
...
static inline void
repctx_add_int_header(struct reply_context_s *ctx, const gchar * name,
	gint value)
{
	GByteArray *gba_code;
	gchar wrk_buf[32];
	...
	gba_code =
		g_byte_array_append(g_byte_array_new(), (const guint8 *) wrk_buf,
...
static inline void
repctx_add_string_header(struct reply_context_s *ctx, const gchar * name,
	const gchar * value)
{
	GByteArray *gba_code =
		g_byte_array_append(g_byte_array_new(), (const guint8 *) value,
		strlen(value));
	reply_context_add_header_in_reply(ctx, name, gba_code);
	g_byte_array_free(gba_code, TRUE);
}
...
static inline gsize
extract_ueid_from_headers(MESSAGE request, GError ** error, gchar * dst,
	gsize dst_size)
{
	void *data;
	gsize data_size;

	if (0 >= message_get_field(request, MSG_HEADER_UEID,
			sizeof(MSG_HEADER_UEID) - 1, &data, &data_size, NULL)) {
...
__manage_event(struct conscience_s *cs, gridcluster_event_t * event,
	GError ** event_error, GError ** error)
{
	...
	if (!gridcluster_event_get_type(event, str_type, sizeof(str_type))) {
		GSETCODE(event_error, CODE_EVT_ERROR_DEF,
			"Invalid event, no TYPE field");
		return TRUE;
	}
	if (0 != fnmatch("broken.*", str_type, 0)) {
		GSETCODE(event_error, CODE_EVT_ERROR_TMP,
			"Invalid event, TYPE not managed (%s)", str_type);
		return TRUE;
	}
	...
	if ((gba_addr = g_hash_table_lookup(event, "ADDR"))) {
		...
		memcpy(str_addr, gba_addr->data, MIN(gba_addr->len + 1,
				sizeof(str_addr)));
		bf.ip = str_addr;
		ptr_port = strchr(str_addr, ':');
		if (!ptr_port) {
			GSETCODE(event_error, CODE_EVT_ERROR_DEF,
				"Invalid event : bad address");
			return TRUE;
		}
		*(ptr_port++) = '\0';
	...
	if ((gba_container = g_hash_table_lookup(event, "CONTAINER")))
		bf.cid =
			(gchar *) g_byte_array_append(gba_container, (const guint8 *) "",
			1)->data;
	if ((gba_path = g_hash_table_lookup(event, "PATH")))
		bf.content =
			(gchar *) g_byte_array_append(gba_path, (const guint8 *) "",
			1)->data;
	if ((gba_cause = g_hash_table_lookup(event, "CAUSE")))
		bf.cause =
			(gchar *) g_byte_array_append(gba_cause, (const guint8 *) "",
			1)->data;
...
handler_event_push(struct request_context_s *req_ctx)
{
	...
	str_ueid_size =
		extract_ueid_from_headers(req_ctx->request, &(ctx.warning), str_ueid,
		sizeof(str_ueid));
	if (!str_ueid_size)
		goto error_label;
	...
	repctx_add_int_header(&ctx, MSG_HEADER_EVENT_STATUS,
		gerror_get_code(error_event));
	repctx_add_string_header(&ctx, MSG_HEADER_EVENT_MESSAGE,
		gerror_get_message(error_event));
	if (error_event)
	...
	reply_context_log_access(&ctx, "NS=%s|UEID=%s",
		conscience_get_namespace(conscience), str_ueid);
	reply_context_clear(&ctx, TRUE);
	return TRUE;
...
handler_get_eventhandler_configuration(struct request_context_s *req_ctx)
{
	...
	gba_config =
		gridcluster_eventhandler_get_configuration(conscience->event_handler,
		&(ctx.warning));
	if (!gba_config) {
		GSETCODE(&(ctx.warning), 500,
			"Failed to get the textual configuration of the eventhandler");
		goto reply_error_label;
	}
	reply_context_set_body(&ctx, gba_config->data, gba_config->len,
...
module_find_handler(gchar * n, gsize l)
{
	...
	static struct cmd_s CMD[] = {
		...
		{NAME_MSGNAME_CS_GET_SRVNAMES, handler_get_services_types,
				&(stats.services.get)},
		{NAME_MSGNAME_CS_PUSH_SRV, handler_push_service,
				&(stats.services.push_stat)},
		{NAME_MSGNAME_CS_RM_SRV, handler_rm_service, &(stats.services.remove)},
		{NAME_MSGNAME_CS_PUSH_BROKEN_CONT, handler_push_broken_containers,
				&(stats.broken.push)},
		{NAME_MSGNAME_CS_GET_BROKEN_CONT, handler_get_broken_containers,
				&(stats.broken.get)},
		{NAME_MSGNAME_CS_RM_BROKEN_CONT, handler_rm_broken_containers,
				&(stats.broken.remove)},
		{NAME_MSGNAME_CS_FIX_BROKEN_CONT, handler_fix_broken_containers,
				&(stats.broken.fix)},
		{NAME_MSGNAME_CS_GET_EVENT_CONFIG,
				handler_get_eventhandler_configuration, &(stats.event.config)},
		{REQ_EVT_PUSH, handler_event_push, &(stats.broken.evt_push)},
		{REQ_EVT_STATUS, handler_event_status, &(stats.broken.evt_status)},
		{NAME_MSGNAME_CS_PUSH_VNS_SPACE_USED,
				handler_push_virtual_namespace_space_used,
				&(stats.services.push_vns)},
		{NULL, NULL, NULL}
	};
...
static inline gboolean
module_check_event_handler(const gchar * cName, const gchar * cfg,
...
static gboolean
module_set_conscience_eventhandlers(struct conscience_s *cs, GError ** err,
	const gchar * raw_path)
{
	...
	if (!g_file_get_contents(path, &data, &size, err)) {
		GSETERROR(err, "[NS=%s] Cannot load the configuration file [%s]",
	...
	if (!module_check_event_handler(cs->ns_info.name, data, size, err)) {
		GSETERROR(err,
			"[NS=%s] Failed to configure an EventHandler with the content of [%s]",
			conscience_get_namespace(cs), path);
		goto error_label;
	}
	...
	if (!cs->event_handler)
		cs->event_handler =
			gridcluster_eventhandler_create(conscience_get_namespace(cs), err,
			NULL, NULL);
	if (data) {
		if (!gridcluster_eventhandler_configure(cs->event_handler, data, size,
				err)) {
			GSETERROR(err,
				"[NS=%s] Failed to configure the event handling rules",
				conscience_get_namespace(cs));
			goto error_label;
		}
	}
	...
	const gchar * type, const gchar * what, const gchar * value)
{
	...
	if (0 == g_ascii_strcasecmp(what, KEY_SCORE_TIMEOUT)) {
		...
		INFO("[NS=%s][SRVTYPE=%s] score expiration interval set to [%ld] seconds", cs->ns_info.name, srvtype->type_name, srvtype->score_expiration);
		return TRUE;
	}
...
{
	g_node_traverse(tree_root, G_IN_ORDER, G_TRAVERSE_ALL, -1, g_free_node_data,
		NULL);
...
_get_vns_quota(gchar * vns_name)
{
	...
	GByteArray *quota_gba =
		g_hash_table_lookup(conscience->ns_info.options, key);
	if (quota_gba)
...
init_vns_in_tree(GNode * tree_root, gchar * vns_name)
{
	...
	void check_new_node_next_slibing(GNode * n)
	{
		...
		do {
			if (g_str_has_prefix(((struct vns_info_s *) tmp->data)->name,
					((struct vns_info_s *) n->data)->name)) {
	...
	while (TRUE) {
		...
		if (g_str_has_prefix(vns_name,
			...
			if (g_str_has_prefix(((struct vns_info_s *) node->data)->name,
					vns_name)) {
				...
				if (node->parent) {
					...
					new_node = g_node_new(vns_info);
					new_node =
						g_node_insert_before(node->parent, node, new_node);
					check_new_node_next_slibing(new_node);
					break;
				}
				else {
					WARN("Virtual Namespace [%s] found in config, but not match with physical namespace [%s]", (gchar *) vns_name, (gchar *) ((struct vns_info_s *) tree_root->data)->name);
					break;
				}
			}
			else {
...
{
	(void) udata;
	DEBUG("Reset total_space_used fo vns %s",
		((struct vns_info_s *) node->data)->name);
...
notify_total_space_used(GNode * node, gpointer udata)
{
	...
	if (n->parent) {
		DEBUG("Updating vns %s space used, new total_space_used = %"
			G_GINT64_FORMAT, ((struct vns_info_s *) n->parent->data)->name,
			((struct vns_info_s *) n->parent->data)->total_space_used);
		((struct vns_info_s *) n->parent->data)->total_space_used +=
			((struct vns_info_s *) n->data)->total_space_used;
	}
	n = NULL;
	return FALSE;
}
...
rebuild_vns_space_used(GNode * tree_root)
{
	...
	g_node_traverse(tree_root, G_IN_ORDER, G_TRAVERSE_NON_LEAVES, -1,
		reset_total_space_used, NULL);
	/* rebuild tot space_used */
	g_node_traverse(tree_root, G_POST_ORDER, G_TRAVERSE_ALL, -1,
		notify_total_space_used, NULL);
}
...
module_reload_vns(struct conscience_s *cs, GError ** err)
{
	...
	root_info->space_used =
		((struct vns_info_s *) cs->virtual_namespace_tree->data)->space_used;
	root_info->total_space_used =
		((struct vns_info_s *) cs->virtual_namespace_tree->data)->
		total_space_used;
	GByteArray *quota_gba =
		g_hash_table_lookup(cs->ns_info.options, cs->ns_info.name);
	if (quota_gba)
	...
	if (vns_gba) {
		...
		if (!buf || !*buf) {
			GSETERROR(err, "[NS=%s] Failed to split virtual namespace list",
				cs->ns_info.name);
			return FALSE;
		}
		...
		while (l && l->data) {
			...
			if (new_node) {
				...
				old_node =
					search_vns_in_tree(cs->virtual_namespace_tree,
					(gchar *) l->data);
				if (old_node) {
					((struct vns_info_s *) new_node->data)->space_used =
						((struct vns_info_s *) old_node->data)->space_used;
					((struct vns_info_s *) new_node->data)->total_space_used =
						((struct vns_info_s *) old_node->data)->
						total_space_used;
				}
				old_node = NULL;
				new_node = NULL;
				...
				WARN("Failed to init node in new tree for VNS [%s]",
					(gchar *) l->data);
			l = l->next;
		}
	...
	INFO("Conscience writable namespace list: [%d] elements",
		g_slist_length(writable_list));
	...
	if (value_regex == NULL) {
		...
		if (valuelist == NULL) {
			...
			while (g_hash_table_iter_next(&params_iterator, &k, &v)) {
				...
				if (!g_regex_match(value_regex, (gchar *) k,
						G_REGEX_MATCH_NOTEMPTY, &match_info))
					TRACE("Parameter skipped : [%s] (does not match %s)",
						(gchar *) k, g_regex_get_pattern(value_regex));
					...
					if (str_opt != NULL)
						g_hash_table_insert(valuelist, str_opt,
							g_byte_array_append(g_byte_array_new(), v,
								strlen(v) + 1));
				}
...
static GError *
fill_hashtable_with_group(GHashTable * ht, GKeyFile * conf_file,
	...
	if (NULL != keys) {
		for (uint i = 0; i < g_strv_length(keys); i++) {
			...
			if (NULL != g_hash_table_lookup(ht, keys[i])) {
				WARN("Duplicate key [%s] in hashtable for group [%s].  The value [%s] will be associated to this key.", keys[i], group_name, v);
			}
			g_hash_table_insert(ht, g_strdup(keys[i]),
				metautils_gba_from_string(v));
		}
		g_strfreev(keys);
		return e;
	}
...
static GError *
module_init_storage_conf(struct conscience_s *cs,
	const gchar * stg_pol_in_option, const gchar * filepath)
{
	GKeyFile *stg_conf_file = g_key_file_new();
	GError *e = NULL;

	if (!filepath
		|| !g_key_file_load_from_file(stg_conf_file, filepath, G_KEY_FILE_NONE,
			&e)) {
		if (NULL != stg_pol_in_option) {
			GSETERROR(&e, "Cannot parse storage configuration from file [%s]",
				filepath);
		}
		else {
			INFO("[NS=%s] storage conf init failed -> ignored as no storage_policy was found as an option", cs->ns_info.name);
		}
		g_key_file_free(stg_conf_file);
		return e;
	}

	cs->ns_info.storage_policy =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_unref);
	e = fill_hashtable_with_group(cs->ns_info.storage_policy, stg_conf_file,
		NAME_GROUPNAME_STORAGE_POLICY);
	if (NULL != e) {
		g_prefix_error(&e,
			"Error collecting storage policy rules from file [%s]", filepath);
		g_key_file_free(stg_conf_file);
		return e;
	}

	/* If the storage policy set by param_option.storage_policy is not present in the storage conf file, set an error. */
	if (stg_pol_in_option
		&& !g_hash_table_lookup(cs->ns_info.storage_policy,
			stg_pol_in_option)) {
		GSETERROR(&e,
			"[NS=%s] storage conf init failed: the policy [%s] wanted as an option is not defined in [%s]",
			cs->ns_info.name, stg_pol_in_option, filepath);
		g_key_file_free(stg_conf_file);
		return e;
	}

	cs->ns_info.data_security =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_unref);
	e = fill_hashtable_with_group(cs->ns_info.data_security, stg_conf_file,
		NAME_GROUPNAME_DATA_SECURITY);
	if (NULL != e) {
		WARN("Data security rules not correctly loaded from file [%s] : %s",
			filepath, e->message);
		g_clear_error(&e);
		e = NULL;
	}

	cs->ns_info.data_treatments =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_unref);
	e = fill_hashtable_with_group(cs->ns_info.data_treatments, stg_conf_file,
		NAME_GROUPNAME_DATA_TREATMENTS);
	if (NULL != e) {
		WARN("Data treatments rules not correctly loaded from file [%s] : %s",
			filepath, e->message);
		g_clear_error(&e);
		e = NULL;
	}

	cs->ns_info.storage_class =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		metautils_gba_unref);
	e = fill_hashtable_with_group(cs->ns_info.storage_class, stg_conf_file,
		NAME_GROUPNAME_STORAGE_CLASS);
	if (NULL != e) {
		WARN("Storage class rules not correctly loaded from file [%s] : %s",
			filepath, e->message);
		g_clear_error(&e);
		e = NULL;
	}
...
static gboolean
module_init_srvtype_from_cfg(struct conscience_s *cs, GHashTable * params,
	GError ** err)
{
	gpointer k = NULL, v = NULL;
	GHashTableIter params_iterator;
	...
	service_param_regex =
		g_regex_new("service\\.([^.]+)\\.(.+)", G_REGEX_CASELESS,
...
module_init_meta0(struct conscience_s *cs, GHashTable * params, GError ** err)
{
	...
	srv =
		conscience_srvtype_register_srv(conscience_get_srvtype(cs, err,
			NAME_SRVTYPE_META0, MODE_STRICT), err, &srvid);
	if (!srv) {
	...
	NOTICE("[NS=%s][SRVTYPE=%s] new locked META0 service at [%s]",
		cs->ns_info.name, NAME_SRVTYPE_META0, str);
	return TRUE;
}
...
static gboolean
module_init_known_service_types(struct conscience_s *cs, GHashTable * params,
	GError ** err)
{
	...
	for (type = types_to_init; type->name && type->expr; type++) {
		...
		if (!config) {
			GSETERROR(err,
				"[NS=%s][SRVTYPE=%s] Failed to init the service type",
				cs->ns_info.name, type->name);
			return FALSE;
		}

		conscience_srvtype_init(config);
		if (!conscience_srvtype_set_type_expression(config, err, type->expr)) {
			GSETERROR(err, "[NS=%s][SRVTYPE=%s] Failed to set expr=[%s]",
				cs->ns_info.name, type->name, type->expr);
			conscience_srvtype_destroy(config);
			return FALSE;
		}

		config->alert_frequency_limit = time_default_alert_frequency;
		NOTICE("[NS=%s][SRVTYPE=%s] service type init done", cs->ns_info.name,
			type->name);
	}
...
plugin_init(GHashTable * params, GError ** err)
{
	...
	if (!(str = g_hash_table_lookup(params, KEY_NAMESPACE))) {
		GSETERROR(err,
			"The configuration must contain a '%s' key with the namespace name",
			KEY_NAMESPACE);
		return -1;
	}
	...
	NOTICE("[NS=%s] Chunk size set to %" G_GINT64_FORMAT,
		conscience->ns_info.name, conscience->ns_info.chunk_size);

	/* Serialization optimizations */
	str = g_hash_table_lookup(params, KEY_SERIALIZE_SRVINFO_CACHED);
	if (NULL != str)
		flag_serialize_srvinfo_cache =
			metautils_cfg_get_bool(str, DEF_SERIALIZE_SRVINFO_CACHED);
	NOTICE("[NS=%s] Cache for serialized service_info  [%s]",
	...
	if (NULL != str)
		flag_serialize_srvinfo_tags =
			metautils_cfg_get_bool(str, DEF_SERIALIZE_SRVINFO_TAGS);
	NOTICE("[NS=%s] Tags in serialized service_info  [%s]",
		conscience->ns_info.name,
		(flag_serialize_srvinfo_tags ? "ENABLED" : "DISABLED"));

	str = g_hash_table_lookup(params, KEY_SERIALIZE_SRVINFO_STATS);
	if (NULL != str)
		flag_serialize_srvinfo_stats =
			metautils_cfg_get_bool(str, DEF_SERIALIZE_SRVINFO_STATS);
	NOTICE("[NS=%s] Stats in serialized service_info  [%s]",
		conscience->ns_info.name,
	...
	if (!(str = g_hash_table_lookup(params, KEY_ALERT_LIMIT)))
		NOTICE
			("[NS=%s] No overall alert_frequency_limit set, default kept to [%ld] seconds",
			conscience->ns_info.name, time_default_alert_frequency);
	else {
		time_default_alert_frequency = g_ascii_strtoll(str, NULL, 10);
		NOTICE("[NS=%s] Overall alert_frequency_limit set to [%ld] seconds",
			conscience->ns_info.name, time_default_alert_frequency);
	}
	...
	*err =
		module_init_storage_conf(conscience,
		namespace_storage_policy(&conscience->ns_info,
			conscience->ns_info.name), g_hash_table_lookup(params,
			KEY_STG_CONF));
	if (NULL != *err) {
	...
	if (!module_init_vns(conscience, err)) {
		GSETERROR(err, "[NS=%s] virtual namespaces init failed",
			conscience->ns_info.name);
		goto error;
	}
	...
	if (!module_init_known_service_types(conscience, params, err)) {
		GSETERROR(err, "[NS=%s] known service types init failed",
			conscience->ns_info.name);
		goto error;
	}

	if (!module_init_meta0(conscience, params, err)) {
		GSETERROR(err, "[NS=%s] Failed to register a locked meta0 service",
			conscience->ns_info.name);
		goto error;
	}
	...
	if (!(str = g_hash_table_lookup(params, KEY_EVENT_HANDLERS)))
		...
		GSETERROR(err, "[NS=%s] failed to loaded the event handler from [%s]",
			conscience_get_namespace(conscience), str);
		goto error;
	}
	else
		NOTICE("[NS=%s] event handlers successfully loaded from [%s]",
			conscience->ns_info.name, str);

	/* Plugin/server stuff */
	if (!srvtimer_register_regular("conscience.expire", timer_expire_services,
			NULL, conscience, 5LL)) {
		GSETERROR(err, "Failed to register the conscience's dump callback");
		goto error;
	}
	if (!srvtimer_register_regular("conscience.check", timer_check_services,
			NULL, conscience, 59LL)) {
		GSETERROR(err, "Failed to register the conscience's dump callback");
		goto error;
	}

	if (!srvtimer_register_regular("conscience.stats", save_counters, NULL,
			NULL, 5LL)) {
...
{
	(void) udata;
	NOTICE("options k = [%s] | v = [%s]", (gchar *) k,
...
plugin_reload(GHashTable * params, GError ** err)
{
	...
	*err =
	...
	if (!module_reload_vns(conscience, err)) {
		GSETERROR(err, "[NS=%s] virtual namespaces reload failed",
			conscience->ns_info.name);
		goto error;
	}
	...
	if (!(str = g_hash_table_lookup(params, KEY_EVENT_HANDLERS))) {
		...
		NOTICE("[NS=%s] event handlers successfully reloaded from [%s]",
			conscience->ns_info.name, str);
