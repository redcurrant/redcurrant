...
static gboolean
namespace_info_handler(GError ** error, gpointer udata, gint code,
...
static gboolean
namespace_info_full_handler(GError ** error, gpointer udata, gint code,
	guint8 * body, gsize bodySize)
{
	namespace_info_t **ns_info = (namespace_info_t **) udata;
...
static gboolean
container_list_content_handler(GError ** error, gpointer udata, gint code,
	guint8 * body, gsize bodySize)
{
	...
	list_from_body =
		meta2_maintenance_names_unmarshall_buffer(body, bodySize, error);
	if (!list_from_body) {
		GSETERROR(error,
			"Cannot unmarshall body of message as broken containers");
		return (FALSE);
	}
...
meta0_info_t *
gcluster_get_meta0_2timeouts(addr_info_t * addr, long to_cnx, long to_req,
	GError ** error)
{
	meta0_info_t *m0;
	GSList *result;
	...
	m0 = NULL;
	if (result) {
		m0 = service_info_convert_to_m0info(g_slist_nth_data(result,
				rand() % g_slist_length(result)));
...
gcluster_get_namespace_info(addr_info_t * addr, long timeout, GError ** error)
{
	...
	/*reads the answers */
	if (!metaXClient_reply_sequence_run_from_addrinfo(error, req, addr, timeout,
			&data)) {
		GSETERROR(error,
			"Cannot execute the query %s and receive all the responses",
			NAME_MSGNAME_CS_GETNS);
		goto error_reply;
	}
...
namespace_info_t *
gcluster_get_namespace_info_full(addr_info_t * addr, long timeout,
	GError ** error)
{
	static struct code_handler_s codes[] = {
	...
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
			NAME_MSGNAME_CS_GET_NSINFO);
		goto error_buildreq;
	}

	(void) message_add_field(req, "VERSION", 7,
		SHORT_API_VERSION, sizeof(SHORT_API_VERSION) - 1, NULL);
...
gint
gcluster_push_broken_container(addr_info_t * addr, long timeout,
	GSList * container_list, GError ** error)
{
	...
	req =
		build_request(NAME_MSGNAME_CS_PUSH_BROKEN_CONT, buf->data, buf->len,
		error);
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
...
gint
gcluster_push_virtual_ns_space_used(addr_info_t * addr, long timeout,
	GHashTable * space_used, GError ** error)
{
	static struct code_handler_s codes[] = {
		{200, REPSEQ_FINAL, NULL, NULL},
		{0, 0, NULL, NULL}
	...
	req =
		build_request(NAME_MSGNAME_CS_PUSH_VNS_SPACE_USED, buf->data, buf->len,
		error);
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
...
gint
gcluster_v2_push_broken_container(struct metacnx_ctx_s * cnx,
	const gchar * ns_name, const container_id_t cid, GError ** error)
{
	...
	gridcluster_event_add_buffer(event, "CONTAINER", cid,
		sizeof(container_id_t));
	gridcluster_event_add_string(event, "NAMESPACE", ns_name);
	g_snprintf(str_ueid, sizeof(str_ueid), "%ld_%d_%p", random(), getpid(),
		&str_ueid);
	rc = gridcluster_push_event(cnx, str_ueid, event, &event_error, error);
	gridcluster_destroy_event(event);
	if (!rc) {
		GSETERROR(error,
			"Failed to push the broken container event with UEID=%s", str_ueid);
		if (event_error)
			g_error_free(event_error);
		return FALSE;
	}
	if (!event_error) {
		GSETERROR(error, "No status received for the event with UEID=%s",
			str_ueid);
		return FALSE;
	}
	g_error_free(event_error);
...
gint
gcluster_v2_push_broken_content(struct metacnx_ctx_s * cnx,
...
gint
gcluster_fix_broken_container(addr_info_t * addr, long timeout,
	GSList * container_list, GError ** error)
{
	static struct code_handler_s codes[] = {
		{200, REPSEQ_FINAL, NULL, NULL},
	...
	req =
		build_request(NAME_MSGNAME_CS_FIX_BROKEN_CONT, buf->data, buf->len,
		error);
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
...
gint
gcluster_rm_broken_container(addr_info_t * addr, long timeout,
	GSList * container_list, GError ** error)
{
	static struct code_handler_s codes[] = {
		{200, REPSEQ_FINAL, NULL, NULL},
	...
	req =
		build_request(NAME_MSGNAME_CS_RM_BROKEN_CONT, buf->data, buf->len,
		error);
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
	...
	GError ** error, gboolean full)
{
	...
	result = service_info_sequence_request(ctx, error,
		NAME_MSGNAME_CS_GET_SRV, NULL, "TYPENAME", gba0, "FULL", gba1, NULL);
...
GSList *
gcluster_get_services(addr_info_t * addr, long timeout, const gchar * type,
	GError ** error)
{
...
gint
gcluster_push_services(addr_info_t * addr, long timeout, GSList * services_list,
	gboolean lock_action, GError ** error)
{
	static struct code_handler_s codes[] = {
		{200, REPSEQ_FINAL, NULL, NULL},
		{0, 0, NULL, NULL}
	...
	if (req == NULL) {
		GSETERROR(error, "Failed to build request %s",
			NAME_MSGNAME_CS_PUSH_SRV);
		goto error_buildreq;
	}

	if (lock_action)
		message_add_field(req, "LOCK", sizeof("LOCK") - 1, "true",
			sizeof("true") - 1, NULL);
...
static gboolean
body_to_gba_handler(GError ** error, gpointer udata, gint code, guint8 * body,
	gsize body_size)
{
...
GByteArray *
gcluster_get_srvtype_event_config(addr_info_t * addr, long to, gchar * name,
	GError ** error)
{
	static struct code_handler_s codes[] = {
	...
	if (req == NULL) {
		GSETERROR(error,
			"Failed to build request " NAME_MSGNAME_CS_GET_EVENT_CONFIG);
		return NULL;
	}

	if (!message_add_field(req, "TYPENAME", sizeof("TYPENAME") - 1, name,
			strlen(name), NULL)) {
		GSETERROR(error,
			"Failed to add the service typ name in the request headers");
		message_destroy(req, NULL);
		return NULL;
	}

	result = g_byte_array_new();
	if (!metaXClient_reply_sequence_run_from_addrinfo(error, req, addr, to,
			&data)) {
		GSETERROR(error,
			"Cannot execute the query " NAME_MSGNAME_CS_GET_EVENT_CONFIG
			" and receive all the responses");
