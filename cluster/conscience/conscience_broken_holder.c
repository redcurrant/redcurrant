...
conscience_create_broken_holder(struct conscience_s *conscience)
{
	...
	bh->ht_meta2 =
		g_hash_table_new_full(addr_info_hash, addr_info_equal, NULL,
		free_broken_m2);
	bh->ht_meta1 =
		g_hash_table_new_full(addr_info_hash, addr_info_equal, NULL, g_free);
...
gchar *
broken_holder_write_content(struct broken_meta2_s *m2,
	struct broken_content_s *c)
{
	gsize s;
	gchar buffer[2048];
	...
	if (c->content_path[0] && c->cause[0])
		g_snprintf(buffer + s, sizeof(buffer) - s, ":%s:%s:%s", c->container_id,
			c->content_path, c->cause);
	else if (c->content_path[0])
		g_snprintf(buffer + s, sizeof(buffer) - s, ":%s:%s:", c->container_id,
			c->content_path);
	else
