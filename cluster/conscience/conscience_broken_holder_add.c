...
static gboolean remove_container(gpointer key, gpointer value,
	gpointer user_data);
...
remove_container(gpointer key, gpointer value, gpointer user_data)
{
	...
	return 0 == g_ascii_strncasecmp(brk_content->container_id, str_cid,
		sizeof(brk_content->container_id));
}
...
create_broken_meta1(addr_info_t * addr)
{
	...
	writen_size = g_strlcpy(brk_m1->string, "META1:", sizeof(brk_m1->string));
	addr_info_to_string(&(brk_m1->addr),
		brk_m1->string + writen_size, sizeof(brk_m1->string) - writen_size);
...
create_broken_content(struct conscience_s *conscience, const gchar * cid,
	const gchar * path, const gchar * cause)
{
	...
	g_strlcpy(brk_content->container_id, cid,
		sizeof(brk_content->container_id));
	if (path) {
		g_strlcpy(brk_content->content_path, path,
			sizeof(brk_content->content_path));
		if (cause)
			g_strlcpy(brk_content->cause, cause, sizeof(brk_content->cause));
	}

	srvtype =
		conscience_get_locked_srvtype(conscience, NULL, "rawx", MODE_STRICT,
		'r');
	if (srvtype) {
...
broken_holder_add_meta1(struct broken_holder_s *bh, struct broken_fields_s *bf)
{
	...
	if (!g_hash_table_lookup(bh->ht_meta1, addr)) {
		...
		do {
			...
			srvtype =
				conscience_get_locked_srvtype(bh->conscience, NULL, "meta2",
				MODE_STRICT, 'r');
			if (srvtype) {
				brk_meta1->counter =
					conscience_srvtype_count_srv(srvtype, FALSE);
				conscience_release_locked_srvtype(srvtype);
			}
		} while (0);
	...
	struct broken_meta2_s *brk_m2, struct broken_fields_s *bf)
{
	...
	if (bf->content) {
		...
		if (brk_content) {
			DEBUG("Content yet broken [%s:%d/%s/%s]", bf->ip, bf->port, bf->cid,
				bf->content);
			return;
		}
		brk_content =
			create_broken_content(bh->conscience, bf->cid, bf->content,
			bf->cause);
	}
	else {
		/*remove all the content entries */
		g_hash_table_foreach_remove(brk_m2->broken_containers, remove_container,
			bf->cid);
		brk_content =
			create_broken_content(bh->conscience, bf->cid, NULL, NULL);
	}

	g_hash_table_insert(brk_m2->broken_containers, g_strdup(key), brk_content);

	DEBUG("Container now broken [%s:%d/%s(%s(/%s))]", bf->ip, bf->port,
		brk_content->container_id, brk_content->content_path,
		brk_content->cause);
}

void
broken_holder_add_in_meta2(struct broken_holder_s *bh,
	struct broken_fields_s *bf)
{
	...
	do {
		...
		if (!wrk_addr) {
			WARN("Invalid META2 address [%s:%d] : %s", bf->ip, bf->port,
	...
	if (brk_m2->totally_broken) {
		DEBUG("META2 broken [%s:%d], element not saved [%s]", bf->ip, bf->port,
			bf->cid);
		return;
	}
