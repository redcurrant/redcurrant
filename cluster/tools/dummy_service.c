		...
		for (l = list_reginfo; l; l = l->next) {
			...
			if (ri->port <= 0) {
				if (!register_namespace_service(ri->si, &err))
					GRID_ERROR("[%s] Registration failed : %s",
				...
				if (fd < 0)
					GRID_WARN("[%s] Connection error with : %s",
						ri->original_str, gerror_get_message(err));
				else {
					do {
					...
					g_byte_array_prepend(gba, (guint8 *) MSG_SRV_PSH,
						sizeof(MSG_SRV_PSH) - 1);
					g_byte_array_prepend(gba, (guint8 *) & (gba->len), 4);

					if (0 > sock_to_write(fd, 2000, gba->data, gba->len, &err))
						GRID_WARN("[%s] Write error with : %s",
							ri->original_str, gerror_get_message(err));
					else {
						s = 0;
						for (;;) {
							...
							rs = sock_to_read(fd, 2000, buffer, sizeof(buffer),
								&err);
							if (rs == -1) {
							...
							if (status == 200U) {
								GRID_DEBUG
									("[%s] registration OK (status=%d msg=%.*s)",
									ri->original_str, status, (int) (rs - 8),
									buffer + 8);
								break;
							}
							GRID_WARN
								("[%s] registration failed, %d bytes received from fd=%d (status=%d msg=%.*s)",
								ri->original_str, (int) rs, fd, status,
								(int) (rs - 8), buffer + 8);
						}
						if (!s)
							GRID_WARN("[%s] No reply : %s", ri->original_str,
...
load_config(int argc, char **args)
{
	...
	error_local = NULL;
	regex =
		g_regex_new("((unix)|(tcp:(.+))):(.+)\\|(.+)\\|(.+:.+)", 0, 0,
		&error_local);
	if (!regex) {
		GRID_ERROR
			("Fatal error : wrong regex for internal arguments parsing : %s",
	...
	for (i = 1; i < argc; i++) {
		...
		if (!g_regex_match(regex, args[i], 0, &match_info)) {
			...
			if (!l4_address_init_with_url(&(si->addr), str_url, &error_local)) {
				GRID_ERROR("Failed to build the address : %s",
					gerror_get_message(error_local));
				abort();
			}
			...
			service_tag_set_value_macro(service_info_ensure_tag(si->tags,
					NAME_MACRO_SPACE_NAME), NAME_MACRO_SPACE_TYPE, "/");
...
main(int argc, char **argv)
{
	...
	GRID_DEBUG("Usage: %s ((unix|tcp:<PORT>):<NS>|<SRVTYPE>|<IP>:<PORT>)...",
		argv[0]);
