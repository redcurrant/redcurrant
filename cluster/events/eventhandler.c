...
gridcluster_eventhandler_create(const gchar * ns_name, GError ** err,
	gpointer udata, struct gridcluster_event_hooks_s *hooks)
{
	...
	if (hooks)
		memcpy(&(result->hooks), hooks,
			sizeof(struct gridcluster_event_hooks_s));

	return result;
}

void
gridcluster_eventhandler_destroy(gridcluster_event_handler_t * h,
...
gridcluster_event_execute_DROP(struct gridcluster_execution_context_s *ctx,
	GError ** err)
{
	ctx->running = FALSE;
	if (!ctx->hooks->on_exit)
		DEBUG("No 'on_drop' hook configured");
	else if (!ctx->hooks->on_drop(ctx->event, ctx->handler->user_data,
			ctx->edata, err)) {
...
static inline gboolean
gridcluster_event_execute_EXIT(struct gridcluster_execution_context_s *ctx,
	GError ** err)
{
	ctx->running = FALSE;
	if (!ctx->hooks->on_exit)
		DEBUG("No 'on_exit' hook configured");
	else if (!ctx->hooks->on_exit(ctx->event, ctx->handler->user_data,
			ctx->edata, err)) {
...
static inline gboolean
gridcluster_event_execute_ADDRESS(const addr_info_t * addr,
	struct gridcluster_execution_context_s *ctx, GError ** err)
{
	if (!ctx->hooks->on_address)
		INFO("Event dropped (no forward_to_address handler)");
	else if (!ctx->hooks->on_address(ctx->event, ctx->handler->user_data,
			ctx->edata, err, addr)) {
		gsize str_size;
		gchar str[STRLEN_ADDRINFO];

		str_size = addr_info_to_string(addr, str, sizeof(str));
		GSETERROR(err, "Failed to forward the event to the address [%.*s]",
			str_size, str);
		return FALSE;
	}
	return TRUE;
...
static inline gboolean
gridcluster_event_execute_SERVICE(const gchar * srv_name,
	struct gridcluster_execution_context_s *ctx, GError ** err)
{
	if (!ctx->hooks->on_service)
		INFO("Event dropped (no forward_to_service handler)");
	else if (!ctx->hooks->on_service(ctx->event, ctx->handler->user_data,
			ctx->edata, err, srv_name)) {
		GSETERROR(err, "Failed to forward the event to the service [%s]",
			srv_name);
		return FALSE;
	}
	return TRUE;
...
gboolean
gridcluster_manage_event_no_defaults(gridcluster_event_handler_t * handler,
	...
	for (pRule = handler->ruleset; ctx.running && (rule = *pRule); pRule++) {
		if (!rule->pattern)
			...
			if (DEBUG_ENABLED()) {
				...
				for (pAction = rule->actions;
					ctx.running && (action = *pAction); pAction++) {
					switch (action->type) {
						case GCEAT_DROP:
							g_byte_array_append(gba_message, (guint8 *) ",drop",
								(guint) sizeof(",drop") - 1);
							break;
						case GCEAT_EXIT:
							g_byte_array_append(gba_message, (guint8 *) ",exit",
								(guint) sizeof(",exit") - 1);
							break;
						case GCEAT_ADDRESS:
							g_byte_array_append(gba_message,
								(guint8 *) ",address",
								(guint) sizeof(",address") - 1);
							break;
						case GCEAT_SERVICE:
							g_byte_array_append(gba_message,
								(guint8 *) ",service",
								(guint) sizeof(",service") - 1);
							break;
						default:
							break;
					}
				}
				DEBUG("Rule: %s = %.*s", rule->pattern, gba_message->len,
					gba_message->data);
				g_byte_array_free(gba_message, TRUE);
			}

			if (0 != fnmatch(rule->pattern, event_type, FNM_CASEFOLD))
				TRACE("Event type '%s' does not match pattern '%s'", event_type,
					rule->pattern);
			else {
				TRACE("Event type '%s' matches pattern '%s'", event_type,
					rule->pattern);
				...
				for (pAction = rule->actions;
					...
					switch (action->type) {
						...
						case GCEAT_SERVICE:
							if (!gridcluster_event_execute_SERVICE(action->
									parameter.service, &ctx, err)) {
								GSETERROR(err,
									"'address' action execution failure");
								return FALSE;
							}
							break;
						case GCEAT_ADDRESS:
							if (!gridcluster_event_execute_ADDRESS(&(action->
										parameter.address), &ctx, err)) {
								GSETERROR(err,
									"'service' action execution failure");
								return FALSE;
							}
							break;
						default:
							GSETERROR(err,
								"EventAction with type [%d] not implemented",
								action->type);
							return FALSE;
					}
				}
...
gboolean
gridcluster_manage_event(gridcluster_event_handler_t * h,
	gridcluster_event_t * e, gpointer edata, GError ** err)
{
	if (!h || !e) {
...
GByteArray *
gridcluster_eventhandler_get_configuration(gridcluster_event_handler_t * h,
	GError ** err)
{
	...
	if (h->ruleset) {
		...
		for (rule_pointer = h->ruleset; *rule_pointer; rule_pointer++) {
			...
			rule = *rule_pointer;
			g_byte_array_append(gba_config, (guint8 *) rule->pattern,
				strlen(rule->pattern));
			g_byte_array_append(gba_config, (guint8 *) "=", 1);
			for (action_pointer = rule->actions;
				action_pointer && *action_pointer; action_pointer++) {
				...
				switch (action->type) {
					case GCEAT_EXIT:
						g_byte_array_append(gba_config, (guint8 *) "exit",
							sizeof("exit") - 1);
						break;
					case GCEAT_DROP:
						g_byte_array_append(gba_config, (guint8 *) "drop",
							sizeof("drop") - 1);
						break;
					case GCEAT_SERVICE:
						g_byte_array_append(gba_config, (guint8 *) "service ",
							sizeof("service ") - 1);
						g_byte_array_append(gba_config,
							(guint8 *) action->parameter.service,
							strlen(action->parameter.service));
						break;
					case GCEAT_ADDRESS:
						g_byte_array_append(gba_config, (guint8 *) "address ",
							sizeof("address ") - 1);
						buffer_size =
							addr_info_to_string(&(action->parameter.address),
							buffer, sizeof(buffer));
						g_byte_array_append(gba_config, (guint8 *) buffer,
							buffer_size);
						break;
				}
			}
...
GSList *
gridcluster_eventhandler_get_patterns(gridcluster_event_handler_t * h,
	GError ** err)
{
...
gboolean
gridcluster_eventhandler_match_wordslist(GSList * patterns,
...
gboolean
gridcluster_eventhandler_match_wordarray(GSList * patterns,
