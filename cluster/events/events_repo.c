 ...
 */
GError *
gridcluster_event_SaveNewEvent(struct event_config_s * evt_config,
	gridcluster_event_t * evt)
{
	...
	gchar *str_ueid =
		gridcluster_event_get_string(evt, GRIDCLUSTER_EVTFIELD_UEID);
	gchar *str_aggrname =
		gridcluster_event_get_string(evt, GRIDCLUSTER_EVTFIELD_AGGRNAME);
	gchar *str_cid =
		gridcluster_event_get_string(evt, GRIDCLUSTER_EVTFIELD_CID);
...
retry:
	...
	if (0 > (fd = g_mkstemp(tmppath))) {
		...
		if (!err) {
			...
			g_snprintf(str_seq, sizeof(str_seq), "%" G_GINT64_FORMAT,
				get_id64(evt_config));
			if ((-1 == fsetxattr(fd, GRIDCLUSTER_EVENT_XATTR_TIME, str_now,
						strlen(str_now), 0))
				|| (-1 == fsetxattr(fd, GRIDCLUSTER_EVENT_XATTR_SEQ, str_seq,
						strlen(str_seq), 0))
				|| (-1 == fsetxattr(fd, GRIDCLUSTER_EVENT_XATTR_CID, str_cid,
						strlen(str_cid), 0))) {
				err =
					NEWERROR(500, "Event error (setxattr) : errno=%d (%s)",
					errno, strerror(errno));
			}
			GRID_INFO("xattr ok");
		}
...
int
gridcluster_eventxattr_get_container_id(const gchar * path, container_id_t * id,
	gchar * str, gsize str_len)
{
	gssize len;
