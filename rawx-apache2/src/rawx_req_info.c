...
typedef const char *(*generator_f) (const dav_resource * resource,
	apr_pool_t * pool);
...
__gen_stats(const dav_resource * resource, apr_pool_t * pool)
{
	...
	apr_uint64_t reqavgtime =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_duration), 4);
	apr_uint64_t req_put =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_req_put_sec), 4);
	apr_uint64_t reqavgtime_put =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_put_duration), 4);
	apr_uint64_t req_get =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_req_get_sec), 4);
	apr_uint64_t reqavgtime_get =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_get_duration), 4);
	apr_uint64_t req_del =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_req_del_sec), 4);
	apr_uint64_t reqavgtime_del =
		rawx_stats_rrd_get_delta(&(stats->body.rrd_del_duration), 4);
	...
	return apr_pstrcat(pool,
		...
		apr_psprintf(pool, "rawx.avputreqtime %" APR_UINT64_T_FMT "\n",
			r_put_time), apr_psprintf(pool, "rawx.reqgetpersec %f\n",
			r_get_rate), apr_psprintf(pool,
			"rawx.avgetreqtime %" APR_UINT64_T_FMT "\n", r_get_time),
...
static dav_resource *
__build_req_resource(const request_rec * r, const dav_hooks_repository * hooks,
	generator_f gen)
{
...
dav_error *
dav_rawx_stat_get_resource(request_rec * r, const char *root_dir,
	const char *label, int use_checked_in, dav_resource ** result_resource)
{
	...
	*result_resource =
		__build_req_resource(r, &dav_hooks_repository_rawxstat, __gen_stats);
	(*result_resource)->info->type = STAT;
	return NULL;
}

dav_error *
dav_rawx_info_get_resource(request_rec * r, const char *root_dir,
	const char *label, int use_checked_in, dav_resource ** result_resource)
{
	(void) root_dir;
	(void) label;
	...
	*result_resource =
		__build_req_resource(r, &dav_hooks_repository_rawxinfo, __gen_info);
...
dav_rawx_deliver_SPECIAL(const dav_resource * resource, ap_filter_t * output)
{
	...
	if (resource->type != DAV_RESOURCE_TYPE_PRIVATE)
		return
			...
			pool, HTTP_CONFLICT, 0, apr_pstrdup(pool,
				"Cannot GET this type of resource."));
	if (resource->collection)
		return
			server_create_and_stat_error(resource_get_server_config(resource),
			pool, HTTP_CONFLICT, 0, apr_pstrdup(pool, "No GET on collections"));
	...
	bkt =
		apr_bucket_heap_create(result, result_len, NULL,
	...
	if ((status = ap_pass_brigade(output, bb)) != APR_SUCCESS)
		return
			server_create_and_stat_error(resource_get_server_config(resource),
			pool, HTTP_FORBIDDEN, 0, apr_pstrdup(pool,
	...
	switch (resource->info->type) {
		case STAT:
			server_inc_request_stat(resource_get_server_config(resource),
				RAWX_STATNAME_REQ_STAT,
				request_get_duration(resource->info->request));
			break;
		case INFO:
			server_inc_request_stat(resource_get_server_config(resource),
				RAWX_STATNAME_REQ_INFO,
				request_get_duration(resource->info->request));
			break;
		default:
			break;
