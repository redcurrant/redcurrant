...
static dav_error *
dav_propdb_open(apr_pool_t * pool, const dav_resource * resource, int ro,
	dav_db ** pdb)
{
	dav_db *db;
...
dav_propdb_define_namespaces(dav_db * db, dav_xmlns_info * xi)
{
	(void) xi;
	/* XXX JFS : TODO */
	return __dav_new_error(db->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
		"not yet implemented");
}

static dav_error *
dav_propdb_output_value(dav_db * db, const dav_prop_name * name,
	dav_xmlns_info * xi, apr_text_header * phdr, int *found)
{
	(void) db;
	(void) name;
	(void) xi;
...
static dav_error *
dav_propdb_map_namespaces(dav_db * db, const apr_array_header_t * namespaces,
...
static dav_error *
dav_propdb_store(dav_db * db, const dav_prop_name * name,
...
static dav_error *
dav_propdb_get_rollback(dav_db * db, const dav_prop_name * name,
