...
static const char *
dav_rawx_cmd_gridconfig_hash_width(cmd_parms * cmd, void *config,
	const char *arg1)
{
...
static const char *
dav_rawx_cmd_gridconfig_hash_depth(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rawx_server_conf *conf;
...
dav_rawx_cmd_gridconfig_docroot(cmd_parms * cmd, void *config, const char *arg1)
{
	...
	do {
		apr_status_t status =
			apr_stat(&(finfo), arg1, APR_FINFO_NORM, cmd->pool);
		...
		if (finfo.filetype != APR_DIR) {
			...
			return apr_pstrcat(cmd->temp_pool,
				"Docroot for GridStorage chunks must be a directory: ", arg1,
				NULL);
		}
	} while (0);
...
static const char *
dav_rawx_cmd_gridconfig_namespace(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rawx_server_conf *conf;
	...
	if (!ns_info) {
		...
		return apr_pstrcat(cmd->temp_pool,
			"Failed to get namespace info from ns: ", conf->ns_name, NULL);
	}
...
static const char *
dav_rawx_cmd_gridconfig_fsync_dir(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rawx_server_conf *conf;
...
rawx_hook_child_init(apr_pool_t * pchild, server_rec * s)
{
	...
	if (APR_SUCCESS != status)
		DAV_ERROR_POOL(pchild, 0,
			"Failed to attach the RAWX statistics support");
	...
	if (shm == NULL) {
		...
		if (rc != APR_SUCCESS) {
			...
			DAV_ERROR_POOL(plog, 0,
				"Failed to create the SHM segment at [%s]: %s", shm_path,
				apr_strerror(rc, buff, sizeof(buff)));
			return rc;
		}
		/* Init the SHM */
		...
		if (ptr_counter) {
			...
			/* init rrd's */
			rawx_stats_rrd_init(&(((struct shm_stats_s *) ptr_counter)->body.
					rrd_req_sec));
			rawx_stats_rrd_init(&(((struct shm_stats_s *) ptr_counter)->body.
					rrd_duration));
			rawx_stats_rrd_init(&(((struct shm_stats_s *) ptr_counter)->body.
	...
	server_rec * server)
{
	...
	if (!(conf->headers_scheme & HEADER_SCHEME_V1) &&
		!(conf->headers_scheme & HEADER_SCHEME_V2)) {
		DAV_ERROR_POOL(plog, 0,
			"You cannot disable both V1 and V2 header scheme");
		return DONE;
	}
	...
	for (s = server; s; s = s->next) {
		...
		for (a = s->addrs; a; a = a->next) {
			...
			switch (state) {
				...
				case ERROR_LS:
					DAV_ERROR_POOL(plog, 0,
						"Failed to check the docroot ownership: %s",
						gerror_get_message(gerr));
					goto label_error;

				case NOLOCK_LS:
					if (!rawx_lock_volume(conf->docroot, conf->ns_name, url, 0,
							&gerr)) {
						DAV_ERROR_POOL(plog, 0,
							"Failed to grab the docroot ownership: %s",
							gerror_get_message(gerr));
						goto label_error;
					}
				...
				case OWN_LS:
					DAV_DEBUG_POOL(plog, 0,
						"Docroot already owned by the current server");
					if (!rawx_lock_volume(conf->docroot, conf->ns_name, url,
							RAWXLOCK_FLAG_OVERWRITE, &gerr))
						DAV_ERROR_POOL(plog, 0,
							"Failed to complete the docroot ownership: %s",
							gerror_get_message(gerr));
					volume_validated = ~0;
					break;
				...
				case OTHER_LS:
					DAV_ERROR_POOL(plog, 0,
						"Another RAWX already used the docroot (see XATTR)"
	...
	if (!volume_validated) {
		DAV_ERROR_POOL(plog, 0,
			"No server found, could not validate the RAWX volume. "
...
static const command_rec dav_rawx_cmds[] = {
	AP_INIT_TAKE1("grid_hash_width", dav_rawx_cmd_gridconfig_hash_width, NULL,
		RSRC_CONF, "hash width on a chunk's name"),
	AP_INIT_TAKE1("grid_hash_depth", dav_rawx_cmd_gridconfig_hash_depth, NULL,
		RSRC_CONF, "hash depth on a chunk's name"),
	AP_INIT_TAKE1("grid_docroot", dav_rawx_cmd_gridconfig_docroot, NULL,
		RSRC_CONF, "chunks docroot"),
	AP_INIT_TAKE1("grid_namespace", dav_rawx_cmd_gridconfig_namespace, NULL,
		RSRC_CONF, "namespace name"),
	AP_INIT_TAKE1("grid_dir_run", dav_rawx_cmd_gridconfig_dirrun, NULL,
		RSRC_CONF, "run directory"),
	AP_INIT_TAKE1("grid_fsync", dav_rawx_cmd_gridconfig_fsync, NULL, RSRC_CONF,
		"do fsync on file close"),
	AP_INIT_TAKE1("grid_fsync_dir", dav_rawx_cmd_gridconfig_fsync_dir, NULL,
		RSRC_CONF, "do fsync on chunk direcory after renaming .pending"),
	AP_INIT_TAKE1("grid_headers", dav_rawx_cmd_gridconfig_headers, NULL,
		RSRC_CONF, "which header scheme to adopt (1, 2, both)"),
	AP_INIT_TAKE1("grid_acl", dav_rawx_cmd_gridconfig_acl, NULL, RSRC_CONF,
		"enabled acl"),
	AP_INIT_TAKE1("grid_upload_blocksize", dav_rawx_cmd_gridconfig_upblock,
		NULL, RSRC_CONF, "upload block size"),
...
{
	dav_hook_gather_propsets(dav_rawx_gather_propsets, NULL, NULL,
		APR_HOOK_MIDDLE);
	dav_hook_find_liveprop(dav_rawx_find_liveprop, NULL, NULL, APR_HOOK_MIDDLE);
	dav_hook_insert_all_liveprops(dav_rawx_insert_all_liveprops, NULL, NULL,
		APR_HOOK_MIDDLE);
