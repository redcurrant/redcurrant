...
rawx_stats_rrd_create(apr_pool_t * pool, time_t period)
{
	...
	result =
		apr_palloc(pool,
		sizeof(struct rawx_stats_rrd_s) + (period * (sizeof(apr_uint32_t))));
	result->last = time(0);
...
rawx_stats_rrd_dup(apr_pool_t * pool, struct rawx_stats_rrd_s *rrd)
{
	...
	result =
		apr_palloc(pool,
		sizeof(struct rawx_stats_rrd_s) +
		(rrd->period * (sizeof(apr_uint32_t))));
...
static void
_rsr_blank_empty_slots(register struct rawx_stats_rrd_s *rsr,
	register apr_uint32_t v, register time_t now)
{
...
rawx_stats_rrd_debug_get_delta(struct rawx_stats_rrd_s *rsr, apr_pool_t * p,
	time_t period)
{
	time_t now;
	...
	result = apr_psprintf(p, "%s| now : %li after bs: %s"
		...
		"(rsr->ten[%u] - rsr->ten[%u] "
		"(%u - %u (%u)))",
		result, now, _dump_rrd(rsr, p), now, rsr->period, now, period,
		rsr->period, nrp, nprp, rsr->ten[nrp], rsr->ten[nprp], op);
