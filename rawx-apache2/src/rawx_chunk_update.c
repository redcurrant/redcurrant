...
static dav_resource *
__get_chunkupdate_resource(const request_rec * r,
...
dav_error *
dav_rawx_chunk_update_get_resource(request_rec * r, const char *root_dir,
	...
	*result_resource = NULL;
	...
	if (r->method_number != M_GET)
		return server_create_and_stat_error(request_get_server_config(r),
			r->pool, HTTP_BAD_REQUEST, 0, apr_pstrdup(r->pool,
				"Invalid request method, only GET"));

	*result_resource =
		__get_chunkupdate_resource(r, &dav_hooks_repository_chunkupdate);
...
static dav_error *
dav_rawx_get_parent_resource_SPECIAL(const dav_resource * resource,
...
static int
dav_rawx_is_same_resource_SPECIAL(const dav_resource * res1,
...
static int
dav_rawx_is_parent_resource_SPECIAL(const dav_resource * res1,
	const dav_resource * res2)
{
	DAV_XDEBUG_RES(res1, 0, "%s(...)", __FUNCTION__);
...
__build_chunk_full_path(const dav_resource * resource, char **full_path)
{
	...
	if (strlen(r->uri) < 65)
		return server_create_and_stat_error(request_get_server_config(r),
			r->pool, HTTP_BAD_REQUEST, 0, apr_pstrcat(r->pool,
				"Cannot parse request uri ", r->uri, NULL));
	char *p = NULL;
	...
	p = apr_palloc(r->pool,
		(65 + 1 + (conf->hash_depth * conf->hash_width) + conf->hash_depth));
	...
	struct storage_policy_s **sp)
{
	...
	if (!pol_name) {
		return server_create_and_stat_error(request_get_server_config(r),
			r->pool, HTTP_BAD_REQUEST, 0, "No storage-policy specified");
	}
	...
	*sp = storage_policy_init(conf->rawx_conf->ni, pol_name);
	apr_pool_cleanup_register(r->pool, *sp, apr_storage_policy_clean,
...
static dav_error *
_load_in_place_chunk_info(const dav_resource * r, const char *path,
	...
	GHashTable ** comp_opt)
{
	...
	if (!get_rawx_info_in_attr(path, &ge, content, chunk)) {
		if (NULL != ge) {
			e = server_create_and_stat_error(conf, p, HTTP_CONFLICT,
				0, apr_pstrcat(p, "Failed to get chunk attributes: ",
					ge->message, NULL));
			g_clear_error(&ge);
		}
		else {
	...
	if (!get_compression_info_in_attr(path, &ge, comp_opt)) {
		if (NULL != ge) {
			e = server_create_and_stat_error(conf, p, HTTP_CONFLICT,
				0, apr_pstrcat(p,
					"Failed to get chunk compression attributes: ", ge->message,
					NULL));
			g_clear_error(&ge);
		}
		else {
			e = server_create_and_stat_error(conf, p, HTTP_CONFLICT,
				0,
				"Failed to get chunk compression attributes: No error specified");
		}
		return e;
	}
...
static int
_is_storage_policy_already_applied(const struct data_treatments_s *dt,
	GHashTable * comp_opt)
{
	const char *c = NULL;
	...
	switch (data_treatments_get_type(dt)) {
		case COMPRESSION:
			...
			if (NULL != c && g_ascii_strcasecmp(c, NS_COMPRESSION_ON)) {
				...
				current_algo =
					g_hash_table_lookup(comp_opt, NS_COMPRESS_ALGO_OPTION);
				current_bs =
					g_hash_table_lookup(comp_opt, NS_COMPRESS_BLOCKSIZE_OPTION);
				if (NULL != pol_algo && NULL != current_algo
					&& 0 == g_ascii_strcasecmp(pol_algo, current_algo)) {
					if (NULL != pol_bs && NULL != current_bs
						&& 0 == g_ascii_strcasecmp(pol_bs, current_bs)) {
						return APR_SUCCESS;
					}
				}
			}
...
static dav_error *
_update_chunk_storage(const dav_resource * resource, const char *path,
	const struct data_treatments_s *dt, GHashTable * comp_opt)
{
	GError *e = NULL;
	dav_error *de = NULL;
	const char *c = NULL;
	...
	if (COMPRESSION == data_treatments_get_type(dt)) {
		...
		if (1 != compress_chunk(path, algo, g_ascii_strtoll(bs, NULL, 10), TRUE,
				&e)) {
			de = server_create_and_stat_error(request_get_server_config(r),
				r->pool, HTTP_INTERNAL_SERVER_ERROR, 0, apr_pstrcat(r->pool,
...
static dav_error *
_ensure_sys_metadata(const dav_resource * resource, const char *path,
	const char *sp, struct content_textinfo_s *content)
{
	...
	if (!content->system_metadata) {
		content->system_metadata =
			apr_pstrcat(resource->pool, "storage-policy=", sp, ";", NULL);
	}
	else {
		const char *p = NULL;
		...
		if (NULL != p) {
			...
			if ((strlen(sp) != (size_t) (end - p))
				...
				content->system_metadata =
					apr_pstrndup(resource->pool, content->system_metadata,
					p - content->system_metadata);
				content->system_metadata =
					apr_pstrcat(resource->pool, content->system_metadata, sp,
					end, NULL);
			}
			else {
				change = 0;
			...
			if (g_str_has_suffix(content->system_metadata, ";")) {
				content->system_metadata =
					apr_pstrcat(resource->pool, content->system_metadata,
					"storage-policy=", sp, NULL);
			}
			else {
				content->system_metadata =
					apr_pstrcat(resource->pool, content->system_metadata,
	...
	if (change && !set_content_info_in_attr(path, &ge, content)) {
		e = server_create_and_stat_error(resource_get_server_config(resource),
			resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
			apr_pstrcat(resource->pool, "Failed to set chunk xattr : ",
...
dav_rawx_deliver_SPECIAL(const dav_resource * resource, ap_filter_t * output)
{
	...
	if (!sp) {
		DAV_DEBUG_REQ(r, 0,
			"Storage policy not initialized with value found in header, don't do anything");
		goto end_deliver;
	}
	...
	apr_pool_cleanup_register(p, comp_opt, apr_hash_table_clean,
		apr_pool_cleanup_null);
	...
	if (NULL != e) {
		DAV_ERROR_REQ(r, 0, "Failed to load in place chunk information: %s",
			e->desc);
		goto end_deliver;
	}

	DAV_DEBUG_REQ(r, 0, "In place chunk info loaded, compression status : %s",
		(gchar *) g_hash_table_lookup(comp_opt, NS_COMPRESSION_OPTION));
	...
	if (APR_SUCCESS != _is_storage_policy_already_applied(dt, comp_opt)) {
		...
		DAV_DEBUG_REQ(r, 0,
			"Storage policy already applied, don't do anything!");
	}


	/* ensure sys-metadata header is valid */
	e = _ensure_sys_metadata(resource, path, storage_policy_get_name(sp),
		content);
	if (NULL != e) {
		DAV_ERROR_REQ(r, 0,
			"Failed to ensure sys-metadata, storage-policy possibly not correctly present in xattr: %s",
			e->desc);
		goto end_deliver;
	}
