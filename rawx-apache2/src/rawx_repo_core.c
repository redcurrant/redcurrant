...
static void
__set_header(request_rec * r, apr_uint32_t h_scheme, const char *n,
	const char *v)
{
	if (r && n && v) {
		if (h_scheme & HEADER_SCHEME_V2) {
			...
			apr_table_setn(r->headers_out, apr_pstrcat(r->pool,
					HEADER_PREFIX_GRID, n, NULL), apr_pstrdup(r->pool, v));
		}
		else {
			DAV_DEBUG_REQ(r, 0, "scheme v1 setting header");
			apr_table_setn(r->headers_out, apr_pstrdup(r->pool, n),
...
_set_chunk_extended_attributes(dav_stream * stream)
{
	...
	g_snprintf(str_hash, sizeof(str_hash),
		...
		"%02X%02X%02X%02X"
		"%02X%02X%02X%02X"
		"%02X%02X%02X%02X", md5_hash[0], md5_hash[1], md5_hash[2], md5_hash[3]
	...
	if (stream->compressed_size) {
		...
		if (!set_rawx_full_info_in_attr(stream->pathname, fileno(stream->f),
				&ge, &(stream->r->info->content), &(stream->r->info->chunk),
				stream->metadata_compress, size)) {
			e = server_create_and_stat_error(resource_get_server_config(stream->
					r), stream->p, HTTP_FORBIDDEN, 0, apr_pstrdup(stream->p,
					gerror_get_message(ge)));
		}
	}
	else {
		if (!set_rawx_full_info_in_attr(stream->pathname, fileno(stream->f),
...
_finalize_chunk_creation(dav_stream * stream)
{
	...
	if (fflush(stream->f)) {
		DAV_ERROR_REQ(stream->r->info->request, 0, "fflush error : %s",
			strerror(errno));
		e = server_create_and_stat_error(resource_get_server_config(stream->r),
			stream->p, HTTP_INTERNAL_SERVER_ERROR, 0, apr_pstrcat(stream->p,
				"fflush error : ", strerror(errno), NULL));
	}

	if (stream->fsync_on_close & FSYNC_ON_CHUNK) {
		if (-1 == fsync(fileno(stream->f))) {
			DAV_ERROR_REQ(stream->r->info->request, 0, "fsync error : %s",
				strerror(errno));
			e = server_create_and_stat_error(resource_get_server_config(stream->
					r), stream->p, HTTP_INTERNAL_SERVER_ERROR, 0,
				apr_pstrcat(stream->p, "fsync error : ", strerror(errno),
					NULL));
		}
	}
	...
	if (0 != status) {
		e = server_create_and_stat_error(resource_get_server_config(stream->r),
			stream->p, MAP_IO2HTTP(status), 0, apr_pstrcat(stream->p, "rename(",
				stream->pathname, ", ", stream->final_pathname, ") failure : ",
				strerror(errno), NULL));
	}
...
_write_data_crumble_COMP(dav_stream * stream, gulong * checksum)
{
	...
	rc = stream->comp_ctx.data_compressor(stream->buffer, stream->bufsize, gba,
		checksum);
	if (0 == rc) {
		...
		/* ### use something besides 500? */
		e = server_create_and_stat_error(resource_get_server_config(stream->r),
			stream->p, HTTP_INTERNAL_SERVER_ERROR, 0,
...
dav_error *
resource_init_decompression(dav_resource * resource,
	dav_rawx_server_conf * conf)
{
	...
	if (!get_compression_info_in_attr(resource_get_pathname(resource), &e,
			&comp_opt)) {
		if (comp_opt)
			g_hash_table_destroy(comp_opt);
		if (e)
			g_clear_error(&e);
		return server_create_and_stat_error(conf, resource->pool, HTTP_CONFLICT,
			0, "Failed to get chunk compression in attr");
	}
	...
	if (resource->info->compression) {
		...
		bzero(resource->info->compress_algo,
		...
		if (0 !=
			resource->info->comp_ctx.chunk_initiator(&(resource->info->
					cp_chunk), (char *) resource->info->fullpath)) {
			r = server_create_and_stat_error(resource_get_server_config
				(resource), resource->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
...
resource_stat_chunk(dav_resource * resource, int xattr_too)
{
	...
	char *tmp_path =
		apr_pstrcat(resource->pool, resource_get_pathname(resource), ".pending",
		NULL);

	status =
		apr_stat(&(resource->info->finfo), tmp_path, APR_FINFO_NORM,
		resource->pool);

	if (status != APR_SUCCESS)
		status =
			apr_stat(&(resource->info->finfo), resource_get_pathname(resource),
		...
		if (xattr_too) {
			...
			if (!rc) {
				DAV_DEBUG_RES(resource, 0,
					"Chunk xattr loading error [%s] : %s",
					resource_get_pathname(resource), apr_pstrdup(resource->pool,
...
content_textinfo_fill_headers(request_rec * r, struct content_textinfo_s *cti)
{
	...
	__set_header(r, conf->headers_scheme, "content_metadata-sys",
		cti->system_metadata);
	__set_header(r, conf->headers_scheme, "content_containerid",
		cti->container_id);
}
...
chunk_textinfo_fill_headers(request_rec * r, struct chunk_textinfo_s *cti)
{
	...
	__set_header(r, conf->headers_scheme, "chunk_containerid",
		cti->container_id);
}
...
dav_error *
rawx_repo_check_request(request_rec * req, const char *root_dir,
	...
	dav_resource ** result_resource)
{
	...
	if (0 == apr_strnatcasecmp(src, "info")) {
		return dav_rawx_info_get_resource(req, root_dir, label, use_checked_in,
			result_resource);
	}

	if (0 == apr_strnatcasecmp(src, "stat")) {
		return dav_rawx_stat_get_resource(req, root_dir, label, use_checked_in,
			result_resource);
	}

	if (0 == apr_strnatcasecmp(src, "update")) {
		return dav_rawx_chunk_update_get_resource(req, root_dir, label,
			use_checked_in, result_resource);
	}

	if (g_str_has_prefix(src, "rawx/")) {
		server_inc_request_stat(request_get_server_config(req),
			RAWX_STATNAME_REQ_RAW, request_get_duration(req));
		return server_create_and_stat_error(request_get_server_config(req),
			req->pool, HTTP_BAD_REQUEST, 0, "Raw request not yet implemented");
	}
	...
	for (i = 0; *src; src++, i++) {
		...
		if (!g_ascii_isdigit(c) && (c < 'A' || c > 'F'))
			return server_create_and_stat_error(request_get_server_config(req),
				req->pool, HTTP_BAD_REQUEST, 0, "Invalid CHUNK id character");
		ctx->hex_chunkid[i] = c;
	}
	if (i != 64) {
		return server_create_and_stat_error(request_get_server_config(req),
			req->pool, HTTP_BAD_REQUEST, 0, apr_psprintf(req->pool,
				"Invalid CHUNK id length: %d", i));
	}
...
rawx_repo_configure_hash_dir(request_rec * req, dav_resource_private * ctx)
{
	...
	needed =
		1 + (sizeof(ctx->hex_chunkid) + (conf->hash_width +
			1) * conf->hash_depth);
	if (remaining < needed)
		return server_create_and_stat_error(request_get_server_config(req),
			req->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
...
rawx_repo_write_last_data_crumble(dav_stream * stream)
{
	...
	if (!e && stream->compression) {
		if (0 != stream->comp_ctx.eof_writer(stream->f, checksum,
...
rawx_repo_rollback_upload(dav_stream * stream)
{
	...
	if (remove(stream->pathname) != 0) {
		...
		return server_create_and_stat_error(resource_get_server_config(stream->
				r), stream->p, HTTP_INTERNAL_SERVER_ERROR, 0,
			"There was a problem removing (rolling " "back) the resource "
...
rawx_repo_commit_upload(dav_stream * stream)
{
	...
	content_textinfo_fill_headers(stream->r->info->request,
		&(stream->r->info->content));
	chunk_textinfo_fill_headers(stream->r->info->request,
		&(stream->r->info->chunk));
...
rawx_repo_ensure_directory(const dav_resource * resource)
{
	...
	status = apr_dir_make_recursive(ctx->dirname,
		...
		| APR_FPROT_WREAD | APR_FPROT_WEXECUTE, resource->info->pool);
	if (status != APR_SUCCESS) {
		return
			server_create_and_stat_error(resource_get_server_config(resource),
			resource->info->pool, MAP_IO2HTTP(status), 0,
			apr_pstrcat(resource->info->pool, "mkdir(", ctx->dirname,
				") failure : ", strerror(errno), NULL));
	}

	DAV_DEBUG_REQ(resource->info->request, status, "mkdir(%s) success",
		ctx->dirname);
	return NULL;
}
...
rawx_repo_stream_create(const dav_resource * resource, dav_stream ** result)
{
	...
	ds->r = resource;
	ds->final_pathname =
		apr_pstrcat(p, ctx->dirname, "/", ctx->hex_chunkid, NULL);
	ds->pathname =
		apr_pstrcat(p, ctx->dirname, "/", ctx->hex_chunkid, ".pending", NULL);
	...
	if (!ds->f) {
		...
		if (0 != setvbuf(ds->f, buf, _IOFBF, (ssize_t) s)) {
			DAV_DEBUG_REQ(resource->info->request, 0,
				"setvbuf failed : (errno=%d) %s", errno, strerror(errno));
		}
	}
	...
	if (NULL != str) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"stg_pol init from req header %s", str);
	...
	if ((!dt || COMPRESSION != data_treatments_get_type(dt)) && (match != 0)) {
		...
		if (NULL != dt && COMPRESSION == data_treatments_get_type(dt)) {
			...
			metadata_compress =
				apr_pstrcat(p, NS_COMPRESSION_OPTION, "=", NS_COMPRESSION_ON,
			...
			if (!ctx->forced_cp_algo || !ctx->forced_cp_bs) {
				return
					...
					(resource), p, 400, 0, apr_pstrcat(p,
						"Failed to get compression info from incoming request",
						NULL));
			}
		...
		ds->metadata_compress =
			apr_pstrndup(p, metadata_compress, strlen(metadata_compress));

		/* writting compression header in busy file */
		guint32 bsize32 = ds->blocksize;

		if (0 != ds->comp_ctx.header_writer(ds->f, bsize32, &checksum,
				&(ds->compressed_size))) {
			return
				server_create_and_stat_error(resource_get_server_config
				(resource), p, 500, 0, apr_pstrcat(p,
					"Failed to write compression headers", NULL));
		}
