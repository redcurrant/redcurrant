 ...
 */
static gboolean
_write_to_xattr(int file, const gchar * key, const gchar * value,
	GError ** error)
{
	if (file < 0 || !key || !value) {
		SETERRCODE(error, EINVAL, "Invalid parameter (%d %p %p)", file, key,
			value);
		return FALSE;
	}

	if (0 > fsetxattr(file, key, value, strlen(value), 0)) {
		SETERRCODE(error, errno, "Failed to add [%s/%s] to file xattr : %s",
 ...
 */
static gboolean
_write_to_attr_file(int file, const gchar * key, const gchar * value,
	GError ** error)
{
	...
	if (file < 0 || !key || !value) {
		SETERRCODE(error, EINVAL, "Invalid argument (%d %p %p)", file, key,
			value);
		return FALSE;
	}
 ...
 */
static gboolean
_write_attributes(GHashTable * attr_hash, int file, attr_writer_f writer,
	GError ** error)
{
...
_commit_attr_handle(struct attr_handle_s *attr_handle, GError ** error)
{
	...
	if (attr_handle->chunk_file_des >= 0) {
		if (_write_attributes(attr_handle->attr_hash,
				attr_handle->chunk_file_des, _write_to_xattr, &local_error)) {
	...
	attr_handle->attr_file_des =
		open(attr_handle->attr_path, O_RDWR | O_CREAT | O_TRUNC, 0644);
	if (attr_handle->attr_file_des < 0) {
		SETERRCODE(error, errno, "File [%s] open failed : %s",
			attr_handle->attr_path, strerror(errno));
		return FALSE;
	}
	else {
		if (!_write_attributes(attr_handle->attr_hash,
				attr_handle->attr_file_des, _write_to_attr_file,
				&local_error)) {
			metautils_pclose(&(attr_handle->attr_file_des));
			SETERROR(error, "Failed to write attributes to chunk.attr : %s",
				local_error->message);
			g_clear_error(&local_error);
			return FALSE;
		}
 ...
 */
static gboolean
_commit_v2_attr_handle(int filedes, struct attr_handle_s *attr_handle,
	GError ** error)
{
	GError *local_error = NULL;
...
retry:
	...
	for (last_name = buf, i = 0; i < size; i++) {
		if (buf[i] == '\0') {
			char *value =
				_getxattr_from_chunk(attr_handle->chunk_path,
	...
	GError ** error)
{
	...
	if (!_load_from_xattr(ah, &local_error)) {
		if (!local_error) {
			SETERRCODE(&local_error, 500,
				"Failed to load xattr : unknown error");
			goto error_and_exit;
		}
 ...
 */
static gboolean
_lazy_load_attr_from_file(const char *chunk_path,
	struct attr_handle_s **attr_handle, GError ** error)
{
...
/* --------------------------------------------------------------------------------------------------------------------------- */
gboolean
set_rawx_full_info_in_attr(const char *pathname, int filedes, GError ** error,
	...
	char *compression_info, char *compressed_size)
{
	...
	if (chunk->id
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_ID, chunk->id))
		goto error_set_attr;
	if (chunk->size
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_SIZE, chunk->size))
		goto error_set_attr;
	if (chunk->hash
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_HASH, chunk->hash))
		goto error_set_attr;
	if (chunk->position
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_POS, chunk->position))
		goto error_set_attr;
	if (chunk->metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_METADATA, chunk->metadata))
		goto error_set_attr;

	if (content->path
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_PATH, content->path))
		goto error_set_attr;
	if (content->size
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_SIZE, content->size))
		goto error_set_attr;
	if (content->chunk_nb
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_NBCHUNK, content->chunk_nb))
		goto error_set_attr;
	if (content->container_id
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_CONTAINER, content->container_id))
		goto error_set_attr;
	if (content->metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_METADATA, content->metadata))
		goto error_set_attr;
	if (content->system_metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_METADATA_SYS, content->system_metadata))
		goto error_set_attr;

	/* Compression info */
	if (compressed_size
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_COMPRESSED_SIZE, compressed_size))
		goto error_set_attr;

	if (compression_info
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_METADATA_COMPRESS, compression_info))
		goto error_set_attr;

	if (!_commit_v2_attr_handle(filedes, attr_handle, &local_error)) {
		SETERROR(error, "Could not write all the attributes on disk : %s",
			local_error->message);
		g_clear_error(&local_error);
...
gboolean
set_rawx_info_in_attr(const char *pathname, GError ** error,
	struct content_textinfo_s * content, struct chunk_textinfo_s * chunk)
{
	struct attr_handle_s *attr_handle;
	GError *local_error = NULL;
...
gboolean
set_chunk_info_in_attr(const char *pathname, GError ** error,
	struct chunk_textinfo_s * cti)
{
	...
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_ID, cti->id))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_PATH, cti->path))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_SIZE, cti->size))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_HASH, cti->hash))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_POS, cti->position))
		goto error_set_attr;
	if (cti->metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_METADATA, cti->metadata))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_CONTAINER, cti->container_id))
		goto error_set_attr;
...
gboolean
set_content_info_in_attr(const char *pathname, GError ** error,
	struct content_textinfo_s * cti)
{
	...
	if (!_lazy_load_attr_from_file(pathname, &attr_handle, &local_error)) {
		SETERRCODE(error, local_error->code,
			"Failed to init the attribute management context : %s",
			local_error->message);
		g_clear_error(&local_error);
		return FALSE;
	}
	...
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
		...
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_SIZE, cti->size))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_NBCHUNK, cti->chunk_nb))
		goto error_set_attr;
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_CONTAINER, cti->container_id))
		goto error_set_attr;
	if (cti->metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_METADATA, cti->metadata))
		goto error_set_attr;
	if (cti->system_metadata
		&& !_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_METADATA_SYS, cti->system_metadata))
		goto error_set_attr;

	if (!_commit_attr_handle(attr_handle, &local_error)) {
		SETERRCODE(error, local_error->code,
			"Could not write all the attributes on disk : %s",
			local_error->message);
		g_clear_error(&local_error);
		_clean_attr_handle(attr_handle, FALSE);
		return FALSE;
...
error_set_attr:
	SETERRCODE(error, local_error->code, "Failed to set attr : %s",
		local_error->message);
	g_clear_error(&local_error);
	_clean_attr_handle(attr_handle, FALSE);
...
gboolean
set_compression_info_in_attr(const char *pathname, GError ** error,
	gchar * metadata_compress)
{
	struct attr_handle_s *attr_handle;
	GError *local_error = NULL;
	...
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_METADATA_COMPRESS, metadata_compress)) {
		SETERROR(error, "Failed to add [%s : %s] in attributes context : %s",
			ATTR_NAME_CHUNK_METADATA_COMPRESS, metadata_compress,
			local_error->message);
		g_clear_error(&local_error);
		goto error_set_attr;
...
gboolean
set_chunk_compressed_size_in_attr(const char *pathname, GError ** error,
	guint32 compressed_size)
{
	...
	if (!_set_attr_in_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CHUNK_COMPRESSED_SIZE, size)) {
		goto error_set_attr;
	}
...
gboolean
get_chunk_compressed_size_in_attr(const char *pathname, GError ** error,
	guint32 * compressed_size)
{
	gchar *tmp = NULL;

	if (!(tmp =
			_getxattr_from_chunk(pathname, -1,
				ATTR_DOMAIN "." ATTR_NAME_CHUNK_COMPRESSED_SIZE))) {
		GSETCODE(error, errno, "compressedsize not found : %s",
			strerror(errno));
		return FALSE;
	}
	...
	struct content_textinfo_s * content, struct chunk_textinfo_s * chunk)
{
	...
	if (!_get_attr_from_handle(attr_handle, &local_error, ATTR_DOMAIN,
		...
		goto error_get_attr;
	if (!_get_attr_from_handle(attr_handle, &local_error, ATTR_DOMAIN,
			ATTR_NAME_CONTENT_METADATA_SYS, &(content->system_metadata), FALSE))
		goto error_get_attr;
	if (!_get_attr_from_handle(attr_handle, &local_error, ATTR_DOMAIN,
	...
	gchar * dst_ns, gsize dst_ns_size, GError ** gerr)
{
	...
	switch (size = getxattr(vol, RAWXLOCK_ATTRNAME_NS, NULL, 0)) {
		case -1:
			if (errno != ENOATTR) {
				SETERRCODE(gerr, errno, "getxattr(%s) : %s",
					RAWXLOCK_ATTRNAME_NS, strerror(errno));
				return FALSE;
			}
			break;
		case 0:
			SETERRCODE(gerr, ENOTSUP, "getxattr(%s) : operation not supported",
				RAWXLOCK_ATTRNAME_NS);
			return FALSE;
		default:
			usize = size;
...
int
rawx_lock_volume(const char *vol, const char *ns, const char *host,
	uint32_t flags, GError ** err)
{
	int xattr_flags = (flags & RAWXLOCK_FLAG_OVERWRITE) ? 0 : XATTR_CREATE;

	if (0 > setxattr(vol, RAWXLOCK_ATTRNAME_URL, host, strlen(host),
			xattr_flags)) {
		if (errno == ENOTSUP) {
			SETERRCODE(err, ENOTSUP,
				"XATTR not supported on volume [%s], no lock set", vol);
			return 0;
		}
...
enum lock_state_e
rawx_get_volume_lock_state(const char *vol, const char *ns, const char *host,
	GError ** err)
{
...
stamp_a_chunk(const char *chunk_path, const char *attr_to_set)
{
	...
	memset(attr_name_buf, '\0', sizeof(attr_name_buf));
	snprintf(attr_name_buf, sizeof(attr_name_buf), "%s.%s", ATTR_DOMAIN,
		attr_to_set);
	...
	setxattr(chunk_path, attr_name_buf, real_time_buf, strlen(real_time_buf),
		0);
}
