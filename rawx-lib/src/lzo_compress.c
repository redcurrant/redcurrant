...
int
lzo_write_compress_header(FILE * file, guint32 blocksize, gulong * checksum,
...
int
lzo_compress_chunk_part(const void *buf, gsize bufsize, GByteArray * result,
	gulong * checksum)
{
	lzo_bytep out = NULL;
	lzo_uint out_len = 0;
...
_fill_decompressed_buffer(struct compressed_chunk_s *chunk, gsize to_skip)
{
	...
	DEBUG
		("_fill_decompressed_buffer: ok, data not corrupted, it's time to work");
...
int
lzo_compressed_chunk_get_data(struct compressed_chunk_s *chunk, gsize offset,
