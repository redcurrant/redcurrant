...
gboolean
init_compression_ctx(struct compression_ctx_s * comp_ctx,
...
check_uncompressed_chunk(const gchar * path, GError ** error)
{
	...
	if (get_compression_info_in_attr(path, error, &compress_opt)) {
		...
		if (compression
			&& g_ascii_strcasecmp(compression, NS_COMPRESSION_ON) == 0) {
		...
		GSETERROR(error,
			"Failed to get compression info from extended attributes\n");
		goto end;
	}
...
copy_fattr(const gchar * src, gchar * dst, GError ** error)
{
	...
	if (!get_rawx_info_in_attr(src, error, content, chunk)) {
		GSETERROR(error,
			"Failed to load extended attributes from source file\n");
		goto err;
	}
	...
	if (!chunk->id) {
		GSETERROR(error,
			"No chunk_id found in source file extended attributes, may be this file is not a chunk\n");
		goto err;
	}

	if (!set_rawx_info_in_attr(dst, error, content, chunk)) {
		GSETERROR(error,
			"Failed to set extended attributes to destination file\n");
		goto err;
	}
		...
		if (nb_read != bsize) {
			...
			if (!feof(src) && ferror(src)) {
				...
				if (nb_read > 0) {
					...
					/* process data */
					if (0 != comp_ctx->data_compressor(buf, nb_read, gba,
							checksum))
						goto end;
...
static gboolean
set_compress_attr(gchar * tmp_path, const gchar * algo, gint64 blocksize,
	GError ** error)
{
	...
	metadata_compress =
		g_strconcat(NS_COMPRESSION_OPTION, "=", NS_COMPRESSION_ON, ";",
		NS_COMPRESS_ALGO_OPTION, "=", algo, ";", NS_COMPRESS_BLOCKSIZE_OPTION,
		"=", bs_str, NULL);
...
int
compress_chunk(const gchar * path, const gchar * algo, const gint64 blocksize,
	gboolean preserve, GError ** error)
{
	...
	if (stat(tmp_path, stat_buf) != -1) {
		GSETERROR(error,
			"stat() success on pending file, cannot process : chunk busy\n");
		goto end;
	}
	...
	if (!copy_fattr(path, tmp_path, &local_error)) {
		if (local_error) {
			GSETERROR(error,
				"Failed to copy extended attributes to destination file:\n%s",
				local_error->message);
			g_clear_error(&local_error);
		}
		else
	...
	while (1) {
		...
		if (nb_read != bsize) {
			...
			if (!feof(src) && ferror(src)) {
				GSETERROR(error,
					"An error occured while reading data from source file\n");
				goto end;
			}
			else {
				if (nb_read > 0) {
					...
					if ((nb_write = fwrite(gba->data, gba->len, 1, dst)) != 1) {
						GSETERROR(error,
							"An error occured while writing data in destination file\n");
						goto end;
					}
	...
	if (comp_ctx->eof_writer(dst, checksum, &compressed_size) != 0) {
		GSETERROR(error,
			"Failed to write compressed file EOF marker and checksum\n");
		goto end;
	}

	if (!set_chunk_compressed_size_in_attr(tmp_path, error, compressed_size)) {
		GSETERROR(error,
			"Failed to set compression information in extended attributes\n");
		goto end;
	}
	...
	if (status == 1) {
		...
		if (preserve) {
			...
			if (stat(path, stat_buf) == -1) {
				GSETERROR(error,
					"Failed to stat old file, cannot keep old file information, abort\n");
				/* remove tmp file */
				...
				if (utime(tmp_path, ut) != 0) {
					GSETERROR(error,
						"Failed to set correct access time to new file");
					status = 0;
				}
				if (ut)
					g_free(ut);
...
uncompress_chunk(const gchar * path, gboolean preserve, GError ** error)
{
	...
	if (!get_compression_info_in_attr(path, error, &compress_opt)) {
		GSETERROR(error,
			"Failed to get compression info in attr, chunk may be not compressed");
		goto end;
	}
	...
	compression =
		(gchar *) g_hash_table_lookup(compress_opt, NS_COMPRESSION_OPTION);

	if (compression != NULL
		&& g_ascii_strncasecmp(compression, NS_COMPRESSION_ON,
			strlen(compression)) != 0) {
	...
	comp_ctx = g_malloc0(sizeof(struct compression_ctx_s));
	init_compression_ctx(comp_ctx, g_hash_table_lookup(compress_opt,
			NS_COMPRESS_ALGO_OPTION));
	...
	if (stat(tmp_path, buf) != -1) {
		DEBUG("Stats failed");
		GSETERROR(error,
			"stat() success on pending file, cannot process : busy chunk\n");
		goto end;
	}
	...
	DEBUG("Starting, total_read = %" G_GINT64_FORMAT ", chunk_size = %"
		G_GINT64_FORMAT, total_read, chunk_size);
	...
	while (total_read < chunk_size) {
		...
		while (nb_read < bufsize) {
			current_read =
				comp_ctx->data_uncompressor(cp_chunk, 0, data + nb_read,
				bufsize - nb_read, &local_error);
			DEBUG("Currently read %" G_GINT64_FORMAT " bytes", current_read);
			if (current_read <= 0) {
				if (local_error) {
					GSETERROR(error,
						"An error occured while decompressing chunk : %s",
						local_error->message);
					g_clear_error(&local_error);
				}
				else
