...
#define WRITE() do { \
	...
	while (nbSent < bufSize) {
		...
		if (rc_poll == -1) {	/*poll error */
			if (errno != EINTR) {
				GSETERROR(err, "Socket error (%s) after %i bytes written",
					strerror(errno), nbSent);
				return (-1);
			}
			else {
		...
		if (p.revents & POLLNVAL) {
			GSETERROR(err, "Socket (%d) is invalid after %i bytes sent", fd,
				nbSent);
			return -1;
		}
		if (p.revents & POLLERR) {
			...
			GSETERROR(err, "Socket (%d) error after %i bytes written : (%d) %s",
				fd, nbSent, sock_err, strerror(sock_err));
			return -1;
		}
		if ((p.revents & POLLHUP)) {
			GSETCODE(err, ERRCODE_CONN_CLOSED,
				"Socket (%d) closed after %i bytes written", fd, nbSent);
			return -1;
		}
...
#define READ() do { \
	...
	for (;;) {
		...
		if (rc < 0 && errno != EINTR) {	/*error */
			GSETCODE(err, errno_to_errcode(errno), "Socket error (%s)",
				strerror(errno));
			return -1;
		}
		if (rc == 1) {
			...
			if (p.revents & POLLERR) {
				...
				GSETCODE(err, ERRCODE_CONN_CLOSED, "Socket %d error : (%d) %s",
...
sock_to_read_size(int fd, gint ms, void *buf, gsize bufSize, GError ** err)
{
	...
	while (nbRead < bufSize) {
		int n =
			sock_to_read(fd, ms, ((guint8 *) buf) + nbRead, bufSize - nbRead,
			err);
		if (n < 0) {
...
sock_get_error(int fd)
{
	...
	rc = metautils_syscall_getsockopt(fd, SOL_SOCKET, SO_ERROR, &sock_err,
		&sock_err_size);
...
sock_set_tcpquickack(int fd, gboolean enabled)
{
	...
	if (!metautils_syscall_setsockopt(fd, IPPROTO_TCP, TCP_QUICKACK,
			(void *) &opt, sizeof(opt)))
		return TRUE;
...
sock_set_reuseaddr(int fd, gboolean enabled)
{
	...
	if (!metautils_syscall_setsockopt(fd, SOL_SOCKET, SO_REUSEADDR,
			(void *) &opt, sizeof(opt)))
		return TRUE;
...
sock_set_keepalive(int fd, gboolean enabled)
{
	...
	if (!metautils_syscall_setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE,
			(void *) &opt, sizeof(opt)))
		return TRUE;
...
sock_set_nodelay(int fd, gboolean enabled)
{
	...
	if (!metautils_syscall_setsockopt(fd, IPPROTO_TCP, TCP_NODELAY,
			(void *) &opt, sizeof(opt)))
		return TRUE;
...
sock_set_cork(int fd, gboolean enabled)
{
	...
	if (!metautils_syscall_setsockopt(fd, IPPROTO_TCP, TCP_CORK, (void *) &opt,
			sizeof(opt)))
		return TRUE;
...
sock_set_linger(int fd, int onoff, int linger)
{
	...
	if (!metautils_syscall_setsockopt(fd, SOL_SOCKET, SO_LINGER, (void *) &ls,
			sizeof(ls)))
		return TRUE;
