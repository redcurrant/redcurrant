...
{
	void _show_option(gpointer _option_name, gpointer _option_value,
		gpointer _unused)
	{
...
static gboolean
_add_option_value(GHashTable * options_ht, gchar * option_name,
	gchar * option_value)
{
	gboolean ret;
	...
	if (*option_name) {
		...
		GRID_WARN("empty option name in url (associated value=[%s])",
			option_value);
		ret = FALSE;
	}
...
_parse_url(struct hc_url_s *url, const gchar * str)
{
	...
	options =
		path ? strrchr(path, '?') : refname ? strrchr(refname, '?') : NULL;
	if (options) {
		if (!*(++options))
			options = NULL;
		if (url->options)
...
hc_url_set(struct hc_url_s *u, enum hc_url_field_e f, const char *v)
{
	...
	switch (f) {
		...
		case HCURL_NSPHYS:
			...
			u->ns = u->vns && u->pns ? g_strconcat(u->pns, ".", u->vns, NULL)
				: g_strdup(u->pns);
			metautils_str_clean(&(u->whole));
			return u;
...
hc_url_has(struct hc_url_s *u, enum hc_url_field_e f)
{
	...
	switch (f) {
		...
		case HCURL_VERSION:
			if (!u->options)
				return 0;
			return NULL != g_hash_table_lookup(u->options,
				HCURL_OPTION_KEY_VERSION);
		case HCURL_SNAPSHOT:
			if (!u->options)
				return 0;
			return NULL != g_hash_table_lookup(u->options,
				HCURL_OPTION_KEY_SNAPSHOT);
		...
		case HCURL_SNAPORVERS:
			return hc_url_has(u, HCURL_SNAPSHOT)
				|| hc_url_has(u, HCURL_VERSION);
	}
		...
		case HCURL_WHOLE:
			if (!u->whole) {
				if (!hc_url_has(u, HCURL_NS) || (!hc_url_has(u, HCURL_REFERENCE)
					...
					return NULL;
				GString *gs =
					_append_options(_append_url(g_string_new(""), u), u);
				if (gs)
