...
grid_lb_init(const gchar * ns, const gchar * srvtype)
{
	...
	lb->id_by_addr = g_hash_table_new(addr_info_hash, addr_info_equal);

	lb->sorted_by_score = g_array_new(TRUE, TRUE, sizeof(struct score_slot_s));
	return lb;
}
...
_lb_relink_services(struct grid_lb_s *lb)
{
	...
	for (guint i = lb->gpa->len; i > 0;) {
		...
		lb->sorted_by_score =
			g_array_append_vals(lb->sorted_by_score, &score, 1);
...
_compute_min_score_by_SD(struct grid_lb_s *lb)
{
	...
	return FUNC_CLAMP(x, 1, lb->sorted_by_score->len);
}
...
_compute_size_greater(struct grid_lb_s *lb, guint32 score)
{
	...
	for (register guint i = 0; i < max; ++i) {
		if (score > g_array_index(lb->sorted_by_score, struct score_slot_s,
				i).score)
			             return i;
	}
	return max;
...
grid_lb_get_service_from_addr(struct grid_lb_s *lb,
...
gboolean
grid_lb_is_srv_available(struct grid_lb_s * lb,
...
__next_SHARED(struct grid_lb_iterator_s *iter, struct service_info_s **si)
{
	if (!iter || !si)
		return FALSE;
	return grid_lb_iterator_next(iter->internals.shared.sub, si);
}
	...
	struct service_info_s ***result, struct lb_next_opt_s *opt)
{
	...
	// Manage the dupplication cases with a well chosen set
	GTree *polled = NULL;
...
grid_lb_iterator_configure(struct grid_lb_iterator_s *iter, const gchar * val)
{
	...
	switch (*val) {
		...
		case 'W':
			...
			else if (g_str_has_prefix(val, "WRAND")
				|| g_str_has_prefix(val, "SRAND")) {
				iter->type = LBIT_SRAND;
			}
...
void
grid_lbpool_reconfigure(struct grid_lbpool_s *glp, struct namespace_info_s *ni)
{
