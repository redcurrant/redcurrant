...
static void
api_gclean(gpointer p1, gpointer p2)
{
	...
	GError ** err)
{
	...
	decRet =
		ber_decode(&codecCtx, h->asn1_descriptor, &(result), asn1_encoded,
		asn1_encoded_size);
		...
		case RC_OK:
			...
			for (i = 0, max = abstract_sequence->list.count; i < max; i++) {
				...
				if (!(api_structure = g_try_malloc0(h->api_size))
					|| !h->map_ASN1_to_API(abstract_sequence->list.array[i],
						api_structure)) {
					GSETERROR(err,
						"Element of type [%s] ASN-to-API conversion error",
						h->type_name);
...
GByteArray *
abstract_sequence_marshall(const struct abstract_sequence_handler_s * h,
	GSList * api_sequence, GError ** err)
{
	...
	void func_fill(gpointer d, gpointer u)
	{
		...
		if (!h->map_API_to_ASN1(d, asn1_form)) {
			g_free(asn1_form);
			GSETERROR(err, "Element of type [%s] serialization failed!",
				h->type_name);
			error_occured = TRUE;
		}
		else {
	...
	if (encRet.encoded == -1) {
		GSETERROR(err, "Cannot encode the ASN.1 sequence (error on %s)",
...
static gboolean
abstract_list_content_handler(GError ** error, gpointer u, gint c, guint8 * b,
	gsize bS)
{
	(void) c;
	...
	if (0 > abstract_sequence_unmarshall(al_req->h, &list_from_body, b, bS,
			error)) {
...
abstract_sequence_request(struct metacnx_ctx_s * cnx, GError ** error,
	const struct abstract_sequence_handler_s * h, const gchar * req_name,
	GByteArray * body, va_list args)
{
	...
	message_destroy(req, NULL);
	GSETERROR(error,
		"Cannot execute the query %s and receive all the responses", req_name);
	if (al_req.result) {
		if (h->clean_API)
