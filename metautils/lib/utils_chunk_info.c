...
chunk_id_to_string(const chunk_id_t * ci, gchar * dst, gsize dstSize)
{
	...
	offset = g_snprintf(dst, dstSize,
		...
		ci->id[0], ci->id[1], ci->id[2], ci->id[3], ci->id[4], ci->id[5],
		ci->id[6], ci->id[7], ci->id[8], ci->id[9], ci->id[10], ci->id[11],
		ci->id[12], ci->id[13], ci->id[14], ci->id[15], ci->id[16], ci->id[17],
		ci->id[18], ci->id[19], ci->id[20], ci->id[21], ci->id[22], ci->id[23],
		ci->id[24], ci->id[25], ci->id[26], ci->id[27], ci->id[28], ci->id[29],
		ci->id[30], ci->id[31], str_addr, LIMIT_LENGTH_VOLUMENAME, ci->vol);
...
chunk_info_to_string(const chunk_info_t * src, gchar * dst, gsize dstSize)
{
	...
	offset += g_snprintf(dst + offset, dstSize - offset,
		":%" G_GINT64_FORMAT ":%" G_GUINT32_FORMAT ":", src->size,
		src->position);
	for (i = 0; i < 16; i++)
		offset +=
...
chunk_info_print_all(const gchar * domain, const gchar * header, GSList * list)
{
	...
	for (l = list; l; l = l->next) {
		...
		chunk_info_to_string((chunk_info_t *) (l->data), str_ci,
			sizeof(str_ci) - 1);
...
gchar *
assemble_chunk_id(const gchar * straddr, const gchar * strvol,
	const gchar * strid)
{
