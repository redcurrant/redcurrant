...
{
	static gchar *array_yes[] =
		{ "yes", "true", "on", "enable", "enabled", NULL };
	static gchar *array_no[] =
		{ "no", "false", "off", "disable", "disabled", NULL };
	gchar **s;
...
void
build_hash_path(const char *file_name, int hash_depth, int hash_size,
	char **hash_path)
{
	int file_len;
