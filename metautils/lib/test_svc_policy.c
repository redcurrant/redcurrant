...
test_configure_str(gboolean expected, const gchar * str)
{
	...
	if (expected) {
		...
		g_test_message("Expected error : (%d) %s", err->code, err->message);
		g_clear_error(&err);
	}
...
test_configure_valid(void)
{
	...
	test_replicas("SVC0=KEEP|1;SVC1=APPEND|2;SVC2=REPLACE|3",
		"SVC0", 1U, "SVC1", 2U, "SVC2", 3U, NULL);

	test_replicas("SVC7=KEEP|1;SVC8=APPEND|2;SVC7=REPLACE|3",
		"SVC7", 3U, "SVC8", 2U, NULL);

	test_tags
		("SVC9=KEEP|7|1|tag.name=tag.value;SVC10=KEEP|2|3|tag.name=tag.value;SVC10=KEEP|1",
		"SVC9", "tag.name", "tag.value", "SVC10", "", "", NULL);
}
