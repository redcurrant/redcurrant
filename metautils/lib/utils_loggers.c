...
{
	return (lvl & 0x7F)
		|| (main_log_level >> G_LOG_LEVEL_USER_SHIFT) >=
		(lvl >> G_LOG_LEVEL_USER_SHIFT);
}
...
parse(const gchar * path, XML_Parser parser)
{
	...
	if (!(in = fopen(path, "r")))
		return NEWERROR(500, "fopen(%s) : (%d) %s", path, errno,
			strerror(errno));
...
log4c_load(const char *path)
{
		...
		switch (*n) {
			...
			case 'c':
				if (!g_ascii_strcasecmp(n, "category")) {
					...
					g_tree_replace(tree_category, g_strdup(_get_attr(attrs,
								"name")), g_memdup(tab, sizeof(tab)));
				}
				return;
			case 'A':
			case 'a':
				if (!g_ascii_strcasecmp(n, "appender")) {
					...
					g_tree_replace(tree_appenders, g_strdup(_get_attr(attrs,
								"name")), g_memdup(tab, sizeof(tab)));
				}
				return;
		}
	...
	tree_appenders =
		g_tree_new_full(cmp, NULL, g_free, (GDestroyNotify) g_strfreev);
	tree_category =
		g_tree_new_full(cmp, NULL, g_free, (GDestroyNotify) g_strfreev);
