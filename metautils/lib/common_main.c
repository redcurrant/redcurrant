...
_set_opt(gchar ** tokens)
{
	...
	for (opt = user_callbacks->options(); opt && opt->name; opt++) {
		if (0 == g_ascii_strcasecmp(opt->name, tokens[0])) {
			if (tokens[1] == NULL && opt->type != OT_BOOL) {
				g_snprintf(errbuff, sizeof(errbuff),
					"Missing parameter, expected '%s=<Value>'", tokens[0]);
				return errbuff;
			}
			switch (opt->type) {
				case OT_BOOL:
					...
					*(opt->data.b) =
						metautils_cfg_get_bool(tokens[1], *(opt->data.b));
					return NULL;
				case OT_INT:
				...
				case OT_LIST:
					*(opt->data.lst) =
						g_slist_prepend(*(opt->data.lst), g_strdup(tokens[1]));
					return NULL;
				default:
					g_snprintf(errbuff, sizeof(errbuff),
						"Invalid option type [%d], possible corruption",
						opt->type);
					return errbuff;
			}
		}
	...
	g_snprintf(errbuff, sizeof(errbuff), "Option '%s' not supported",
		tokens[0]);
	return errbuff;
}
...
_dump_xopts()
{
	...
	for (o = user_callbacks->options(); o && o->name; o++) {
		switch (o->type) {
			case OT_BOOL:
				g_snprintf(name, sizeof(name), "%s=%s", o->name,
					(*(o->data.b) ? "on" : "off"));
				break;
			case OT_INT:
			...
			case OT_UINT:
				...
				break;
			case OT_INT64:
				g_snprintf(name, sizeof(name), "%s=%" G_GINT64_FORMAT, o->name,
					*(o->data.i64));
				break;
			case OT_TIME:
...
grid_main_usage(void)
{
	...
	g_printerr
		("  -v         verbose mode, this activates stderr traces (default FALSE)\n");
	g_printerr("  -p PATH    pidfile path, no pidfile if unset\n");
	g_printerr
		("  -l PATH    activates the log4c emulation and load PATH as a log4c file\n");
...
grid_main_delete_pid_file(void)
{
	...
	if (-1 == stat(pidfile_path, &current_pidfile_stat)) {
		GRID_WARN("Unable to remove pidfile at [%s] : %s", pidfile_path,
			strerror(errno));
		return;
	}
...
grid_main_init(int argc, char **args)
{
	...
	for (;;) {
		...
		switch (c) {
			...
			case 'p':
				memset(pidfile_path, 0, sizeof(pidfile_path));
				if (sizeof(pidfile_path) <= g_strlcpy(pidfile_path, optarg,
						sizeof(pidfile_path) - 1)) {
					GRID_WARN("Invalid '-p' argument : too long");
					grid_main_usage();
					return FALSE;
				}
				GRID_DEBUG("Explicitely configured pidfile_path=[%s]",
					pidfile_path);
				break;
			case 'q':
				logger_quiet();
			...
			case 's':
				memset(syslog_id, 0, sizeof(syslog_id));
				if (sizeof(syslog_id) <= g_strlcpy(syslog_id, optarg,
						sizeof(syslog_id) - 1)) {
			...
			case 'v':
				...
				break;
			case ':':
				GRID_WARN("Unexpected option at position %d ('%c')", optind,
					optopt);
				return FALSE;
			default:
				GRID_WARN("Unknown option at position %d ('%c')", optind,
					optopt);
				return FALSE;
		}
	}
...
metautils_ignore_signals(void)
{
	...
	if (0 > sigprocmask(SIG_BLOCK, &new_set, &old_set)) {
		g_message("LIBC Some signals could not be blocked : %s",
			strerror(errno));
	}
	...
	if (0 > pthread_sigmask(SIG_BLOCK, &new_set, &old_set)) {
		g_message("PTHREAD Some signals could not be blocked : %s",
			strerror(errno));
	}
}
