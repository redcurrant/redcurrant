...
_copy_hash(GHashTable * src)
{
	...
	if (src) {
		...
		while (g_hash_table_iter_next(&iter, &k, &v))
			g_hash_table_insert(res, g_strdup((gchar *) k),
				metautils_gba_dup(v));
	}
...
#undef NSI_COPY_TABLE_REF
	...
	if (src->writable_vns != NULL) {
		...
		g_slist_foreach(src->writable_vns, _copy_list_element,
			&(dst->writable_vns));
		if (old)
...
namespace_info_list2map(GSList * list_nsinfo, gboolean auto_free)
{
	...
	ht = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
		auto_free ? (GDestroyNotify) namespace_info_free : NULL);
	for (l = list_nsinfo; l; l = l->next) {
		if (l->data)
			g_hash_table_insert(ht, ((struct namespace_info_s *) l->data)->name,
				l->data);
	}
	return ht;
}
...
namespace_info_extract_name(GSList * list_nsinfo, gboolean copy)
{
	...
	for (l = list_nsinfo; l; l = l->next) {
		if (l->data)
			result =
				g_slist_prepend(result, (copy ? g_strndup((gchar *) l->data,
						LIMIT_LENGTH_NSNAME) : l->data));
	}
	return result;
}
...
gchar *
namespace_info_get_data_security(namespace_info_t * ni,
...
gchar *
namespace_info_get_data_treatments(namespace_info_t * ni,
...
namespace_info_is_vns_writable(namespace_info_t * ni, const gchar * vns)
{
	if (ni && ni->writable_vns && vns) {
		return NULL != g_slist_find_custom(ni->writable_vns, vns,
...
gchar *
namespace_info_get_storage_class(namespace_info_t * ni,
