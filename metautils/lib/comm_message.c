...
message_unmarshall(MESSAGE m, void *s, gsize * sSize, GError ** error)
{
	...
	if (!m || !s || !sSize || *sSize < 4) {
		GSETERROR(error, "Invalid parameter (m=%p s=%p sSize=%p/%i)", m, s,
			sSize, sSize ? *sSize : ~0U);
		return 0;
	}
...
gint
message_get_param(MESSAGE m, enum message_param_e mp, void **s, gsize * sSize,
	GError ** error)
{
	const char *name_mp;
	OCTET_STRING_t *os;
...
gint
message_set_param(MESSAGE m, enum message_param_e mp, const void *s,
	gsize sSize, GError ** error)
{
	...
	switch (mp) {
		case MP_ID:
			m->asnMsg->id =
				OCTET_STRING_new_fromBuf(&asn_DEF_OCTET_STRING, s, sSize);
			return 1;
		case MP_NAME:
			m->asnMsg->name =
				OCTET_STRING_new_fromBuf(&asn_DEF_OCTET_STRING, s, sSize);
...
gint
message_add_field(MESSAGE m, const void *name, gsize nameSize,
	const void *value, gsize valueSize, GError ** error)
{
	...
	/*fill the field */
	rc = OCTET_STRING_fromBuf(&(pMember->name), name, strlen_len(name,
			nameSize));
	if (0 != rc) {
...
gint
message_get_field(MESSAGE m, const void *name, gsize name_size, void **value,
	gsize * valueSize, GError ** error)
{
...
message_del_field(MESSAGE m, const void *name, gsize nameSize, GError ** error)
{
	...
	for (i = 0; i < m->asnMsg->content.list.count; i++) {
		...
		if (p) {
			...
			if (0 == strncmp((const char *) pStr->buf, name, MIN((gsize) l,
						nameSize))) {
...
message_get_fields(MESSAGE m, GHashTable ** hash, GError ** error)
{
	...
	*hash =
	...
	for (fn_ptr = field_names; *fn_ptr; fn_ptr++) {
		...
		if (message_get_field(m, *fn_ptr, strlen(*fn_ptr), &value, &value_size,
				error) > 0) {
...
GError *
message_extract_cid(struct message_s * msg, const gchar * n,
	container_id_t * cid)
{
	void *f;
	gsize f_size;
...
message_extract_body_string(struct message_s * msg, gchar ** result)
{
	...
	for (c = b, last = b + bsize; c < last; c++) {
		if (!g_ascii_isprint(*c))
			return NEWERROR(400, "Body contains non printable characters");
	}
