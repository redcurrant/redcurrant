...
static int
_load_data_security(struct storage_policy_s *sp, const char *key,
	namespace_info_t * ni)
{
	int status = 0;
	gchar *datasec = NULL;
	...
	if ((0 == g_ascii_strcasecmp(key, "NONE"))
		|| (0 == g_ascii_strcasecmp(key, "OFF"))) {
...
static int
_load_data_treatments(struct storage_policy_s *sp, const char *key,
	namespace_info_t * ni)
{
	int status = 0;
...
static int
_load_storage_class(struct storage_policy_s *sp, const char *key,
	namespace_info_t * ni)
{
	int status = 0;
...
{
	struct data_treatments_s *result =
		g_malloc0(sizeof(struct data_treatments_s));
...
_dummy_stgpol(void)
{
	struct storage_policy_s *result =
		g_malloc0(sizeof(struct storage_policy_s));
	result->name = g_strdup("none");
...
gint64
data_security_get_int64_param(const struct data_security_s * ds,
	...
	const gchar * asc, gboolean strict)
{
	...
	if (wsc == NULL || storage_class_is_satisfied(wsc->name, asc)) {
		...
		for (GSList * cursor = wsc->fallbacks;
			cursor != NULL && cursor->data != NULL; cursor = cursor->next) {
...
{
	GError *err = NULL;
	const struct data_security_s *old_dsec =
		storage_policy_get_data_security(old);
	const struct data_security_s *new_dsec =
		storage_policy_get_data_security(new);
	...
	if (err != NULL) {
		g_prefix_error(&err,
			...
			data_security_get_name(old_dsec), data_security_get_name(new_dsec));
