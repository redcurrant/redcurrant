...
gboolean
addr_info_get_addr(const addr_info_t * a, gchar * d, gsize dsize,
	guint16 * port)
{
...
gsize
grid_addrinfo_to_string(const struct addr_info_s * a, gchar * dst,
	gsize dst_size)
{
	gchar tmp[256];
	uint16_t port = 0;
...
gboolean
grid_string_to_addrinfo(const gchar * start, const gchar * end,
...
gint
addrinfo_to_sockaddr(const addr_info_t * ai, struct sockaddr *sa,
	gsize * saSize)
{
