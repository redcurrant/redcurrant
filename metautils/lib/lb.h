 ...
 * @return
 */
struct grid_lb_iterator_s *grid_lb_iterator_weighted_round_robin(struct
	grid_lb_s *lb);
 ...
 * @return
 */
struct grid_lb_iterator_s *grid_lb_iterator_scored_round_robin(struct grid_lb_s
	*lb);
 ...
 * @return
 */
struct grid_lb_iterator_s *grid_lb_iterator_scored_random(struct grid_lb_s *lb);

typedef gboolean(*service_filter) (struct service_info_s * si,
	gpointer hook_data);
...
struct service_info_s *grid_lbpool_get_service_from_url(struct grid_lbpool_s
	*glp, const gchar * srvtype, const gchar * url);
