...
service_tag_ASN2API(ServiceTag_t * asn, service_tag_t * api)
{
	...
	/*name */
	memcpy(api->name, asn->name.buf, MIN((size_t) asn->name.size,
			sizeof(api->name)));
	...
	switch (asn->value.present) {
		...
		case ServiceTag__value_PR_s:
			api->type = STVT_STR;
			api->value.s =
				g_strndup((const gchar *) asn->value.choice.s.buf,
				asn->value.choice.s.size);
			return TRUE;
		case ServiceTag__value_PR_macro:
			api->type = STVT_MACRO;
			api->value.macro.type =
				g_strndup((const gchar *) asn->value.choice.macro.type.buf,
				asn->value.choice.macro.type.size);
			api->value.macro.param =
				g_strndup((const gchar *) asn->value.choice.macro.param.buf,
				asn->value.choice.macro.param.size);
			return TRUE;
...
service_info_ASN2API(ServiceInfo_t * asn, service_info_t * api)
{
	...
	memcpy(api->ns_name, asn->nsName.buf, MIN((size_t) asn->nsName.size,
			sizeof(api->ns_name)));
	memcpy(api->type, asn->type.buf, MIN((size_t) asn->type.size,
			sizeof(api->type)));
...
service_tag_API2ASN(service_tag_t * api, ServiceTag_t * asn)
{
	...
	switch (api->type) {
		case STVT_STR:
			...
			OCTET_STRING_fromBuf(&(asn->value.choice.s), api->value.s,
				strlen(api->value.s));
			break;
		case STVT_BUF:
			asn->value.present = ServiceTag__value_PR_s;
			OCTET_STRING_fromBuf(&(asn->value.choice.s), api->value.buf,
				strlen_len((const guint8 *) api->value.buf,
		...
		case STVT_MACRO:
			...
			OCTET_STRING_fromBuf(&(asn->value.choice.macro.type),
				api->value.macro.type, strlen(api->value.macro.type));
			if (api->value.macro.param)
				OCTET_STRING_fromBuf(&(asn->value.choice.macro.param),
					api->value.macro.param, strlen(api->value.macro.param));
			else
...
service_info_API2ASN(service_info_t * api, ServiceInfo_t * asn)
{
	...
	/*header */
	OCTET_STRING_fromBuf(&(asn->type), api->type,
		strlen_len((const guint8 *) api->type, sizeof(api->type)));
	OCTET_STRING_fromBuf(&(asn->nsName), api->ns_name,
		strlen_len((const guint8 *) api->ns_name, sizeof(api->ns_name)));
	...
	const gchar * req_name, GByteArray * body, ...)
{
	...
	va_start(args, body);
	result =
		abstract_sequence_request(cnx, error, &seq_descriptor, req_name, body,
		args);
	va_end(args);
...
service_info_marshall_1(service_info_t * si, GError ** err)
{
	...
	if (encRet.encoded == -1) {
		GSETERROR(err, "Serialization error on '%s'", encRet.failed_type->name);
