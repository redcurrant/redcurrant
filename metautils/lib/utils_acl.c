...
_rule_match(const gchar * addr, addr_rule_t * rule)
{
	...
	if (g_strv_length(mask_tokens) != 4 || g_strv_length(mask_tokens) != 4
		|| g_strv_length(network_tokens) != 4) {
		DEBUG
			("Failed to process acl mask [%s] on client addr [%s] (network [%s])",
			mask, addr, rule->network_addr);
		goto label_exit;
	}
...
_range2netmask(gchar * ip_range)
{
	...
	g_snprintf(buf, sizeof(buf), "%s %d.%d.%d.%d", ip_range_split[0],
...
_parse_acl_bytes(const GByteArray * acl_byte)
{
	...
	range_regex =
		g_regex_new("([0-9]{1,3}.){3}[0-9]{1,3}/[0-9]{1,2}", 0, 0, &error);
	...
	for (i = 0, max = g_strv_length(access_rules); i < max; i++) {
		...
		if (g_str_has_prefix(access_rules[i], "host")) {
			...
			if (g_strv_length(line_splits) == 3) {
				...
				if (g_regex_match_full(range_regex, line_splits[0],
						strlen(line_splits[0]), 0, 0, NULL, &error)) {
...
parse_acl_conf_file(const gchar * file_path, GError ** error)
{
	...
	if (fd == -1) {
		GSETERROR(error, "Failed to open file [%s] : %s", file_path,
			strerror(errno));
		return NULL;
	}
	...
	if (r < 0) {
		GSETERROR(error, "Failed to read data from file [%s] : %s", file_path,
			strerror(errno));
...
parse_acl(const GByteArray * acl_byte, gboolean authorize)
{
	...
	for (int i = 0; i < ((int) g_strv_length(access_rules)); i++) {
		...
		if (g_regex_match_full(range_regex, access_rules[i],
				strlen(access_rules[i]), 0, 0, NULL, &error)) {
			gchar *tmp = NULL;
...
access_rule_to_string(const addr_rule_t * addr_rule)
{
	...
	g_snprintf(tmp, sizeof(tmp),
		"network = [%s] | netmask = [%s] | authorize = [%d]",
		addr_rule->network_addr, addr_rule->network_mask, addr_rule->authorize);
	return g_strdup(tmp);
}
