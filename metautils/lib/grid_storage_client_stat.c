...
#define LIST_NB 7
	...
	if (TRACE_ENABLED()) {
		...
		TRACE("Services added to spy by gscstat %s\n %s\n", ((result < 0)
				? "(not all services selected are added to spy)" : ""), tmp);
		g_free(tmp);
	}
...
gscstat_dumpAllServices(void)
{
	...
	for (i = 0; i < nbElt; i++) {
		...
		tmp = _gscstat_dumpServiceByStruct(pSvc,
			(list == g_gscstat_info.services), GSCSTAT_TAGS_REQPROCTIME);
...
gscstat_tags_end(char *pServiceType, gscstat_service_tags_lst tag)
{
	...
	if ((tag & GSCSTAT_TAGS_REQPROCTIME) != 0) {
		GSCSTAT_TH_LOCK();
		gscstat_service_tag_reqprctime_t *val = &(pSvc->tag_recproctime);

		GSCSTAT_GETCLOCK(val->proctime_end);
		val->last_req = GSCSTAT_CALC(val->proctime_start, val->proctime_end);
		...
		if ((pSvc->tagsEnabled & GSCSTAT_TAGS_REQPROCTIME) != 0) {
			...
			if (val->nb_req == 0)
				...
				g_string_append_printf(str,
					...
					val->sum_req / 1000, val->average_req / 1000, val->nb_req);
		}
		GSCSTAT_TH_UNLOCK();
	}
