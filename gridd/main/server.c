...
_expand_gridd_macro(service_info_t * si, const service_tag_t * tag)
{
	...
	if (g_strv_length(params) != 2) {
		ERROR("Failed to parse gridd special macro param : [%s]",
			tag->value.macro.param);
		g_strfreev(params);
		return;
	}
	else {
		if (g_ascii_strncasecmp(params[0], "i64", strlen(params[0])) == 0) {
			...
			if (!srvstat_get_i64(params[1], &value))
				NOTICE("Failed to get param [%s] in gridd stats map (int64).",
					params[1]);
			service_tag_set_value_i64(service_info_ensure_tag(si->tags,
					tag->name), value);
		}
			...
			if (!srvstat_get_bool(params[1], &value))
				NOTICE("Failed to get param [%s] in gridd stats map (bool).",
					params[1]);
			service_tag_set_value_boolean(service_info_ensure_tag(si->tags,
					tag->name), value);
		}
		else if (g_ascii_strncasecmp(params[0], "double",
				strlen(params[0])) == 0) {
			gdouble value = FALSE;

			if (!srvstat_get_double(params[1], &value))
				NOTICE("Failed to get param [%s] in gridd stats map (double).",
					params[1]);
			service_tag_set_value_float(service_info_ensure_tag(si->tags,
					tag->name), value);
		}
			...
			if (!srvstat_get_string(params[1], &value)) {
				NOTICE("Failed to get param [%s] in gridd stats map (str).",
					params[1]);
				value = g_strdup("");
			}
...
self_register_in_cluster(GError ** err)
{
	...
	if (!serv_tags)
		...
		for (i = 0; i < serv_tags->len; i++) {
			...
			if (tag->type != STVT_MACRO)
				...
				if (0 != g_ascii_strncasecmp(tag->value.macro.type,
						NAME_MACRO_GRIDD_TYPE, strlen(tag->value.macro.type)))
					g_ptr_array_add(si->tags, service_tag_dup(tag));
				else
	...
	if (!register_namespace_service(si, err)) {
		ERROR("Failed to register service in cluster : %s",
...
srv_periodic_register(gpointer d)
{
	...
	if (!self_register_in_cluster(&err))
		NOTICE("Failed to register service in cluster : %s",
			gerror_get_message(err));
	if (err)
		g_clear_error(&err);
}
...
thread_monitoring_reserve(struct server_s *srv)
{
	...
	gboolean too_many_spare_workers, too_many_workers, too_few_spare_workers,
		too_few_workers;
...
thread_monitoring_release(struct server_s *srv)
{
	...
	gboolean too_many_spare_workers =
		spare_workers > srv->mon.max_spare_workers;
...
thread_monitoring_periodic_debug(struct server_s *srv)
{
	...
	/*and then print all */
	DEBUG_DOMAIN(DOMAIN_THREADS,
		"%s wake=%" G_GUINT64_FORMAT " variation=+%" G_GUINT64_FORMAT "-%"
		G_GUINT64_FORMAT, str_dbg, (mon.wake - mon0.wake),
		(mon.creation - mon0.creation), (mon.destruction - mon0.destruction));
	g_free(str_dbg);

	if (too_many_workers) {
		WARN_DOMAIN(DOMAIN_THREADS, "%s ALL THREADS are currently used!",
			srv->name);
		if (server_alert_possible(srv)) {
			str_pool_size =
				accept_pool_to_string(srv->ap, str_pool, sizeof(str_pool));
			SRV_SEND_ERROR(ALERTID_SRV_THREADS,
				"Server [%s] on [%.*s] currently uses all its threads",
				srv->name, str_pool_size, str_pool);
			server_notify_alert(srv);
		}
	}
	else if (max_reached) {
		WARN_DOMAIN(DOMAIN_THREADS, "%s ALL THREADS have been used!",
			srv->name);
		if (server_alert_possible(srv)) {
			str_pool_size =
				accept_pool_to_string(srv->ap, str_pool, sizeof(str_pool));
			SRV_SEND_WARNING(ALERTID_SRV_THREADS,
				"Server [%s] on [%.*s] used all its threads", srv->name,
				str_pool_size, str_pool);
			server_notify_alert(srv);
		}
	}
}
...
thread_monitoring_periodic_stats(struct server_s *srv)
{
	...
	/*  */
	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.current",
		srv->name);
	...
	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.idle",
		srv->name);
	srvstat_set_int(keyTab, (mon.nb_workers - mon.used_workers));

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.max_spare",
		srv->name);
	srvstat_set_int(keyTab, mon.max_spare_workers);

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.max",
		srv->name);
	srvstat_set_int(keyTab, mon.max_workers);

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.min_spare",
		srv->name);
	srvstat_set_int(keyTab, mon.min_spare_workers);

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.gauge.min",
		srv->name);
	srvstat_set_int(keyTab, mon.min_workers);

	/* counters : 64 bits */
	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.counter.max_reached",
		srv->name);
	srvstat_set_u64(keyTab, mon.max_reached);

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.counter.creation",
		srv->name);
	srvstat_set_u64(keyTab, mon.creation);

	g_snprintf(keyTab, sizeof(keyTab), "server.%s.threads.counter.destruction",
		srv->name);
		...
		for (i = 0; i < s->nbHandlers; i++) {
			if (s->handlers[i]->handler == h) {
				for (id = 0; id < ap->count; id++) {
					...
					if (!getsockname(ap->srv[id],
							(struct sockaddr *) &sock_name, &sock_len)
						&& (sock_name.ss_family == PF_INET
							|| sock_name.ss_family == PF_INET6)) {
						if (format_addr((struct sockaddr *) &sock_name, host,
								sizeof(host), str_port, sizeof(str_port),
								error)) {
							*addr = strdup(host);
							*port = atoi(str_port);
							return (1);
...
trace_message(MESSAGE m)
{
	...
	if (err) {
		DEBUG("failed to retrieved the {id,name} of the message : %s",
...
static gint
manage_message(SERVER srv, GByteArray * gba, struct request_context_s *ctx,
	GError ** err)
{
	MESSAGE m = NULL;
	gint found = 0;
...
main_thread(gpointer arg)
{
	...
	while (may_continue) {
		...
		addr_info_to_string(ctx->remote_addr, str_addr_src,
			sizeof(str_addr_src));
		...
		while (may_continue) {
			...
			gba =
				l4v_read_2to(clt, srv->to_connection, srv->to_operation, &gErr);
			if (!gba) {
		...
		if (gErr) {
			switch (gErr->code) {
				...
				case ERRCODE_CONN_CLOSED:
					DEBUG("Connection CLOSED/RESET fd=%i [%s]", clt,
						str_addr_src);
					break;
	...
	for (i = 0; i < nbgroups; i++) {
		...
		if (0 == fnmatch("Plugin.*", group, 0)) {
			...
			if (!g_key_file_has_key(cfgFile, group, NAME_PATH, err)
				|| !(fileName =
					g_key_file_get_string(cfgFile, group, NAME_PATH, err))) {
				GSETERROR(err,
					"The group %s must contain the path of the plugin in the key %s",
					group, NAME_PATH);
				goto errorLabel;
			}
			...
			if (!params) {
				GSETERROR(err,
					"Cannot load the parameters hash for the plugin (group %s in the configuration file)",
					group);
				goto errorLabel;
			}
			...
			if (!mod) {
				GSETERROR(err, "Cannot load the plug-in from file %s (%s)",
...
set_srv_addr(const gchar * url)
{
	...
	if (!l4_address_init_with_url(newaddr, url, &local_error))
		ERROR("Failed to init the server address to [%s] : %s", url,
	...
	for (i = 0; i < nbgroups; i++) {
		...
		if (0 == fnmatch("Server.*", group, 0)) {
			...
			if (!g_key_file_has_key(cfgFile, group, NAME_PLUGINS, err)) {
				GSETERROR(err, "the key '%s' was not found the section %s (1)",
					NAME_PLUGINS, section);
				goto errorLabel;
			}
			...
			memcpy(&(srv->alert_cfg), &default_alert_cfg,
				sizeof(struct alert_cfg_s));
			...
			srvList =
				g_key_file_get_string_list(cfgFile, group, NAME_LISTEN, 0, err);
			if (!srvList) {
				GSETERROR(err, "the key '%s' was not found the section %s (2)",
					NAME_LISTEN, group);
				goto errorLabel;
			}
			...
			pluginList =
				g_key_file_get_string_list(cfgFile, group, NAME_PLUGINS,
				&nbPlugins, err);
			if (!pluginList || nbPlugins <= 0) {
				GSETERROR(err,
					"the key '%s' was not found the section %s, or it is empty",
					NAME_PLUGINS, section);
				goto errorLabel;
			}

			srv->handlers =
				g_try_malloc0(nbPlugins * sizeof(struct message_handler_s *));
			...
			for (; nbPlugins > 0; nbPlugins--) {
				...
				for (h = BEACON_MSGHANDLER.next; h; h = h->next) {
					if (0 == strcmp(h->name, pluginList[nbPlugins - 1])) {
						...
						DEBUG
							("The message handler '%s' has been prepended to the server '%s'",
							pluginList[nbPlugins - 1], BEACON_SRV.next->name);
						break;
					}
				}
				...
				if (!BEACON_SRV.next->handlers[nbPlugins - 1]) {
					GSETERROR(err,
						"The server '%s' requires the message handler '%s'",
						BEACON_SRV.next->name, pluginList[nbPlugins - 1]);
					return 0;
				}
			}
			...
			DEBUG
				("New server created SRV=%s POOL=%p WORKER={min:%d max:%d min_spare:%d max_spare:%d} TO_CNX=%i TO_OP=%i",
				srv->name, (void *) srv->ap, srv->mon.min_workers,
				srv->mon.max_workers, srv->mon.min_spare_workers,
				srv->mon.max_spare_workers, srv->to_connection,
				srv->to_operation);
...
load_defaults(GKeyFile * cfgFile, GError ** err)
{
	...
	must_daemonize =
		g_key_file_get_boolean(cfgFile, NAME_GENERAL, NAME_DAEMON, err);
	if (err && *err)
		return 0;
	...
	if (!pid_file) {
		...
		if (0 > pid_fd) {
			GSETERROR(err,
				"Cannot create and open a new temporary file in %s (%s)",
			...
			DEBUG
				("no pidfile found in the configuration section=%s key=%s : %s",
				NAME_GENERAL, NAME_PIDFILE, pid_file);
		}
	}
	else {
		DEBUG("pidfile found in the configuration section=%s key=%s : %s",
			NAME_GENERAL, NAME_PIDFILE, pid_file);
	}
...
load_service_tags(GKeyFile * cfgFile, GError ** err)
{
	...
	if (g_key_file_has_group(cfgFile, NAME_SERVICETAGS)) {
		tags =
			g_key_file_get_keys(cfgFile, NAME_SERVICETAGS, &number_of_tags,
			err);
		for (cur_tag = tags; *cur_tag; cur_tag++) {
			tag_name = g_strconcat("tag.", *cur_tag, NULL);
			tag_value =
				g_key_file_get_value(cfgFile, NAME_SERVICETAGS, *cur_tag, err);
...
load_service_info(GKeyFile * cfgFile, GError ** err)
{
	if (!g_key_file_has_group(cfgFile, NAME_SERVICE)) {
		DEBUG("No '%s' group in configuration, Old style service declaration",
			NAME_SERVICE);
		old_style = TRUE;
		return 1;
	}
	...
	service_type =
		g_key_file_get_string(cfgFile, NAME_SERVICE, NAME_SRV_TYPE, err);
	/* TODO: service type ok */

	rec_service =
		g_key_file_get_boolean(cfgFile, NAME_SERVICE, NAME_REGISTER, err);
	if (!rec_service)
		load_ns_info =
			g_key_file_get_boolean(cfgFile, NAME_SERVICE, NAME_LOAD_NS_INFO,
			err);
	else
		load_ns_info = TRUE;
...
reload_config(void)
{
	...
	if (!reload_defaults(cfgFile, &err)) {
		if (err && err->message)
			ERROR("Cannot set the default value from configuration: %s",
				err->message);
		else
...
_close_servers(void)
{
	...
	for (srv = BEACON_SRV.next; srv; srv = srv->next) {
		if (srv->ap) {
			...
			if (nb > 0)
				ERROR("%d server sockets could not be stopped : %s", nb,
					error ? error->message : "?");
			if (error)
				g_clear_error(&error);
		}
...
main(int argc, char **args)
{
	...
	srvtimer_register_regular("server core statistics", srv_inner_gauges_update,
		NULL, NULL, 1);
	...
	if (!start_threads()) {
		...
		if (load_ns_info)
			srvtimer_register_regular(service_type,
				srv_periodic_refresh_ns_info, NULL, NULL,
				PERIOD_REFRESH_NS_INFO);
		...
		for (s = BEACON_SRV.next; s && s != &BEACON_SRV; s = s->next) {
			srvtimer_register_regular(s->name,
				(srvtimer_f) thread_monitoring_periodic_debug, NULL, s,
				PERIOD_DEBUG);
			srvtimer_register_regular(s->name,
				(srvtimer_f) thread_monitoring_periodic_stats, NULL, s,
				PERIOD_STATS);
		}
	}
