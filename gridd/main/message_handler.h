...
typedef gint(*message_handler_f) (MESSAGE m, gint cnx, void *param,
	GError ** err);
typedef gint(*message_handler_v2_f) (struct request_context_s * ctx,
	GError ** err);

gint message_handler_add(const char *name, message_matcher_f m,
	message_handler_f h, GError ** err);
gint message_handler_add_v2(const char *name, message_matcher_f m,
	message_handler_v2_f h, const GPtrArray * tags, GError ** err);
 ...
 * @param msg
 */
void reply_context_set_message(struct reply_context_s *ctx, gint code,
	const gchar * msg);
 ...
 * @param bodySize
 */
void reply_context_set_body(struct reply_context_s *ctx, void *body,
	gsize bodySize, guint32 flags);
...
void reply_context_add_header_in_reply(struct reply_context_s *ctx,
	const char *k, GByteArray * v);

void reply_context_add_strheader_in_reply(struct reply_context_s *ctx,
...
gint get_network_socket(message_handler_f h, char **addr, int *port,
	GError ** error);
