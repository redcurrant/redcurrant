...
void
reply_context_set_message(struct reply_context_s *ctx, gint code,
	const gchar * msg)
{
	if (!ctx)
		return;
	...
	TRACE("couple message/code set to (%i %s)", ctx->header.code,
		ctx->header.msg);
}


void
reply_context_set_body(struct reply_context_s *ctx, void *body, gsize bodySize,
	guint32 flags)
{
	if (!ctx)
		return;
	...
	if (body && bodySize > 0) {
		ctx->body.copy =
			(flags & REPLYCTX_DESTROY_ON_CLEAN) | (flags & REPLYCTX_COPY);
		ctx->body.size = bodySize;
		ctx->body.buffer =
			flags & REPLYCTX_COPY ? g_memdup(body, bodySize) : body;
		TRACE("content set to (%p size=%" G_GSIZE_FORMAT ")", ctx->body.buffer,
			ctx->body.size);
	}
}

static void
reply_context_add_bufheader_in_reply(struct reply_context_s *ctx, const char *k,
...
void
reply_context_add_strheader_in_reply(struct reply_context_s *ctx, const char *k,
	const gchar * v)
{
	if (!ctx || !k || !v)
		return;
...
void
reply_context_add_header_in_reply(struct reply_context_s *ctx, const char *k,
	GByteArray * v)
{
	if (!ctx || !k || !v)
		return;
...
reply_ctx_header_adder(gpointer k, gpointer v, gpointer u)
{
	...
	if (!gba->data || gba->len <= 0)
		return;
	rc = message_add_field(answer, (char *) k, strlen((char *) k), gba->data,
		gba->len, NULL);
	if (rc != 1)
...
reply_context_reply(struct reply_context_s *ctx, GError ** err)
{
	...
	if (ctx->extra_headers) {
		...
		g_hash_table_foreach(ctx->extra_headers, reply_ctx_header_adder,
			answer);
	}

	if (ctx->body.buffer && (ctx->body.size > 0)) {
		GRID_TRACE("body set to %p %" G_GSIZE_FORMAT, ctx->body.buffer,
			ctx->body.size);
		if (0 >= message_set_BODY(answer, ctx->body.buffer, ctx->body.size,
				err)) {
...
gint
message_handler_add(const char *name,
...
gint
message_handler_add_v2(const char *name,
	message_matcher_f m, message_handler_v2_f h, const GPtrArray * tags,
	GError ** err)
{
	struct message_handler_s *mh;
...
request_context_create(int fd, addr_info_t * fd_peer)
{
	...
	addrinfo_from_sockaddr(ctx->remote_addr, (struct sockaddr *) &ss_dst,
		ss_dst_len);
	...
	if (fd_peer)
		...
		addrinfo_from_sockaddr(ctx->local_addr, (struct sockaddr *) &ss_src,
			ss_src_len);
	}
