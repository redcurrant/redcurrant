...
gint
format_addr(struct sockaddr *sa, gchar * h, gsize hL, gchar * p, gsize pL,
	GError ** err)
{
	if (!sa) {
	...
	*p = '?';
	...
	if (0 != getnameinfo(sa, ADDRLEN(sa), h, hL, p, pL,
...
gint
resolve(struct sockaddr_storage * sa, const gchar * h, const gchar * p,
	GError ** err)
{
	gint retCode;
	...
	if (retCode != 0) {
		GSETERROR(err, "Cannot resolve [%s]:%s (%s)", (h ? h : "0.0.0.0"),
			(p ? p : "NOPORT"), gai_strerror(retCode));
		return 0;
	}
	if (!ai) {
		GSETERROR(err, "Cannot resolve [%s]:%s (%s)", (h ? h : "0.0.0.0"),
			(p ? p : "NOPORT"), "Unknown error");
		return 0;
	}
...
parse_option_mode(struct working_parameter_s *pParam, char *pV)
{
	...
	if (errno != 0) {
		WARN("Invalid mode found for UNIX socket [%s] : %s", pParam->path.ptr,
			strerror(errno));
	}
	else if (!u64 && pVEnd == pV) {
		WARN("Invalid mode found for UNIX socket [%s] : not an integer",
			pParam->path.ptr);
	}
	else if (u64 > max) {
		WARN("Invalid mode found for UNIX socket [%s] : out of range",
			pParam->path.ptr);
	}
	else {
		pParam->mode = u64;
		pParam->mode |= S_IRUSR | S_IWUSR;
		NOTICE("UNIX socket [%s] will have permissions [%o]", pParam->path.ptr,
			pParam->mode);
	}
}
	...
	if (!pEq) {
		...
		*pEq = '\0';
		...
		if (0 == g_ascii_strcasecmp(pK, "mode")) {
			...
			ERROR("Unrecongnized UNIX socket option [%s] with value [%s]", pK,
				pV);
		}
	}
}
...
static void
init_socket_options_defaults(const gchar * l,
	struct working_parameter_s *pParam)
{
	char *pStr;
...
check_socket_is_absent(struct working_parameter_s *pParam, GError ** err)
{
	...
	for (attempts = 3; attempts > 0; attempts--) {
		if (-1 == stat(pParam->path.ptr, &lStat)) {
			...
			switch (errno) {
				...
				case EACCES:
				case ELOOP:
					GSETERROR(err, "cannot access the socket path : %s",
						strerror(errno));
					return -1;
				default:
					return 0;
			...
			if (!S_ISSOCK(lStat.st_mode)) {
				GSETERROR(err,
					"path %s exists and is not a socket, remove it and restart the server",
					pParam->path.ptr);
				return -1;
			}
			...
			metautils_pclose(&fd);
			if (!rc) {
				GSETERROR(err, "Server socket already up at [%s]",
					pParam->path.ptr);
				return -1;
			}
			else if (-1 == unlink(pParam->path.ptr)) {
				GSETERROR(err, "Cannot remove the socket at [%s] : %s",
					pParam->path.ptr, strerror(errno));
				return -1;
	...
	GSETERROR(err, "Supposed socket [%s] could not be removed",
		pParam->path.ptr);
	return -1;
}
...
accept_add_local(ACCEPT_POOL ap, const gchar * l, GError ** err)
{
	...
	if (-1 == check_socket_is_absent(&wrkParam, err)) {
		GSETERROR(err, "A socket seems already present at [%s]",
			wrkParam.path.ptr);
	}
	...
	if (-1 == (srv = socket(PF_UNIX, SOCK_STREAM, 0))) {
		GSETERROR(err, "Cannot open a new socket PF_UNIX : %s",
			strerror(errno));
		goto errorLabel;
	}

	if (!sock_set_reuseaddr(srv, TRUE)) {
		GSETERROR(err, "Cannot set SO_REUSEADDR on %d [%s] (%s)", srv, l,
			strerror(errno));
		goto errorLabel;
	}
	...
	if (-1 == bind(srv, (struct sockaddr *) &sun, sizeof(struct sockaddr_un))) {
		GSETERROR(err, "cannot bind srv=%d to %s (%s)", srv, l,
			strerror(errno));
		goto errorLabel;
	}

	if (-1 == listen(srv, AP_BACKLOG)) {
		GSETERROR(err, "cannot listen to inbound connections : %s",
			strerror(errno));
		goto errorLabel;
	}
	...
	if (0 != chmod(wrkParam.path.ptr, wrkParam.mode)) {
		...
		ERROR("Failed to set mode [%o] on UNIX socket [%s] : %s", wrkParam.mode,
			wrkParam.path.ptr, strerror(errsav));
		SRV_SEND_WARNING("server",
			"UNIX socket might be not accessible : failed to set mode [%o] on UNIX socket [%s] (%s)",
...
accept_add_inet(ACCEPT_POOL ap, const gchar * h, const gchar * p, GError ** err)
{
	...
	if (-1 == (srv = socket(FAMILY(&sa), SOCK_STREAM, 0))) {
		GSETERROR(err, "Cannot open a %s socket (%s)",
			_get_family_name(FAMILY(&sa)), strerror(errno));
		goto errorLabel;
	}

	if (!sock_set_reuseaddr(srv, TRUE)) {
		GSETERROR(err, "Cannot set SO_REUSEADDR on %d [%s]:%s (%s)", srv, h, p,
			strerror(errno));
		goto errorLabel;
	}
	...
	if (-1 == bind(srv, (struct sockaddr *) (&sa), sizeof(struct sockaddr_in))) {
		GSETERROR(err, "Cannot bind %d on [%s]:%s (%s)", srv, h, p,
			strerror(errno));
		goto errorLabel;
	}

	if (-1 == listen(srv, AP_BACKLOG)) {
		GSETERROR(err, "Cannot listen on %d [%s]:%s (%s)", srv, h, p,
			strerror(errno));
		goto errorLabel;
	}

	if (!accept_add_any(ap, srv, err)) {
		GSETERROR(err, "Cannot monitor %s srv=%d",
			_get_family_name(FAMILY(&sa)), srv);
		goto errorLabel;
	}
...
static int
UNSAFE_accept_do(ACCEPT_POOL ap, struct sockaddr *sa, socklen_t * saSize,
	GError ** err)
{
	if (ap->count <= 0) {
...
remove_unix_socket(int fd)
{
	...
	if (0 != getsockname(fd, (struct sockaddr *) &ss, &ss_size)) {
		...
		/*remove ... now! */
		if (0 != unlink(path)) {
			ERROR("Listen socket (%s) cannot be removed : %s", path,
				strerror(errno));
		}
		else {
	...
	if (ap->srv) {
		...
		for (i = 0, max = ap->count; i < max; i++) {
			...
			if (0 != getsockname(ap->srv[i], (struct sockaddr *) &ss, &ss_size))
				continue;
			if (!format_addr((struct sockaddr *) &ss, host, sizeof(host), port,
					sizeof(port), NULL))
				continue;
			writen =
				g_snprintf(dst + writen_size, dst_size - writen_size, fmt, host,
				port);
			fmt = fmt1;
			if (writen > 0)
				writen_size += writen;
