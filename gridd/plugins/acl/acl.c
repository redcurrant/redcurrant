...
plugin_init(GHashTable * params, GError ** error)
{
	...
	if (!str) {
		DEBUG("Option [%s] not found in acl config, setting default value",
			ACL_REFRESH_CONF_OPTION);
	...
	if (acl_period_refresh_conf < LIMIT_MIN_REFRESH_RATE
		|| acl_period_refresh_conf > LIMIT_MAX_REFRESH_RATE) {
		WARN("[acl_refresh_rate] parameter out of range [%u,%u]",
			LIMIT_MIN_REFRESH_RATE, LIMIT_MAX_REFRESH_RATE);
		acl_period_refresh_conf =
			CLAMP(acl_period_refresh_conf, LIMIT_MIN_REFRESH_RATE,
	...
	srvtimer_register_regular(MODULE_NAME " refresh configuration",
		acl_refresh_conf, NULL, NULL, acl_period_refresh_conf);
