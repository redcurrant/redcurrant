...
GHashTable *
gridd_stats_remote(addr_info_t * ai, gint ms, GError ** err,
	const gchar * pattern)
{
	...
	GByteArray *gba_pattern =
		g_byte_array_append(g_byte_array_new(), (guint8 *) pattern,
		strlen(pattern));
	request =
		message_create_request(err, NULL /*id */ , MSG_NAME, NULL /*body */ ,
		MSGKEY_PATTERN, gba_pattern, NULL);
