	...
	const gchar * pPrefixCt_In, container_id_t * pPrefix_Out)
{
	...
	if (pPrefixCt_In != NULL) {
		...
		if (!hex2bin(pPrefixCt_In, *pPrefix_Out,
				MIN(strlen(pPrefixCt_In) / 2, sizeof(container_id_t)), &err)) {
			ALCS_WARN("Invalid hex prefix [%s] : %s", pPrefixCt_In,
				err->message);
			return err;
		}
	}
	...
	gchar * srvtype, gchar * srvurl, GByteArray ** result)
{
	...
	if (bAllelseByService == TRUE)
		err =
			meta1v2_remote_list_references(m1addr, namespace, *prefix, result);
...
static gchar **
alcs_meta1_GetListServices(gchar * namespace, addr_info_t * m1addr,
 ...
 */
static GError *
alcs_gridclient_container_Create(gs_grid_storage_t * hc, gchar * namespace,
	gchar * sContainer)
{
	gs_error_t *err = NULL;
	...
	if (c == NULL) {
		...
		if (g_dryrun_mode == FALSE) {
			...
			if (c != NULL) {
				ALCS_DEBUG("CREATE Container [%s] created in namespace [%s].",
					sContainer, namespace);
			}
			else {
				// dryrun mode
				DRYRUN_GRID
					("CREATE Container [%s] created in namespace [%s].\n",
					sContainer, namespace);
			...
			if (err) {
				ALCS_SETERR(&g_err, "Cannot create container: (%d) %s",
					err->code, err->msg);
			}
			else
				ALCS_SETERR(&g_err, "Cannot create container: %s",
					"Unknown Error");
		}
...
static ssize_t
_alcs_gridclient_content_feed_from_stream(void *uData, char *b, size_t bSize)
{
	ssize_t nbRead = 0;
	int i;
	...
	ALCS_DEBUG("%s: pHandleStream->position= %d / %d - contentSize=%d",
		__FUNCTION__, pHandleStream->position, (int) bSize, nbBytesTotal);
	...
	if (nbBytesTotal <= pHandleStream->position) {
		...
		ALCS_DEBUG("%s: pHandleStream->position=%d, nbRead=%d\n", __FUNCTION__,
			pHandleStream->position, (int) nbRead);
		for (i = 0; i < nbRead; i++) {
			b[i] = ptr[i];
		}
...
static GError *
alcs_gridclient_content_PUTorAPPEND(gs_grid_storage_t * hc, gchar * namespace,
	gchar * sContainer, gchar * sContent, gchar * buf, int64_t len,
	gboolean bAppendUsed)
{
	GError *g_err = NULL;
	gs_error_t *err = NULL;
	gs_container_t *c = NULL;
	...
	if (c) {
		...
		if (g_dryrun_mode == FALSE) {
			if (bAppendUsed == FALSE) {
				...
				// don't exists, PUT it
				s = gs_upload_content_v2(c, sContent,
					pStream->content_size /*s.st_size */ ,
					_alcs_gridclient_content_feed_from_stream,
					pStream_handle /*&in */ , stgpol, NULL, &err);
				//
			}
			else {
				...
				s = gs_append_content(c, sContent, pStream->content_size,
					_alcs_gridclient_content_feed_from_stream, pStream_handle,
					&err);
			}
		}
		else {
			...
			DRYRUN_GRID("%s content [%s/%s/%s](%dB)\n",
				((bAppendUsed == FALSE) ? "PUT" : "APPEND"), namespace,
				sContainer, sContent, pStream->content_size);
		}
...
alcs_listener_connect(const char *listenerUrl)
{
	...
	if (g_zmq_sock == NULL) {
		...
		g_zmq_sock =
			listener_remote_connect(&err, g_zmq_ctx, listenerUrl, 2000, -1);
		if (err != NULL) {
	...
	if (alcs_listener_connect(listenerUrl) == TRUE) {
		...
		if (buf != NULL) {
			...
			if (err != NULL) {
				...
				ALCS_DEBUG("zmq_send ok");
			}
		}
		else {
			ALCS_ERROR("Faile to convert JSON to String");
		}
	}
}
...
static void
alcs_listener_buildHead(TLstJSONHeader * msgH, char *name, int pid,
	char *status, char *idcrawl)
{
	...
	const gchar * prefixCt, struct _ActionListCtStatData_s *data)
{
	...
	if ((listener_remote_json_addStringToSection(j_datah, "PREFIX",
				(char *) prefixCt) != NULL)
		|| (listener_remote_json_addStringToSection(j_datah, "M1_ID",
				(char *) meta1_url) != NULL)
		|| (listener_remote_json_addStringToSection(j_datah, "SVC_ID",
				(char *) svc_url) != NULL)
		|| (listener_remote_json_addStringToSection(j_datah, "SVC_TYPE",
				(char *) svc_type) != NULL)) {
	...
	if (bMeta1Only == FALSE) {
		...
		if ((listener_remote_json_addIntToSection(j_data, "SVC_NB_PREFIX",
					data->svc_nbPrefix) != NULL)
			|| (listener_remote_json_addIntToSection(j_data,
					"SVC_NB_CONTAINER_M2onM1",
					data->svc_nbContainerTotal) != NULL)
			|| (listener_remote_json_addIntToSection(j_data,
					"SVC_ERR_REQ_META1", data->svc_nbErrReq) != NULL)) {
		...
		if (listener_remote_json_addIntToSection(j_data, "M1_NB_CONTAINER",
				data->m1_nbContainerTotal) != NULL) {
			listener_remote_json_clean(j_data);
			listener_remote_json_clean(j_root);
			return NULL;
		}
	...
	if ((data->contentName != NULL) && (strlen(data->contentName) > 0)) {
		if (listener_remote_json_addStringToSection(j_data, "LISTCONTAINER",
	...
	if ((listener_remote_json_addIntToSection(j_data, "M1_NB_PREFIX",
				data->m1_nbPrefixTotal) != NULL)
		|| (listener_remote_json_addIntToSection(j_data, "M1_NB_NOTMASTER",
				data->m1_notMaster) != NULL)
		|| (listener_remote_json_addIntToSection(j_data, "M1_ERR_BADBDDFILE",
				data->m1_nbBadBddFile) != NULL)
		|| (listener_remote_json_addIntToSection(j_data, "M1_ERR_BADPREFIX",
				data->m1_nbBadPrefix) != NULL)
		|| (listener_remote_json_addIntToSection(j_data, "M1_ERR_REQ_META1",
				data->m1_nbErrReq) != NULL)) {
		listener_remote_json_clean(j_data);
		listener_remote_json_clean(j_root);
		return NULL;
	}
	...
	struct _ActionListCtStatData_s *data)
{
	...
	if ((svc_type == NULL) || (svc_url == NULL)) {
		...
		ALCS_DEBUG("meta1:%s [%s] [%s|%s]\n", meta1_url, prefixCt, svc_type,
			svc_url);
	}
	...
	jobj =
		alcs_listener_request_buildData(bMeta1Only, &msgH, meta1_url, svc_type,
		svc_url, prefixCt, data);
...
alcs_sharedmem_get(gchar * namespace, gchar * contentName)
{
	...
	if (ptr == NULL) {
		...
		ptr->contentName =
			(gchar *) g_malloc0((strlen(contentName) + 2) * sizeof(gchar));
		...
		ptr->namespace =
			(gchar *) g_malloc0((strlen(namespace) + 2) * sizeof(gchar));
...
alcs_sharedmem_free_all(gchar * crawlerIDonly)
{
	...
	while (list) {
		...
		if ((crawlerIDonly == NULL)
			|| (g_ascii_strncasecmp(ptr->contentName, crawlerIDonly,
					strlen(crawlerIDonly)) == 0)) {
...
alcs_sharedmem_saveOnGrid(TAlcsSharedMem * pData, gchar * buf, int len)
{
	...
	g_err =
		alcs_gridclient_container_Create(g_hc, pData->namespace,
		LISTENER_RESULT_CONTAINER_NAME);
	if (g_err) {
	...
	g_err = alcs_gridclient_content_PUTorAPPEND(g_hc, pData->namespace, LISTENER_RESULT_CONTAINER_NAME, pData->contentName, buf, len, pData->bExists);	//FALSE: put command
	if (!g_err)
		pData->bExists = TRUE;
...
static GError *
alcs_sharedmem_add(gchar * namespace, gchar * contentName, gchar * pDataHead,
	GByteArray * result)
{
	GError *g_err = NULL;
	...
	if (len >= ALCS_SAVEBUF_MAX_BYTES) {
		...
		g_snprintf(&(ptr->data[ptr->datalen]),
			ALCS_SAVEBUF_MAX_BYTES - ptr->datalen, "%s\n%s\n",
			((pDataHead != NULL) ? pDataHead : ""), result->data);
...
do_analyzeListContainer(GByteArray * result, gboolean bShowAll)
{
	...
	for (numCt = 0; numCt < nbCt; numCt++) {
		if (listCt[numCt] == NULL)
			continue;
		if (strlen((char *) listCt[numCt]) == 0)
			continue;
	...
	const gchar * prefixCt, container_id_t * prefix)
{
	...
	if ((prefixCt == NULL) || (strlen(prefixCt) == 0)) {
		...
		if (g_ascii_strncasecmp(prefixCt, "R003", 4) == 0)
			data->m1_notMaster++;
		else
			data->m1_nbBadBddFile++;
		return FALSE;
...
do_Execute(const char *listenerUrl, const gchar * meta1_url, gchar * namespace,
	const gchar * prefixCt, gchar * srvtype, gchar * srvurl,
	gboolean ShowDetails, struct _ActionListCtStatData_s *pData,
	GByteArray * pResultListContainer)
{
	GError *g_err = NULL;
	...
	if (ShowDetails == TRUE) {
		...
		if (pResultListContainer->len > 0) {
			...
			g_snprintf(pData->contentName, ALCS_CONTENTNAME_MAX_BYTES, "%s_%s_listcontainer-meta1_%s_%d", ((g_crawlerID == NULL) ? "XXXX" : g_crawlerID), g_cfg_action_name, meta1_url, g_service_pid);	//, numContent);

			//build head of content data
			if ((srvtype != NULL) && (srvurl != NULL)) {
				g_snprintf(&dataHead[0], 150, "[%s: %s ] (prefix=%s)", srvtype,
					srvurl, prefixCt);
			}
			else {
			...
			g_err =
				alcs_sharedmem_add(namespace, pData->contentName, &dataHead[0],
				pResultListContainer);
			if (g_err) {
	...
	alcs_listener_sendData(listenerUrl, meta1_url, srvtype, srvurl, prefixCt,
		pData);
	...
	const gchar * prefixCt, gboolean ShowDetails)
{
	...
	if (do_verifAndInitPrefix(&data, prefixCt, &prefix) == FALSE) {
		// send error...
		alcs_listener_sendData(listenerUrl, meta1_url, NULL, NULL, prefixCt,
			&data);
		return NULL;
	}
	...
	if (meta2cfg == NULL) {
		...
		if (err) {
			...
			data.m1_nbContainerTotal =
				do_analyzeListContainer(result, ShowDetails);
		}

		do_Execute(listenerUrl, meta1_url, namespace, prefixCt, NULL, NULL,
		...
		gchar **result_m2cfg =
			alcs_meta1_GetListServices(namespace, &m1addr, &prefix, &err);
		if (err) {
			data.m1_nbErrReq++;
			g_clear_error(&err);
			...
			while (result_m2cfg[num]) {
				if (result_m2cfg[num] == NULL)
					break;;
				if (strlen((char *) result_m2cfg[num]) == 0) {
					num++;
					continue;
				}
				...
				ALCS_DEBUG("services check: srvtype=[%s], srvurl[%s]", srvtype,
					srvurl);

				err =
					alcs_meta1_GetListContainer(namespace, &m1addr, &prefix,
					FALSE, srvtype, srvurl, &result);
				if (err) {
					data.svc_nbErrReq = 1;
					g_clear_error(&err);

					alcs_listener_sendData(listenerUrl, meta1_url, srvtype,
						srvurl, prefixCt, &data);

				}
				else {

					data.svc_nbContainerTotal =
						do_analyzeListContainer(result, ShowDetails);

					do_Execute(listenerUrl, meta1_url, namespace, prefixCt,
						srvtype, srvurl, ShowDetails, &data, result);
...
void
init_paramMsgRx(struct SParamMsgrx *pParam)
{
	if (pParam == NULL)
		return;
...
void
clean_paramMsgRx(struct SParamMsgrx *pParam)
{
	if (pParam == NULL)
		return;

	if (pParam->namespaceCfg)
		g_free(pParam->namespaceCfg);
	if (pParam->displayListContainer)
		g_free(pParam->displayListContainer);
	if (pParam->listenerUrl)
		g_free(pParam->listenerUrl);
	if (pParam->crawlerID)
		g_free(pParam->crawlerID);
	if (pParam->dryrun)
		g_free(pParam->dryrun);
	...
	struct SParamMsgrx *pParam, gboolean * bShowAll)
{
	...
	if (NULL == (pParam->namespaceCfg =
			get_argv_value(pActParam->argc, pActParam->argv, g_cfg_action_name,
				g_cfg_namespace_cmd_opt_name))) {
		GRID_TRACE("Failed to get namespace from args");
		return FALSE;
	}

	// show all details configuratio
	if (NULL == (pParam->displayListContainer =
			get_argv_value(pActParam->argc, pActParam->argv, g_cfg_action_name,
				g_cfg_displayListContainer_cmd_opt_name))) {
		ALCS_TRACE("Failed to get displayListContainer from args");
	}

	// show all details configuration
	if (NULL == (pParam->listenerUrl =
			get_argv_value(pActParam->argc, pActParam->argv, g_cfg_action_name,
	...
	if ((pParam->crawlerID != NULL) && (strlen(pParam->crawlerID) > 0)) {
		...
		g_strlcpy(g_crawlerID, pParam->crawlerID,
			strlen(pParam->crawlerID) + 1);
	}
...
gboolean
action_set_data_trip_ex(TCrawlerBusObject * obj, const char *sender,
	const char *alldata, GError ** error)
{
	...
	// save response
	char *temp_msg =
		(char *) g_malloc0((SHORT_BUFFER_SIZE * sizeof(char)) +
		sizeof(guint64));
	...
	char *status =
		act_buildResponse(g_cfg_action_name, g_service_pid, actparam.context_id,
		temp_msg);
	g_free(temp_msg);
	...
	GError *err =
		crawler_bus_req_init(g_conn, &req, sender, SERVICE_PATH,
		SERVICE_IFACE_CONTROL);
	if (err) {
		g_prefix_error(&err, "Failed to connectd to crawler services %s : ",
			sender);
		GRID_WARN("Failed to send ack [%s]: %s", msgRx.source_path,
			err->message);
		g_clear_error(&err);
	}
	...
	if (e) {
		ALCS_WARN("Failed to list containers [%s] : %s", msgRx.source_path,
			e->message);
		g_clear_error(&e);
	}
...
gboolean
action_command(TCrawlerBusObject * obj, const char *cmd, const char *alldata,
