	...
	struct SParamMsgrx *pParam)
{
	...
	if (allParam) {
		...
		if (!(pParam->namespace = get_argv_value(pActParam->argc,
					pActParam->argv, action_name, namespace_cmd_opt_name))) {
			GRID_TRACE("Failed to get namespace from args");
			return FALSE;
		}

		if (!(pParam->dryrun = get_argv_value(pActParam->argc,
					pActParam->argv, action_name, dryrun_cmd_opt_name))) {
			g_dryrun_mode = FALSE;
		}
		else {
			g_dryrun_mode = TRUE;
		...
		if (NULL != (tmp = get_argv_value(pActParam->argc,
					pActParam->argv, action_name, timeout_cmd_opt_name))) {
			int itmp = 0;
	...
	if (polix_action_purge_byurl(url, msgRx->meta2_url, msgRx->timeout_request,
		...
		if (0 < result.nb_del) {
			...
			GRID_DEBUG("No chunks deleted from %" G_GINT64_FORMAT
				" sized chunk's list of %s", result.del_size, hc_url_get(url,
					HCURL_WHOLE));
		}
	}
	else {
		if (error)
			GRID_ERROR("ERROR: (%d) %s", gerror_get_code(error),
...
gboolean
action_command(TCrawlerBusObject * obj, const char *cmd,
