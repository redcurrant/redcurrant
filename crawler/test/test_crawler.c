 ...
 * */
static gboolean
_binredir_manage_error(TBinRedir * hbin, gboolean bTestTrueIsGood,
	gchar * msgIfFailed, gboolean * bEnd, gboolean * bErr)
{
	int nb = 0;
	char errmsg[1024] = "";
...
__test_trip_loadandstart(char *path, char *trip, char *args,
	gboolean bTestTrueIsGood, gchar * errMsg)
{
	...
	if (ep) {
		...
		TEST_END_WITH_ERROR("Trip library Failed to load\n");
...
static void
__crawl(struct trip_lib_entry_points *ep)
{
	GVariant *occur = NULL;
...
static gpointer
_thread_srvstub_run(gpointer d)
{
...
static void
__spy_crawler_action(char *cmdline_action, char *cmdline_crawler,
	gboolean bTestTrueIsGood, char *msgIfFailed)
{
	...
	while (bEnd == FALSE) {
		...
		if (g_hbin_crawler) {
			_binredir_manage_error(g_hbin_crawler, bTestTrueIsGood, msgIfFailed,
				&bEnd, &bErr);
		}

		// check error from action
		if (g_hbin) {
			_binredir_manage_error(g_hbin, bTestTrueIsGood, msgIfFailed, &bEnd,
				&bErr);
		}
	};
...
static void
_test_trip_start(gconstpointer userdata)
{
	...
	ep = __test_trip_loadandstart(path_triplib, t->bin_name, t->args,
		t->startNotFailed, t->msgIfFailed);
	if (ep) {
...
static void
_test_crawler_simple(gconstpointer userdata)
{
	TEST_START();
	...
	g_snprintf(cmdline_crawler, 1023, "%s %s %s", t->bin_name,
		(g_bdebug ? "-vvvvvvvv" : ""), t->args);

	__spy_crawler_action(NULL, cmdline_crawler, t->startNotFailed,
		t->msgIfFailed);
...
static void
_test_action_simple(gconstpointer userdata)
{
	TEST_START();
	...
	g_snprintf(cmdline_bin, 1023, "%s %s", t->bin_name,
		(g_bdebug ? "-vvvvvvvv" : ""));
	...
	if (tcss) {
		fprintf(stdout, "tcss->url=[%s], tcss->sscmd=[%d]\n", tcss->url,
			tcss->sscmd);
		g_hsrvstub =
			srvstub_init(tcss->url, tcss->sscmd, tcss->name,
			tcss->responsedata);
		if (!g_hsrvstub)
		...
		GThread *th =
			g_thread_create(_thread_srvstub_run, g_hsrvstub, TRUE, NULL);

		sleep(2);
	}

	// surveillance crawler/action
	__spy_crawler_action(cmdline_bin, cmdline_crawler, t->startNotFailed,
		t->msgIfFailed);
...
static void
_test_action_timeout(gconstpointer userdata)
{
	// enabled timeout
...
getoption(int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "b:a:s:n:dv", long_options, &option_index);
		if (c == -1)
			break;
		...
		switch (c) {
			...
			case 'v':
				...
				break;
			case 'a':
				g_strlcpy(g_args, optarg, ARG_MAX);
				break;
			case 'b':
				g_strlcpy(g_bin_name, optarg, ARG_MAX);
				break;
			case '?':
				ret = FALSE;
				break;
...
main(int argc, char **argv)
{
	...
	static TTestCase testcase_trip[] = {
		{g_bin_name, "/tmp", FALSE, NULL, "xattr_not_found", _test_trip_start,
				TEST_ERRMSG_TRIP_MISSINGARG},
		{g_bin_name, "/toto", FALSE, NULL, "sourcepath_not_found",
				_test_trip_start, TEST_ERRMSG_TRIP_MISSINGARG},
		{g_bin_name, g_args, TRUE, NULL, "simple", _test_trip_start,
				TEST_ERRMSG_TRIP_FAILEDSTART},
	...
	static TTestCase testcase_crawl[] = {
		{g_bin_name,
				"-Otrip_action_purge_container -Otrip=trip_container -- -trip_container.s=/tmp",
				FALSE, NULL, "bad args", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_BADARGS},
		{g_bin_name,
				"-Oaction=action_toto_service -Otrip=trip_container -- -trip_container.s=/tmp",
				FALSE, NULL, "bad_action", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_BADACTION},
		{g_bin_name, "-Otrip=trip_container -- -trip_container.s=/tmp", FALSE,
				NULL, "missing_action", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_BADACTION},
		{g_bin_name, "-Otrip=trip_container11 -- -trip_container.s=/tmp", FALSE,
				NULL, "bad_libtrip", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_BADTRIP},
		{g_bin_name, "-- -trip_container.s=/tmp", FALSE, NULL,
				"missing_libtrip", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_BADTRIP},
		{g_bin_name, "-trip_container.s=/tmp", FALSE, NULL, "bad_args",
				_test_crawler_simple, TEST_ERRMSG_CRAWLER_BADARGS},
		{g_bin_name, g_args, TRUE, NULL, "simple", _test_crawler_simple,
				TEST_ERRMSG_CRAWLER_FAILEDSTART},
		{NULL, NULL, 0, NULL, NULL, NULL, NULL}
	};
	...
	//---------
	// action
	TTestCaseSrvStub tcss_allok[] =
		{ {url_srvstub, SSCMD_ALL_OK_WITHOUTDATA, "", NULL}, {NULL, 0, "",
			NULL} };
	TTestCaseSrvStub tcss_allerr[] =
		{ {url_srvstub, SSCMD_ALL_ERR_WITHOUTDATA, "", NULL}, {NULL, 0, "",
			NULL} };
	TTestCaseSrvStub tcss_allnone[] =
		{ {url_srvstub, SSCMD_ALL_NONE, "", NULL}, {NULL, 0, "", NULL} };


	TTestCase testcase_act[] = {
		{g_bin_name, g_args, FALSE, &tcss_allerr[0],
				"metaX/with_error_without_data", _test_action_simple,
				TEST_ERRMSG_ACTION_MISSINGERR},
		{g_bin_name, g_args, FALSE, &tcss_allok[0], "metaX/timeout",
				_test_action_timeout, TEST_ERRMSG_ACTION_MISSINGERR},
		{g_bin_name, g_args, FALSE, NULL, "metaX/connect_refused",
				_test_action_simple, TEST_ERRMSG_ACTION_MISSINGERR},
		{g_bin_name, g_args, TRUE, &tcss_allok[0],
				"metaX/no_error_without_data", _test_action_simple,
				TEST_ERRMSG_ACTION_FAILEDSRV},
	...
	for (; pT->bin_name; pT++) {
		...
		testcase =
			g_strdup_printf("/crawler/%s/%s", pT->bin_name, pT->testcase_name);
