	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if (NULL != g_err) {
		GRID_DEBUG("Container purge failed (%d) : %s", g_err->code,
			g_err->message);
...
//
static gboolean
meta2_dispatch_all(struct gridd_reply_ctx_s *reply, gpointer gdata,
	gpointer hdata)
{
	gridd_filter *fl;
...
static gboolean
meta2_dispatch_ok(struct gridd_reply_ctx_s *reply, gpointer gdata,
	gpointer hdata)
{
...
static gboolean
meta2_dispatch_error(struct gridd_reply_ctx_s *reply, gpointer gdata,
	gpointer hdata)
{
	GError *error = NULL;
...
static gboolean
meta2_dispatch_none(struct gridd_reply_ctx_s *reply, gpointer gdata,
	gpointer hdata)
{
	return TRUE;
}
...
static gridd_filter M2V2_PURGE_FILTERS[] =
	{ meta2_filter_action_purge_container, NULL };
static gridd_filter M2V2_DEDUP_FILTERS[] =
	{ meta2_filter_action_deduplicate_container, NULL };
...
_get_requests(ESrvStubCmd sscmd, char *name)
{
	...
	if (!name) {
		if (sscmd == SSCMD_ONE_ERR_WITHOUTDATA) {
			GRID_WARN
				("srvstub, %s: name == NULL: SSCMD_ONE_ERR_WITHOUTDATA --> SSCMD_ALL_ERR_WITHOUTDATA",
				__FUNCTION__);
 ...
 */
TSrvStubHandle *
srvstub_init(char *url, ESrvStubCmd sscmd, char *name, void *responsedata)
{
	GError *err = NULL;
	...
	transport_gridd_dispatcher_add_requests(s->dispatcher, _get_requests(sscmd,
			name), NULL);
...
GError *
srvstub_run(TSrvStubHandle * s)
{
	GError *err = NULL;

	if (NULL != (err = network_server_run(s->server))) {
		GRID_WARN("GRIDD run failure : code=%d message=%s", err->code,
			err->message);
		return err;
	}
...
int
srvstub_close(TSrvStubHandle ** s)
{
	TSrvStubHandle *ss = *s;
