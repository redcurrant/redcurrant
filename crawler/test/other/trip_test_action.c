...
trip_start(int argc, char **argv)
{
	...
	if (NULL != (triptest =
			get_argv_value(argc, argv, trip_name, triptest_cmd_opt_name))) {
	}

	if (NULL != (triptestdata =
			get_argv_value(argc, argv, trip_name, triptestdata_cmd_opt_name))) {
	}
	...
	if (NULL == (source_directory_path_ref =
		...
		GRID_ERROR("Bad or missing -%s.%s argument", trip_name,
			source_cmd_opt_name);
		return EXIT_FAILURE;
	}
	/* ------- */
	...
	if (getxattr(source_directory_path_ref, "user.meta2_server.address",
			meta2_url, sizeof(meta2_url)) <= 0) {
		GRID_ERROR
			("Failed during get xattr(meta2) parameter about repository [%s]: (errno:%d)",
			source_directory_path_ref, errno);
	}

	/* Meta1 URL extraction */
	if (getxattr(source_directory_path_ref, "user.meta1_server.address",
			meta1_url, sizeof(meta1_url)) <= 0) {
		GRID_ERROR
			("Failed during get xattr(meta2) parameter about repository [%s]: (errno:%d)",
			source_directory_path_ref, errno);
	}
	...
	if (err) {
		GRID_ERROR("Failed during exploring repository [%s]: %s",
	...
	do {
		...
		do {
			...
			if (item->bTitle == TRUE) {
				...
				switch (item->argc) {
					case 1:
						ret = g_variant_new("(s)", item->argv[0]);
						break;

					case 2:
						ret =
							g_variant_new("(ss)", item->argv[0], item->argv[1]);
						break;

					case 3:
						ret =
							g_variant_new("(sss)", item->argv[0], item->argv[1],
							item->argv[2]);
						break;

					default:
						fprintf(stdout,
							"%d item: Failed to generate GVariant, NOT IMPLEMENTED\n",
							item->argc);
						test_id = 0;
						return NULL;
				};
