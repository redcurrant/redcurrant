...
alcs_listener_sendJSON(const gchar * listenerUrl, json_object * jobj)
{
	if (alcs_listener_connect(listenerUrl) == TRUE) {
		...
		if (buf != NULL) {
			...
			TLstError *err =
				listener_remote_sendBuffer(g_zmq_sock, buf, strlen(buf));
			if (err != NULL) {
				GRID_ERROR("Error (%d) %s", err->code, err->message);
				listener_remote_error_clean(err);
			}
			else
				GRID_TRACE("zmq_send ok");
		}
		else
			GRID_ERROR("Faile to convert JSON to String");
	}
}
 ...
 *   */
struct json_object *
alcs_listener_request_buildData(TLstJSONHeader * msgH, gchar * filename)
{
	...
	if ((listener_remote_json_addStringToSection(j_datah, "NAME",
				(char *) filename) != NULL)) {
	...
	if ((listener_remote_json_addIntToSection(j_data, "NB_FILENAME",
				1) != NULL)) {
		...
		listener_remote_json_addSection(j_root, LST_SECTION_DATAR, j_datah);
...
void
alcs_listener_sendData(const char *listenerUrl, gchar * filename)
{
	TLstJSONHeader msgH;
		...
		if (dbus_message_is_signal(msg, SERVICE_IFACE_ACTION, action_name)) {
			...
			if (!dbus_message_iter_init(msg, &iter)) {
				...
				if (DBUS_TYPE_STRING != dbus_message_iter_get_arg_type(&iter)) {
					...
					GRID_TRACE("crawlerID=[%s]",
						(crawlerID == NULL) ? "null" : crawlerID);
					...
					if (FALSE == ending_signal) {
						...
						GRID_TRACE("source_directory_path=[%s]",
						...
						if (NULL == occur
							...
							GRID_TRACE
								("Faled to get specific signal parameter occur from message");
							continue;
						}
						...
						gchar *value = NULL;
						GVariant *temp_value =
							g_variant_get_child_value(occur, 0);
						if (temp_value) {
						...
						if (value != NULL) {
							source_path =
								g_strconcat(source_directory_path,
								G_DIR_SEPARATOR_S, value, NULL);
							GRID_TRACE("source_path=[%s]", source_path);
						}
						else
							GRID_TRACE("source_path=[null]");
						...
						if ((crawlerID != NULL) && (strlen(crawlerID) > 0)) {
							...
							g_strlcpy(g_crawlerID, crawlerID,
								strlen(crawlerID) + 1);
						}
						...
						sprintf(temp_msg,
							"%s on ACTION_TEST for the context %llu and the file %s",
							ACK_OK, (long long unsigned) context_id,
							source_path);
						...
						if (EXIT_FAILURE == send_signal(conn,
								SERVICE_OBJECT_NAME, SERVICE_IFACE_ACK, ACK_OK,
								ack_parameters))
							GRID_ERROR
