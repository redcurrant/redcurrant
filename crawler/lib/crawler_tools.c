...
void
buildServiceName(char *svc_name, int max_size_svc_name,
	char *prefix_name, char *action_name, int pid, gboolean bPrefixOnly)
{
	if (bPrefixOnly)
		g_snprintf(svc_name, max_size_svc_name, "%s_%s", prefix_name,
			action_name);
	else
		g_snprintf(svc_name, max_size_svc_name, "%s_%s_%d", prefix_name,
			action_name, pid);
}


char *
getBusAddress(char *userdata)
{
	char *add = NULL;
...
guint64
get_child_value_uint64(GVariant * gv, int order)
{
	guint64 value = 0;
...
GVariant *
get_child_value_variant(GVariant * gv, int order)
{
	GVariant *value = NULL;
...
int
get_child_value_int(GVariant * gv, int order)
{
	int value = 0;
...
const gchar *
get_child_value_string(GVariant * gv, int order)
{
...
void
get_child_value_strv(GVariant * gv, int order, gchar *** value)
{
	GVariant *tmp = g_variant_get_child_value(gv, order);
...
int
disassemble_context_occur_argc_argv_uid(GVariant * gv_glued,
	guint64 * context_id, GVariant ** gv_src, int *argc, char ***argv,
	guint64 * service_uid)
{
...
GVariant *
assemble_context_occur_argc_argv_uid(GVariant ** b, guint64 context_id,
	GVariant * gv, int argc, char **argv, guint64 service_uid)
{
...
gchar *
get_argv_value(int argc, char **argv, gchar * module_name,
	gchar * variable_name)
{
	int i;
	gchar *temp = NULL;
...
int
move_file(const char *source_file_path, const char *destination_file_path,
...
load_trip_library(char *path, char *trip_library_name)
{
	...
	if (path && strlen(path) > 0)
		plugin_path =
			g_strconcat(path, G_DIR_SEPARATOR_S, "lib", trip_library_name,
			".so", NULL);
	else
		plugin_path =
			g_strconcat(TRIP_INSTALL_PATH, G_DIR_SEPARATOR_S, "lib",
			trip_library_name, ".so", NULL);

	ret =
		(struct trip_lib_entry_points *) g_malloc0(sizeof(struct
			trip_lib_entry_points));
	...
	if (!g_module_symbol(ret->lib_ref, "trip_start",
			(void **) &(ret->trip_start))) {
		g_free(plugin_path);
		g_free(ret);
		return NULL;
	...
	if (!g_module_symbol(ret->lib_ref, "trip_next",
			(void **) &(ret->trip_next))) {
		g_free(plugin_path);
		g_free(ret);
		return NULL;
	...
	if (!g_module_symbol(ret->lib_ref, "trip_progress",
			(void **) &(ret->trip_progress))) {
		g_free(plugin_path);
		g_free(ret);
		return NULL;
	}
