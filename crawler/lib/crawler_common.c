...
const TCrawlerBusObjectInfo *
crawler_getObjectInfo(void)
{
...
GError *
crawler_ServiceAction_AddServices(TCrawlerBus * handle,
	TCrawlerSvcActList * list_svc, gchar * svc_name)
{
	...
	error =
		crawler_bus_req_init(handle, &r, svc_name, SERVICE_PATH,
		SERVICE_IFACE_ACTION);
	if (error) {
		g_prefix_error(&error,
			"Failed to connectd to new finded actions services %s : ",
			svc_name);
		return error;
	}
	...
	if (!elt) {
		...
		return NEWERROR(-1, "%s: Failed to allocated memory (TCrawlerSvcAct)",
			__FUNCTION__);
	}
	elt->req = r;
	...
	if (!elt->svc_name) {
		crawler_bus_req_clear(&r);
		g_free(elt);
		return NEWERROR(-1, "%s: Failed to allocated memory (svc_name)",
			__FUNCTION__);
	}
...
GError *
crawler_ServiceAction_RemoveServices(TCrawlerSvcActList * list_svc,
	TCrawlerSvcAct * elt)
{
...
TCrawlerSvcAct *
crawler_ServiceAction_GetCrtService(TCrawlerSvcActList * list_svc)
{
	return (TCrawlerSvcAct *) list_svc->crt->data;
}

TCrawlerSvcAct *
crawler_ServiceAction_GetNextService(TCrawlerSvcActList * list_svc,
	gboolean bEnabled)
{
	GSList *pList = NULL;
	GSList *oldCrt = NULL;
...
GError *
crawler_ServiceAction_InitList(TCrawlerBus * handle,
	TCrawlerSvcActList ** list_svc, char *prefix_service_action_name,
	char *action_name)
{
	GError *error = NULL;
...
void
crawler_ServiceAction_ClearList(TCrawlerSvcActList ** list_svc)
{
	TCrawlerSvcAct *elt = NULL;
	GSList *pList = NULL;
...
GError *
crawler_ServiceAction_UpdateList(TCrawlerBus * handle,
	TCrawlerSvcActList * list_svc, char *prefix_service_action_name,
	char *action_name)
{
	char **listnames = NULL;
	...
	buildServiceName(tmp_action_name, 1024, prefix_service_action_name,
		action_name, 0, TRUE);
...
void
crawler_ServiceAction_DumpList(TCrawlerSvcActList * list_svc)
{
	TCrawlerSvcAct *elt = NULL;
...
char *
crawler_ServiceAction_ListToStr(TCrawlerSvcActList * list_svc)
{
	TCrawlerSvcAct *elt = NULL;
	GSList *pList = NULL;
	GString *str = NULL;
...
{
	struct action_context *new_action_ctx =
		g_try_malloc0(sizeof(struct action_context));
...
void
free_action_context(struct action_context *ac)
{
	if (NULL != ac) {
		if (NULL != ac->occur) {
