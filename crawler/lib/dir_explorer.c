...
static GError *
_dir_explore_rec(gchar * current_path, dir_explorer_t * exp)
{
	GError *error = NULL;
	const gchar *fn = NULL;
	...
	if (error) {
		g_prefix_error(&error, "about source directory path[%s]: ",
			current_path);
		return error;
	}
...
GError *
dir_explore(gchar * current_path, dir_explorer_t * exp)
{
	GError *err = NULL;
...
gint
dir_progress(dir_explorer_t * exp)
{
	if (exp->dir_count == 0)
		return 0;
...
gchar *
dir_next_match(dir_explorer_t * exp, GPatternSpec * exclude_pattern,
	dir_explorer_match match, gpointer udata)
{
	...
	while (exp->src_dir_cursor != NULL) {
		...
		if (!exclude_pattern || !g_pattern_match_string(exclude_pattern, dname)) {
...
gchar *
dir_next_file(dir_explorer_t * exp, gchar * exclude_dir_pattern)
{
...
void
dir_explorer_clean(dir_explorer_t * exp)
{
	if (exp == NULL)
		return;
