...
static void
crawler_object_class_init(TCrawlerBusObjectClass * klass)
{
	(void) klass;
}
...
typedef struct
{
	...
	void (*_notify_callback) (TCrawlerReq * req, GError * error,
		void *msgReceived, void *user_data);
} TCrawlerBusCmdCall;
...
typedef struct
{
	...
	void (*_notify_callback) (TCrawlerReq * req, char *msgReceived,
		void *user_data);
} TCrawlerBusCmdSig;
 ...
 */
GError *
crawler_bus_Open(TCrawlerBus ** handle, gchar * address)
{
	GError *error = NULL;
	...
	if ((address == NULL) || (strlen(address) == 0)) {
		...
		if (!h->conn) {
			g_prefix_error(&error,
				"Failed to open connection to bus(system): ");
			return error;
		}
		...
		if (h->conn) {
			...
			if (!dbus_bus_register((DBusConnection *)
					dbus_g_connection_get_connection(h->conn), &dberror)) {
				//if (dberror != NULL) {                
				if (error)
					g_clear_error(&error);
				error =
					NEWERROR(-1, "Failed to register connection to bus %s: %s",
 ...
 */
GError *
crawler_bus_Close(TCrawlerBus ** handle)
{
	// obj
	if ((*handle)->obj) {
		if ((*handle)->conn)
			dbus_g_connection_unregister_g_object((*handle)->conn,
				(GObject *) (*handle)->obj);
...
GError *
_crawler_bus_VerifIsConnected(TCrawlerBus * handle)
{
	if (!handle)
		return NEWERROR(-1, "Bad Handle");
 ...
 */
GError *
crawler_bus_Register(TCrawlerBus * handle, char *service_name,
	char *service_path, const TCrawlerBusObjectInfo * object_info)
{
	...
	if (error) {
		g_prefix_error(&error, "Failed to init request to %s: ",
	...
	if (!handle->obj)
		return NEWERROR(-1,
			"cannot allocate memory for g_object_new() function");

	dbus_g_connection_register_g_object(handle->conn, service_path,
		G_OBJECT(handle->obj));
 ...
 */
GError *
crawler_bus_flush(TCrawlerBus * handle)
{
 ...
 */
GError *
crawler_bus_req_init(TCrawlerBus * handle, TCrawlerReq ** req,
 ...
 */
void
crawler_bus_req_clear(TCrawlerReq ** req)
{
	if (!req)
		return;
 ...
 */
GError *
crawler_bus_reqBase_init(TCrawlerBus * handle, TCrawlerReq ** req)
{
 ...
 */
GError *
crawler_bus_reqBase_RegisterName(TCrawlerReq * req, const char *name)
{
	GError *error = NULL;
 ...
 */
GError *
crawler_bus_reqBase_IsNameExists(TCrawlerReq * req, const char *name,
	gboolean * name_exist)
{
 ...
 */
GError *
crawler_bus_reqBase_GetListNames(TCrawlerReq * req, char ***listnames)
{
	GError *error = NULL;

	if (!dbus_g_proxy_call(req->proxy, "ListNames", &error,
			G_TYPE_INVALID, G_TYPE_STRV, listnames, G_TYPE_INVALID)) {
 ...
 */
GError *
crawler_bus_req_Send_ss_s(TCrawlerReq * req, char *method, int timeout,
	char *dataToSend1, char *dataToSend2, char **dataToreceiv)
{
	...
	if (!method)
		return NEWERROR(-1, "Bad method");
	if (!dataToSend1)
		return NEWERROR(-1, "Bad dataToSend1");
	if (!dataToSend2)
		return NEWERROR(-1, "Bad dataToSend2");
	if (!dataToreceiv)
		return NEWERROR(-1, "Bad dataToreceiv");
...
static void
reply__nc(DBusGProxy * proxy, DBusGProxyCall * call, void *user_data)
{
	GError *error = NULL;
	...
	switch (c->resp_typ) {
		case RESP_AS:{
				...
				if (result)
					c->_notify_callback(c->req, error, reply_list,
						c->user_data);
				if (reply_list)
		...
		case RESP_S:{
				...
				result =
					dbus_g_proxy_end_call(proxy, call, &error, G_TYPE_STRING,
	...
	if (result == FALSE) {
		if (!error)
			error =
				NEWERROR(-1,
				"Failed to get value from previous request: Unknown error\n");
...
GError *
crawler_bus_reqcCall_verifArg(TCrawlerReq * req, char *method_call)
{
	if (!req)
		return NEWERROR(-1, "Request/proxy not available to %s service",
			"Unknown");
	...
	if (!method_call)
		return NEWERROR(-1, "Methode not available to %s service",
			req->svc_name);
 ...
 */
GError *
crawler_bus_reqcCall_Send_s_as(TCrawlerReq * req, char *method_call,
	int timeout, void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char **msgReceived, void *user_data), void *user_data, char *dataToSend)
{
	...
	if (_notify_callback == NULL) {
		dbus_g_proxy_call_no_reply(req->proxy, method_call,	/* data to send */
			/* parameter */
			G_TYPE_STRING, dataToSend, G_TYPE_INVALID);

	}
	else {
		c = g_new0(TCrawlerBusCmdCall, 1);
		CRAWLER_BUSCMD_SAVEUSERDATA(c, req, RESP_AS, user_data,
			_notify_callback);

		call = dbus_g_proxy_begin_call_with_timeout(req->proxy, method_call,	/* data to send */
 ...
 */
GError *
crawler_bus_reqcCall_Send_s_s(TCrawlerReq * req, char *method_call, int timeout,
	void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data, char *dataToSend)
{
	DBusGProxyCall *call = NULL;
	...
	if (!_notify_callback) {
		...
		c = g_new0(TCrawlerBusCmdCall, 1);
		CRAWLER_BUSCMD_SAVEUSERDATA(c, req, RESP_S, user_data,
			_notify_callback);
 ...
 */
GError *
crawler_bus_reqcCall_Send_ss_s(TCrawlerReq * req, char *method_call,
	int timeout, void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data, char *dataToSend1,
	char *dataToSend2)
{
	DBusGProxyCall *call = NULL;
	TCrawlerBusCmdCall *c = NULL;
	...
	if (!_notify_callback) {
		dbus_g_proxy_call_no_reply(req->proxy, method_call,
			/* parameter */
			G_TYPE_STRING, dataToSend1,
			G_TYPE_STRING, dataToSend2, G_TYPE_INVALID);
 ...
 *    */
GError *
crawler_bus_reqcCall_Send_sss_s(TCrawlerReq * req, char *method_call,
	int timeout, void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data, char *dataToSend1,
	char *dataToSend2, char *dataToSend3)
{
	DBusGProxyCall *call = NULL;
	TCrawlerBusCmdCall *c = NULL;
	GError *error = NULL;
	...
	if (!_notify_callback) {
		dbus_g_proxy_call_no_reply(req->proxy, method_call,
			...
			G_TYPE_STRING, dataToSend3, G_TYPE_INVALID);
 ...
 */
GError *
crawler_bus_reqcCall_Send_ts_s(TCrawlerReq * req, char *method_call,
	int timeout, void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data,
	guint64 dataToSend1, char *dataToSend2)
{
	DBusGProxyCall *call = NULL;
	TCrawlerBusCmdCall *c = NULL;
	GError *error = NULL;
...
static void
reply_sig_cb(DBusGProxy * proxy, const char *message, gpointer user_data)
{
	(void) proxy;
	(void) message;
	(void) user_data;
...
GError *
crawler_bus_reqSig_OpenInputQueue(TCrawlerReq * req, const char *signal_name,
	void *user_data)
{
	TCrawlerBusCmdSig *sig = NULL;

	// signal with one argument string (notification callback
	dbus_g_proxy_add_signal(req->proxy, signal_name, G_TYPE_STRING,
		G_TYPE_INVALID);
...
GError *
crawler_bus_reqSig_CloseInputQueue(TCrawlerReq * req, const char *signal_name)
{
	dbus_g_proxy_disconnect_signal(req->proxy, signal_name,
		G_CALLBACK(reply_sig_cb), NULL);
...
GError *
crawler_bus_reqSig_SendMsg(TCrawlerReq * req, gboolean broadcast,
	...
	const char *signal_name, const char *signal_data, void *data)
{
	...
	if (signal_data) {
		dbus_message_append_args(msg, DBUS_TYPE_STRING, &signal_data,
			DBUS_TYPE_INVALID);
	}
	else {
		static char *tmp = "";
	...
	if (broadcast == FALSE) {
		...
		// broadcast
		if (!dbus_connection_send((DBusConnection *)
				dbus_g_connection_get_connection(req->handle->conn), msg,
				NULL)) {
			dbus_message_unref(msg);
			return NEWERROR(-1, "Failed to send msg to %s on %s", service_path,
				service_iface);
		}
	}
