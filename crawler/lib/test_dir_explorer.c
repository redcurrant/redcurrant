...
_erase(const gchar * dn)
{
	...
	if (dir == NULL) {
		g_warning("g_dir_open(%s) failed : (%d) %s", dn, errno,
			strerror(errno));
		g_clear_error(&err);
		return;
	}
...
_population_create(const gchar * basedir, struct citizen_s *population)
{
	for (; population->dirname; ++population) {
		...
		if (0 != g_mkdir_with_parents(dn, 0700))
			g_error("Try error: mkdir(%s) : (%d) %s", dn, errno,
				strerror(errno));
		if (population->basename && population->basename[0]) {
			...
			if (fd < 0)
				g_error("Try error: open(%s) : (%d) %s", full, errno,
					strerror(errno));
			metautils_pclose(&fd);
			g_free(full);
		}
...
main(int argc, char **argv)
{
	...
	global_base =
		g_strdup_printf("/tmp/crawler-test-%d-%ld", getpid(), time(0));
	if (0 != g_mkdir_with_parents(global_base, 0700))
		g_error("Preparation failure [%s] : (%d) %s", global_base, errno,
			strerror(errno));
