 ...
 */
static void
tc_sqliterepo_file_locator(gpointer ignored, const gchar * n, const gchar * t,
	GString * result)
{
	(void) ignored;
	(void) t;
 ...
 */
GError *
tc_sqliterepo_initRepository(const gchar * basedir, gchar * svc_type,
 ...
 */
gchar *
tc_sqliterepo_admget(sqlx_repository_t * repo, gchar * type_,
	gchar * bddnameWithExtension, gchar * key)
{
	...
	err = sqlx_repository_open_and_lock(repo, type_,
		bddnameWithExtension, SQLX_OPEN_LOCAL, &sq3, NULL);

	if (err != NULL) {
		if (err->code < 300 || err->code > 399)
			g_prefix_error(&err, "Open/Lock error: ");

		GRID_WARN("not open and lock: [%s]: %s", bddnameWithExtension,
			err->message);
		g_clear_error(&err);
		return NULL;
	}
	...
	if (!value)
		GRID_WARN("Unknown field \"%s\" on db \"%s\"", key,
...
void
tc_sqliterepo_free(sqlx_repository_t ** repo)
{
	if (!repo)
		return;
