 ...
 */
GError *
tlc_init_connection(TCrawlerBus ** conn, char *service_name, char *service_path,
	char *bus_address, TCrawlerBusObjectInfo * object_info)
{
	GError *error = crawler_bus_Open(conn, bus_address);

	if (error) {
		g_prefix_error(&error, "Failed to open connection to %s: ",
			bus_address);
		return error;
	}

	error =
		crawler_bus_Register(*conn, service_name, service_path, object_info);
	if (error) {
 ...
 */
GError *
tlc_Send_CmdProc(TCrawlerReq * req, int timeout,
	void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data, char *cmd,
	char *alldata)
{
...
GError *
tlc_Send_CmdProcEx(TCrawlerReq * req, int timeout,
	void (*_notify_callback) (TCrawlerReq * req, GError * error,
		char *msgReceived, void *user_data), void *user_data, char *cmd,
	const char *sender, char *alldata)
{
...
GError *
tlc_Send_DataTripEx_noreply(TCrawlerReq * req, void *user_data, char *addSender,
	char *alldata)
{
...
GError *
tlc_Send_Ack_noreply(TCrawlerReq * req, void *user_data, char *cmd,
	char *alldata)
{
