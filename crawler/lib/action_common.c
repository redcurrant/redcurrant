...
void
act_paramact_init(TActParam * pActParam)
{
	if (pActParam == NULL)
		return;
...
void
act_paramact_clean(TActParam * pActParam)
{
	if (pActParam == NULL)
		return;
...
GVariant *
act_disassembleParam(char *param_print, TActParam * pActParamOut)
{
	GVariant *param = NULL;
...
char *
act_buildResponse(gchar * action_name, int pid, guint64 context_id,
	gchar * temp_msg)
{
