...
main_set_defaults(void)
{
	...
	action_ctx_table =
		...
		(GDestroyNotify) free_action_context);
	action_list = NULL;
...
main_configure(int argc, char **args)
{
	g_mutex_lock(mutex_ctx_update);
	if (NULL == action_ctx_table) {
		GRID_ERROR("%s (%d) : Context table failed to create", g_service_name,
			service_pid);
		g_mutex_unlock(mutex_ctx_update);
		return FALSE;
	}
	...
	if (NULL == trip_ep
		|| (EXIT_FAILURE == (int) (trip_ep->trip_start) (argc, args))) {
		GRID_ERROR
			("%s (%d) : Trip library failed to load (Please ensure the path while contain libtrip_*.so are on LD_LIBRARY_PATH)",
	...
	if (NULL != console.action_names) {
		...
		action_list =
			g_strsplit(temp_action_names, opt_value_list_separator, -1);
	...
	my_argc = argc;
	my_argv = args;
	GRID_INFO("%s (%d) : Additional parameters stored", g_service_name,
		service_pid);
...
static gboolean
action_timeout(gpointer key, gpointer val, gpointer udata)
{
	(void) key;
	(void) udata;
...
static gpointer
thread_action_timeout_check(gpointer data)
{
	time_t current_time_stamp;
 ...
 */
static void
action_ctx_update(guint64 context_id)
{
...
gboolean
crawler_command(TCrawlerBusObject * obj, const char *cmd,
	const char *sender, const char *alldata, GError ** error)
{
	...
	GRID_DEBUG
		("%s (%d): command %s received on the system D-Bus control interface",
...
gboolean
crawler_ack(TCrawlerBusObject * obj, const char *cmd,
	const char *alldata, GError ** error)
{
	...
	if ((!strstr(cmd, ACK_OK)) && (!strstr(cmd, ACK_KO))) {
		GRID_DEBUG
			("%s (%d): ERROR: Signal %s received on the system D-Bus acknowledgement, BAD cmd",
			g_service_name, service_pid, cmd);
		return FALSE;
	}
	...
	if (NULL != ack_params) {
		...
		GRID_DEBUG
			("%s (%d): ERROR: Signal %s received on the system D-Bus acknowledgement, bad ack_params",
			g_service_name, service_pid, cmd);
...
static gboolean
sendToListener(gchar * listenerUrl, gchar * crawlerID)
{
	void *zmq_ctx = NULL;
	void *zmq_sock = NULL;
	...
	if (bInit == TRUE) {
		...
		msgH.idmsg = 0;
		g_strlcpy(msgH.status, LISTENER_JSON_KEYNAME_HEAD_STATUS_setcrawlerid,
			LSTJSON_STATUS_MAX_CARACT);
		...
		if (console.dryrun_mode == FALSE) {
			...
			if (err2) {
				GRID_ERROR
					("Error send JSON message ID_crawl to listener (%d) %s",
...
static gboolean
buildCrawlerID(gchar * crawlerID, int sizec, int arg)
{
	time_t t;
	struct tm *tmp;
	...
	g_snprintf(&crawlerID[strlen(crawlerID)], sizec - strlen(crawlerID), "_%d",
		arg);
	return TRUE;
}
...
static void
addItemsToArgvArgc(gchar * crawlerID, gchar * listenerUrl)
{
	...
	if (newv != NULL) {
		if ((console.action_names != NULL) && (console.action_names->len > 0)) {
			...
			if (crawlerID != NULL) {
				//prepare crawlerID
				int len1 =
					strlen(console.action_names->str) + strlen(crawlerID) + 20;
				char *tmp1 = g_malloc0(sizeof(char) * len1);

				g_snprintf(tmp1, len1, "-%s.crawlerID=%s",
					console.action_names->str, crawlerID);
				newv[my_argc] = tmp1;
				my_argc++;
			}
			...
			if (listenerUrl != NULL) {
				//prepare listenerUrl
				int len2 =
					strlen(console.action_names->str) + strlen(listenerUrl) +
					20;
				char *tmp2 = g_malloc0(sizeof(char) * len2);

				g_snprintf(tmp2, len2, "-%s.l=%s", console.action_names->str,
					listenerUrl);
				newv[my_argc] = tmp2;
				my_argc++;
			}
			...
			GRID_ERROR
				("Failed to allocated memory for add crawlerID to message to action send (na action_name)");
	}
	else
		GRID_ERROR
			("Failed to allocated memory for add crawlerID to message to action send");
}

static gboolean
send_startend_signal(gchar * signal_tile)
{
	...
	assemble_context_occur_argc_argv_uid(&signal_param, 0, pOccur, my_argc,
		my_argv, service_uid);
	...
	for (i = 0; i < action_list_length; i++) {
		if (console.dryrun_mode == TRUE) {
			//dryrun mode
			DRYRUN_SENDTOACTION
				("%s (%d) : %s signal %s sent on the system D-Bus interface %s\n",
		...
		for (int s = 0; s < nb_svc_act; s++) {
			TCrawlerSvcAct *svc_act =
				crawler_ServiceAction_GetNextService(g_list_svcAction, FALSE);
			if ((svc_act == NULL) || (svc_act->bEnabled == FALSE)) {
				continue;
			}

			error = tlc_Send_CmdProc(svc_act->req, MAX_ACTION_TIMEOUT * 1000,
				NULL /*crawler_command */ , NULL, signal_tile,
				s_signal_parameters);
			if (error) {
				GRID_ERROR
					("%s (%d) : System D-Bus %s signal sending failed in crawler",
...
gboolean
send_DataTrip_toAction(TCrawlerSvcAct * svc_act,
	struct action_context * temp_action_ctx, GVariant ** occur)
{
	...
	if (console.dryrun_mode == FALSE) {
		...
		if (error)
			GRID_ERROR
				("%s (%d): System D-Bus signal sending failed in crawler %s",
...
static gpointer
thread_manage_trip_data(gpointer data)
{
	GVariant *occur = NULL;
	...
	if ((console.listenerUrl) && (console.listenerUrl->len > 0)) {
		...
		addItemsToArgvArgc(crawlerid, NULL);
	...
	while (!stop_thread && NULL != occur) {
		...
		g_mutex_lock(mutex_ctx_update);
		TCrawlerSvcAct *svc_act =
			crawler_ServiceAction_GetNextService(g_list_svcAction, TRUE);
		g_mutex_unlock(mutex_ctx_update);
		if ((svc_act == NULL) || (svc_act->bEnabled == FALSE)) {
			...
			GRID_DEBUG("%s (%d): Context %llu added to the context table",
				g_service_name, service_pid,
				(unsigned long long) temp_action_ctx->id);

			if (send_DataTrip_toAction(svc_act, temp_action_ctx, &occur)) {
				g_mutex_lock(mutex_ctx_update);
				g_hash_table_insert(action_ctx_table, &(temp_action_ctx->id),
					temp_action_ctx);
				temp_action_ctx = NULL;
				g_mutex_unlock(mutex_ctx_update);
			}
		...
		if (console.ctxWait_sup > 0) {
			...
			if (action_ctx_table_size >= console.ctxWait_sup) {
				do {
					GRID_DEBUG("nb context: %d: Waiting...",
						action_ctx_table_size);
					sleep(1);
					g_mutex_lock(mutex_ctx_update);
					action_ctx_table_size = g_hash_table_size(action_ctx_table);
					g_mutex_unlock(mutex_ctx_update);
				} while ((action_ctx_table_size > console.ctxWait_inf)
					&& (!stop_thread));
			}
		}
		...
		if (console.ctxmax > 0) {
			...
			if (nbctx >= console.ctxmax) {
				stop_thread = TRUE;
				GRID_INFO("%d/%d context sending to action: stop crawler",
					nbctx, console.ctxmax);
			}
		}
...
init_bus(void)
{
	...
	// init bus
	error = tlc_init_connection(&g_tl_conn, g_service_name, SERVICE_PATH, "",	//tmp, //g_dbusdaemon_address, /*pour le bus system: =""*/
	...
	crawler_ServiceAction_InitList(g_tl_conn, &g_list_svcAction,
		SERVICE_ACTION_NAME, action_list[0]);

	// init update action list
	crawler_ServiceAction_UpdateList(g_tl_conn, g_list_svcAction,
		SERVICE_ACTION_NAME, action_list[0]);
	GRID_INFO("%s (%d) : %d Services Action to find", g_service_name,
		service_pid, g_slist_length(g_list_svcAction->list));
...
main_action(void)
{
	...
	control_timeout_thread =
		g_thread_create(thread_action_timeout_check, NULL, FALSE, NULL);
	if (NULL == control_timeout_thread)
		GRID_INFO
			("%s (%d) : System Timeout Ack management thread failed to start...",
			g_service_name, service_pid);
	else
		GRID_INFO("%s (%d) : System Timtout Ack management thread started...",
			g_service_name, service_pid);


	// launch thread for manage trip_data
	control_tripmanage_thread =
		g_thread_create(thread_manage_trip_data, NULL, FALSE, NULL);
	if (NULL == control_tripmanage_thread)
		GRID_INFO
			("%s (%d) : System Trip Data management thread failed to start...",
			g_service_name, service_pid);
	else
		GRID_INFO("%s (%d) : System Trip Data management thread started...",
			g_service_name, service_pid);
...
{
	return
		"-Otrip=<trip_name> -Oaction=<action_name><:...:...> [[-Octxmax=<nb max manage before end>] [-Oreloadaction=<waitTime>] [-Octxwait=<nb_contextWainting_if action more slowly than trip> [-Octxwait_timeout=<nb second>]] [-Olistener=<listenerUrl] -Otriplibpath=<specific_trip_library_dir_path>] -- -trip_name.param_name=<value> -action_name.param_name=<value> [...]\n";
}
