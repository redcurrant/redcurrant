...
_trip_next_content(void)
{
	...
	gchar *temp_val = g_strconcat(current_container_id_str, G_DIR_SEPARATOR_S,
		temp_cur_cont, NULL);
	GVariant *ret =
		g_variant_new(trip_occur_format_string, temp_val, meta2_url);
...
trip_end(void)
{
	...
	if (NULL != current_content_list)
		g_slist_free_full(current_content_list,
			(GDestroyNotify) path_info_clean);
