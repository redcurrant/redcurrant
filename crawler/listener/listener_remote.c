...
/******************************************************************************/
TLstError *
listener_remote_error_new(int code, char *format, ...)
{
	gchar message[1024];
	va_list args;
...
void
listener_remote_error_clean(TLstError * err)
{
	if (err == NULL)
		return;
 ...
 */
void
listener_remote_close(void *zmq_ctx, void *zmq_sock)
{
	if (zmq_sock != NULL)
		zmq_close(zmq_sock);
	if (zmq_ctx != NULL)
 ...
 */
TLstError *
listener_remote_sendBuffer(void *zmq_sock, const char *buf, int buflen)
{
	int rc = 0;
	zmq_msg_t msg;
	TLstError *err = NULL;
	...
	if (rc == 0) {
		...
		err = listener_remote_error_new(-2, "zmq_send failed (%d)", errno);
...
TLstError *
listener_remote_sendJSON(void *zmq_sock, json_object * j_root)
{
 ...
 */
void *
listener_remote_connect(TLstError ** err, void *zmq_ctx,
	const char *listenerUrl, int linger, int64_t hwm)
{
	int rc = 0;
	...
	if (zmq_sock != NULL) {
		...
		// size of out queue
		if (hwm >= 0)
			zmq_setsockopt(zmq_sock, ZMQ_SNDHWM, (uint64_t *) & hwm,
				sizeof(hwm));
		...
		if (rc == 0) {
			...
			*err =
				listener_remote_error_new(-1, "zmq_connect failed (%d)", errno);
	}
	else
		*err = listener_remote_error_new(-1, "zmq_socket failed (%d)", errno);
...
void
listener_remote_closesocket(void *zmq_sock)
{
	if (zmq_sock != NULL) {
		zmq_close(zmq_sock);
		zmq_sock = NULL;
...
struct json_object *
listener_remote_json_init(TLstJSONHeader * msgH, unsigned char bAllDataSection)
{
...
struct json_object *
listener_remote_json_buildHEADObj(TLstJSONHeader * msgH)
{
	...
	if ((listener_remote_json_addStringToSection(j_head, "SRC_NAME",
				msgH->action_name) != NULL)
		|| (listener_remote_json_addIntToSection(j_head, "SRC_ID",
				msgH->action_pid) != NULL)
		|| (listener_remote_json_addStringToSection(j_head, "STATUS",
				msgH->status) != NULL)
		|| (listener_remote_json_addIntToSection(j_head, "MSG_ID",
				msgH->idmsg) != NULL)
		|| (listener_remote_json_addStringToSection(j_head, "CRAWL_ID",
				msgH->idcrawl) != NULL)) {
		return NULL;
	}
	else
...
TLstError *
listener_remote_json_addStringToSection(json_object * j_section, char *key,
	char *value)
{
	json_object *j_obj;
...
TLstError *
listener_remote_json_addIntToSection(json_object * j_section, char *key,
	int value)
{
	json_object *j_obj;
...
TLstError *
listener_remote_json_addSection(json_object * j_root,
...
json_object *
listener_remote_json_newSection(void)
{
...
void
listener_remote_json_clean(json_object * j_obj)
{
	if (j_obj)
...
char *
listener_remote_json_getStr(json_object * object)
{
	if (object)
