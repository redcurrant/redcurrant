...
//socket management
void *listener_remote_connect(TLstError ** err, void *zmq_ctx,
	const char *listenerUrl, int linger, int64_t hwm);
TLstError *listener_remote_sendBuffer(void *zmq_sock, const char *buf,
	int buflen);
...
struct json_object *listener_remote_json_init(TLstJSONHeader * msgH,
	unsigned char bAllDataSection);
TLstError *listener_remote_json_addStringToSection(json_object * j_section,
	char *key, char *value);
TLstError *listener_remote_json_addIntToSection(json_object * j_section,
	char *key, int value);
