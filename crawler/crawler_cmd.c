...
static GError *
tl_init_command(TCrawlerBus * conn, TCrawlerReq ** req, gchar * crawlerName)
{
	GError *err = NULL;
	...
	if (!crawlerName) {
		...
		err =
			crawler_bus_req_init(conn, req, crawlerName, SERVICE_PATH,
...
static char **
tl_send_command_strv(TCrawlerBus * conn, gchar * crawlerName, gchar * cmd,
	gchar * alldata)
{
	char **listnames = NULL;
...
static TCrawlerReq *
tl_send_command(TCrawlerBus * conn, GError ** error,
...
gboolean
crawlerCmd_ack(TCrawlerBusObject * obj, const char *cmd,
...
thread_timeout()
{
	...
	if (MAX_ACTION_TIMEOUT > difftime(current_time_stamp, begining_time_stamp))
		...
		while (!g_stop_thread && g_pending_command) {
			time(&current_time_stamp);
			if (MAX_ACTION_TIMEOUT < difftime(current_time_stamp,
					begining_time_stamp))
				break;
...
{
	return
		"./crawler_cmd [-Oname=<crawler_name>] -Ocommand=<command>]\n./crawler_cmd -Ocommand=help\n";
}
...
usage(void)
{
	...
	g_print
		("\tprogress\t:\tShows the progress percentage and the status of the crawler\n");
	g_print("\n");
}
...
main_action(void)
{
	...
	if (g_crawler_name != NULL) {
		if (!g_strcmp0("list", command))
			g_strlcpy(g_ctrl_command, CTRL_LIST, 30);
		else if (!g_strcmp0("stop", command))
			g_strlcpy(g_ctrl_command, CTRL_STOP, 30);
		else if (!g_strcmp0("pause", command))
			g_strlcpy(g_ctrl_command, CTRL_PAUSE, 30);
		else if (!g_strcmp0("slow", command))
			g_strlcpy(g_ctrl_command, CTRL_SLOW, 30);
		else if (!g_strcmp0("resume", command))
			g_strlcpy(g_ctrl_command, CTRL_RESUME, 30);
		else if (!g_strcmp0("progress", command))
			g_strlcpy(g_ctrl_command, CTRL_PROGRESS, 30);
	}
	else {
		if (!g_strcmp0("list", command))
			g_strlcpy(g_ctrl_command, CTRL_LIST, 30);
		else if (!g_strcmp0("help", command))
			g_strlcpy(g_ctrl_command, "help", 30);
	}
	...
	if (strlen(g_ctrl_command) == 0) {
		...
		if (!g_progress_thread) {
			GRID_ERROR
				("%s (%d) : System D-Bus returned progress thread failed to start...",
