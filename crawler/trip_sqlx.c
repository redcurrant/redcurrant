...
gboolean
extract_from_bdd(gchar * file_path, gchar ** type_, gchar ** seq, gchar ** cid)
{
	...
	// properties admin
	container_name =
		tc_sqliterepo_admget(global_repo, *type_, file_name, "container_name");
	...
	TRIP_INFO
		("file_path=%s: file_name=[%s], *type_=[%s], container_name=[%s], *seq=[%s], *cid=[%s]",
...
trip_start(int argc, char **argv)
{
	...
	if (getxattr(source_directory_path, "user.sqlx_server.address", sqlx_url,
			sizeof(sqlx_url)) <= 0) {
		GRID_ERROR
	...
	err =
		tc_sqliterepo_initRepository(source_directory_path, SQLX_TYPE,
		&global_repo);
	if (err) {
	...
	while (file_path != NULL) {
		...
		if (extract_from_bdd(file_path, &type_, &seq, &cid)) {
			TRIP_INFO("Pass container [%s|%s|%s|%s|%s] to actions",
				/*source_directory_path */ file_path, seq, cid, type_,
				sqlx_url);
