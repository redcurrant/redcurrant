	...
	const char *alldata, GError ** error)
{
	...
	gchar *chexid =
		g_substr(msgRx.source_path, strlen(msgRx.source_path) - 64,
		strlen(msgRx.source_path));
	hc_url_set(url, HCURL_HEXID, chexid);
	GRID_INFO("Sending DEDUP to container [%s]", hc_url_get(url, HCURL_WHOLE));
	gchar *result = NULL;

	e = m2v2_remote_execute_DEDUP(msgRx.meta2_url, NULL, url, g_dryrun_mode,
		&result);
	if (NULL != result) {
		GRID_INFO("Container [%s] deduplicated %s : %s", hc_url_get(url,
				HCURL_WHOLE), ((g_dryrun_mode == TRUE) ? "(DRYRUN mode)" : ""),
			result);
		g_free(result);
	}
	...
	if (e) {
		GRID_WARN("Failed to send DEDUP to container [%s]: %s",
			msgRx.source_path, e->message);
		g_clear_error(&e);
	}
