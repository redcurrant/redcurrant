...
get_content_info(const gchar * meta2_url, gchar * container_id_str,
	gchar * content_name)
{
	GError *error = NULL;
	...
	container_id_hex2bin(container_id_str, strlen(container_id_str),
		&container_id, &error);
	struct meta2_raw_content_s *ret =
		meta2_remote_stat_content(&cnx, container_id, content_name,
		strlen(content_name), &error);
	if (NULL != error) {
		g_clear_error(&error);
		metacnx_close(&cnx);
...
static int
do_work(gchar * namespace, gint8 source_type,
	const struct SActionRulesMotorDataWork *data_work)
{
#define EXIT_IF_NULL_OR_EMPTY(str) if ((NULL == str)||(!g_strcmp0("", str)) ) 	return EXIT_FAILURE
	...
	if (CHUNK_TYPE_ID == source_type) {	/* If the source is a chunk */
		...
		struct crawler_chunk_data_pack_s *data_block =
			g_malloc(sizeof(struct crawler_chunk_data_pack_s));
		...
		if (!get_rawx_info_in_attr(data_work->source_path, &local_error,
				&content_info, &chunk_info)
			|| !get_extra_chunk_info(data_work->source_path, &local_error,
				&chunk_info_extra)) {
		...
		motor_args_init(&args, (gpointer) data_block, source_type, &motor_env,
			namespace);
		...
		struct crawler_sqlx_data_pack_s *data_block =
			g_malloc0(sizeof(struct crawler_sqlx_data_pack_s));

		struct motor_args args;

		sqlx_crawler_data_block_init(data_block, data_work->source_path,
			(char *) data_work->seq, (char *) data_work->cid,
			(char *) data_work->type, (char *) data_work->svc_url);
		motor_args_init(&args, (gpointer) data_block, source_type, &motor_env,
			namespace);
		pass_to_motor((gpointer) (&args));
		...
		struct crawler_meta2_data_pack_s *data_block =
			g_malloc0(sizeof(struct crawler_meta2_data_pack_s));

		struct motor_args args;

		meta2_crawler_data_block_init(data_block, data_work->source_path,
			(char *) data_work->svc_url);
		motor_args_init(&args, (gpointer) data_block, source_type, &motor_env,
			namespace);
		pass_to_motor((gpointer) (&args));
		...
		gchar **my_tokens =
			g_strsplit(data_work->source_path, G_DIR_SEPARATOR_S, -1);
		...
		struct meta2_raw_content_s *raw_info =
			get_content_info(data_work->svc_url, my_tokens[0], my_tokens[1]);
		if (NULL == raw_info) {
			if (NULL != my_tokens)
				g_strfreev(my_tokens);
		...
		content_info.size =
			g_strdup_printf("%" G_GINT64_FORMAT, raw_info->size);
		if (raw_info->metadata)
			content_info.metadata =
				g_strndup((gchar *) raw_info->metadata->data,
				raw_info->metadata->len);
		if (raw_info->system_metadata)
			content_info.system_metadata =
				g_strndup((gchar *) raw_info->system_metadata->data,
				raw_info->system_metadata->len);

		struct motor_args args;

		chunk_crawler_data_block_init(data_block, &content_info, NULL, NULL,
			NULL, NULL);
		motor_args_init(&args, (gpointer) data_block, source_type, &motor_env,
			namespace);
		pass_to_motor((gpointer) (&args));
	...
	struct SParamMsgrx *pParam)
{
	...
	if (allParam) {
		...
		if (NULL == (pParam->namespace =
				get_argv_value(pActParam->argc, pActParam->argv, action_name,
		...
		gchar *temp_source_type =
			get_argv_value(pActParam->argc, pActParam->argv,
			action_name, source_type_cmd_opt_name);

		if (temp_source_type == NULL)
			return FALSE;
		pParam->source_type =
			(gint8) g_ascii_strtoll(temp_source_type, NULL, 10);
		...
		if (NULL == pActParam->occur
			|| FALSE == g_variant_is_of_type(pActParam->occur, gvt)) {
		...
		/* Source path */
		pParam->data_work.source_path =
			(gchar *) get_child_value_string(pActParam->occur, 0);

		if ((META2_TYPE_ID == pParam->source_type)
			|| (CONTENT_TYPE_ID == pParam->source_type)) {
			pParam->data_work.svc_url =
				(gchar *)
				g_variant_get_string(g_variant_get_child_value(pActParam->occur,
					1), NULL);

		}
		else if (SQLX_TYPE_ID == pParam->source_type) {
			pParam->data_work.seq =
				(gchar *)
				g_variant_get_string(g_variant_get_child_value(pActParam->occur,
					1), NULL);
			pParam->data_work.cid =
				(gchar *)
				g_variant_get_string(g_variant_get_child_value(pActParam->occur,
					2), NULL);
			pParam->data_work.type =
				(gchar *)
				g_variant_get_string(g_variant_get_child_value(pActParam->occur,
					3), NULL);
			pParam->data_work.svc_url =
	...
	const char *alldata, GError ** error)
{
	...
	char *temp_msg =
		...
		sizeof(guint64));
	if (EXIT_FAILURE == do_work(msgRx.namespace, msgRx.source_type,
			&(msgRx.data_work))) {
		sprintf(temp_msg, "%s on %s for the context %llu and the file %s",
			ACK_KO, action_name, (long long unsigned) actparam.context_id,
			msgRx.data_work.source_path);
	}
	else {
		sprintf(temp_msg, "%s on %s for the context %llu and the file %s",
			ACK_OK, action_name, (long long unsigned) actparam.context_id,
			msgRx.data_work.source_path);
	}

	char *status =
		act_buildResponse(action_name, service_pid, actparam.context_id,
		temp_msg);
	g_free(temp_msg);
	...
	e = crawler_bus_req_init(conn, &req, sender, SERVICE_PATH,
