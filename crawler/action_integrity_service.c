...
typedef gboolean(*check_func_t) (check_info_t * info, check_result_t * result,
...
static int
_move_temp_db(const gchar * source_path, const gchar * volume_path,
	guint64 * p_service_uid)
{
	...
	if (!g_strcmp0("", source_path)) {
		...
		db_final_path =
			g_strconcat(volume_path, G_DIR_SEPARATOR_S, db_base_name, NULL);

		if (g_dryrun_mode == FALSE) {
			//real mode
			if (EXIT_FAILURE == move_file(db_complete_path, db_final_path,
					TRUE))
				goto label_error;
		}
		else {
			//dryrun mode
			db_complete_path_dryrun =
				g_strconcat(db_complete_path, ".dryrun", NULL);
			DRYRUN_GRID("[%s] --> [%s] not executed, dryrun copy to [%s]\n",
				db_complete_path, db_final_path, db_complete_path_dryrun);
			if (EXIT_FAILURE == move_file(db_complete_path,
					db_complete_path_dryrun, TRUE))
				goto label_error;
		}
...
static int
_fill_temp_db(const gchar * source_path, guint64 * p_service_uid,
	check_info_t * p_check_info)
{
	int ret = EXIT_FAILURE;
	sqlite3 *db = NULL;
	...
	if (NULL == fp) {
		...
		if (SQLITE_OK != sqlite3_prepare(db, CONTAINER_DB_SCHEMA, -1, &stmt,
				NULL))
			goto label_error;
	...
	req_string = g_strconcat("INSERT OR REPLACE INTO chunks VALUES ( '",
		...
		source_path, "', '", p_check_info->ct_info->path, "');", NULL);
...
do_work(const gchar * source_path, guint64 service_uid, GSList * checks)
{
	...
	if (!rawx_get_lock_info(volume_path,
		...
		|| local_error != NULL)
		goto label_error;
	GRID_DEBUG("Volume path: %s, namespace: '%s'", volume_path,
		check_info.ns_name);
	...
	if (!get_rawx_info_in_attr(source_path, &local_error, check_info.ct_info,
			check_info.ck_info)
		|| !get_extra_chunk_info(source_path, &local_error,
			check_info.ck_extra))
		goto label_error;
	...
	for (; c_iter; c_iter = g_slist_next(c_iter)) {
		check = c_iter->data;
		GRID_DEBUG("Starting check [%s] on chunk [%s]", check->name,
	...
	const char *alldata, GError ** error)
{
	...
	if (extract_paramMsgRx(TRUE, &actparam, &msgRx) == FALSE) {
	 ...
	 /**/ if ((msgRx.source_path) && (strlen(msgRx.source_path) > 0)) {
		...
		if (NULL == g_hash_table_lookup(volume_path_table,
				&(actparam.service_uid)))
			g_hash_table_insert(volume_path_table, &(actparam.service_uid),
				get_volume_path(msgRx.source_path));
	}
	...
	GError *err =
		crawler_bus_req_init(conn, &req, sender, SERVICE_PATH,
	...
	tlc_Send_Ack_noreply(req, NULL,
		((resultat != EXIT_FAILURE) ? ACK_OK : ACK_KO), status);
	g_free(status);
