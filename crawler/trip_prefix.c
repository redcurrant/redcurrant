	...
	if (!(err = gridd_client_loop(client))) {
		if (!(err = gridd_client_error(client))) {
			if (message != NULL) {
				...
				TRIP_TRACE("(a)master or slave: [%s]:master=%d", message,
					master);
				g_free(message);
			}
		}
		else
			TRIP_WARN("%s: [%s]: (a)%s", __FUNCTION__, bddname, err->message);
	}
	else
		TRIP_WARN("%s: [%s]: (b)%s", __FUNCTION__, bddname, err->message);
...
trip_start(int argc, char **argv)
{
	...
	if (NULL != (temp_verbose =
			get_argv_value(argc, argv, trip_name, verbose_cmd_opt_name))) {
	...
	if (NULL != (temp_allprefix =
			get_argv_value(argc, argv, trip_name, allprefix_cmd_opt_name))) {
	...
	if (NULL != (temp_infinite =
			get_argv_value(argc, argv, trip_name, infinite_cmd_opt_name))) {
	...
	if (NULL == (source_directory_path =
			get_argv_value(argc, argv, trip_name, source_cmd_opt_name))) {
		TRIP_ERROR("Bad or missing -%s.%s argument", trip_name,
			source_cmd_opt_name);
		return EXIT_FAILURE;
	}
	/* ------- */
	...
	if (getxattr(source_directory_path, "user.meta1_server.address", meta1_url,
			sizeof(meta1_url)) <= 0) {
		GRID_ERROR
			("Cannot get xattr parameters of repository [%s]: (errno %d) %s",
	...
	if (!g_strcmp0("", meta1_url)) {
		TRIP_ERROR
			("Bad xattr azttribute (bad repository?) about repository [%s]",
	...
	TRIP_INFO("Initialize and scan directory [%s]... please Wait...",
		source_directory_path);


	err =
		tc_sqliterepo_initRepository(source_directory_path, META1_TYPE_NAME,
		&global_repo);
	if (err) {
		TRIP_ERROR("Failed on repository init [%s] : %s", source_directory_path,
			err->message);
		if (global_repo)
	...
	if (bAllPrefix == FALSE) {
		...
		if (err) {
			TRIP_ERROR("Failed during exploring repository [%s]: %s",
	...
	while (file_path != NULL) {
		...
		if (bAllPrefix == FALSE) {
			if (file_name) {
				// search prefix
				prefixCt =
					tc_sqliterepo_admget(global_repo, META1_TYPE_NAME,
					file_name, "base_name");
				if (prefixCt == NULL) {
				...
				TRIPERR_ALLOC_AND_FORMAT(prefixCt, TRIPERR_NOTFILE);
		}
		else {
			gchar *tmp = g_malloc0(sizeof(gchar) * 8);
		...
		if (bRetryAnotherPrefix == FALSE) {
			TRIP_DEBUG("Pass prefix [%s|%s|%s](num %d) to actions",
				source_directory_path, prefixCt, meta1_url, current_bdd_nb);
			pVariant =
				g_variant_new(trip_occur_format_string, source_directory_path,
				prefixCt, meta1_url);
		}
