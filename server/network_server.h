...
struct network_transport_s
{
	...
	void (*clean_context) (struct transport_client_context_s *);
...
struct network_client_s
{
	int fd;
	enum
	{ CLT_READ = 0X01, CLT_WRITE = 0X02, CLT_ERROR = 0X04 } events;
 ...
 * @return
 */
struct grid_stats_holder_s *network_server_get_stats(struct network_server_s
	*srv);
...
static inline void
transport_devnull_factory(gpointer factory_udata, struct network_client_s *clt)
{
	(void) factory_udata;
	(void) clt;
}
