...
struct http_parser_s
{
	...
	void (*command_provider) (const gchar * req, const gchar * sel,
		const gchar * ver);
...
struct http_parsing_result_s
{
	gsize consumed;
	enum
	{ HPRC_SUCCESS = 0, HPRC_MORE, HPRC_ERROR } status;
};
...
http_request_create(struct network_client_s *client)
{
	...
	req->client = client;
	req->tree_headers =
		g_tree_new_full(metautils_strcmp3, NULL, g_free, g_free);
...
_access_log(struct req_ctx_s *r, gint status, gsize out_len)
{
	...
	g_string_append_printf(gstr,
		...
		tv_diff1.tv_sec, tv_diff1.tv_usec, status, out_len);
...
http_notify_input(struct network_client_s *clt)
{
	...
	while (!done && data_slab_sequence_has_data(&clt->input)) {
		...
		if (rc.status == HPRC_SUCCESS) {
			...
			if (err) {
				GRID_INFO("Request management error : %d %s", err->code,
					err->message);
				g_clear_error(&err);
