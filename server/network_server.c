...
static struct network_client_s *_endpoint_manage_event(struct network_server_s
	*srv, struct endpoint_s *e);
	...
	network_transport_factory factory, guint32 flags)
{
	...
	srv->endpointv =
		g_realloc(srv->endpointv, sizeof(struct endpoint_s *) * (len + 2));
	srv->endpointv[len] = e;
	srv->endpointv[len + 1] = NULL;
}

void
network_server_bind_host(struct network_server_s *srv, const gchar * url,
	gpointer u, network_transport_factory factory)
{
...
void
network_server_bind_host_throughput(struct network_server_s *srv,
	const gchar * url, gpointer u, network_transport_factory factory)
{
...
network_server_run(struct network_server_s * srv)
{
	...
	for (pu = srv->endpointv; (u = *pu); pu++) {
		if (u->fd < 0)
			return NEWERROR(EINVAL, "DESIGN ERROR : some servers are not open");
	}
	if (!srv->flag_continue)
		return NULL;
...
retry:
	...
	if (0 > fd) {
		...
		if (errno != EAGAIN && errno != EWOULDBLOCK)
			GRID_WARN("fd=%d ACCEPT error (%d %s)", e->fd, errno,
				strerror(errno));
		return NULL;
	}
...
static void
_server_start_one_worker(struct network_server_s *srv,
	gboolean counters_changed)
{
	if (!srv->flag_continue)
		return;
...
_server_update_main_stats(struct network_server_s *srv)
{
	grid_stats_holder_set(srv->stats,
		...
		"server.cnx.counter.close", srv->cnx_close, NULL);
}
...
_thread_cb_worker(gpointer td)
{
	...
	last_update = last_not_idle = srv->bogonow;
...
_ds_feed(int fd, struct data_slab_s *ds)
{
	while (ds->data.buffer.end < ds->data.buffer.alloc) {
		...
		if (r < 0)
			return MACRO_COND((errno == EAGAIN
					|| errno == EWOULDBLOCK), RC_NOTREADY, RC_ERROR);
		if (r == 0)
			return RC_NODATA;
...
int
network_client_send_slab(struct network_client_s *client,
