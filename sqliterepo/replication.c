		...
		switch (sqlite3_column_type(stmt, i)) {
			...
			case SQLITE_FLOAT:
				if (!noreal) {
					...
					OCTET_STRING_fromBuf(&(rf->value.choice.s), (char *) t,
						strlen((char *) t));
			...
			case SQLITE_BLOB:
				do {
					...
					OCTET_STRING_fromBuf(&(rf->value.choice.b), (char *) b,
						bsize);
	...
	Table_t * table)
{
	...
	sql =
		g_strdup_printf("SELECT * FROM %s WHERE ROWID = ?", hashstr_str(name));
		...
		for (pc = clients; pc && *pc; pc++) {
			...
			if (!e)
				...
				if (e->code == CODE_PIPEFROM || e->code == CODE_PIPETO
					...
					++count_success;
					g_ptr_array_add(ctx->resync_todo,
						g_strdup(gridd_client_url(*pc)));
				}
				else
					++count_errors;
				g_clear_error(&e);
	...
	struct sqlx_repctx_s **result)
{
	...
	*result = NULL;
	...
	if (sqlx_repository_replication_configured(sq3->repo)) {
		GError *err =
			election_has_peers(sqlx_repository_get_elections_manager(sq3->repo),
	...
	if (has) {
		...
		sqlite3_update_hook(sq3->db, (sqlite3_update_hook_f) hook_update,
			repctx);
	}
