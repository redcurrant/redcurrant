...
typedef void (*sqlx_file_locator_f) (gpointer locator_data,
	const gchar * base_name, const gchar * base_type, GString * file_name);
...
GError *sqlx_repository_init(const gchar * vol,
	const struct sqlx_repo_config_s *cfg, sqlx_repository_t ** result);
...
struct election_manager_s *sqlx_repository_get_elections_manager(struct
	sqlx_repository_s *repo);

gboolean sqlx_repository_replication_configured(const struct sqlx_repository_s
	*r);
 ...
 * effect. */
void sqlx_repository_configure_maxbases(sqlx_repository_t * repo, guint max);
