...
static gboolean
_destroy_if_no_ref(struct election_member_s *m, gboolean check_age)
{
	if (m->refcount != 0)
		return FALSE;

	if (check_age && (m->last_USE + MMANAGER(m)->delay_max_wait) > time(0)) {
		GRID_INFO("Avoid destroying member %s (too young)", m->name);
		return FALSE;
	}
...
step_LeaveElection_completion(int zrc, const void *d)
{
	...
	if (_destroy_if_no_ref(member, TRUE))
		return;
	if (zrc != ZOK) {
		if (zrc == ZNONODE) {
...
step_AskMaster_start(struct election_member_s *member)
{
	...
	if (NULL != (path = member_masterpath(member))) {
		...
		g_free(path);
		if ((zrc == ZOK || zrc == ZMARSHALLINGERROR) && !member->master_watched) {
			member_ref(member);
...
step_WatchNode_start(struct election_member_s *member)
{
	...
	if (NULL != (path = member_fullpath(member))) {
		...
		// callback may be installed even on marshalling error
		if ((zrc == ZOK || zrc == ZMARSHALLINGERROR) && !member->node_watched) {
			member_ref(member);
...
election_manager_expire(struct election_manager_s *manager, time_t delay)
{
	...
	gboolean _traverse(gpointer k, gpointer v, gpointer u) {
		...
		if ((m->last_USE + delay) < now) {
			if (m->step == STEP_FAILED) {
				to_remove = g_slist_prepend(to_remove, m->key);
			} else if (m->step != STEP_LOST) {
				become_leaver(m);
			}
		}
		return FALSE;
	...
	void _destroy_member(gpointer key, gpointer _unused) {
		(void) _unused;
		lru_tree_remove(manager->lrutree_members, key);
	}
	...
	if (!manager->exiting) {
		lru_tree_foreach_TREE(manager->lrutree_members, _traverse, NULL);
		if (to_remove) {
			g_slist_foreach(to_remove, _destroy_member, NULL);
