...
static void restart_election(struct election_member_s *member);
		...
		struct election_manager_s **result)
{
	...
	*result = NULL;
	...
	manager->url = g_strdup(config->get_local_url(config->ctx));
...
manager_count_active(struct election_manager_s *manager)
{
	...
	gboolean _count(gpointer k, gpointer v, gpointer u) {
		...
		MEMBER_CHECK(v);
		step = MEMBER(v)->step;
		if (step != STEP_NONE && step != STEP_FAILED && step != STEP_LEFT)
			++ count;
		return FALSE;
	}
...
static void
step_DeleteOutdated_completion(int zrc, const void *d)
{
	...
	if (zrc != ZOK && zrc != ZNONODE) {
		GRID_WARN("Could not delete outdated node for [%s,%s]",
				member->name, member->type);
		transition_error(member, EVT_MASTER_KO, zrc);
	} else {
		GRID_INFO("Deleted outdated node for [%s,%s]",
				member->name, member->type);
		...
		const struct Stat *s, const void *d)
{
	...
	if (zrc != ZOK) {
		...
		if (!master) {
			...
			if (g_strcmp0(master, member->manager->url)) {
				transition(member, EVT_MASTER_OK, master);
			} else {
				gchar *master_path = member_masterpath(member);
				if (master_path) {
					GRID_INFO("Found master with same url as member, deleting it "
							"(outdated): %s", master_path);
					int rc = sqlx_sync_adelete(member->manager->sync, master_path,
							-1, step_DeleteOutdated_completion, member);
					if (!_is_zhandle_error(rc))
						member_ref(member);
					g_free(master_path);
				}
			}
...
step_WatchNode_completion(int zrc, const struct Stat *s, const void *d)
{
	...
	if (zrc != ZOK) {
		...
		// watcher was successfully registered
...
static inline gboolean
_ignore_change(int type, int state)
{
	return type == ZOO_SESSION_EVENT &&
			(state == ZOO_CONNECTED_STATE || state == ZOO_EXPIRED_SESSION_STATE);
}
			...
			const char *path, void *d)
{
	...
	if (_ignore_change(type, state)) {
		GRID_INFO("Ignoring WatchMaster (state=%i)", state);
		return;
	}
	...
	if (member->refcount == 0) {
		gchar descr[256];
		member_descr(member, descr, sizeof(descr));
		GRID_INFO("Ignoring WatchMaster because refcount is 0, path=%s type=%i state=%i %s",
				path, type, state, descr);
		member_unlock(member);
		return;
	}
	member_unref(member);
	...
	if (type == ZOO_SESSION_EVENT || type == ZOO_NOTWATCHING_EVENT) {
		...
		if (type == ZOO_DELETED_EVENT) {
			transition(member, EVT_MASTER_KO, NULL);
		} else {
		...
		const char *path, void *d)
{
	...
	if (_ignore_change(type, state)) {
		GRID_INFO("Ignoring WatchNode (state=%i)", state);
		return;
	}
	...
	if (member->refcount == 0) {
		gchar descr[256];
		member_descr(member, descr, sizeof(descr));
		GRID_INFO("Ignoring WatchNode because refcount is 0, path=%s type=%i state=%i %s",
				path, type, state, descr);
		member_unlock(member);
		return;
	}
...
wait_for_final_status(struct election_member_s *member,
		GTimeVal *pmax)
{
	enum WAIT_RESULT ret = STATUS_OK;
	GTimeVal tmp;
	struct election_manager_s *manager = MMANAGER(member);
	GMutex *mutex = member_get_lock(member);
	GCond *cond = member_get_cond(member);
	hashstr_t *key = hashstr_dup(member->key);
	gchar *name = g_strdup(member->name);
	gchar *type = g_strdup(member->type);
	...
	while (!STATUS_FINAL(member->step)) {
		...
		(void) g_cond_timed_wait(cond, mutex, &tmp);
		if (member != manager_get_member(manager, key)) {
			GRID_INFO("member destroyed while waiting for its status [%s,%s]",
					name, type);
			ret = MEMBER_DESTROYED;
			goto end;
		}
		if (member->step == STEP_LEFT) {
			GRID_DEBUG("member left election while waiting for its status [%s,%s]",
					name, type);
			ret = MEMBER_LEFT_ELECTION;
			goto end;
		}
	}
...
static void become_candidate(struct election_member_s *member);
...
init:
	...
	switch(wait_for_final_status(member, &max)) {
	case MEMBER_LEFT_ELECTION:
		member->last_USE = 0;
		restart = TRUE;
		goto init;
	case MEMBER_DESTROYED:
		member = NULL;
		restart = TRUE;
		goto init;
	case STATUS_KO:
		...
		guint max, GTimeVal *end)
{
	...
	MANAGER_CHECK(manager);
	if (manager->exiting || !sqlx_sync_is_connected(manager->sync))
		return 0;
...
become_leaver(struct election_member_s *member)
{
	...
	if (member->myid >= 0) {
		if (step_LeaveElection_start(member) != ZOK)
...
restart_election(struct election_member_s *member)
{
	...
	member_reset(member);
	if (member->manager->exiting || !sqlx_sync_is_connected(MMANAGER(member)->sync)) {
...
static gboolean
_too_many_nodes(struct election_member_s *member, GArray *i64v)
{
	gchar **peers = NULL;
	guint npeers;

	member_get_peers(member, FALSE, &peers);
	if (!peers)
		return FALSE;

	npeers = g_strv_length(peers);
	g_strfreev(peers);

	return i64v->len > npeers + 1;
}

static void
_delete_all_nodes(struct election_member_s *member, GArray *i64v)
{
	guint i;
	gchar *path;

	for (i = 0; i < i64v->len; i++) {
		if ((member->myid = g_array_index(i64v, gint64, i)) < 0)
			continue;
		path = member_fullpath(member);
		sqlx_sync_delete(member->manager->sync, path, -1);
		g_free(path);
	}
...
manage_list(struct election_member_s *member, GArray *i64v)
{
	...
	if (_too_many_nodes(member, i64v)) {
		GRID_WARN("Too many nodes found for [%s.%s], deleting all nodes and restarting election",
				member->name, member->type);
		member_trace(__FUNCTION__, "DELNODES", member);
		_delete_all_nodes(member, i64v);
		restart_election(member);
		return;
	}
	...
	switch (member->step) {
		...
		case STEP_LEAVING:
			switch (evt) {
				...
				case EVT_LEAVE_OK:
					if (GRID_DEBUG_ENABLED())
						GRID_DEBUG("LEFT election [%s.%s]", member->name, member->type);
					member_reset(member);
					member_set_status(member, STEP_LEFT);
					return;
				case EVT_LEAVE_KO:
				case EVT_EXITING:
					become_leaver(member);
...
election_manager_expire(struct election_manager_s *manager, time_t delay)
{
	...
	gboolean _traverse(gpointer k, gpointer v, gpointer u) {
		...
		if (!sqlx_sync_is_connected(manager->sync) || manager->exiting)
			return TRUE;

		if ((m->last_USE + delay) < now) {
			if (m->step == STEP_LEFT) {
				if (m->refcount == 0)
					to_destroy = g_slist_prepend(to_destroy, m);
			} else if (m->step != STEP_LEAVING) {
	...
	void _mdestroy(gpointer _m, gpointer udata) {
		(void) udata;
		struct election_member_s *m = _m;
		lru_tree_remove(manager->lrutree_members, m->key);
	}

	now = time(0);

	if (!sqlx_sync_is_connected(manager->sync))
		return;
	...
	if (!manager->exiting)
		lru_tree_foreach_TREE(manager->lrutree_members, _traverse, NULL);
	if (to_destroy) {
		g_slist_foreach(to_destroy, _mdestroy, NULL);
