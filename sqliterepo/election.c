...
defer_USE(struct election_member_s *member, time_t now)
{
	...
	member_trace(__FUNCTION__ , "defer_USE:begin", member);
	if (!peers || !*peers)
		pending = 0;
	else {
...
on_end_GETVERS(struct event_client_s *mc)
{
	...
	member_trace(__FUNCTION__ , "on_end_GETVERS:begin", member);
	err = gridd_client_error(mc->client);
...
on_reply_GETVERS(gpointer ctx, struct message_s *reply)
{
	...
	if (0 < message_get_BODY(reply, &b, &bsize, NULL)) {
		...
		if (udata->version) {
			GRID_DEBUG("on_reply_GETVERS:begin delete version");
			g_tree_destroy(udata->version);
		} else {
			GRID_DEBUG("on_reply_GETVERS:begin no version");
		}
		udata->version = version;
	}
	GRID_DEBUG("on_reply_GETVERS:end");
