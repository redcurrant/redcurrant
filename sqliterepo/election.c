...
struct election_member_s
{
	...
	GTimeVal zkrq_awget;
	GTimeVal zkrq_awexists;
	GTimeVal zkrq_awsiblings;
	GTimeVal zkrq_awcreate;
	GTimeVal zkrq_awdelete;
...
member_descr(const struct election_member_s *m, gchar *d, gsize ds)
{
	g_snprintf(d, ds,
			...
			"/%"G_GUINT16_FORMAT"/%"G_GUINT16_FORMAT"/%u [%.*s] [%s.%s]",
...
{
	member_trace(__FUNCTION__ , "get_peers:begin", m);
	GError *ret = sqlx_config_get_peers2(MCFG(m), m->name, m->type, nocache, peers);
	member_trace(__FUNCTION__ , "get_peers:end", m);
	return ret;
}
...
{
	member_trace(__FUNCTION__ , "decache:begin", m);
	GError *err = member_get_peers(m, TRUE, NULL);
	g_clear_error(&err);
	member_trace(__FUNCTION__ , "decache:end", m);
}

static inline void
member_kickoff(struct election_member_s *m)
{
	member_trace(__FUNCTION__ , "kickoff:begin", m);
	transition(m, EVT_NONE, NULL);
	member_trace(__FUNCTION__ , "kickoff:end", m);
}
...
{
	member_trace(__FUNCTION__ , "About to lock", m);
	g_mutex_lock(member_get_lock(m));
	member_trace(__FUNCTION__ , "Just locked", m);
}

static inline void
member_unlock(struct election_member_s *m)
{
	member_trace(__FUNCTION__ , "About to unlock", m);
	g_mutex_unlock(member_get_lock(m));
	member_trace(__FUNCTION__ , "Just unlocked", m);
}
		...
		const struct Stat *s, const void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_AskMaster_completion", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	...
	GTimeVal now, diff;
	g_get_current_time(&now);
	timersub(&now, &member->zkrq_awget, &diff);
	GRID_DEBUG("ZKRQ awget took %ld.%06lds", diff.tv_sec, diff.tv_usec);
	if (zrc != ZOK) {
	...
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_AskMaster_completion", member);
		...
		const void *data)
{
	...
	member_lock(member);
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_ListGroup_completion", member);
	MEMBER_CHECK(member);
	member_unref(member);
	member_trace(__FUNCTION__, "DONE", member);
	...
	GTimeVal now, diff;
	g_get_current_time(&now);
	timersub(&now, &member->zkrq_awsiblings, &diff);
	GRID_DEBUG("ZKRQ awsiblings took %ld.%06lds", diff.tv_sec, diff.tv_usec);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_ListGroup_completion", member);
}
...
step_LeaveElection_completion(int zrc, const void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_LeaveElection_completion", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	member_trace(__FUNCTION__, "DONE", member);
	...
	GTimeVal now, diff;
	g_get_current_time(&now);
	timersub(&now, &member->zkrq_awdelete, &diff);
	GRID_DEBUG("ZKRQ awdelete took %ld.%06lds", diff.tv_sec, diff.tv_usec);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_LeaveElection_completion", member);
}
...
step_WatchNode_completion(int zrc, const struct Stat *s, const void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_WatchNode_completion", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	member_trace(__FUNCTION__, "DONE", member);
	...
	GTimeVal now, diff;
	g_get_current_time(&now);
	timersub(&now, &member->zkrq_awexists, &diff);
	GRID_DEBUG("ZKRQ awexists took %ld.%06lds", diff.tv_sec, diff.tv_usec);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_WatchNode_completion", member);
}
...
step_StartElection_completion(int zrc, const char *path, const void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_StartElection_completion", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	member_trace(__FUNCTION__, "DONE", member);
	...
	GTimeVal now, diff;
	g_get_current_time(&now);
	timersub(&now, &member->zkrq_awcreate, &diff);
	GRID_DEBUG("ZKRQ awcreate took %ld.%06lds", diff.tv_sec, diff.tv_usec);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_StartElection_completion", member);
}
			...
			const char *path, void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_WatchMaster_change", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_WatchMaster_change", member);
}
		...
		const char *path, void *d)
{
	...
	member_trace(__FUNCTION__ , "MEMBERLOCK:step_WatchNode_change", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	member_trace(__FUNCTION__, "CHANGE", member);
	...
	member_unlock(member);
	member_trace(__FUNCTION__ , "MEMBERUNLOCK:step_WatchNode_change", member);
}
...
step_StartElection_start(struct election_member_s *member)
{
	...
	member_ref(member);
	g_get_current_time(&member->zkrq_awcreate);
...
step_ListGroup_start(struct election_member_s *member)
{
	...
	path = member_fullpath(member);
	member_ref(member);
	g_get_current_time(&member->zkrq_awsiblings);
...
step_LeaveElection_start(struct election_member_s *member)
{
	...
	path = member_fullpath(member);
	member_ref(member);
	g_get_current_time(&member->zkrq_awdelete);
...
_election_trigger_RESYNC(struct election_manager_s *manager,
		const gchar *name, const gchar *type)
{
	GRID_DEBUG("Election resync %s", name);
...
_election_init(struct election_manager_s *manager, const gchar *name,
		const gchar *type)
{
	GRID_DEBUG("Election init %s", name);
...
_election_start(struct election_manager_s *manager, const gchar *name,
		const gchar *type)
{
	GRID_DEBUG("Election start %s", name);
...
_election_exit(struct election_manager_s *manager, const gchar *name,
		const gchar *type)
{
	GRID_DEBUG("Election exit %s", name);
...
on_end_USE(struct event_client_s *mc)
{
	...
	member_lock(member);
	member_trace(__FUNCTION__ , "on_end_USE:begin", member);
	MEMBER_CHECK(member);
	member_unref(member);
	transition(member, EVT_USE_RES, NULL);
	member_trace(__FUNCTION__ , "on_end_USE:end", member);
	member_unlock(member);
...
defer_USE(struct election_member_s *member, time_t now)
{
	...
	if (!peers || !*peers) {
		member_trace(__FUNCTION__, "defer_USE NOPEERS", member);
		pending = 0;
	} else {
		if (member->pending_USE) {
			member_trace(__FUNCTION__, "defer_USE NOPEND", member);
			pending = 0;
		} else {
			pending = g_strv_length(peers);
			member_trace(__FUNCTION__, "defer_USE FOUNDPEND", member);
		}
	}
...
on_end_PIPEFROM(struct event_client_s *mc)
{
	...
	member_trace(__FUNCTION__ , "on_end_PIPEFROM:begin", member);
	member_lock(member);
	MEMBER_CHECK(member);
	member_unref(member);
	...
	transition(member, EVT_RESYNC_DONE, NULL);

	member_unlock(member);
	member_trace(__FUNCTION__ , "on_end_PIPEFROM:end", member);
...
defer_PIPEFROM(struct election_member_s *member)
{
	...
	member_trace(__FUNCTION__ , "defer_PIPEFROM:begin", member);
	...
	member_trace(__FUNCTION__ , "defer_PIPEFROM:end", member);
}
...
on_end_GETVERS(struct event_client_s *mc)
{
	...
	udata->version = NULL;
	GRID_DEBUG("on_end_GETVERS vremote=%p", vremote);
	...
	GRID_DEBUG("on_end_GETVERS vlocal=%p", vlocal);
	...
	member_unref(member);
	if (!err)
		transition(member, EVT_GETVERS_OK, NULL);
	else if (err->code == CODE_PIPEFROM)
		transition(member, EVT_GETVERS_OUTDATED, NULL);
	else if (err->code == CODE_CONCURRENT)
		transition(member, EVT_GETVERS_CONCURRENT, NULL);
	else {
		...
		transition(member, EVT_GETVERS_ERROR, NULL);
	}
	member_unlock(member);
	member_trace(__FUNCTION__ , "on_end_GETVERS:end", member);
...
on_reply_GETVERS(gpointer ctx, struct message_s *reply)
{
	...
	if (0 < message_get_BODY(reply, &b, &bsize, NULL)) {
		member_trace(__FUNCTION__ , "ONREPLY withversion", udata->member);
		GTree *version;
		...
		if (udata->version) {
			GRID_DEBUG("ONREPLY udataversion=%p", udata->version);
			g_tree_destroy(udata->version);
		} else {
			GRID_DEBUG("ONREPLY version=%p", version ? version : NULL);
		}
		udata->version = version;
	} else {
		member_trace(__FUNCTION__ , "ONREPLY noversion", udata->member);
	}
...
defer_GETVERS(struct election_member_s *member)
{
	...
	member_trace(__FUNCTION__ , "defer_GETVERS:begin", member);
	gchar **peers = NULL;
	GError *err = member_get_peers(member, FALSE, &peers);
	if (err != NULL) {
		GRID_WARN("[%s] Election initiated (%s) but get_peers error: (%d) %s",
	...
	member_trace(__FUNCTION__ , "defer_GETVERS:end", member);
	g_strfreev(peers);
}
...
{
	member_trace(__FUNCTION__ , "restart_election:begin", member);
	if (member->myid > 0)
	...
	member_trace(__FUNCTION__ , "restart_election:end", member);
}
...
manage_list(struct election_member_s *member, GArray *i64v)
{
	...
	member_trace(__FUNCTION__ , "manage_list:begin", member);
	member_set_status(member, STEP_CANDOK);
	...
	if (!member_in_group(member, i64v)) {
		...
		if (member_group_master(member, i64v)) {
			member_trace(__FUNCTION__ , "manage_list:master:begin", member);
			member_set_master_id(member, member->myid);
			member_set_status(member, STEP_PRELEAD);
			defer_GETVERS(member);
			member_trace(__FUNCTION__ , "manage_list:master:end", member);
		}
		else {
			member_trace(__FUNCTION__ , "manage_list:slave:begin", member);
			...
			member_trace(__FUNCTION__ , "manage_list:slave:end", member);
		}
	}
}
...
member_ping(struct election_member_s *member)
{
	...
	member_trace(__FUNCTION__ , "ping:begin", member);
	now = time(0);
	...
	if (!member->last_USE || member->last_USE + delay < now
		...
		if (!defer_USE(member, now))
			become_leaver(member);
	member_trace(__FUNCTION__ , "ping:end", member);
}
	...
	switch (member->step) {
		...
		case STEP_PRELOST:
			switch (evt) {
				...
				case EVT_DISCONNECTED:
				case EVT_USE_RES:
					GRID_INFO("USE_when_in_prelost %s  pasbien:begin", member->name);
					return;
		...
		case STEP_PRELEAD:
			switch (evt) {
				...
				case EVT_DISCONNECTED:
				case EVT_USE_RES:
					GRID_INFO("USE_when_in_prelead %s  pasbien:begin", member->name);
					return;
				case EVT_CREATE_OK:
				case EVT_CREATE_KO:
				case EVT_NODE_LEFT:
