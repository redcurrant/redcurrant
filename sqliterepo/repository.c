...
__test_schema(const gchar * schema, const gchar * version, GByteArray ** raw)
{
	...
	tmp_path = g_strdup_printf("/tmp/schema.%d.%ld.sqlite", getpid(), time(0));
...
__get_schema(sqlx_repository_t * repo, const gchar * type, GByteArray ** res)
{
	...
	if (!raw)
		return (subtype != NULL)
			? (NEWERROR(EINVAL, "Unmanaged schema type [%s] [%s]",
				hashstr_str(ht), type))
	...
	sqlx_repository_t ** result)
{
	...
	repo->flag_autocreate =
		!cfg ? TRUE : BOOL(cfg->flags & SQLX_REPO_AUTOCREATE);
	...
	guint width, guint depth)
{
	...
	/* some sanity checks */
	if (width > 4)
		GRID_WARN("width = %u be careful to the number of files in each level",
			width);
	if (depth > 4)
...
label_retry:
	...
	do {
		...
		if (SQLITE_OK == sqlite3_open(path, &h)) {
			...
			_admin_entry_set_str_noerror(h, "container_name",
				args->logical_name);
...
__admin_load_from_table(struct sqlx_sqlite3_s *sq3)
{
	...
	sqlite3_prepare_debug(rc, sq3->db, "SELECT k,v FROM admin", -1, &stmt,
		NULL);
	if (rc == SQLITE_OK) {
		EXTRA_ASSERT(stmt != NULL);
		while (SQLITE_ROW == (rc = sqlite3_step(stmt))) {
		...
		case SQLITE_CANTOPEN:
			...
			if (attempts-- && (args->create || args->repo->flag_autocreate)) {
				...
				if (!(error = __create_base(args, args->realpath, args->raw))) {
					GRID_TRACE("Base created, retrying open [%s]",
						args->realpath);
					goto retry;
				}
	...
	struct sqlx_sqlite3_s **result, gchar ** pmaster)
{
	...
	if (!expected || !election_configured || !args->is_replicated) {
		...
		GRID_TRACE("Status got=%d expected=%d master=%s", status, expected,
			url);
	...
	if (!err) {
		err = args->repo->cache ? __open_maybe_cached(args, result)
	...
	const gchar * type, const gchar * name)
{
	...
	switch (status) {
		...
		case ELECTION_LEADER:
			...
			break;
		case ELECTION_FAILED:
			err = NEWERROR(500, "Election failed for %s.%s", name, type);
			break;
	}
...
_backup_main(sqlite3 * src, sqlite3 * dst)
{
	...
	if (!backup)
		err = NEWERROR(sqlite3_errcode(dst), sqlite3_errmsg(dst));
	else {
