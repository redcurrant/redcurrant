...
label_retry:
	GRID_DEBUG("create_base:open:begin");
	fd = open(path, O_CREAT|O_EXCL|O_RDWR, 0640);
	GRID_DEBUG("create_base:open:end");
	if (fd < 0) {
		switch (errno) {
			case EINTR:
				goto label_retry;
			...
			case ENOENT: {
				GError *err;
				if (retry--) {
					GRID_DEBUG("create_base:create_dir:begin");
					if (NULL != (err = __create_directory(path)))
						return err;
					GRID_DEBUG("create_base:create_dir:end");
					goto label_retry;
				}
			}
			default: /* FALLTROUGH */
	...
	GRID_DEBUG("create_base:write_rawcontent:begin");
	guint written;
	...
	GRID_DEBUG("create_base:write_rawcontent:end");

	/* Save the base admin fields */
	do {
		GRID_DEBUG("create_base:save_fields:begin");
		sqlite3 *h;
	...
	GRID_DEBUG("create_base:pclose:begin");
	metautils_pclose(&fd);
	GRID_DEBUG("create_base:pclose:end");
	return NULL;
}
