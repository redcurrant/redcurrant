...
static GError *
_get_peers(gpointer ctx, const gchar * n, const gchar * t, gchar *** result)
{
...
static GError *
_get_vers(gpointer ctx, const gchar * n, const gchar * t, GTree ** result)
{
	(void) ctx, (void) n, (void) t;
...
test_create_bad_config(void)
{
	...
	g_assert_error(err, g_quark_from_static_string("sqliterepo"),
		ERRCODE_PARAM);
	g_clear_error(&err);
