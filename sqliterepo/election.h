...
struct election_manager_vtable_s
{
	...
	const struct replication_config_s *(*get_config) (const struct
		election_manager_s *);
	...
	/** Run all the elections failed/pending for too long, then kick them off */
		 ...
		 guint(*retry_elections) (struct election_manager_s *, guint,
		GTimeVal *);

	/** Exit all elections older than 'max' */
	void (*exit_all) (struct election_manager_s *, GTimeVal * max,
		gboolean persist);
	...
	enum election_status_e (*election_get_status) (struct election_manager_s *
		manager, const gchar * name, const gchar * type, gchar ** master_url);
