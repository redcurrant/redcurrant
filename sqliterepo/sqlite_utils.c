	...
	const guint8 * v, gsize vlen)
{
	...
	sqlite3_prepare_debug(rc, db, repl
		...
		: "INSERT OR IGNORE  INTO admin (k,v) VALUES (?,?)", -1, &stmt, NULL);
	if (rc != SQLITE_OK && rc != SQLITE_DONE)
...
{
	GError *e =
		sqlite_admin_entry_set(db, 1, k, v ? v->data : NULL, v ? v->len : 0);
	if (e) {
...
_admin_entry_del(sqlite3 * db, const gchar * k)
{
	...
	sqlite3_prepare_debug(rc, db, "DELETE FROM admin WHERE k = ?", -1, &stmt,
		NULL);
	if (rc != SQLITE_OK && rc != SQLITE_DONE)
		err = NEWERROR(500, "DB error: (%d) %s", rc, sqlite3_errmsg(db));
	else {
...
void
sqlx_admin_set_gba(struct sqlx_sqlite3_s *sq3, const gchar * k,
	GByteArray * gba)
{
	GByteArray *prev;
...
void
sqlx_admin_init_str(struct sqlx_sqlite3_s *sq3, const gchar * k,
	const gchar * v)
{
...
gint64
sqlx_admin_get_i64(struct sqlx_sqlite3_s * sq3, const gchar * k,
	const gint64 def)
{
...
void
sqlx_admin_inc_i64(struct sqlx_sqlite3_s *sq3, const gchar * k,
	const gint64 delta)
{
	gchar *s = sqlx_admin_get_str(sq3, k);
...
void
sqlx_admin_inc_version(struct sqlx_sqlite3_s *sq3, const gchar * k,
	const int delta)
{
	...
	*(p++) = '\0';
	g_snprintf(buf, sizeof(buf), "%" G_GINT64_FORMAT ":%" G_GINT64_FORMAT,
		g_ascii_strtoll(prev, NULL, 10) + delta, g_ascii_strtoll(p, NULL, 10));
	g_free(prev);
	sqlx_admin_set_str(sq3, k, buf);
}
...
sqlx_admin_inc_all_versions(struct sqlx_sqlite3_s *sq3, const int delta)
{
	gboolean runner(gchar * k0, GByteArray * v, gpointer ignored)
	{
		...
		*(p++) = '\0';
		...
		g_snprintf(prev, v->len + 3, "%" G_GINT64_FORMAT ":%" G_GINT64_FORMAT,
			v0, v1);
