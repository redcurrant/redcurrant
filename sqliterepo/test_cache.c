...
test_fail(sqlx_cache_t * cache)
{
	...
	g_debug("sqlx_cache_lock_base(-1) : failed as expected : code=%d %s",
	...
	g_debug("sqlx_cache_lock_base(32767) : failed as expected : code=%d %s",
		err->code, err->message);
	g_error_free(err);
	...
	g_debug("sqlx_cache_lock_base(0) : failed as expected : code=%d %s",
		err->code, err->message);
	g_error_free(err);
}
