...
GError *
create_zk_node(struct zk_manager_s * manager, gchar * subdir, gchar * name,
	gchar * data)
{
	EXTRA_ASSERT(manager != NULL);
	...
	if (rc != ZOK && rc != ZNODEEXISTS)
		return NEWERROR(0, "Failed to create Zk node [%s], zk code [%d]", name,
			rc);
	return NULL;
}
...
GError *
list_zk_children_node(struct zk_manager_s * manager, gchar * sub_dir,
	GSList ** result)
{
	...
	for (i = 0; i < sv.count; i++) {
		...
		if (rc != ZOK) {
			err =
				NEWERROR(0, "Failed to get node [%s] , zk code [%d]", fullpath,
				rc);
			goto end_error;
		}
...
delete_zk_node(struct zk_manager_s * manager, gchar * subdir, gchar * name)
{
	...
	if (rc != ZOK && rc != ZNONODE)
		return NEWERROR(0, "Failed to delete zk  node [%s], zk code [%d]", name,
			rc);
	return NULL;
}
