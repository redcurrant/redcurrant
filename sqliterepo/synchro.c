...
static void _set_exit_hook(struct sqlx_sync_s *ss, void (*on_exit) (void *),
	void *on_exit_ctx);
...
static void
_set_exit_hook(struct sqlx_sync_s *ss, void (*on_exit) (void *),
	void *on_exit_ctx)
{
	EXTRA_ASSERT(ss != NULL);
	EXTRA_ASSERT(ss->vtable == &VTABLE);
	...
	strings_completion_t completion, const void *data)
{
	...
	int rc =
		zoo_awget_children(ss->zh, p, watcher, watcherCtx, completion, data);
