...
/* update the rules */
int update_rules(struct rules_motor_env_s **motor_env, const gchar * ns_name);

/* check reload condition */
gboolean is_time_to_reload_rules(void);

/* pass the c datas to python for further process */
void pass_to_motor(gpointer args);

/* pass the c datas to python for further process (multi-threads version) */
gpointer pass_to_motor_v_multi_thread(gpointer args);
...
void meta2_crawler_data_block_init(struct crawler_meta2_data_pack_s *data_block,
	const char *container_path, char *meta2_url);
void meta2_crawler_data_block_free(struct crawler_meta2_data_pack_s
	*data_block);
...
motor_args_init(struct motor_args *args,
	gpointer data_block,
	gint8 type_id, struct rules_motor_env_s **motor_env, gchar * ns_name);

/* free the chunk extra textinfo structure */
void chunk_textinfo_extra_free_content(struct chunk_textinfo_extra_s *ctie);

/* Read extra content info from chunk attributes */
gboolean get_extra_chunk_info(const char *pathname, GError ** error,
...
void destroy_crawler_meta2_data_block(struct crawler_meta2_data_pack_s
	*data_block);
...
void content_info_test(struct crawler_chunk_data_pack_s *c_struct);

/* compress a non-compressed chunk */
void
motor_compress_chunk(const char *chunk_path, const char *algo, const int bsize,
	gboolean preserve);

/* decompress a compressed chunk */
void motor_decompress_chunk(const char *chunk_path, gboolean preserve);

/* get name space informations */
void motor_list_namespace_services(const char *ns_name, GError ** error);
...
void motor_check_storage_policy(const gchar * ns_name,
