	...
	gboolean preserve)
{
	...
	if (rc != 1)
		ERROR("Chunk [%s] compression failed : %s", chunk_path,
...
motor_decompress_chunk(const char *chunk_path, gboolean preserve)
{
	...
	if (rc != 1)
		ERROR("Chunk [%s] decompression failed : %s", chunk_path,
			error ? error->message : "unknown error");
	else
 ...
 */
void
motor_delete_content(const gchar * ns_name, const gchar * container_id,
	const gchar * content_name)
{
	...
	if (gs == NULL) {
		ERROR("Failed to get grid connection to ns [%s] : %s", ns_name,
	...
	if (container == NULL) {
		ERROR("Failed to get container [%s/%s] : %s", ns_name, container_id,
			gs_error_get_message(error));
		gs_error_free(error);
	...
	if (!gs_delete_content_by_name(container, content_name, &error)) {
		ERROR("Failed to delete content [%s/%s/%s] : %s", ns_name, container_id,
			content_name, gs_error_get_message(error));
		gs_error_free(error);
 ...
 */
void
motor_check_storage_policy(const gchar * ns_name, const gchar * container_id,
	const gchar * content_name)
{
	GError *error = NULL;

	if (!check_content_storage_policy(ns_name, container_id, content_name,
			FALSE, &error)) {
		ERROR("Failed to check content [%s/%s/%s] storage policy : %s", ns_name,
			container_id, content_name, error->message);
		g_clear_error(&error);
	}
	else
		INFO("Storage policy for content [%s/%s/%s] was checked and valid",
			ns_name, container_id, content_name);
}
