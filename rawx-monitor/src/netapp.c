...
/*filer actions*/
static struct volume_s **netapp_load_volumes(struct filer_s *filer,
	GError ** error);
static struct volume_s **netapp_get_filer_luns(netsnmp_session * session,
	GError ** error);
static struct volume_s **netapp_get_filer_volumes(netsnmp_session * session,
	GError ** error);
static struct volume_s **netapp_get_volumes(struct filer_s *filer,
	GError ** error);
static struct volume_s *netapp_get_volume(struct filer_s *filer,
	const char *name, GError ** error);
static gboolean netapp_monitor_volume(struct volume_s *vol,
	struct volume_statistics_s *st, GError ** err);
...
zapi_get_vol2aggr(na_server_t * s, GError ** err)
{
	...
	if (na_results_status(out) != NA_OK) {
		na_elem_free(out);
		GSETERROR(err, "NetApp filer error : (%d) %s", na_results_errno(out),
	...
	for (iter_aggr = na_child_iterator(aggr_info_array);
		(aggr_info = na_iterator_next(&iter_aggr));) {
		vol_info_array = na_elem_child(aggr_info, "volumes");
		for (iter_vol = na_child_iterator(vol_info_array);
			(vol_info = na_iterator_next(&iter_vol));) {
...
zapi_get_disk2aggr(na_server_t * s, GError ** err)
{
	...
	for (iter_disk = na_child_iterator(disk_info_array);
		(disk_info = na_iterator_next(&iter_disk));) {
...
zapi_get_disk2idle(na_server_t * s, GError ** err)
{
	...
	for (iter_i = na_child_iterator(instances);
		(instance = na_iterator_next(&iter_i));) {
		const char *iname;
		...
		for (iter_c = na_child_iterator(counters);
			(counter = na_iterator_next(&iter_c));) {
			const char *cname;
			gint i;
			...
			if (cname && *cname == 'b'
				&& 0 == g_ascii_strcasecmp(cname, "base_for_disk_busy"))
				busy_base = i > 0 ? i : 1;
			if (cname && *cname == 'd'
				&& 0 == g_ascii_strcasecmp(cname, "disk_busy"))
				busy = i > 0 ? i : 1;
		}
...
netapp_get_filer_volumes(netsnmp_session * session, GError ** error)
{
	...
	if (types_len != names_len) {
		GSETERROR(error, "netapp : no data match (#types=%u and #names=%u)",
			types_len, names_len);
		goto exit_label;
	}
	...
	for (i = 0; i < types_len; i++) {
		if (types[i]->i64 == NETAPP_VOLTYPE_NFS) {
			...
			XTRACE("Reguler volume matched (type=%" G_GINT64_FORMAT ")",
				types[i]->i64);
			vol = g_try_malloc0(sizeof(struct volume_s));
			vol->type = VT_VOL;
			vol->id.vol.vol_id = i;
			...
			XTRACE("Unexpected Volume type (type=%" G_GINT64_FORMAT ")",
				types[i]->i64);
	}
...
netapp_load_volumes(struct filer_s *filer, GError ** error)
{
	...
	if (luns) {
		if (luns_size)
			memcpy(all + volumes_size, luns,
				luns_size * sizeof(struct volume_s *));
		g_free(luns);
	}
	...
	for (volumes = all; (vol = *volumes); volumes++) {
		...
		for (ptr = str + strlen(str) - 1;
			ptr > str && (g_ascii_isspace(*ptr) || *ptr == '/'); ptr--)
			*ptr = '\0';
		...
		if (g_str_has_prefix(str, "/vol/"))
			g_strlcpy(vol->name, str + sizeof("/vol/") - 1,
				sizeof(vol->name) - 1);
		else
...
netapp_refresh_network_definitions(struct filer_s *filer, GError ** err)
{
	...
	if (!snmp_get_interface_speed(filer->ctx->session, itfIndex, &itfSpeed,
			err)) {
...
netapp_init_filer(struct filer_s *filer, GError ** err)
{
	...
	ctx->session =
		snmp_init(&(ctx->snmp_session), filer->str_addr, &(filer->auth.snmp),
		err);
	if (!ctx->session) {
		g_free(ctx);
	...
	if (!ctx->na_session) {
		...
		g_free(ctx);
		GSETERROR(err,
			"OnTap Management API error : failed to start a session");
		LOG_RETURN(NULL, "Failure (na_session)");
	}
	na_server_style(ctx->na_session, NA_STYLE_LOGIN_PASSWORD);
	na_server_set_transport_type(ctx->na_session, NA_SERVER_TRANSPORT_HTTP,
		NULL);
	na_server_adminuser(ctx->na_session, filer->auth.filer.user,
		filer->auth.filer.passwd);
	...
	if (time_is_up || !filer->ctx->fixed.volumes) {
		...
		if (aggregates) {
			if (filer->ctx->fixed.aggregates) {
				g_list_foreach(filer->ctx->fixed.aggregates, (GFunc) g_free,
					NULL);
		...
		if (ht_vol2aggr) {
			...
			XTRACE("Mappings saved : vol2aggr (%u)",
				g_hash_table_size(filer->ctx->fixed.vol2aggr));
		}
		...
		if (ht_disk2aggr) {
			...
			XTRACE("Mappings saved : disk2aggr (%u)",
				g_hash_table_size(filer->ctx->fixed.disk2aggr));
		}
	}
...
netapp_refresh_filer_data(struct filer_s *filer, GError ** err)
{
	...
	/* get CPU-idle */
	if (!snmp_get_template_int(filer->ctx->session, oid_cpuIdle,
			oid_cpuIdle_size, 0, &(filer->ctx->variable.cpu_idle), err)) {
		GSETERROR(err, "Failed to get network output on interface %d",
			filer->ctx->fixed.net_itf_index);
		LOG_RETURN(FALSE, "Failure (oid_ifNetOut)");
	}
	XTRACE("cpuIdle = %" G_GINT64_FORMAT, filer->ctx->variable.cpu_idle);

	/* compute IO-idle */
	if (!snmp_get_template_int(filer->ctx->session, oid_ifNetIn,
			oid_ifNetIn_size, filer->ctx->fixed.net_itf_index, &in64, err)) {
		GSETERROR(err, "Failed to get network input on interface %d",
			filer->ctx->fixed.net_itf_index);
		LOG_RETURN(FALSE, "Failure (oid_ifNetIn)");
	}
	XTRACE("ifNetIn = %" G_GINT64_FORMAT, in64);

	if (!snmp_get_template_int(filer->ctx->session, oid_ifNetOut,
			oid_ifNetOut_size, filer->ctx->fixed.net_itf_index, &out64, err)) {
		GSETERROR(err, "Failed to get network output on interface %d",
			filer->ctx->fixed.net_itf_index);
		LOG_RETURN(FALSE, "Failure (oid_ifNetOut)");
	}
	...
	for (l = filer->ctx->fixed.aggregates; l; l = l->next) {
		g_hash_table_insert(ht_disk_counters, g_strdup(l->data),
			GINT_TO_POINTER(0));
		g_hash_table_insert(filer->ctx->variable.aggr2idle, g_strdup(l->data),
			GINT_TO_POINTER(0));
	}
	...
	while (g_hash_table_iter_next(&iter, &k, &v)) {
		...
		if (aggr_name) {
			...
			p_sum =
				g_hash_table_lookup(filer->ctx->variable.aggr2idle, aggr_name);

			count = GPOINTER_TO_INT(p_count) + 1;
			sum = GPOINTER_TO_INT(p_sum) + GPOINTER_TO_INT(v);

			g_hash_table_insert(ht_disk_counters, aggr_name,
				GINT_TO_POINTER(count));
			g_hash_table_insert(filer->ctx->variable.aggr2idle,
				g_strdup(aggr_name), GINT_TO_POINTER(sum));
	...
	for (l = filer->ctx->fixed.aggregates; l; l = l->next) {
		...
		g_hash_table_insert(filer->ctx->variable.aggr2idle, g_strdup(aggr_name),
			GINT_TO_POINTER(idle));
	}
	...
	LOG_RETURN(TRUE,
		"Success (idle=%" G_GINT64_FORMAT " out=%" G_GINT64_FORMAT " in=%"
		G_GINT64_FORMAT ")", filer->ctx->variable.net_idle, out64, in64);
}
...
static gint
netapp_get_volume_disk_idle(struct filer_s *filer, struct volume_s *vol,
	GError ** err)
{
	...
	if (!p_idle) {
		GSETERROR(err, "Aggregate [%s] not monitored (for volume [%s])",
			aggr_name, vol->name);
		return -1;
	}
...
gboolean
netapp_monitor_volume(struct volume_s * vol, struct volume_statistics_s * st,
	GError ** err)
{
	struct filer_s *filer;
	...
	if (!snmp_get_template_int(filer->ctx->session, oid_fsUsedSpace,
			oid_fsUsedSpace_size, netapp_get_volume_id(vol), &(st->used_space),
			err)) {
		GSETERROR(err, "Failed to get the FS usage of this volume");
		LOG_RETURN(FALSE, "Failure (FS usage)");
	}
	if (!snmp_get_template_int(filer->ctx->session, oid_fsFreeSpace,
			oid_fsFreeSpace_size, netapp_get_volume_id(vol), &(st->free_space),
			err)) {
	...
	if (st->io_idle < 0) {
		GSETERROR(err, "Failed to collect the disk-idle for vol [%s]",
			vol->path);
		LOG_RETURN(FALSE, "Failure (disk idle)");
	}
	...
	LOG_RETURN(TRUE,
		"Success (net=%" G_GINT64_FORMAT " cpu=%" G_GINT64_FORMAT ")",
		st->net_idle, st->cpu_idle);
}
