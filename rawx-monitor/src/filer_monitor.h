...
typedef struct filer_ctx_s *(*filer_ctx_initiator_f) (struct filer_s * filer,
	GError ** err);

typedef void (*filer_ctx_cleaner_f) (struct filer_ctx_s * ctx);

typedef gboolean(*filer_ctx_refresher_f) (struct filer_s * filer,
	GError ** err);

/*filer actions*/

typedef struct volume_s **(*filer_vol_lister_f) (struct filer_s * filer,
	GError ** err);

typedef struct volume_s *(*filer_vol_getter_f) (struct filer_s * filer,
	const char *name, GError ** err);

typedef int (*filer_vol_monitor_f) (struct volume_s * vol,
...
struct string_mapping_s **snmp_get_strings(netsnmp_session * session,
	oid * prefix, size_t prefix_size, GError ** err);
...
struct int_mapping_s **snmp_get_integers(netsnmp_session * session,
	oid * prefix, size_t prefix_size, GError ** err);

gboolean snmp_get_int(netsnmp_session * s, oid * what, size_t what_len,
	gint64 * i64, GError ** err);

gboolean snmp_get_template_int(netsnmp_session * s, oid * what, size_t what_len,
	oid which, gint64 * i64, GError ** err);

/* RFC1213 */

gboolean snmp_get_enterprise_code(netsnmp_session * session, oid * code,
	GError ** error);

gboolean snmp_get_interface_index(netsnmp_session * s, oid * itfIndex,
	GError ** err);

gboolean snmp_get_interface_speed(netsnmp_session * s, oid itfIndex,
	gint64 * itfSpeed, GError ** err);
