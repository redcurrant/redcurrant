...
thread_function_monitor(gpointer p)
{
	...
	if (strlen(fs_info.docroot) > 0) {
		GRID_INFO("Starting the monitoring of %s at [%s] in [%s]",
				service.type_name, service.str_addr, fs_info.docroot);
	} else {
		GRID_INFO("Starting the monitoring of %s at [%s]",
				service.type_name, service.str_addr);
	}
