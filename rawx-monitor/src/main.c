...
{
	service_tag_set_value_boolean(service_info_ensure_tag(si->tags, TAGNAME_UP),
		FALSE);
}
...
_srvinfo_populate_with_rawx_stats(struct service_info_s *si)
{
	...
	service_tag_set_value_float(service_info_ensure_tag(si->tags,
			"stat.total_reqpersec"), reqpersec);
	service_tag_set_value_i64(service_info_ensure_tag(si->tags,
			"stat.total_avreqtime"), reqavgtime);
	service_tag_set_value_float(service_info_ensure_tag(si->tags,
			"stat.put_reqpersec"), putpersec);
	service_tag_set_value_i64(service_info_ensure_tag(si->tags,
			"stat.put_avreqtime"), putavgtime);
	service_tag_set_value_float(service_info_ensure_tag(si->tags,
			"stat.get_reqpersec"), getpersec);
	service_tag_set_value_i64(service_info_ensure_tag(si->tags,
			"stat.get_avreqtime"), getavgtime);
	service_tag_set_value_float(service_info_ensure_tag(si->tags,
			"stat.del_reqpersec"), delpersec);
	service_tag_set_value_i64(service_info_ensure_tag(si->tags,
			"stat.del_avreqtime"), delavgtime);
...
static void
_srvinfo_populate_with_filer_info(struct service_info_s *si,
	struct filer_s *filer, const char *filer_volume)
{
	GError *error_local;
	struct volume_s *vol;
	...
	if (!enterprise->refresh_filer(filer, &error_local)) {
		...
		if (!vol) {
			ERROR("Volume not found [%s]:[%s] : %s", filer->str_addr,
				filer_volume, gerror_get_message(error_local));
			_srvinfo_set_down(si);
		}
		else {
			bzero(&vol_stats, sizeof(vol_stats));
			if (!enterprise->monitor_volume(vol, &vol_stats, &error_local)) {
				ERROR("Failed to monitor [%s]:[%s] : %s", filer->str_addr,
					filer_volume, gerror_get_message(error_local));
				_srvinfo_set_down(si);
			}
			else {
				...
				service_tag_set_value_i64(service_info_ensure_tag(si->tags,
						NAME_MACRO_IOIDLE_NAME), MIN(vol_stats.cpu_idle,
						MIN(vol_stats.io_idle, vol_stats.perf_idle)));
				service_tag_set_value_i64(service_info_ensure_tag(si->tags,
						NAME_MACRO_SPACE_NAME), space_idle);
			}
		}
	}
...
thread_function_monitor(gpointer p)
{
	...
	if (!FILER_CONFIGURED())
		...
		while (flag_continue && !filer) {
			...
			if (!filer) {
				ERROR("Failed to init the filer : %s",
					gerror_get_message(error_local));
				_srvinfo_set_down(si);
...
static gboolean
_path_dereference_symlinks(const gchar * path, gchar * dst, gsize dst_size,
	GError ** error)
{
	gsize dst_offset;
	gchar **tokens, **tok;
	...
	for (tok = tokens; *tok; tok++) {
		...
		g_snprintf(current_path, sizeof(current_path), "%.*s/%s", offset_int,
			dst, *tok);
		...
		if (g_file_test(current_path, G_FILE_TEST_IS_SYMLINK)) {
			...
			if (*current_link == '/')
				...
				dst_offset +=
					g_snprintf(dst + dst_offset, dst_size - dst_offset, "/%s",
					current_link);
		}
		else {
			dst_offset +=
...
static gboolean
_path_get_mountpoint(const gchar * path, struct volume_cfg_s *mfs,
	GError ** error)
{
	FILE *stream_proc;

	bzero(mfs, sizeof(*mfs));
	if (!_path_dereference_symlinks(path, mfs->docroot,
			sizeof(mfs->docroot) - 1, error)) {
	...
	DEBUG("Volume config : docroot=%s mountpoint=%s device=%s", mfs->docroot,
		mfs->mount_point, mfs->device_path);
	return TRUE;
}
...
{
	return val && (0 == g_ascii_strcasecmp(val, "true")
...
_cfg_section_service(GKeyFile * kf, const gchar * section, GError ** error)
{
	...
	if (NULL != str) {
		g_strlcpy(service.location_name, str,
			sizeof(service.location_name) - 1);
		g_free(str);
	}
	else {
		g_clear_error(error);
	...
	if (sections) {
		for (p_section = sections; *p_section; p_section++) {
			if (!g_ascii_strcasecmp(*p_section, "default")) {
				INFO("Loading main configuration from section [%s]",
					*p_section);
				...
				INFO("Loading service configuration from section [%s]",
					*p_section);
				...
				INFO("Loading volume configuration from section [%s]",
					*p_section);
				if (!_cfg_section_volume(kf, *p_section, error)) {
					GSETERROR(error, "Error in section [%s]", *p_section);
					NOTICE
						("If you are running a rainx, please don't set [Volume] section.");
					goto label_exit;
				}
			}
			else if (!g_ascii_strcasecmp(*p_section, "child")) {
				INFO("Loading child configuration from section [%s]",
					*p_section);
...
delete_pid_file(void)
{
	...
	if (-1 == stat(pidfile_path, &current_pidfile_stat)) {
		WARN("Unable to remove pidfile at [%s] : %s", pidfile_path,
			strerror(errno));
		return;
	}
...
main_prepare_srvinfo(void)
{
	...
	service_tag_set_value_macro(service_info_ensure_tag(si->tags,
			NAME_MACRO_CPU_NAME), NAME_MACRO_CPU_TYPE, NULL);
	if (strlen(fs_info.docroot) > 0) {
		service_tag_set_value_macro(service_info_ensure_tag(si->tags,
				NAME_MACRO_IOIDLE_NAME), NAME_MACRO_IOIDLE_TYPE,
			fs_info.docroot);
		service_tag_set_value_macro(service_info_ensure_tag(si->tags,
				NAME_MACRO_SPACE_NAME), NAME_MACRO_SPACE_TYPE, fs_info.docroot);
...
main(int argc, char **argv)
{
	...
	if (!_cfg_check(&error_local)) {
		GSETERROR(&error_local,
			"Invalid configuration for the grid version this progra was compiled for");
		goto label_error;
	}
