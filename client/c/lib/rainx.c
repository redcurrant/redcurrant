...
stg_pol_is_rainx(namespace_info_t * ni, const gchar * stgpol)
{
	...
	const struct data_security_s *datasec =
		storage_policy_get_data_security(sp);
	gboolean ret;
...
get_rainx_from_conscience(const gchar * nsname, GError ** error)
{
	struct service_info_s *si =
		get_one_namespace_service(nsname, "rainx", error);
	if (!si)
		return NULL;
 ...
 */
static ne_session *
rainx_opensession(const addr_info_t * rainx_addr, gs_content_t * hollow_content,
	GError ** err)
{
	...
	if (NULL == (session =
			opensession_common(rainx_addr, connect_timeout, read_timeout, err)))
		return NULL;
...
_chunk_to_rawxlist_element(chunk_info_t * chunk_info)
{
	...
	g_string_append(res, "/");
	buffer2str(chunk_info->id.id, sizeof(chunk_info->id.id), tmpstr,
		sizeof(tmpstr));
	g_string_append(res, tmpstr);
...
create_sparerawxlist_from_chunk_list(GSList * spare_chunks_list)
{
	...
	void _append_to_res(gpointer _chunk_in_error, gpointer _last_chunk_in_error)
	{
		...
		gchar *sparerawxlist_element =
			_chunk_to_sparerawxlist_element(chunk_in_error);
	...
	g_slist_foreach(spare_chunks_list, _append_to_res,
		g_slist_last(spare_chunks_list)->data);

	return g_string_free(res, FALSE);
}

static char *
create_rainx_request_from_chunk_list(ne_request ** req, ne_session * session,
	const char *method, GSList * chunk_list, gs_content_t * hollow_content,
	GByteArray * system_metadata, const gchar * storage_policy,
	guint32 current_metachunk_pos, guint32 current_metachunk_size,
	GError ** err)
{
	gchar *str_req_id;
	gchar *rawxlist_str;
	...
	first_chunk.ci =
		g_memdup(g_slist_nth_data(chunk_list, 0), sizeof(chunk_info_t));
	...
	str_req_id =
		create_rawx_request_from_chunk(req, session, method, &first_chunk,
		system_metadata, err);
...
static GError *
_extract_size_and_md5(gchar * header, chunk_size_t * new_size,
	chunk_hash_t * new_md5)
{
	GError *err = NULL;
	gchar **header_tokens = g_strsplit(header, "|", 0);

	if (g_strv_length(header_tokens) < 3)
		err = NEWERROR(CODE_INTERNAL_ERROR, "could not split '%s'", header);
	else {
		*new_size = g_ascii_strtoll(header_tokens[1], NULL, 10);
		hex2bin(header_tokens[2], new_md5, sizeof(*new_md5), &err);
	}
	if (err) {
		GRID_DEBUG("Cannot extract md5 from header: %s", err->message);
	}

	g_strfreev(header_tokens);
	return err;
}

static gchar *
_find_header(chunk_id_t chunkid, gchar ** split_header)
{
	gchar **cursor = NULL, **header_tokens = NULL;
	gchar *header_chunkid_str = NULL;
	hash_sha256_t header_chunk_id;

	if (NULL == split_header || NULL == *split_header)
		return NULL;

	memset(&header_chunk_id, 0, sizeof(header_chunk_id));

	for (cursor = split_header; NULL != *cursor; cursor++) {
		// Split into "/"-separated tokens, then copy the first 64 bytes of the 2nd token
		// to get the chunkid string.
		header_tokens = g_strsplit(*cursor, "/", 0);
		header_chunkid_str = g_strndup(header_tokens[1], 64);
		// Convert chunkid from string to bin.
		hex2bin(header_chunkid_str, header_chunk_id, sizeof(header_chunk_id),
			NULL);
		// cleanup
		g_strfreev(header_tokens);
		g_free(header_chunkid_str);
		// If the two hashes are equal, return the full header.
		if (0 == memcmp(header_chunk_id, chunkid.id, sizeof(header_chunk_id)))
			return *cursor;
	}
...
GSList *
rainx_get_spare_chunks(gs_container_t * container, gchar * content_path,
	gint64 count, gint64 distance, GSList * notin_list,
	GSList * broken_rawx_list, gs_error_t ** err)
{
	...
	if (container->meta2_addr.port <= 0) {
		tmp =
			gs_get_storage_container(container->info.gs, NULL,
			C0_NAME(container), container->ac, err);
		if (NULL != tmp) {
	...
	new_spare =
		meta2_remote_content_spare_in_fd_full(C0_CNX(container),
		C0_M2TO(container), &local_gerr, C0_ID(container), content_path, count,
		distance, notin_str, broken_str);
	g_free(notin_str);
	g_free(broken_str);
	if (!new_spare) {
...
static GSList *
_update_chunk_list(GSList * chunk_list, const gchar * header_chunklist,
	GError ** err)
{
	gchar **split_header = g_strsplit(header_chunklist, ";", 0);
	GSList *updated_list = NULL;

	void _update_chunk_info(gpointer _chunk_info, gpointer _unused)
	{
		GError *local_err = NULL;
		chunk_info_t *chunk_info = _chunk_info;
		gchar *header = NULL;
		chunk_size_t new_size = 0;
		chunk_hash_t new_md5;
		...
		if (NULL == (header = _find_header(chunk_info->id, split_header))) {
			// This is not an error, rainx just used less chunks than expected
			if (DEBUG_ENABLED()) {
				char strid[128];

				chunk_id_to_string(&(chunk_info->id), strid, sizeof(strid));
				GRID_DEBUG("chunk not found in response header: %s", strid);
			}
			return;
		}

		local_err = _extract_size_and_md5(header, &new_size, &new_md5);
		if (local_err != NULL) {
			*err = local_err;
			g_prefix_error(err, "Failed to update chunk list: ");
			return;
		}
		chunk_info->size = new_size;
		memcpy(&(chunk_info->hash), &new_md5, sizeof(new_md5));

		updated_list =
			g_slist_append(updated_list, g_memdup(chunk_info,
				sizeof(chunk_info_t)));
	}

	g_slist_foreach(chunk_list, _update_chunk_info, NULL);
	g_strfreev(split_header);

	if (err && *err) {
		g_slist_free_full(updated_list, g_free);
		updated_list = NULL;
	}
...
static void
_compute_real_positions(guint pos, gint k, guint * metachunkpos,
	guint * subchunkpos)
{
	if (metachunkpos)
		*metachunkpos = pos / k;
...
_convert_filtered(GSList * list)
{
	...
	void _create_chunk_info(gpointer _bean, gpointer _unused)
	{
		...
		if (DESCR(_bean) == &descr_struct_CONTENTS) {
			...
			gint64 pos64 =
				g_ascii_strtoll(CONTENTS_get_position(bean)->str, NULL, 10);
			guint32 pos32 = pos64;
...
static void
_get_all_chunks(GSList ** p_all_chunks, GSList * agregated_chunks,
	GSList * filtered, guint metachunkpos, guint k, guint m, guint metachunknb,
	guint * metachunksize)
{
	...
	for (l = agregated_chunks; l; l = l->next) {
		...
		if (metachunkpos == mcp) {
			ci->nb = metachunknb;
			all_chunks =
				g_slist_prepend(all_chunks, g_memdup(ci, sizeof(chunk_info_t)));
...
static gboolean
_rainx_reconstruct_init(struct dl_status_s *dl_status, gs_content_t * content,
	GSList * aggregated_chunks, GSList ** p_all_chunks,
	GSList ** p_spare_chunks, GSList * filtered, GSList * broken_rawx_list,
	GHashTable * failed_chunks, const gchar * storage_policy,
	guint * p_metachunkpos, guint * p_metachunksize, guint * p_metachunkoffset,
	guint * p_k, gs_error_t ** err)
{
	...
	if (FALSE ==
		stg_pol_rainx_get_param(&(content->info.container->info.gs->ni),
			storage_policy, DS_KEY_DISTANCE, &distance))
		distance = 1;

	if (stg_pol_rainx_get_param(&(content->info.container->info.gs->ni),
			storage_policy, DS_KEY_K, &k_signed))
		k = k_signed;
	else
		return FALSE;

	if (stg_pol_rainx_get_param(&(content->info.container->info.gs->ni),
			storage_policy, DS_KEY_M, &m_signed))
		m = m_signed;
	else
		return FALSE;
	...
	*p_spare_chunks =
		rainx_get_spare_chunks(content->info.container, content->info.path,
...
_find_chunk_from_hash(GSList * chunks, chunk_hash_t chunk_hash)
{
	...
	if (NULL != (found_l =
			g_slist_find_custom(chunks, chunk_hash, _compare_chunk_to_hash)))
		return found_l->data;
	return NULL;
}
...
_change_raw_content_id(meta2_raw_content_t * raw_co, GSList * spare_chunks)
{
	...
	for (l = raw_co->raw_chunks; l; l = l->next) {
		...
		if (NULL != (found_spare =
				_find_chunk_from_hash(spare_chunks, rc->hash))) {
...
_create_meta2_raw_content(GSList * beans, gs_content_t * content,
	guint32 metachunkpos, GSList * spare_chunks)
{
	...
	memcpy(&(new_m2rc->container_id), &(content->info.container->cID),
	...
	for (l = beans; l && l->data; l = l->next) {
		if (DESCR(l->data) == &descr_struct_CONTENTS) {
			...
			gint64 pos64 =
				g_ascii_strtoll(CONTENTS_get_position(bc)->str, NULL, 10);
			current_pos = pos64;
			if (current_pos == metachunkpos) {
				...
				struct bean_CHUNKS_s *ck =
					get_chunk_matching_content(beans, bc);
				GByteArray *hash_gba = CHUNKS_get_hash(ck);

				memcpy(hash, hash_gba->data, hash_gba->len);
				if (NULL != (found_spare =
						_find_chunk_from_hash(spare_chunks, hash))) {
...
static GError *
_commit_reconstruct(gs_content_t * content, GSList * beans,
	GSList * spare_chunks, guint metachunkpos)
{
	GError *local_gerr = NULL;
	...
	if (FALSE == (ret =
			meta2_remote_container_open(&(ctx.addr), MAX(ctx.timeout.cnx,
					ctx.timeout.req), &local_gerr, container->cID)))
		goto label_error;

	raw_co =
		_create_meta2_raw_content(beans, content, metachunkpos, spare_chunks);

	/* Delete the old chunk */
	if (FALSE == (ret =
			meta2raw_remote_delete_chunks(&ctx, &local_gerr, raw_co)))
		goto label_error;

	/* Insert the new chunk */
	_change_raw_content_id(raw_co, spare_chunks);
	bzero(pos_prefix, sizeof(pos_prefix));
	g_snprintf(pos_prefix, sizeof(pos_prefix), "%" G_GUINT32_FORMAT ".",
		metachunkpos);
	if (FALSE == (ret =
			meta2raw_remote_update_chunks(&ctx, &local_gerr, raw_co, TRUE,
				pos_prefix)))
		goto label_error;
...
gboolean
rainx_ask_reconstruct(struct dl_status_s * dl_status, gs_content_t * content,
	GSList * aggregated_chunks, GSList * filtered, GSList * beans,
	GSList * broken_rawx_list, GHashTable * failed_chunks,
	const gchar * storage_policy, gs_error_t ** err)
{
	...
	addr_info_t *rainx_addr =
		get_rainx_from_conscience(content->info.container->info.gs->ni.name,
		&local_error);
	if (rainx_addr == NULL) {
	...
	int output_wrapper(void *uData, const char *b, const size_t bSize)
	{
		...
		for (; offset < bSize;) {
			...
			nbW =
				dl_status->dl_info.writer(dl_status->dl_info.user_data,
				b + offset, bSize - offset);
			if (nbW < 0) {
				return -1;
			}
			if (nbW == 0) {
	...
	if (rainx_addr) {
		if (NULL == (session =
				rainx_opensession(rainx_addr, content, &local_error)))
			goto error_label;

		if (FALSE == _rainx_reconstruct_init(dl_status, content,
				aggregated_chunks, &all_chunks, &spare_chunks, filtered,
				broken_rawx_list, failed_chunks, storage_policy, &metachunkpos,
				&metachunksize, &metachunkoffset, &k, err))
			goto error_label;

		if (NULL == (str_req_id =
				create_rainx_request_from_chunk_list(&request, session,
					RAINX_DOWNLOAD, all_chunks, content, NULL, storage_policy,
					metachunkpos, metachunksize, &local_error))) {
			goto error_label;
		}

		if (request) {
			sparerawxlist_str =
				create_sparerawxlist_from_chunk_list(spare_chunks);
			GRID_DEBUG("sparerawxlist=%s", sparerawxlist_str);
			ne_print_request_header(request, "sparerawxlist", "%s",
				sparerawxlist_str);
			...
			switch (ne_request_dispatch(request)) {
				...
				case NE_ERROR:
					GSERRORCAUSE(err, local_error,
						"error from the RAINX WebDAV server (%s) (ReqId:%s)",
						ne_get_error(session), str_req_id);
					goto error_label;
				default:
					GSERRORCAUSE(err, local_error,
						"unexpected error from the RAINX WebDAV server (%s) (ReqId:%s)",
						ne_get_error(session), str_req_id);
					goto error_label;
			}
		}
	...
	if (NULL == (local_error =
			_commit_reconstruct(content, beans, spare_chunks, metachunkpos))) {
		GRID_INFO("Commit successful after reconstruct.");
	}
	else {
		GSERRORCAUSE(err, local_error,
			"Reconstruct succeeded, but commit failed.");
	}
...
gs_status_t
rainx_upload(GSList * chunk_list, gs_content_t * hollow_content,
	addr_info_t * rainx_addr, gs_input_f input, void *user_data,
	GByteArray * system_metadata, GSList ** p_returned_chunk_list,
	const gchar * storage_policy, guint32 current_metachunk_pos,
	guint32 current_metachunksize, GError ** err)
{
	ne_session *session = NULL;
	ne_request *request = NULL;
	gchar *str_req_id = NULL;
	const gchar *header_chunklist = NULL;
	int64_t size_to_be_read = 0;
	gs_chunk_t first_chunk;
	GSList *updated_chunk_list = NULL;

	/*sanity checks and variables initiations */
	if (!chunk_list || !input || !p_returned_chunk_list) {
		GSETERROR(err, "Invalid parameter");
		return 0;
	}

	if (0 == g_slist_length(chunk_list)) {
		GSETERROR(err, "Invalid parameter : size out of range");
		return 0;
	}

	first_chunk.ci = g_slist_nth_data(chunk_list, 0);
	size_to_be_read = first_chunk.ci->size;
	...
	session = rainx_opensession(rainx_addr, hollow_content, err);

	/*create an upload request */
	if (NULL == (str_req_id =
			create_rainx_request_from_chunk_list(&request, session,
				RAINX_UPLOAD, chunk_list, hollow_content, system_metadata,
				storage_policy, current_metachunk_pos, current_metachunksize,
				err))) {
		goto error_label;
	}

	/*tell we'll read data from the provided callback */
	ne_set_request_body_provider(request, size_to_be_read, limited_input,
		user_data);
	...
	switch (ne_request_dispatch(request)) {
		case NE_OK:
			if (ne_get_status(request)->klass != 2) {
				GSETERROR(err, "cannot upload (%s) (ReqId:%s)",
					ne_get_error(session), str_req_id);
				goto error_label;
			}
		...
		case NE_TIMEOUT:
		case NE_ERROR:
			GSETERROR(err, "error from the RAINX WebDAV server (%s) (ReqId:%s)",
				ne_get_error(session), str_req_id);
			goto error_label;
		default:
			GSETERROR(err,
	...
	/* The response header "chunklist" contains size and md5sum. */
	if (NULL != (header_chunklist =
			ne_get_response_header(request, "chunklist"))) {
		GRID_DEBUG("header_chunklist=%s", header_chunklist);
		updated_chunk_list =
			_update_chunk_list(chunk_list, header_chunklist, err);
		if (err && *err) {
			goto error_label;
		}
		*p_returned_chunk_list =
			g_slist_concat(*p_returned_chunk_list, updated_chunk_list);
	}
	else {
		GSETERROR(err, "Could not find chunklist header in response (ReqId:%s)",
			str_req_id);
		goto error_label;
	}
	...
	if (DEBUG_ENABLED()) {
		TRACE("%s uploaded (ReqId:%s)", hollow_content->info.path, str_req_id);
	}
...
error_label:
	...
	if (session)
		ne_session_destroy(session);
	TRACE("could not upload %s (ReqId:%s)", hollow_content->info.path,
		str_req_id);
	g_free(str_req_id);
	return FALSE;
}
