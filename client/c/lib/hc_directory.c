...
gs_error_t *
hc_link_service_to_reference(gs_grid_storage_t * hc, const char *reference,
	const char *srv_type, char ***result)
{
	GError *cb(addr_info_t * a, const container_id_t ref_id, gchar ** master)
	{
		GError *e = NULL;
		...
		*result = meta1v2_remote_link_service(a, &e,
			hc->ni.name, ref_id, srv_type,
			gs_grid_storage_get_timeout(hc, GS_TO_M1_CNX),
			gs_grid_storage_get_timeout(hc, GS_TO_M1_OP), master);
		return e;
	}
...
gs_error_t *
hc_list_reference_services(gs_grid_storage_t * hc, const char *reference,
	const char *srv_type, char ***result)
{
	GError *cb(addr_info_t * a, const container_id_t ref_id, gchar ** master)
	{
...
gs_error_t *
hc_unlink_reference_service(gs_grid_storage_t * hc, const char *reference,
	const char *srv_type)
{
	GError *cb(addr_info_t * a, const container_id_t ref_id, gchar ** master)
	{
		...
		if (!strchr(srv_type, '|')) {
			...
			if ((g_strv_length(toks) < 2)
				|| (0 >= (seq = g_ascii_strtoll(toks[0], NULL, 10)))) {
				...
				meta1v2_remote_unlink_one_service(a, &e,
					...
					gs_grid_storage_get_timeout(hc, GS_TO_M1_CNX),
					gs_grid_storage_get_timeout(hc, GS_TO_M1_OP), master, seq);
			}
			g_strfreev(toks);
