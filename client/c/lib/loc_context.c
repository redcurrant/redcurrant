...
static gboolean
_open_meta2_connection(struct metacnx_ctx_s *cnx, const gchar * m2_url,
	gs_error_t ** e)
{
	GError *ge = NULL;
	gboolean ret = FALSE;
...
_check_chunk(const char *cid)
{
	...
	if (NULL != (session =
			ne_session_create("http", addr_tok[0], atoi(addr_tok[1])))) {
		...
		if (NULL != (request =
				ne_request_create(session, "HEAD", strrchr(cid, '/')))) {
...
_get_container_user_properties(gs_grid_storage_t * hc, struct hc_url_s *url,
	container_id_t cid, char ***props, gs_error_t ** gserr)
{
	GError *gerr = NULL;
	gboolean rc;
	addr_info_t *m1 = NULL;
	...
	for (;;) {
		...
		if (!m1) {
			*gserr =
				gs_error_new(500, "No META1 found for [%s]", hc_url_get(url,
					HCURL_REFERENCE));
			break;
		}

		rc = meta1v2_remote_reference_get_property(m1, &gerr, hc_url_get(url,
				HCURL_NS), cid, NULL, props, -1, -1);
		...
		if (!rc) {
			...
			if (gerr) {
				if (gerr->code < 100) {	/* network error */
					...
					GSERRORCAUSE(gserr, gerr,
						"Cannot get container user properties");
					break;
				}
			}
		}
...
static status_t
_get_container_global_property(gs_grid_storage_t * hc,
	struct metacnx_ctx_s *cnx, container_id_t cid, GHashTable ** ht,
	gs_error_t ** gserr)
{
	...
	if (!meta2_remote_list_all_container_properties(cnx, cid, &prop_list,
			&gerr)) {
		...
		if (!m2_list) {
			GSERRORCAUSE(gserr, gerr,
				"Failed to get container admin entries, Cannot find meta2(s)");
			if (gerr)
				g_error_free(gerr);
			return 0;
		}
		...
		for (m2 = m2_list; m2; m2 = m2->next) {
			...
			if (addr) {
				...
				DEBUG
					("Failed to get container admin entries -> test with next meta2 [%s]",
					str_addr);

				if (!_open_meta2_connection(&cnxOther,
						str_addr /*ctx->loc->m2_url[0] */ , &e)) {
				...
				if (!meta2_remote_list_all_container_properties(&cnxOther, cid,
						&prop_list, &gerr)) {
					_close_meta2_connection(&cnxOther, cid);
					GSERRORCAUSE(gserr, gerr,
						"Failed to get container admin entries: %s\n",
						str_addr);
					continue;
	...
	*ht = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
	...
	if (!g_hash_table_lookup(*ht, GS_CONTAINER_PROPERTY_STORAGE_POLICY))
		g_hash_table_insert(*ht, g_strdup(GS_CONTAINER_PROPERTY_STORAGE_POLICY),
			g_strdup("namespace default"));
	if (!g_hash_table_lookup(*ht, GS_CONTAINER_PROPERTY_VERSIONING))
		g_hash_table_insert(*ht, g_strdup(GS_CONTAINER_PROPERTY_VERSIONING),
			g_strdup("namespace default"));
	if (!g_hash_table_lookup(*ht, GS_CONTAINER_PROPERTY_QUOTA))
		g_hash_table_insert(*ht, g_strdup(GS_CONTAINER_PROPERTY_QUOTA),
			g_strdup("namespace default"));
...
_beans_to_content(const GSList * beans)
{
	...
	for (; beans; beans = beans->next) {
		if (DESCR(beans->data) == &descr_struct_CHUNKS)
			...
			c->properties =
				g_slist_append(c->properties, _bean_dup(beans->data));
	}
...
static gs_error_t *
_get_content(struct metacnx_ctx_s *cnx, struct hc_url_s *url,
	struct beans_content_s **content)
{
	GSList *beans = NULL;
	char target[64];
	GError *e = NULL;
	...
	const char *prefix)
{
	...
	if (NULL != si) {
		...
		g_string_append_printf(result,
			...
			prefix, (strlen(avail) > 0) ? "no" : "yes", prefix);
		...
		g_string_append_printf(result,
			...
			"%s     </location>\n",
			prefix, prefix, cid, prefix, prefix, prefix);
	}
...
_dump_chunks_xml_NORMAL(const struct loc_context_s *lc, GString ** s)
{
	...
	for (guint i = 0; i < lc->rc->pairs->len; i++) {
		...
		g_string_append_printf(*s,
			...
			"   </chunk>\n",
			loc,
			current->position.meta, CHUNKS_get_size(current->chunk), str_hash);
		g_free(loc);
	}
}
...
_dump_chunks_xml_DUPLI(const struct loc_context_s *lc, GString ** s)
{
	...
	void _append(void)
	{
		...
		g_string_append_printf(*s,
			...
			copies,
			urls->str,
			base->position.meta, CHUNKS_get_size(base->chunk), str_hash);
...
_dump_chunks_xml_RAIN(const struct loc_context_s *lc, GString ** s)
{
	...
	void _append_subchunk(chunk_pair_t * pair)
	{
		...
		g_string_append_printf(*s,
			...
			CHUNKS_get_size(pair->chunk), str_hash);
		g_free(loc);
	}
...
__dump_content_xml(const struct loc_context_s *lc, GString ** s)
{
	...
	sp = _init_storage_policy(lc->namespace,
		CONTENTS_HEADERS_get_policy(lc->rc->header)->str);
	...
	g_string_append_printf(*s,
		...
		g_array_index(lc->rc->pairs, chunk_pair_t,
			lc->rc->pairs->len).position.meta + 1,
...
_dump_chunks_NORMAL(const struct loc_context_s *lc, GString ** s)
{
	...
	for (guint i = 0; i < lc->rc->pairs->len; i++) {
		...
		g_string_append_printf(*s,
			...
			loc, CHUNKS_get_size(current->chunk), str_hash);
		g_free(avail);
		g_free(loc);
	}
}
...
_dump_chunks_RAIN(const struct loc_context_s *lc, GString ** s)
{
	...
	void _append_subchunk(chunk_pair_t * pair)
	{
		...
		g_string_append_printf(*s,
			...
			avail, loc, CHUNKS_get_size(pair->chunk), str_hash);
		g_free(loc);
		g_free(avail);
	}
...
__dump_content(const struct loc_context_s *lc, GString ** s)
{
	...
	gint chunk_nb = ((content->pairs->len == 0) ?
		0 : g_array_index(content->pairs, chunk_pair_t,
			content->pairs->len - 1).position.meta + 1);
...
_close_meta2_connection(struct metacnx_ctx_s *cnx, container_id_t cid)
{
	...
	if (NULL != ge) {
		GRID_DEBUG("Meta2 connection closure failed : (%d), %s", ge->code,
			ge->message);
		g_clear_error(&ge);
		return FALSE;
	}
...
__write_admin_info(gpointer k, gpointer v, gpointer gstr)
{
	...
	*s = g_string_append(*s, "\t\t");
	...
	for (uint i = 0; (key_len < max_key_len) && (i < max_key_len - key_len);
		i++)
		*s = g_string_append(*s, " ");
	g_string_append_printf(*s, "%s : %s\n", (char *) k, (char *) v);
}
...
_loc_context_to_xml(const struct loc_context_s *lc)
{
	...
	g_string_append_printf(s, " </directory>\n"
		" <storage>\n"
		"  <name>%s</name>\n"
		"  <id>%s</id>\n", lc->loc->container_name, lc->loc->container_hexid);
	...
	gs_error_t ** p_e)
{
	...
	if (!ctx->loc) {
		gs_error_set(&e, e ? e->code : 0,
			"Container reference not resolvable : %s\n",
			gs_error_get_message(e));
		goto label_error;
	}

	if (!ctx->loc->m0_url || !ctx->loc->m1_url || !ctx->loc->m2_url
		|| !ctx->loc->m2_url[0]) {
		gs_error_set(&e, e ? e->code : 0,
			"Container reference partially missing (%p %p %p): %s\n",
	...
	if (!_get_container_user_properties(hc, url, cid, &ctx->container_props,
			&e)) {
		gs_error_set(&e, e ? e->code : 0,
			"Container properties not found : %s\n", gs_error_get_message(e));
		goto label_error_close_cnx;
	}

	if (!_get_container_global_property(hc, &cnx, cid, &ctx->admin_info, &e)) {
		gs_error_set(&e, e ? e->code : 0,
			"Container admin entries not found : %s\n",
			gs_error_get_message(e));
		goto label_error_close_cnx;
	}
...
struct loc_context_s *
loc_context_init_retry(gs_grid_storage_t * hc, struct hc_url_s *url,
	gs_error_t ** p_e)
{
...
char *
loc_context_getstgpol_to_string(const struct loc_context_s *lc,
	gboolean bContent)
{
	if (bContent == TRUE) {
		...
		return (char *) g_hash_table_lookup(lc->admin_info,
			GS_CONTAINER_PROPERTY_STORAGE_POLICY);
	}
}
