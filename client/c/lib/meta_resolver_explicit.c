...
void
resolver_direct_clear(resolver_direct_t * r)
{
	if (!r)
		return;
...
build_meta0_cache(struct resolver_direct_s *r, GError ** err)
{
	...
	if (!(m0_list =
			meta0_remote_get_meta1_all(&(r->meta0), r->timeout.m0.op, err))) {
		gchar str_addr[128];

		memset(str_addr, 0x00, sizeof(str_addr));
		addr_info_to_string(&(r->meta0), str_addr, sizeof(str_addr));
		GSETERROR(err, "META0 error : Cannot get the reference from %s",
			str_addr);
...
static int
UNSAFE_resolver_direct_reload(struct resolver_direct_s *r, gboolean locked,
	GError ** err)
{
	int rc = 0;
...
addr_info_t *
resolver_direct_get_meta1(resolver_direct_t * r, const container_id_t cID,
...
int
resolver_direct_set_meta1_master(resolver_direct_t * r,
...
GSList *
resolver_direct_get_meta2_once(resolver_direct_t * r, const char *ns,
	const container_id_t cid, GSList ** m1_exclude, GError ** err)
{
	char **m2 = NULL;
	addr_info_t *m1 = NULL;
	GError *e = NULL;
	...
	if (!m1) {
		GSETCODE(err, e ? e->code : 500, "META1 Resolution error : %s",
			e ? e->message : "no error specified");
		if (NULL != e)
			g_clear_error(&e);
		return NULL;
	}
	...
	m2 = meta1v2_remote_list_reference_services(m1, &e, ns, cid, "meta2",
		(r->timeout.m1.cnx) / 1000, (r->timeout.m1.op) / 1000);
	...
	if (!m2 || 0 == g_strv_length(m2)) {
		if (NULL != e) {
			...
			GSETCODE(err, e->code,
				"Cannot directly resolve META2 : %s [META1=%s]", e->message,
				strm1);
			...
			GSETCODE(err, CODE_CONTAINER_NOTFOUND,
				"No service meta2 found for this reference");
			g_free(m1);
		}
		return NULL;
	}
...
GSList *
resolver_direct_get_meta2(resolver_direct_t * resolver, const char *ns,
	const container_id_t cID, GError ** e, int max_attempts)
{
	GSList *m1_exclude = NULL;
	GSList *result = NULL;

	GSList *__directly_resolve(resolver_direct_t * r, GSList ** exclude,
		GError ** err)
	{
		GSList *m2 = NULL;
...
void
resolver_direct_free(resolver_direct_t * r)
{
...
resolver_direct_t *
resolver_direct_create2(const char *const config, gint to_cnx, gint to_req,
	GError ** err)
{
	return resolver_direct_create_with_metacd(config, NULL, to_cnx, to_req,
		err);
}
...
{
	return resolver_direct_create2(config, CS_TOCNX_DEFAULT, CS_TOREQ_DEFAULT,
		err);
}

resolver_direct_t *
resolver_direct_create_with_metacd(const gchar * const config,
	struct metacd_s * metacd, gint to_cnx, gint to_req, GError ** err)
{
	...
	if (r->metacd && resolver_metacd_is_up(r->metacd)) {
		...
		if (m0_addr) {
			...
			g_free(m0_addr);
			DEBUG
				("Explicit resolver created for %s (empty), META0 got from the metacd",
				config);
			return r;
		}
		else {
	...
	meta0_info =
		get_meta0_info2(config, r->timeout.conscience.cnx,
		r->timeout.conscience.op, err);
	if (meta0_info) {
		memcpy(&(r->meta0), &(meta0_info->addr), sizeof(addr_info_t));
		g_free(meta0_info);
		DEBUG("Explicit resolver created for %s, META0 got from gridcluster",
			config);
		return r;
	}
	...
	if (!meta0_addr) {
		GSETERROR(err,
			"Cannot build the META0 address from its URL (host=%s port=%s/%i)",
			host, port, atoi(port));
		g_free(host);
		g_free(port);
		free(r);
	...
	g_free(meta0_addr);
	DEBUG
		("Explicit resolver created for %s (empty), META0 got from gridcluster",
		config);
