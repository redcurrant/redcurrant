...
extern gboolean
set_solr_service(gs_grid_storage_t * grid, const gchar * container_name,
	const gchar * new_solr_service)
{
	...
	if (new_solr_service) {
		...
		if (!l4_address_init_with_url(&si->addr, new_solr_service, &gerror)) {
			g_printerr
				("Error creating addr_info from field [%s]. Error: [%s]\n",
				new_solr_service,
	...
	DEBUG("Setting solr service to [%s] for container [%s].",
		new_solr_service ? new_solr_service : "", container_name);
	...
	if (!meta2raw_remote_set_admin_entry(ctx, &gerror, cid, "service_solr",
