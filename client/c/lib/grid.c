	...
	const char *m1)
{
	...
	int _try(void)
	{
		...
		if (resolver_metacd_is_up(gs->metacd_resolver)) {
			if (resolver_metacd_set_meta1_master(gs->metacd_resolver, cID, m1,
					&e)) {
	...
	addr_info_t *_try(void)
	{
		...
		if (!ref_exists || !metacd_is_up) {
			...
			if (meta1v2_remote_create_reference(pA, &gErr, gs->ni.name,
					cID, cname,
					gs_grid_storage_get_timeout(gs, GS_TO_M1_CNX),
					gs_grid_storage_get_timeout(gs, GS_TO_M1_OP), NULL)) {
...
gs_resolve_meta1v2(gs_grid_storage_t * gs, const container_id_t cID,
	const gchar * cname, int read_only, GSList * exclude, GError ** err)
{
	return gs_resolve_meta1v2_v2(gs, cID, cname, read_only, exclude, FALSE,
		err);
}
...
GSList *
gs_resolve_meta2(gs_grid_storage_t * gs, container_id_t cID, GError ** err)
{
	...
	GSList *_try(GSList ** exclude)
	{
		...
		if (gErr) {
			...
			GSETERROR(&gErr, "Unknown error, not retrying a direct resolution");
...
static void
_fill_meta1_tabs(char ***p_m1_url_tab, addr_info_t ** p_addr_tab,
	gs_grid_storage_t * gs, container_id_t cid, gchar * cname)
{
	addr_info_t *addr;
	...
	while (NULL != (addr =
			gs_resolve_meta1v2(gs, cid, cname, 0, meta1_list, NULL)))
	...
	// Fill result tabs
	for (i = 0, iter_list = meta1_list; iter_list;
		iter_list = iter_list->next, i++) {
...
gs_locate_container(gs_container_t * container, gs_error_t ** gserr)
{
	...
	/*resolve meta0 */
	addr_info_to_string(&(container->info.gs->direct_resolver->meta0), str_addr,
		sizeof(str_addr));
	...
	_fill_meta1_tabs(&(location->m1_url), NULL, container->info.gs,
		C0_ID(container), C0_NAME(container));

	return location;
}

static struct gs_container_location_s *
_gs_locate_container_by_cid(gs_grid_storage_t * gs, container_id_t cid,
	char **out_nsname_on_m1, gs_error_t ** gserr)
{
	addr_info_t *addr;
	addr_info_t *m1_addr;
	...
	/*resolve meta0 */
	addr_info_to_string(&(gs->direct_resolver->meta0), str_addr,
		sizeof(str_addr));
	location->m0_url = strdup(str_addr);
	...
	do {
		...
		m1_raw = meta1_remote_get_container_by_id(&cnx_tmp, cid, &gerror_local,
			gs_grid_storage_get_timeout(gs, GS_TO_M1_CNX) / 1000,
			gs_grid_storage_get_timeout(gs, GS_TO_M1_OP) / 1000);
		if (!m1_raw) {
			GSERRORCAUSE(gserr, gerror_local, "Container ID=[%s] not found",
				str_cid);
...
struct gs_container_location_s *
gs_locate_container_by_hexid(gs_grid_storage_t * gs, const char *hexid,
	gs_error_t ** gserr)
{
...
struct gs_container_location_s *
gs_locate_container_by_hexid_v2(gs_grid_storage_t * gs, const char *hexid,
	char **out_nsname_on_m1, gs_error_t ** gserr)
{
	container_id_t cid;
...
struct gs_container_location_s *
gs_locate_container_by_name(gs_grid_storage_t * gs, const char *name,
	gs_error_t ** gserr)
{
	container_id_t cid;
