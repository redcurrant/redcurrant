...
static int _create(gs_container_t * container,
	struct m2v2_create_params_s *params, GError ** err, int nb,
	GSList ** exclude, int max);
...
static int
__create_in_meta2(addr_info_t * m1, gs_container_t * c,
	struct m2v2_create_params_s *params, GError ** err)
{
	GError *e = NULL;
	addr_info_t *new_master = NULL;
	char *master = NULL;
	...
	if (!m2 || 0 == g_strv_length(m2)) {
		if (NULL != e) {
			...
			GSETCODE(err, 500,
				"Failed to link service meta2 , no error specified");
		}
		rc = GS_ERROR;
		goto cleanup_label;
	}
	...
	GError ** err, int nb, GSList ** exclude, int max)
{
	...
	pM1 =
		gs_resolve_meta1v2(container->info.gs, C0_ID(container),
	...
	if (__create_in_meta2(&m1Addr, container, params, err)) {
		...
		return _get(container, params, err, 0, NB_ATTEMPTS_GET_CONTAINER,
			0 /*no more autocreate */ );
	}
	...
	if (err && *err) {
		...
		if (code == CODE_CONTAINER_EXISTS) {
			...
			*exclude = g_slist_prepend(*exclude, pM1);
			DEBUG("Local/Network error, retrying the creation of [%s]",
				C0_NAME(container));
			gs_decache_all(container->info.gs);
			return _create(container, params, err, nb + 1, exclude, max);
		}
			...
			GSETERROR(err, "Failed to create container [%s] on META1=[%s]",
				C0_NAME(container), str_addr);
			return GS_ERROR;
		}
	}
	...
	GSETERROR(err, "Cannot autocreate [%s] on META1=[%s] (unknown error)",
		C0_NAME(container), str_addr);
	return GS_ERROR;
}
	...
	GError ** err, int retry_count, int max_retries, int ac)
{
	...
	DEBUG("Resolution attempt NAME=[%s] ID=[%s]", C0_NAME(container),
		C0_IDSTR(container));
	...
	if ((*err)->code == CODE_CONTAINER_NOTFOUND) {
		if (!ac) {
			...
			GSList *exclude = NULL;
			int r =
				_create(container, params, err, 0, &exclude,
				NB_ATTEMPTS_AUTOCREATE);
			if (NULL != exclude) {
				g_slist_foreach(exclude, addr_info_gclean, NULL);
				g_slist_free(exclude);
			}
...
static int
container_resolve_and_get(gs_container_t * container,
	struct m2v2_create_params_s *params, int auto_create, GError ** err)
{
	return _get(container, params, err, 0, NB_ATTEMPTS_GET_CONTAINER,
		auto_create);
}
...
gs_container_t *
gs_init_container(gs_grid_storage_t * gs, const char *cname, int ac,
	gs_error_t ** gs_err)
{
	...
	container_id_to_string(container->cID, container->str_cID,
		sizeof(container->str_cID));
	container->ac = ac;
	...
	int auto_create, gs_error_t ** gs_err)
{
	...
	if (GS_ERROR == container_resolve_and_get_v2(container, &localError)) {
		if (!localError)
			GSETERROR(&localError, "Failed to resolve container [%s]",
				C0_NAME(container));
		goto error_label;
	}
...
error_label:
	...
	GSERRORCAUSE(gs_err, localError, "Failed to get container [%s]",
		container_name);
	g_clear_error(&localError);
	return NULL;
}
...
gs_container_t *
gs_get_container_by_hexid(gs_grid_storage_t * gs, const char *hex_id,
	int auto_create, gs_error_t ** gs_err)
{
	...
	TRACE("hash done : name=[%s] -> id=[%s]", C0_NAME(container),
		C0_IDSTR(container));

	/*get the container location */
	if (GS_ERROR == container_resolve_and_get(container, &params, auto_create,
			&localError)) {
		if (!localError)
			GSETERROR(&localError, "Failed to resolve container [%s]",
				C0_NAME(container));
...
_alias_to_path_info(gpointer alias)
{
	...
	pi->system_metadata =
		...
		(const guint8 *) ALIASES_get_mdsys(alias)->str,
		ALIASES_get_mdsys(alias)->len);
	char tmp[64];
	...
	pi->system_metadata =
		g_byte_array_append(pi->system_metadata, (const guint8 *) tmp,
		strlen(tmp));
	return pi;
}
...
_list_v2_wrapper(gs_container_t * c, const char *version, GError ** e)
{
	...
	le = m2v2_remote_execute_LIST(target, NULL, url, M2V2_FLAG_ALLVERSION,
		&beans);
	...
	gs_error_t ** err)
{
	...
	for (nbResult = 0, current = contents; current; current = current->next) {
		...
		memcpy(content->info.path, pi->path, MIN(sizeof(content->info.path) - 1,
				sizeof(pi->path) - 1));
		content->info.size = pi->hasSize ? pi->size : -1;
		if (NULL != pi->system_metadata)
			content->gba_sysmd =
				g_byte_array_append(g_byte_array_new(),
				pi->system_metadata->data, pi->system_metadata->len);
		if (NULL != pi->version)
		...
		if (filter) {
			...
			switch ((localRC = filter(content, user_data))) {
				...
				default:
					gs_content_free(content);
					GSETERROR(&localError,
						"bad return code from the user filter (%i)", localRC);
					goto end_label;
			}
		}
		else if (result) {
			gs_content_t **newTab =
				realloc(*result, (nbResult + 1) * sizeof(gs_content_t *));
			if (!newTab) {
				GSETERROR(&localError, "Memory allocation failure (%s)",
					strerror(errno));
				goto end_label;
			}
			newTab[nbResult - 1] = content;
...
end_label:
	...
	if (!rc) {
		...
		if (!localError)
			GSETERROR(&localError, "unknown error");
		GSERRORCAUSE(err, localError, "Cannot list the content of %s/%s",
...
gs_content_t *
gs_get_content_from_path(gs_container_t * container, const char *name,
	gs_error_t ** err)
{
	return gs_get_content_from_path_full(container, name, NULL, NULL, NULL,
		err);
}

gs_content_t *
gs_get_content_from_path_and_version(gs_container_t * container,
	const char *name, const char *version, gs_error_t ** err)
{
	return gs_get_content_from_path_full(container, name, version, NULL, NULL,
		err);
}

gs_content_t *
gs_get_content_from_path_full(gs_container_t * container, const char *name,
	const char *version, void *_p_filtered, void *_p_beans, gs_error_t ** err)
{
	...
	if (!gs_content_reload_with_filtered(content, TRUE, FALSE, p_filtered,
			p_beans, err)) {
		gs_content_free(content);
		return NULL;
	}
...
gs_container_load(gs_container_t * container, GError ** err)
{
	...
	gs_container_close_cnx(container);
	 ...
	 /**/
		m2Locations =
		gs_resolve_meta2(container->info.gs, C0_ID(container), err);
	if (!m2Locations) {
		GSETERROR(err, "Resolution error for NAME=[%s] ID=[%s]",
			C0_NAME(container), C0_IDSTR(container));
		return GS_ERROR;
	}
	...
	if (!gs_container_check_cnx(container)) {
		GSETERROR(err, "no META2 accepted to serve %s/%s", C0_NAME(container),
			C0_IDSTR(container));
	}
	else {
		rs = (*err == NULL);
...
gs_status_t
gs_container_reconnect_and_refresh(gs_container_t * container, GError ** err,
	gboolean may_refresh)
{
	/*sanity checks */
	if (!container) {
	...
	container->meta2_cnx =
		addrinfo_connect(&(container->meta2_addr), C0_M2TO_CNX(container), err);

	if (!gs_container_check_cnx(container)) {
		/*the connection failed */
		if (!may_refresh) {
			GSETERROR(err,
				"Not authorized to refresh once again the meta2 reference for %s/%s",
				C0_NAME(container), C0_IDSTR(container));
			return GS_ERROR;
		}
		if (!gs_container_refresh(container, err)) {
			GSETERROR(err, "Cannot reconnect/refresh to meta2 for %s/%s",
				C0_NAME(container), C0_IDSTR(container));
			return GS_ERROR;
		}
	}
...
{
	return gs_container_check_cnx(container) ? GS_OK :
		gs_container_reconnect(container, err);
}
...
_allm2(gs_container_t * container, gchar *** targets)
{
	...
	GSList *l, *allm2 =
		resolver_direct_get_meta2(container->info.gs->direct_resolver,
...
_destroy_on_allm2(gs_container_t * container, guint32 flags)
{
	...
	hc_url_set(url, HCURL_HEXID, container->str_cID);
	GByteArray *req = m2v2_remote_pack_DESTROY(NULL, url, flags);

	hc_url_clean(url);

	struct client_s **clients =
		gridd_client_create_many(targets, req, NULL, NULL);
	g_strfreev(targets);
	targets = NULL;
	...
	guint32 flags)
{
	...
	if (!m1) {
		...
		if (!meta1v2_remote_unlink_service(m1, &err,
				container->info.gs->ni.name, C0_ID(container), "meta2", 10000,
				30000, NULL))
...
gs_flush_container(gs_container_t * container, gs_error_t ** err)
{
	...
	int _filter(gs_content_t * content, void *user_data)
	{
		...
		if (GS_OK != _flusher()) {
			...
			GSETERROR(&gerr, "cannot flush the content : %s",
	...
	if (GS_OK != gs_list_container(container, NULL, _filter, NULL, err)) {
		if (!gerr)
			GSETERROR(&gerr, "unknown cause");
		GSERRORCAUSE(err, gerr, "cannot run the content's list : %s",
			g_error_get_message(gerr));
		g_clear_error(&gerr);
		return GS_ERROR;
	}
	else {
...
{
	return gs_download_content_by_name_full(container, name, NULL, NULL,
		dl_info, err);
}

gs_status_t
gs_download_content_by_name_and_version(gs_container_t * container,
	const char *name, const char *version, gs_download_info_t * dl_info,
	gs_error_t ** err)
{
	return gs_download_content_by_name_full(container, name, version, NULL,
		dl_info, err);
}

gs_status_t
gs_download_content_by_name_full(gs_container_t * container, const char *name,
	const char *version, const char *stgpol, gs_download_info_t * dl_info,
	gs_error_t ** err)
{
	int rc = GS_ERROR;
	gs_content_t *content = NULL;
	...
	if (gs_content_reload(content, FALSE, TRUE, &gserr_cache)) {
		if (content->deleted) {
			...
			if (gs_download_content_full(content, dl_info, stgpol, NULL, NULL,
					&gserr_cache)) {
				/* SUCCESS */
				gs_content_free(content);
				if (gserr_cache)
...
gs_status_t
gs_delete_content_by_name(gs_container_t * container, const char *name,
	gs_error_t ** err)
{
	int rc = GS_ERROR;
	gs_content_t *content;
...
hc_get_container_admin_entries(gs_container_t * container, gs_error_t ** gs_err)
{
	...
	if (!localError)
		GSETERROR(&localError, "unknown error");
	GSERRORCAUSE(gs_err, localError,
		"Failed to get container admin entries [%s]", C0_NAME(container));
	g_clear_error(&localError);
	return NULL;
}
