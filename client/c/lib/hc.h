 ...
 * @return 0 if an error occured, another value otherwise.
 */
gs_error_t *hc_put_content(gs_grid_storage_t * hc, struct hc_url_s *url,
	const char *local_path, const char *metadata, const char *sys_metadata,
	int ac);
 ...
 * @return 0 if an error occured, another value otherwise.
 */
gs_error_t *hc_append_content(gs_grid_storage_t * hc, struct hc_url_s *url,
 ...
 * @return 0 if an error occured, another value otherwise.
 */
gs_error_t *hc_list_contents(gs_grid_storage_t * hc, struct hc_url_s *url,
...
gs_error_t *hc_get_content(gs_grid_storage_t * hc, struct hc_url_s *url,
	const char *dest, int force, int cache, gchar * stgpol);
 ...
 */
gs_error_t *hc_object_info(gs_grid_storage_t * hc, struct hc_url_s *url,
	int xml, char **result);
 ...
 * @return a pointer to a gs_erorr_t if an error occured, NULL otherwise
 */
gs_error_t *hc_func_get_content_properties(gs_grid_storage_t * hc,
	struct hc_url_s *url, char ***result);
 ...
 * @return a pointer to a gs_erorr_t if an error occured, NULL otherwise
 */
gs_error_t *hc_func_delete_property(gs_grid_storage_t * hc,
	struct hc_url_s *url, char **keys);
 ...
 * @return a pointer to a gs_erorr_t if an error occured, NULL otherwise
 */
gs_error_t *hc_func_copy_content(gs_grid_storage_t * hc, struct hc_url_s *url,
	const char *source);
 ...
 * @param url The URL of the container, with the snapshot name as a query string
 */
gs_error_t *hc_func_delete_snapshot(gs_grid_storage_t * hc,
	struct hc_url_s *url);
 ...
 */
gs_error_t *hc_func_restore_snapshot(gs_grid_storage_t * hc,
	struct hc_url_s *url, int hard_restore);
