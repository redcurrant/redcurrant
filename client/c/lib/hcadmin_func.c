...
static gs_error_t *
_m1policy_update(gchar * action, gchar * ns, gboolean isprefix,
	container_id_t cid, gchar * type, gchar * exludesrv, gboolean checkonly,
	gchar *** result)
{
	GError *err = NULL;
	...
	while (m1addr) {
		hc_result =
			meta1v2_remote_update_m1_policy(m1addr, &err, ns,
			(isprefix ? cid : NULL), (isprefix ? NULL : cid), type, action,
			checkonly, exludesrv, 300, 300);
		if (err) {
			if (err->code < 100 || err->code > 500) {
				...
				GRID_WARN("META1 request error (%d) : %s", err->code,
					err->message);
...
_m2_touch(struct hc_url_s *hcurl, guint32 flags)
{
	...
	if (!gserr) {
		if (!cid) {
			gs_grid_storage_free(hc);
			GSERRORCODE(&gserr, -1, "Invalid container_name %s/%s", ns,
				container_name);
			return gserr;
		}
	}
	...
	if (!m2list) {
		GSERRORCODE(&gserr, -1, "Meta2 Resolution error for NAME=[%s] ID=[%s]",
			C0_NAME(cid), C0_IDSTR(cid));
		if (cid)
...
static void
_pack_result(GPtrArray ** gpa, gchar * id, gchar ** result, gs_error_t * err,
	struct global_cpt_s *cpts)
{
	if (!*gpa)
		*gpa = g_ptr_array_new();
	...
	if (err)
		...
		for (; tmp && *tmp; tmp++) {
			...
			if (g_strv_length(tokens) == 3) {
				if (cpts) {
					cpts->checkedRef =
						cpts->checkedRef + g_ascii_strtoull(tokens[0], NULL,
						10);
					cpts->updatedRef =
						cpts->updatedRef + g_ascii_strtoull(tokens[1], NULL,
						10);
					cpts->failedRef =
						cpts->failedRef + g_ascii_strtoull(tokens[2], NULL, 10);
				}
				g_ptr_array_add(*gpa,
					g_strdup_printf
					("%s : References checked %s , updated %s , failed %s", id,
...
_m1policy_update_thread(gpointer data, gpointer p)
{
	...
	err =
		_m1policy_update(user_data->action, user_data->ns, TRUE, cid, "meta2",
		user_data->excludesrv, user_data->checkonly, &tmp);
...
gs_error_t *
hcadmin_meta1_policy_update(char *ns, gchar * action, gboolean checkonly,
	gchar ** globalresult, gchar *** result, char **args)
{
	...
	if (g_strcmp0(hexID, "ALL") == 0) {
		...
		user_data->gpa = NULL;
		pool =
			g_thread_pool_new(_m1policy_update_thread, user_data, 10, TRUE,
			&gerr);
		for (idx = 0; idx < 65536; idx++) {
			...
			if (gerr != NULL) {
				GRID_WARN("Failed to push new data thread %d, %d %s", idx,
		...
		if (strlen(hexID) == 4) {
			...
			err =
				_m1policy_update(m1pAction, ns, TRUE, cid, srvtype, excludesrv,
				checkonly, &tmp);
			...
			err =
				_m1policy_update(m1pAction, ns, FALSE, cid, srvtype, excludesrv,
				checkonly, &tmp);
			_pack_result(&gpa, hexID, tmp, err, NULL);
			if (err) {
				gs_error_free(err);
	...
	if (gpa) {
		...
		if (cpts) {
			*globalresult =
				g_strdup_printf
				("Global result : References checked %lu , updated %lu , failed %lu",
				cpts->checkedRef, cpts->updatedRef, cpts->failedRef);
		}
...
gs_error_t *
hcadmin_touch(char *url, gchar * action, gboolean checkonly,
	gchar ** globalresult, gchar *** result, char **args)
{
	gchar *option = NULL;
	guint32 flags = 0;
