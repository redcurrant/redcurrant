...
static GError *
_fill_sorted_chunk_array(GArray * in, GArray * sorted_chunks)
{
	gboolean ok = FALSE;
	GError *err = NULL;
	...
	for (guint i = 0; i < in->len; i++) {
		...
		ok = m2v2_parse_chunk_position(CONTENTS_get_position(pair->content)->
			str, &pos, &par, &sub);
		if (!ok) {
...
static GError *
_find_gaps_and_dispatch_chunks(struct rainx_params_s *params,
	...
	for (gint64 i = 0; i < k + m; i++) {
		...
		if (pair->chunk == NULL) {
			...
			*gap_positions = g_slist_prepend(*gap_positions, (gpointer) i);
			...
			GRID_DEBUG
				("Missing chunk at position %ld, looking for unavailable chunk '%s'",
				i, s_pos);
			for (guint j = 0; j < params->unavail_chunk_pairs->len; j++) {
				m2v2_chunk_pair_t *pair2 =
					&g_array_index(params->unavail_chunk_pairs,
					m2v2_chunk_pair_t, j);
				...
				if (!strcmp(s_pos, CONTENTS_get_position(pair2->content)->str)
					...
					*broken_chunks =
						g_slist_prepend(*broken_chunks, pair2->chunk);
					*broken_chunks =
						g_slist_prepend(*broken_chunks, pair2->content);
					GRID_DEBUG
						("-> found matching broken chunk/content, will reuse hash");
					break;
				}
			}
	...
	GSList * gap_positions, GSList ** spare_chunks)
{
	...
	for (GSList * l = gap_positions; l != NULL && err == NULL; l = l->next) {
		...
		struct bean_CHUNKS_s *spare =
			(struct bean_CHUNKS_s *) (*spare_chunks)->data;
		m2v2_chunk_pair_t *pair = &g_array_index(sorted_chunks,
...
static gchar *
_build_semicol_spare_rawx_list(GArray * chunks, GSList * gap_positions)
{
	gchar *hash_str = NULL;
	...
	for (GSList * l = gap_positions; l != NULL; l = l->next) {
		...
		m2v2_chunk_pair_t *pair =
			&g_array_index(chunks, m2v2_chunk_pair_t, pos);
...
static size_t
_drop_curl_data(void *buffer, size_t size, size_t nmemb, void *userp)
{
	(void) buffer;
	(void) userp;
	return size * nmemb;
...
static GError *
_ask_reconstruct(struct rainx_params_s *params,
	...
	GSList * gap_positions)
{
	...
	spare_rawx_list =
		_build_semicol_spare_rawx_list(sorted_chunks, gap_positions);
...
#define ADD_HEADER(header,valueformat,value) do {\
	...
	ADD_HEADER("contentmetadata-sys", "%s",
		ALIASES_get_mdsys(params->alias)->str);

#undef ADD_HEADER

	struct service_info_s *si =
		get_one_namespace_service(hc_url_get(url, HCURL_NS), "rainx", &err);
	if (!si) {
		if (err != NULL)
...
_free_sorted_chunks(GArray * sorted_chunks)
{
	for (guint i = 0; i < sorted_chunks->len; i++) {
		m2v2_chunk_pair_t *pair =
			&g_array_index(sorted_chunks, m2v2_chunk_pair_t, i);
	...
	struct rainx_params_s *params, gboolean reuse_broken)
{
	...
	stg_pol_rainx_get_param(nsinfo,
		CONTENTS_HEADERS_get_policy(params->content_header)->str, DS_KEY_K, &k);
	stg_pol_rainx_get_param(nsinfo,
		CONTENTS_HEADERS_get_policy(params->content_header)->str, DS_KEY_M, &m);
