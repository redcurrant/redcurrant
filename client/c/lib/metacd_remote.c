...
connect_to_unix_socket(const struct metacd_connection_info_s *mi, GError ** err)
{
	...
	if (-1 == (connect(fd, (struct sockaddr *) &sun, sizeof(sun)))) {
		if (errno != EINTR && errno != EINPROGRESS) {
			GSETERROR(err, "cannot connect to %s (%s)", mi->metacd.path,
				strerror(errno));
			goto errorLabel;
		}
			...
			for (;;) {
				...
				switch (rc) {
					case 0:	/*timeout */
						GSETCODE(err, ERRCODE_CONN_TIMEOUT,
							"METAcd connection timeout");
						goto errorLabel;
					case -1:
						if (errno == EINTR)
							break;	/*interrupted */
						GSETERROR(err,
							"Cannot connect to METACD through %s (%s)",
					...
					case 1:
						if ((pfd.revents & POLLERR) || (pfd.revents & POLLHUP)
							...
							GSETERROR(err,
								"Cannot connect to %s (poll events : %04X)",
								sun.sun_path, pfd.revents);
							goto errorLabel;
						}
					...
					default:
						GSETERROR(err,
							"Cannot connect to %s (unexpected event)",
							sun.sun_path);
						goto errorLabel;
				}
			}
...
inline const gchar *
make_metacd_path(const gchar * content_path, const gchar * content_version)
{
...
inline const gchar *
make_metacd_path2(const gchar * content_path, content_version_t content_version)
{
	return g_strdup_printf("%s?version=%" G_GINT64_FORMAT, content_path,
		content_version);
}

inline void
destroy_metacd_path(const gchar * metacd_path)
{
	g_free((gpointer) metacd_path);
}

static MESSAGE
metacd_create_request(const struct metacd_connection_info_s *mi,
	const container_id_t cID, GError ** err)
{
	...
	if (cID) {
		if (!message_add_field(request, MSGKEY_CID, sizeof(MSGKEY_CID) - 1, cID,
	...
	if (!message_add_field(request, MSGKEY_NS, sizeof(MSGKEY_NS) - 1,
			mi->metacd.nsName, strlen(mi->metacd.nsName), err)) {
	...
	if (mi->cnx_id && mi->cnx_id_size > 0) {
		TRACE("requesting METAcd mi=%p id=%p size=%lu", mi,
			(mi->cnx_id ? mi->cnx_id : NULL),
			(mi->cnx_id ? mi->cnx_id_size : 0));
...
GSList *
metacd_remote_get_meta0(const struct metacd_connection_info_s * mi,
	GError ** err)
{
	int fd = -1;
	GSList *result = NULL;
	...
	if (!message_set_NAME(request, MSGNAME_METACD_GETM0,
			sizeof(MSGNAME_METACD_GETM0) - 1, err)) {
	...
	if (!metaXClient_reply_sequence_run(err, request, &fd,
			mi->metacd.timeout.op, &rs_data)) {
...
GSList *
metacd_remote_get_meta1(const struct metacd_connection_info_s * mi,
	const container_id_t cID, int ro, gboolean * p_ref_exists, GSList * exclude,
	GError ** err)
{
	int fd = -1;
	GSList *result = NULL;
	...
	gboolean _get_meta1_reply_cb(GError ** local_err, gpointer udata, gint code,
		MESSAGE rep)
	{
		(void) local_err;
		(void) udata;
		...
		if (NULL == p_ref_exists)
			return TRUE;
		message_get_field(rep, "REF_EXISTS", strlen("REF_EXISTS"),
			(void **) (&ref_exists), &ref_value_size, local_err);
	...
	struct code_handler_s rs_codes[] = {
		{200, REPSEQ_FINAL | REPSEQ_BODYMANDATORY, addr_info_concat,
				_get_meta1_reply_cb},
		{0, 0, NULL, NULL}
	};
	struct reply_sequence_data_s rs_data = { &result, 0, rs_codes };
	...
	if (!message_set_NAME(request, MSGNAME_METACD_GETM1,
			sizeof(MSGNAME_METACD_GETM1) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
	...
	if (ro) {
		if (!message_add_field(request, "RO", strlen("RO"), "TRUE",
				strlen("TRUE"), err)) {
...
gboolean
metacd_remote_set_meta1_master(const struct metacd_connection_info_s * mi,
	const container_id_t cid, const char *master, GError ** e)
{
	int fd = -1;
	MESSAGE request = NULL;
	...
	if (!message_set_NAME(request, MSGNAME_METACD_SET_M1_MASTER,
			sizeof(MSGNAME_METACD_SET_M1_MASTER) - 1, e)) {
	...
	/*send the request and read the responses */
	if (!metaXClient_reply_sequence_run(e, request, &fd, mi->metacd.timeout.op,
			&rs_data)) {
...
GSList *
metacd_remote_get_meta2(const struct metacd_connection_info_s * mi,
	const container_id_t cID, GError ** err)
{
	int fd = -1;
	GSList *result = NULL;
	...
	if (!message_set_NAME(request, MSGNAME_METACD_GETM2,
			sizeof(MSGNAME_METACD_GETM2) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
...
gboolean
metacd_remote_decache(const struct metacd_connection_info_s * mi,
	const container_id_t cID, GError ** err)
{
	int fd = -1;
	MESSAGE request = NULL;
	...
	if (!message_set_NAME(request, MSGNAME_METACD_DECACHE,
			sizeof(MSGNAME_METACD_DECACHE) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
...
gboolean
metacd_remote_decache_all(const struct metacd_connection_info_s * mi,
	GError ** err)
{
	int fd = -1;
	MESSAGE request = NULL;
...
metacd_remote_get_content(const struct metacd_connection_info_s *mi,
	const container_id_t cID, const gchar * path, GError ** err)
{
	...
	if (!message_add_field(request, MSGKEY_PATH, sizeof(MSGKEY_PATH) - 1, path,
			strlen(path), err)) {
		GSETERROR(err, "Cannot set the path in the request");
		goto errorLabel;
	}
	if (!message_set_NAME(request, MSGNAME_METACD_V2_CHUNKS_GET,
			sizeof(MSGNAME_METACD_V2_CHUNKS_GET) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
	...
	const container_id_t cID, const gchar * path, GError ** err)
{
	...
	if (!message_set_NAME(request, MSGNAME_METACD_V1_CHUNKS_DEL,
			sizeof(MSGNAME_METACD_V1_CHUNKS_DEL) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
...
metacd_remote_flush_content(struct metacd_connection_info_s * mi, GError ** err)
{
	...
	if (!message_set_NAME(request, MSGNAME_METACD_V1_CHUNKS_FLUSH,
			sizeof(MSGNAME_METACD_V1_CHUNKS_FLUSH) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
...
gboolean
metacd_remote_save_content(struct metacd_connection_info_s * mi,
	struct meta2_raw_content_s * content, GError ** err)
{
	...
	destroy_metacd_path(metacd_path);
	if (!message_set_NAME(request, MSGNAME_METACD_V2_CHUNKS_PUT,
			sizeof(MSGNAME_METACD_V2_CHUNKS_PUT) - 1, err)) {
		GSETERROR(err, "Cannot set the name of the request");
		goto errorLabel;
	}
