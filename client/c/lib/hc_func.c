...
_filter_info_xml(gs_content_t * content, struct list_content_s *lc)
{
	...
	g_string_append_printf(tmp, "  <Content>\n"
		...
		content->version, sysmd, content->deleted);
	g_free(sysmd);
...
_filter_info(gs_content_t * content, struct list_content_s *lc)
{
	...
	g_string_append_printf(tmp, "%" G_GINT64_FORMAT " %s %s",
		content->info.size, content->version, content->info.path);
	if (content->deleted)
	...
	if (!local_path || !g_ascii_strcasecmp(local_path, "-")) {
		...
		*out = open(local_path, O_WRONLY | O_CREAT | (force ? O_TRUNC : O_EXCL),
		...
		if (-1 == *out) {
			if (errno == ENOENT) {
				*out =
					open(local_path, O_WRONLY | O_CREAT | O_EXCL, file_perms);
				if (-1 == *out) {
					g_printerr
						("Cannot create and open the local file %s (%s)\n",
...
static gs_error_t *
hc_upload_content(gs_grid_storage_t * hc, struct hc_url_s *url,
	const char *local_path, const char *stgpol, const char *sys_metadata,
	int ac, gboolean is_append)
{
	int in = -1;
	struct stat64 s;
	gs_container_t *c = NULL;
	...
	if (-1 == stat64(local_path, &s)) {
		...
		e->code = errno;
		e->msg =
			g_strdup_printf("Cannot stat the local file (%s)\n",
			strerror(errno));
		return e;
	}
	...
	if (-1 == (in = open(local_path, O_RDONLY | O_LARGEFILE))) {
		...
		e->code = errno;
		e->msg =
			g_strdup_printf("Cannot open the local file (%s)\n",
			strerror(errno));
		goto end_put;
	}
	...
	if (!(c =
			gs_get_storage_container(hc, hc_url_get(url, HCURL_REFERENCE), NULL,
				ac, &e))) {
		g_printerr
			("Failed to resolve and/or create meta2 entry for reference %s\n",
			hc_url_get(url, HCURL_REFERENCE));
		goto end_put;
	}
	...
	/*upload the content */
	if (is_append) {
		if (!gs_append_content(c, hc_url_get(url, HCURL_PATH), s.st_size,
				_feed_from_fd, &in, &e)) {
			goto end_put;
		}
	}
	else {
		if (!gs_upload_content_v2(c, hc_url_get(url, HCURL_PATH), s.st_size,
				_feed_from_fd, &in, stgpol, sys_metadata, &e)) {
			goto end_put;
		}
	}
...
gs_error_t *
hc_put_content(gs_grid_storage_t * hc, struct hc_url_s * url,
	const char *local_path, const char *stgpol, const char *sys_metadata,
	int ac)
{
	return hc_upload_content(hc, url, local_path, stgpol, sys_metadata, ac,
		FALSE);
}

gs_error_t *
hc_func_copy_content(gs_grid_storage_t * hc, struct hc_url_s * url,
	const char *source)
{
	gs_error_t *e = NULL;
	gs_container_t *c = NULL;
...
gs_error_t *
hc_append_content(gs_grid_storage_t * hc, struct hc_url_s * url,
...
_sort_listed(struct list_content_s *lc)
{
	...
	for (l = lc->listed; l && l->data; l = l->next) {
		...
		if (lc->show_info) {
			lc->buffer =
				g_string_append(lc->buffer,
				((struct ls_utils_s *) l->data)->str);
			lc->nb_elts++;
		}
		else {
...
gs_error_t *
hc_list_contents(gs_grid_storage_t * hc, struct hc_url_s *url, int output_xml,
	int show_info, char **result)
{
	gs_error_t *e = NULL;
	gs_container_t *c = NULL;
...
gs_error_t *
hc_get_content(gs_grid_storage_t * hc, struct hc_url_s * url,
	const char *local_path, int force, int cache, gchar * stgpol)
{
	gs_error_t *e = NULL;
	...
	if (NULL != c) {
		if (_open_destination(local_path, force, &out)) {
			...
			g_snprintf(tmp, sizeof(tmp),
				"Failed to open the destination file descriptor to path=%s\n",
				local_path ? local_path : "<stdout>");
			e = g_malloc0(sizeof(gs_error_t));
			e->code = 0;
			e->msg = g_strdup(tmp);
...
gs_error_t *
hc_object_info(gs_grid_storage_t * hc, struct hc_url_s * url, int xml,
	char **result)
{
...
hc_delete_content(gs_grid_storage_t * hc, struct hc_url_s * url)
{
	...
	if (NULL != c) {
		...
		// First try
		content =
			gs_get_content_from_path_and_version(c, hc_url_get(url, HCURL_PATH),
			version, &e);
		if (content == NULL && e != NULL && e->code == CODE_CONTENT_NOTFOUND
			&& !version) {
			...
			// Second try
			content =
				gs_get_content_from_path_and_version(c, hc_url_get(url,
					HCURL_PATH), version, &e);
		}
		if (NULL != content) {
...
gs_error_t *
hc_func_set_property(gs_grid_storage_t * hc, struct hc_url_s * url, char **args)
{
	gs_error_t *e = NULL;
	gs_container_t *c = NULL;
	gs_content_t *content = NULL;
...
gs_error_t *
hc_func_get_content_properties(gs_grid_storage_t * hc, struct hc_url_s * url,
	char ***result)
{
	gs_error_t *e = NULL;
	gs_container_t *c = NULL;
...
gs_error_t *
hc_func_delete_property(gs_grid_storage_t * hc, struct hc_url_s * url,
	char **keys)
{
	gs_error_t *e = NULL;
	gs_container_t *c = NULL;
