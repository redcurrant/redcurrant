...
_free_content_internals(gs_content_t * content)
{
	...
	if (content->chunk_list) {
		...
		TRACE("Freeing %u old chunks in [grid://%s/%s/%s]",
			g_slist_length(content->chunk_list),
			content->info.container->info.gs->ni.name, C1_IDSTR(content),
			C1_PATH(content));
...
_filter_and_sort(GSList * chunk_list, GSList ** p_filtered)
{
	...
	gint _contents_cmp(gconstpointer _bean1, gconstpointer _bean2)
	{
		...
		m2v2_parse_chunk_position(CONTENTS_get_position((struct bean_CONTENTS_s
					*) _bean1)->str, &mainpos1, &par1, &secondpos1);
		m2v2_parse_chunk_position(CONTENTS_get_position((struct bean_CONTENTS_s
					*) _bean2)->str, &mainpos2, &par2, &secondpos2);
...
_has_dotted_position(GSList * beans)
{
	...
	for (; l && l->data; l = l->next) {
		if (DESCR(l->data) == &descr_struct_CONTENTS) {
			pos =
				CONTENTS_get_position((struct bean_CONTENTS_s *) l->data)->str;
...
gboolean
map_raw_content_from_beans(struct meta2_raw_content_s * raw_content,
	GSList * beans, GSList ** p_filtered, gboolean force_keep_position)
{
	...
	for (; l && l->data; l = l->next) {
		if (DESCR(l->data) == &descr_struct_CONTENTS) {
			...
			struct meta2_raw_chunk_s *ci =
				g_malloc0(sizeof(struct meta2_raw_chunk_s));
			...
			raw_content->raw_chunks =
				g_slist_prepend(raw_content->raw_chunks, ci);
			if (pos > maxpos)
				maxpos = pos;
		}
		else if (DESCR(l->data) == &descr_struct_CONTENTS_HEADERS) {
			raw_content->size = CONTENTS_HEADERS_get_size(l->data);
			raw_content->storage_policy =
				strdup(CONTENTS_HEADERS_get_policy(l->data)->str);
		}
		else if (DESCR(l->data) == &descr_struct_ALIASES) {
			char *mdsys = ALIASES_get_mdsys(l->data)->str;

			g_strlcpy(raw_content->path, ALIASES_get_alias(l->data)->str,
				sizeof(raw_content->path));
...
map_policy_from_beans(gchar ** policy, GSList * beans)
{
	...
	for (l = beans; l && l->data; l = l->next) {
		if (DESCR(l->data) == &descr_struct_CONTENTS_HEADERS) {
			struct bean_CONTENTS_HEADERS_s *bch =
				(struct bean_CONTENTS_HEADERS_s *) l->data;
...
void
map_content_from_raw(gs_content_t * content,
	struct meta2_raw_content_s *raw_content)
{
	...
	for (l = raw_content->raw_chunks; l; l = l->next) {
		...
		if (!raw_chunk->flags) {
			...
			content->chunk_list =
				g_slist_prepend(content->chunk_list, g_memdup(&chunk,
					sizeof(chunk_info_t)));
		}
	}
...
static gboolean
_reload_content(gs_content_t * content, GSList ** p_filtered, GSList ** p_beans,
	GError ** err)
{
	struct meta2_raw_content_s *raw_content;
	...
	*err = m2v2_remote_execute_GET(target, NULL, url, flags, &beans);
	...
	TRACE("Content [grid://%s/%s/%s] reloaded (%u chunks)",
		content->info.container->info.gs->ni.name, C1_IDSTR(content),
		C1_PATH(content), g_slist_length(content->chunk_list));
...
_get_content_version(gs_content_t * content)
{
	...
	if (NULL == C1_VERSION(content)) {
		...
		if (!gs_content_reload(content, TRUE, FALSE, &reloadErr)) {
			...
			DEBUG
				("_get_content_version: found version [%s] for content [%s] from meta2",
 ...
 */
gboolean
gs_content_reload(gs_content_t * content, gboolean allow_meta2,
	gboolean allow_cache, gs_error_t ** err)
{
	return gs_content_reload_with_filtered(content, allow_meta2, allow_cache,
		NULL, NULL, err);
}

gboolean
gs_content_reload_with_filtered(gs_content_t * content, gboolean allow_meta2,
	gboolean allow_cache, GSList ** p_filtered, GSList ** p_beans,
	gs_error_t ** err)
{
	gboolean rc = FALSE;
	...
	if (allow_cache) {			/* Try with the METACD */
		...
		if (resolver_metacd_is_up(metacd)) {
			...
			metacd_path =
				make_metacd_path(C1_PATH(content),
				_get_content_version(content));
			raw_content =
				resolver_metacd_get_content(metacd, C1_ID(content), metacd_path,
				&localError);
...
#define MAX_ATTEMPTS_COMMIT 2
	...
	for (nb_refreshes = MAX_ATTEMPTS_COMMIT; nb_refreshes > 0; nb_refreshes--) {
		...
		if (NULL != localError) {
			if (localError->code == CODE_CONTENT_NOTFOUND
				&& nb_refreshes < MAX_ATTEMPTS_COMMIT) {
				/*content already removed */
				break;
			}
			CONTAINER_REFRESH(C1_C0(content), localError, end_label,
				C1_PATH(content));
		}
		else {
			raw_content = g_malloc0(sizeof(struct meta2_raw_content_s));
	...
	TRACE("COMMIT path=%s %s/%s", C1_PATH(content), C1_NAME(content),
		C1_IDSTR(content));
	if (localError)
		g_clear_error(&localError);
	...
	for (GSList * cursor = beans; cursor != NULL; cursor = cursor->next) {
		if (DESCR(cursor->data) == &descr_struct_CHUNKS) {
			if (!rawx_delete_v2(cursor->data, &localError)) {
				gchar *cid =
					CHUNKS_get_id((struct bean_CHUNKS_s *) cursor->data)->str;
...
end_label:
	if (remove_done) {
		(void) gs_container_reconnect_if_necessary(C1_C0(content), NULL);
		for (nb_refreshes = MAX_ATTEMPTS_ROLLBACK_DELETE;
			!C1_ROLLBACK(content, &localError) && nb_refreshes > 0;
			nb_refreshes--) {
			CONTAINER_REFRESH(C1_C0(content), localError, error_label,
				C1_PATH(content));
		}
	}
...
gs_status_t
gs_content_get_info(const gs_content_t * content, gs_content_info_t * info,
	gs_error_t ** err)
{
	if (!content || !info) {
		GSERRORSET(err, "Invalid parameter");
...
gs_status_t
gs_content_get_metadata(gs_content_t * content, uint8_t * dst,
	size_t * dst_size, gs_error_t ** err)
{
	size_t real_size;
...
gs_status_t
gs_content_get_system_metadata(gs_content_t * content, uint8_t * dst,
	size_t * dst_size, gs_error_t ** err)
{
	size_t real_size;
...
gs_status_t
gs_content_set_metadata(gs_content_t * content, uint8_t * src, size_t src_size,
	gs_error_t ** err)
{
	(void) content;
	(void) src;
...
gs_decache_chunks_in_metacd(gs_content_t * content)
{
	...
	metacd_path =
		make_metacd_path(C1_PATH(content), _get_content_version(content));
	if (!resolver_metacd_del_content(metacd, C0_ID(container), metacd_path,
			&flush_error)) {
		WARN("METACD flush failed for [%s/%s/%s] : %s", client->ni.name,
			C0_IDSTR(container), C1_PATH(content),
...
_update_sys_metadata(gpointer data, gpointer metadata)
{
	...
	rawx_update_chunk_attr((struct meta2_raw_chunk_s *) data, "sys-metadata",
		(const char *) metadata, &e);
	if (NULL != e) {
		g_clear_error(&e);
	}
...
gs_status_t
hc_set_content_storage_policy(gs_container_t * c, const char *path,
	const char *stgpol, gs_error_t ** e)
{
	GError *ge = NULL;
	...
	if (c->meta2_addr.port <= 0 && !gs_reload_container(c, &ge)) {
	...
	if (NULL != rc->system_metadata) {
		...
		g_hash_table_insert(unpacked, g_strdup("storage-policy"),
			g_strdup(stgpol));
		...
		GSERRORCAUSE(e, ge, "No metadata on content [%s], cannot process",
			path);
		if (NULL != ge)
			g_clear_error(&ge);
		meta2_raw_content_clean(rc);
	...
	int r0 =
		meta2_remote_modify_metadatasys(&ctx, C0_ID(c), path, metadata_str,
		&ge);
	c->meta2_cnx = ctx.fd;
	if (!r0) {
		GSERRORCAUSE(e, ge, "Failed to update system metadata of content [%s]",
			stgpol, path);
		g_clear_error(&ge);
		meta2_raw_content_clean(rc);
		return GS_ERROR;
...
hc_set_content_property(gs_content_t * content, char **props, gs_error_t ** e)
{
	...
	for (i = 0; i < g_strv_length(props); i++) {
		...
		if (g_strv_length(kv) != 2) {
			GSERRORCODE(e, 0, "Invalid property [%s] : format  key=value",
				props[i]);
			status = GS_ERROR;
			goto enderror;
		}
		PROPERTIES_set2_alias(bp, hc_url_get(url, HCURL_PATH));
		PROPERTIES_set_alias_version(bp, (guint64) hc_url_get(url,
				HCURL_VERSION));
...
gs_status_t
hc_get_content_properties(gs_content_t * content, char ***result,
	gs_error_t ** e)
{
	GError *ge = NULL;
	char target[64];
	...
	ge = m2v2_remote_execute_PROP_GET(target, NULL, url, M2V2_FLAG_NODELETED,
		&beans);

	if (NULL != ge) {
		GSERRORCAUSE(e, ge, "Failed to get  properties of content [%s]",
			C1_PATH(content));
		g_clear_error(&ge);
		status = GS_ERROR;
		goto enderror;
	...
	for (l = beans; l && l->data; l = l->next) {
		if (DESCR(l->data) == &descr_struct_PROPERTIES) {
			...
			final[max - 1] =
				g_strdup_printf("%s=%s", PROPERTIES_get_key(bp)->str, buf);
			final[max] = NULL;
		}
	}
...
hc_delete_content_property(gs_content_t * content, char **keys, gs_error_t ** e)
{
	...
	for (i = 0; i < g_strv_length(keys); i++) {
		for (l = out_beans; l && l->data; l = l->next) {
			if (DESCR(l->data) == &descr_struct_PROPERTIES) {
				if (g_ascii_strcasecmp(PROPERTIES_get_key((struct
								bean_PROPERTIES_s *) l->data)->str,
						keys[i]) == 0) {
...
gs_status_t
hc_copy_content(gs_container_t * c, const char *src, const char *dst,
	gs_error_t ** e)
{
	GError *ge = NULL;
	char target[64];
