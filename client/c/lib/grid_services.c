...
static gs_service_t *
_make_service_from_srvinfo(gs_container_t * container,
...
static gs_service_t *
_make_service_from_addr(gs_container_t * container, const gchar * service,
	const addr_info_t * ai)
{
	return _make_service_from_srvinfo(container,
		_make_srvinfo(container->info.gs->ni.name, service, ai));
}

static gs_service_t **
_make_service_array_from_services(gs_container_t * container,
	const char *srvtype, GSList * list_of_srvinfo)
{
	gs_service_t **result;
	GSList *l;
	GPtrArray *pa;
...
static gs_service_t **
_make_service_array_from_addresses(gs_container_t * container,
	const char *srvtype, GSList * list_addr)
{
	gs_service_t **result;
	GSList *l;
	GPtrArray *pa;
...
static gs_service_t **
addrlist_make_service_array(GSList * argl, const char *srvtype,
...
socklen_t
gs_service_get_address(const gs_service_t * service, struct sockaddr * sa,
	socklen_t sa_size)
{
	gsize result_gsize;
	gint rc;
	...
	char **paths, gs_error_t ** err)
{
	...
	for (path = paths; *path; path++) {
		...
		gerr = NULL;
		ai = meta2_remote_service_get_content_service(&ctx, C0_ID(container),
			srvtype, *path, &gerr);
		if (!ai)
			metacnx_close(&ctx);
		merge_cnx_in_container(&ctx, container);
		if (!ai) {
			GSERRORCAUSE(err, gerr, "Failed to get the content for [%s]",
				*path);
			if (gerr)
				g_error_free(gerr);
	...
	char **paths, gs_error_t ** err)
{
	...
	init_cnx_with_container(&ctx, container);
	si = meta2_remote_service_add_contents(&ctx, C0_ID(container), srvtype,
		paths_list, &gerr);
	if (!si)
		metacnx_close(&ctx);
	merge_cnx_in_container(&ctx, container);
...
gs_delete_services_for_paths(gs_container_t * container, const char *srvtype,
	char **paths, char ***really_removed, gs_service_t *** services_used,
	gs_error_t ** err)
{
	status_t rc;
	char **path;
	...
	if (!rc) {
		...
		if (services_used)
			*services_used =
				addrlist_make_service_array(used, srvtype, container);
	}
...
gs_service_t **
gs_get_all_services_used(gs_container_t * container, const gchar * srvtype,
	gs_error_t ** err)
{
	gs_service_t **result;
	GError *gerr = NULL;
	...
	tmp =
		meta1v2_remote_list_reference_services(meta1_addr, &gerr,
	...
	for (i = 0; i < g_strv_length(tmp); i++) {
		...
		if (g_strv_length(addr_tok) != 2)
			continue;
		if ((addr =
				build_addr_info(addr_tok[0], atoi(addr_tok[1]), &gerr)) != NULL)
...
gs_service_t **
gs_service_flush(gs_container_t * container, const char *srvtype,
	gs_error_t ** err)
{
	gboolean rc;
	gs_service_t **result;
	...
	init_cnx_with_container(&ctx, container);
	rc = meta2_remote_service_flush(&ctx, C0_ID(container), srvtype, &list_addr,
		&gerr);
	if (rc != GS_OK)
		metacnx_close(&ctx);
	merge_cnx_in_container(&ctx, container);
	...
	if (!rc || gerr) {
		GSERRORCAUSE(err, gerr,
			"Failed to flush the indexes of type [%s] used by [%s]", srvtype,
			C0_NAME(container));
		result = NULL;
	}
	else {
		result =
			_make_service_array_from_addresses(container, srvtype, list_addr);
	}
...
gs_service_t **
gs_container_service_get_all(gs_container_t * container, const char *srvtype,
	gs_error_t ** err)
{
	...
	if (!container || !srvtype || !*srvtype) {
		GSERRORCODE(err, EINVAL, "Invalid parameter (%p %p)", container,
			srvtype);
		return NULL;
	}
	...
	while (nb_try < 3) {
		...
		if (!meta1_addr) {
			if (gerr) {
				GSERRORCAUSE(err, gerr,
					"Failed to resolve meta1 for container [%s]",
					C0_NAME(container));
			}
			result = NULL;
			goto end_label;
		...
		if (!srv_str_list) {
			if (gerr) {
				...
				if (gerr->code >= 500 || gerr->code < 100) {
					...
					GSERRORCAUSE(err, gerr,
						"No service of type [%s] found for container [%s]",
			...
			result =
				_make_service_array_from_services(container, srvtype,
				list_of_srvinfo);
			break;
		}
	}
...
gs_service_t **
gs_container_service_get_available(gs_container_t * container,
	const char *srvtype, gs_error_t ** err)
{
	...
	while (1) {
		...
		if (!meta1_addr) {
			GSERRORCAUSE(err, gerr,
				"No service of type [%s] found for container [%s]", srvtype,
				C0_NAME(container));
			result = NULL;
			goto end_label;
		}
		str_srv =
			meta1v2_remote_link_service(meta1_addr, &gerr,
			container->info.gs->ni.name, C0_ID(container), srvtype,
