...
help(void)
{
	...
	g_printerr
		("\tList the contents available in a container and downloads them in a local directory\n");
	g_printerr("Options:\n");
...
write_to_fd(void *uData, const char *b, const size_t bSize)
{
	...
	if (b && bSize > 0) {
		...
		if (nbW > 0) {
			PRINT_DEBUG("wrote %" G_GSSIZE_FORMAT " bytes among %"
				G_GSIZE_FORMAT " (from %p) in fd=%d\n", nbW, bSize, b, fd);
		}
		else {
...
dump_container_properties(gs_container_t * container)
{
	...
	if (!meta2_remote_list_all_container_properties(&cnx, container->cID,
			&properties, &error)) {
	...
	if (output_fd < 0) {
		PRINT_ERROR
			("Failed to open [%s] as the container properties target : %s\n",
			path, strerror(errno));
		return FALSE;
	}
...
dump_content_properties(gs_content_t * content)
{
	...
	if (!meta2_remote_list_all_content_properties(&cnx, container->cID,
			info->path, &properties, &error)) {
	...
	if (output_fd < 0) {
		PRINT_ERROR
			("Failed to open [%s] as the content properties target : %s\n",
			path, strerror(errno));
		return FALSE;
	}
...
my_content_filter(gs_content_t * content, void *user_data)
{
	...
	PRINT_DEBUG("About to download [grid://%s/%s/%s] (%" G_GINT64_FORMAT
		" bytes) in [%s/%s]\n", meta0_url, container_info.name, info.path,
		info.size, base_dir, original_path);
	...
	if (GS_OK != gs_download_content(content, &dl_info, &err)) {
		...
		if (flag_verbose) {
			PRINT_ERROR
				("grid storage error: cannot download [grid://%s/%s/%s] in [%s]. Cause:\n\t%s\r\n",
				meta0_url, container_name, info.path, path,
				gs_error_get_message(err));
		}
		else {
			PRINT_ERROR
				("grid storage error: cannot download [grid://%s/%s/%s] in [%s]\n",
				meta0_url, container_name, info.path, path);
		}
		return -1;
	}
	...
	fchmod(output_fd, S_IRGRP | S_IRUSR | S_IWUSR);
	close(output_fd);
	g_print("download: [grid://%s/%s/%s] in [%s] (%" G_GINT64_FORMAT
		" bytes)\n", meta0_url, container_name, info.path, path, info.size);
	nb_elements++;
...
main(int argc, char **args)
{
	...
	if (!parse_opt(argc, args)) {
		PRINT_ERROR
			("cannot parse the options, see the help section (--help option)\n");
		return ~0;
	}
	...
	if (!conf_check()) {
		PRINT_ERROR
			("invalid configuration, see the help section (--help option)\n");
		help();
		return ~0;
	}
