...
help_meta1_policy_update()
{
	g_printerr("\n");
	g_printerr
		("usage: hcadmin meta1_policy_apply NS ALL|PREFIX|REFERENCE_ID [SRVTYPE]\n\n");
	g_printerr
		("\t              NS : Honeycomb namespace, if you don't know this, please contact your Honeycomb namespace administrator\n\n");
	g_printerr
		("\t             ALL : apply the meta1 policy to all references defined in the namespace\n");
	g_printerr
		("\t          PREFIX : apply the meta1 policy only to the references defined for this prefix. (Prefix is defined by 4 hexadecimal digits)\n");
	g_printerr
		("\t    REFERENCE_ID : apply the meta1 policy only to this reference. (REFERENCE_ID is defined by 32 hexadecimal digits)\n");
	g_printerr
		("\t        [SRVTYPE : the service type to be updated (defaults to \"meta2\")]\n");
	g_printerr("\n");
}
...
help_meta1_policy_exclude()
{
	g_printerr("\n");
	g_printerr
		("usage: hcadmin meta1_policy_exclude NS ALL|PREFIX|REFERENCE_ID SRV_URL\n\n");
	g_printerr
		("\tThis action can be use to exclude a meta2 service and replace it by another service for the replication mechanism\n\n");
	g_printerr
		("\t            NS : Honeycomb namespace, if you don't know this, please contact your Honeycomb namespace administrator\n");
	g_printerr
		("\t           ALL : apply the meta1 policy to all references defined in the namespace\n");
...
help_touch(void)
{
	g_printerr("\n");
	g_printerr
		("usage: hcadmin touch <NS>/<CONTAINER_NAME> [UPDATE_CSIZE|RECALC_CSIZE]\n");
	g_printerr("or     hcadmin touch <NS>/<CONTAINER_NAME>/<CONTENT_NAME>\n\n");
	g_printerr
		("\tExecute a touch command on a container or on a simgle content\n\n");
	...
	g_printerr
		("\t   UPDATE_CSIZE: Option for update container_size data on meta1 by container_size from meta2 \n");
	g_printerr("\t                 (without recalculation)\n");
	g_printerr
		("\t   RECALC_CSIZE: Option for recalculation container_size form content_header(meta2) and update data on meta1\n");
	g_printerr("\n");
}
...
_func_meta1_policy_update(void)
{
	...
	err =
		hcadmin_meta1_policy_update(g_url, action, flag_checkonly,
		&globalresult, &result, action_args);
...
_func_touch(void)
{
	...
	err =
		hcadmin_touch(g_url, action, flag_checkonly, &globalresult, &result,
		action_args);
...
hcadmin_get_options(void)
{
	static struct grid_main_option_s hcdir_options[] = {
		{"Checkonly", OT_BOOL, {.b =
						&flag_checkonly},
			"Not commit modification on data base"},
		{NULL, 0, {.i = 0}, NULL}
	};
