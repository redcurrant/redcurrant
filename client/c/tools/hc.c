...
__display_friendly_error(gs_error_t * error)
{
	...
	switch (error->code) {
		...
		case CODE_BAD_REQUEST:
			g_printerr
				("The server doesn't recognize the request. Be sure to use a compatible client\n");
			break;
		...
		case CODE_CONTENT_NOTFOUND:
			g_printerr("Content [%s] not found in container [%s]\n",
				copy_source != NULL ? copy_source->str : hc_url_get(url,
					HCURL_PATH), hc_url_get(url, HCURL_REFERENCE));
			break;
		...
		case CODE_CONTAINER_NOTEMPTY:
			g_printerr("Container [%s] not empty [%s].\n", hc_url_get(url,
					HCURL_REFERENCE), hc_url_get(url, HCURL_NS));
			break;
		...
		case CODE_NAMESPACE_FULL:
			if (hc_url_has(url, HCURL_NSVIRT)) {
				g_printerr("Quota of namespace [%s%s] has been reached.\n",
					hc_url_get(url, HCURL_NSPHYS), hc_url_get(url,
						HCURL_NSVIRT));
			}
			else {
		...
		case CODE_CONTAINER_PROP_NOTFOUND:
			g_printerr
				("This kind of service is not managed by your Honeycomb namespace.\n");
			break;
		case CODE_CONTENT_PROP_NOTFOUND:
			g_printerr
				("No more service of this type available. Please ensure your services are correctly started.\n");
			break;
		...
		case CODE_POLICY_NOT_SATISFIABLE:
			g_printerr
				("The storage policy could not be satisfied by namespace. %s\n",
				hc_url_get(url, HCURL_NSPHYS));
			break;
		...
		default:
			if (!flag_verbose) {
				g_printerr
					("Unexpected server error, please run action with -v option for more informations.\n");
			}
			return;
	}
...
func_stgpol(gs_grid_storage_t * hc)
{
	...
	if (NULL != c) {
		if (sp == NULL) {
			...
			if (lc) {
				value =
					loc_context_getstgpol_to_string(lc, hc_url_has(url,
						HCURL_PATH));
				if (value != NULL) {
					g_print("%s\n", value);
					status = TRUE;
		...
		if (hc_url_has(url, HCURL_PATH)) {
			/* policy to a content */
			if ((!hc_set_content_storage_policy(c, hc_url_get(url, HCURL_PATH),
						sp, &e))) {
				__display_friendly_error(e);
			}
			else {
				GRID_DEBUG("Storage policy %s set to content %s/%s\n", sp,
					hc_url_get(url, HCURL_REFERENCE), hc_url_get(url,
						HCURL_PATH));
			}
		}
		else {
...
func_srvlink(gs_grid_storage_t * hc)
{
	...
	e = hc_link_service_to_reference(hc, hc_url_get(url, HCURL_REFERENCE),
		action_args[0], &urlv);
	if (NULL != e) {
		__display_friendly_error(e);
		gs_error_free(e);
...
func_srvunlink(gs_grid_storage_t * hc)
{
	...
	if (!(e =
			hc_unlink_reference_service(hc, hc_url_get(url, HCURL_REFERENCE),
				action_args[0]))) {
...
func_srvpoll(gs_grid_storage_t * hc)
{
	...
	if (!(e =
			hc_poll_service(hc, hc_url_get(url, HCURL_REFERENCE),
				action_args[0], &u))) {
...
func_srvforce(gs_grid_storage_t * hc)
{
	...
	if (NULL != (e =
			hc_force_service(hc, hc_url_get(url, HCURL_REFERENCE),
				action_args[0]))) {
		__display_friendly_error(e);
		gs_error_free(e);
		return FALSE;
...
func_srvconfig(gs_grid_storage_t * hc)
{
	...
	if (!(e =
			hc_configure_service(hc, hc_url_get(url, HCURL_REFERENCE),
				action_args[0]))) {
	...
	if (url) {
		...
		if (!hc) {
			g_printerr("Failed to load namespace [%s]: %s\n"
				...
				hc_url_get(url, HCURL_NS), hc_error->msg, hc_url_get(url,
					HCURL_NS));
			action_result = -1;
			return;
		}
...
hc_configure(int argc, char **argv)
{
	...
	if (!hc_url_has(url, HCURL_PATH) && argv[2]
		&& 0 == g_ascii_strcasecmp(action, "put")) {
