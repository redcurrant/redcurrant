...
output_firstbytes(void *uData, const char *b, const size_t bSize)
{
	...
	if (!first) {
		g_printerr("# %" G_GSIZE_FORMAT
			" bytes received pointer=%p ... +++ STOP WANTED +++\n", bSize, b);
		return 0;
	}
	first = 0;
...
main(int argc, char **args)
{
	...
	g_printerr("# gs_download_content_by_name(%s,%s,%s,offset=%" G_GINT64_FORMAT
		",size=%" G_GINT64_FORMAT ",TOTAL) %d %s\n", ns, cName, cPath,
		dl_info.offset, dl_info.size, gs_error_get_code(err),
	...
	status = gs_download_content_by_name(container, cPath, &dl_info, &err);
	g_printerr("# gs_download_content_by_name(%s,%s,%s,offset=%" G_GINT64_FORMAT
		",size=%" G_GINT64_FORMAT ",PARTIAL) %d %s\n", ns, cName, cPath,
		dl_info.offset, dl_info.size, gs_error_get_code(err),
		gs_error_get_message(err));
	if (!status || err != NULL)
		goto error_get;
	...
	status = gs_download_content_by_name(container, cPath, &dl_info, &err);
	g_printerr("# gs_download_content_by_name(%s,%s,%s,offset=%" G_GINT64_FORMAT
		",size=%" G_GINT64_FORMAT ",FIRST_BYTES) %d %s\n", ns, cName, cPath,
		dl_info.offset, dl_info.size, gs_error_get_code(err),
		gs_error_get_message(err));
	if (!status || err != NULL)
		goto error_get;
