...
help(void)
{
	...
	g_printerr
		("\t -f : force flag, the target local file will be truncated if it exists."
	...
	g_printerr
		("\t -p <PATH> : a path to a local file (must not exist or must be writeable"
		" and set the force flag)\n");
	g_printerr("\t -V <version> : version of content to get\n");
	g_printerr
		("\t -C <PATH> : a path to a directory where the downloaded file must be put\n");
	g_printerr
		("\t -o <NUMBER> : The offset from which to start the content downloading. (default : 0)\n\n");
}
...
check_args(t_gs_tools_options * gto, gchar ** extra_args)
{
	...
	if (!has_content_in_url && !has_content_in_options) {
		g_printerr
			("Error: content name must be supplied.  Please refer to usage.\n");
		return FALSE;
	}
