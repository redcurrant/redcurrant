...
static gs_status_t
dl_nocache(gs_container_t * container, const gchar * name,
	gs_download_info_t * dlinfo, gs_error_t ** err)
{
...
main(int argc, char **args)
{
	...
	status =
		gs_upload_content(container, path, local_stats.st_size, feed_from_fd,
		&input_fd, &err);
	if (status != GS_OK && err) {
		printf("upload error \n");
