...
chunk_key_equal(gconstpointer k1, gconstpointer k2)
{
	...
	key1 = k1;
	key2 = k2;
	return (k1 == k2) || (!memcmp(key1->cid, key2->cid, sizeof(container_id_t))
...
namespace_cache_init(const char *name, GError ** err)
{
	...
	if (nsCache)
		...
		nsCache =
			(struct namespace_cache_s *) g_try_malloc0(sizeof(struct
				namespace_cache_s));
		if (!nsCache) {
		...
		nsCache->m0 =
			resolver_direct_create2(name, cs_timeout.cnx, cs_timeout.req, err);
		if (!nsCache->m0) {
		...
		nsCache->m1 =
			limited_cache_create(meta1_cache_size, meta1_cache_expiration,
			&cid_callbacks, 0, err);
		if (!nsCache->m1) {
		...
		nsCache->chunks_cache =
			limited_cache_create(chunks_cache_size, chunks_cache_expiration,
			&chunks_callbacks, (chunks_cache_noatime ? LCFLAG_NOATIME : 0),
			err);
		if (!nsCache->chunks_cache) {
...
reply_addr_info_list(struct reply_context_s *ctx, GSList * aL)
{
	...
	for (cursor = list_of_lists; cursor; cursor = cursor->next) {
		...
		reply_context_set_body(ctx, bufAI, bufAISize,
...
static GSList *
_locate_meta2(struct namespace_cache_s *nsCache, const gchar * ns,
	container_id_t cid, const gchar * str_cid, GError ** err)
{
	...
	if (m2_ai) {
		DEBUG("Successfully resolved META2 from cache NS=[%s] ID=[%s]",
	...
	if (!m2L) {
		if (err != NULL)
			g_prefix_error(err,
				"Cannot explicitely resolve META2 for NS=[%s] ID=[%s]: ",
				nsCache->ns_name, str_cid);
		else
			GSETERROR(err,
				"Cannot explicitely resolve META2 for NS=[%s] ID=[%s]",
	...
	if (!(m2L->data)) {
		...
		g_slist_free(m2L);
		GSETERROR(err,
			"Cannot explicitely resolve META2 (invalid return) for NS=[%s] ID=[%s]",
			nsCache->ns_name, str_cid);
		return NULL;
	}

	DEBUG("Successfully resolved META2 from META1 for NS=[%s] ID=[%s]",
		nsCache->ns_name, str_cid);
...
static GByteArray *
_stat_content(const gchar * ns, container_id_t cid, gchar * str_cid,
	gchar * path, GError ** err)
{
	...
	for (raw_content = NULL, l = list_of_addr; l && !raw_content; l = l->next) {
		...
		raw_content =
			meta2raw_remote_get_chunks(&cnx, err, cid, key.content_path,
			strlen(key.content_path));
		metacnx_close(&cnx);
		metacnx_clear(&cnx);
	}
...
handler_get_meta0(struct request_context_s *req_ctx)
{
	...
	m0_ai =
		nsCache ? g_memdup(&(nsCache->m0->meta0), sizeof(addr_info_t)) : NULL;
	RESOLVERS_UNLOCK();
	if (!nsCache) {
...
handler_get_meta1(struct request_context_s *req_ctx)
{
	...
	e = message_extract_body_encoded(req_ctx->request, &exclude,
		addr_info_unmarshall);
	if (NULL != e) {
	...
	m1_ai =
		resolver_direct_get_meta1(nsCache->m0, cid, ro, exclude,
		&(ctx.warning));
	if (!m1_ai) {
		JUMPERR(&ctx, 500, MSG_INTERNAL_ERROR);
	}
	...
	if (!ref_exists) {
		...
		DEBUG("Created META1 reference for NS=[%s] ID=[%s] master=[]", nsName,
			str_cid);
	}
	else {
		DEBUG("META1 reference already exists for NS=[%s] ID=[%s]", nsName,
			str_cid);
	}
	...
	do {						/*reply the address sequence */
		...
		reply_context_set_body(&ctx, bufAI, bufAISize,
			REPLYCTX_DESTROY_ON_CLEAN);
		reply_context_add_strheader_in_reply(&ctx, "REF_EXISTS",
			ref_exists ? "TRUE" : "FALSE");
		reply_context_reply(&ctx, &(ctx.warning));
	} while (0);

	g_free(m1_ai);
	LOG_ACCESS(&ctx, "/%.*s/%.*s", (int) sizeof(nsName), nsName,
...
handler_set_m1_master(struct request_context_s *req_ctx)
{
	...
	EXTRACT_CONTAINER_ID(req_ctx, ctx, cid, str_cid);
	ctx.warning =
		message_extract_string(req_ctx->request, NAME_MSGKEY_M1_MASTER, master,
		sizeof(master));
	if (NULL != ctx.warning)
		JUMPERR(&ctx, 400, MSG_BAD_REQUEST);
	...
	RESOLVERS_UNLOCK();
	if (!nsCache) {
		GSETERROR(&(ctx.warning), "Cannot get the cache for namespace=%s",
			nsName);
		ERROR("%s", ctx.warning->message);
	}
	else {
		resolver_direct_set_meta1_master(nsCache->m0, cid, master,
			&(ctx.warning));
	}
...
handler_decache(struct request_context_s *req_ctx)
{
	...
	do {
		...
		if (!message_get_field(req_ctx->request, MSGKEY_CID,
				sizeof(MSGKEY_CID) - 1, &field, &fieldLen, &(ctx.warning))) {
			cid_present = FALSE;
		}
		if (!field || fieldLen != sizeof(container_id_t)) {
			GSETCODE(&(ctx.warning), 400, "Bad Request (invalid container id)");
...
handler_chunks_get(struct request_context_s *req_ctx)
{
	...
	gba_content =
		_stat_content(args.ns, args.cid, args.str_cid, args.path,
		&(ctx.warning));
	if (!gba_content) {
	...
	reply_context_set_body(&ctx, gba_content->data, gba_content->len,
		REPLYCTX_DESTROY_ON_CLEAN);
	reply_context_reply(&ctx, &(ctx.warning));
...
handler_chunks_save(struct request_context_s *req_ctx)
{
	...
	DEBUG_DOMAIN("chunks", "put(%s/%s/%s) size=%u", nsName, str_cid,
		key.content_path, gba_content->len);
...
handler_chunks_flush(struct request_context_s *req_ctx)
{
	...
	LOG_ACCESS(&ctx, "cache=%d;NS=%.*s", cache_size, (int) sizeof(nsName),
		nsName);
	reply_context_clear(&ctx, TRUE);
	return TRUE;
...
plugin_init(GHashTable * params, GError ** err)
{
	...
	meta1_cache_expiration =
		_read_param_i64(params, KEY_PARAM_META1CACHE_EXPIRATION,
		DEFAULT_META1CACHE_EXPIRATION);
	CLAMP_IT((meta1_cache_expiration), 1, 604800);

	chunks_cache_expiration =
		_read_param_i64(params, KEY_PARAM_CHUNKSCACHE_EXPIRATION,
		DEFAULT_CHUNKSCACHE_EXPIRATION);
	CLAMP_IT(chunks_cache_expiration, 1, 604800);

	meta1_cache_size =
		_read_param_i64(params, KEY_PARAM_META1CACHE_SIZE,
		DEFAULT_META1CACHE_SIZE);
	CLAMP_IT(meta1_cache_size, 1, 1000000);

	chunks_cache_size =
		_read_param_i64(params, KEY_PARAM_CHUNKSCACHE_SIZE,
		DEFAULT_CHUNKSCACHE_SIZE);
	CLAMP_IT(chunks_cache_size, 1, 100000);

	chunks_cache_noatime =
		_read_param_bool(params, KEY_PARAM_CHUNKSCACHE_NOATIME, TRUE);
	...
	do {
		...
		NOTICE("Conscience time-out set to [%d,%d]", cs_timeout.cnx,
			cs_timeout.req);
	} while (0);
	...
	if (!message_handler_add("metacd", plugin_matcher, plugin_handler, err)) {
		...
		GSETERROR(err,
			"metacd module error : cannot register the plugin handler");
		return 0;
	}
