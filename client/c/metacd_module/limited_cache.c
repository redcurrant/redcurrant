...
limited_cache_t *
limited_cache_create(gssize limit, time_t expiration,
	struct limited_cache_callbacks *callbacks, guint32 flags, GError ** err)
{
	...
	ht = g_hash_table_new_full(callbacks->hash_k, callbacks->equal_k, NULL,
		NULL);
	if (!ht) {
		g_mutex_free(mutex);
...
static void
UNCHECKED_remove_element(limited_cache_t * lc,
...
void
limited_cache_destroy(limited_cache_t * lc)
{
	GMutex *mutex;
...
void
limited_cache_clean(limited_cache_t * lc)
{
	guint count = 0;
...
void
limited_cache_put(limited_cache_t * lc, gpointer k, gpointer v)
{
...
static gpointer
limited_cache_lookup(limited_cache_t * lc, gconstpointer k,
	gboolean * p_cache_has_key)
{
	time_t now;
	gpointer eData = NULL;
	...
	if (p_cache_has_key) {
		*p_cache_has_key =
			g_hash_table_lookup_extended(lc->ht, k, NULL, (gpointer *) & e);
	}
	else {
