...
static int
read_known_service_from_file(GArray * known_services, const char *file_name,
	GError ** error)
{
	int fd;
	guint8 buff[256];
	...
	if (fd < 0) {
		GSETERROR(error, "Failed to open file [%s] : %s", file_name,
			strerror(errno));
		return (0);
	}
	...
	if (r < 0) {
		GSETERROR(error, "Failed to read data from file [%s] : %s", file_name,
			strerror(errno));
	...
	service_number =
		(data->len * sizeof(guint8)) / sizeof(struct grid_service_data);
	services = (struct grid_service_data *) g_byte_array_free(data, FALSE);

	known_services =
		g_array_append_vals(known_services, services, service_number);
...
static int
save_known_service_to_file(GArray * known_services, const char *file_name,
	GError ** error)
{
	...
	if (fd < 0) {
		GSETERROR(error, "Failed to open file [%s] : %s", new_file_name,
			strerror(errno));
		return (0);
	}
	...
	for (i = 0; i < known_services->len; i++) {
		...
		while (written < (int) sizeof(struct grid_service_data)) {
			wl = write(fd, service + written,
				sizeof(struct grid_service_data) - written);
			if (wl < 0) {
				GSETERROR(error, "Failed to write to file [%s] : %s",
	...
	if (0 > rename(new_file_name, file_name)) {
		GSETERROR(error, "Failed to move file [%s] to [%s] : %s", new_file_name,
...
int
get_idx_of_service(const char *service_type, struct grid_service_data *service,
	GError ** error)
{
	...
	if (rc < 0 && errno != ENOENT && errno != ENOTDIR) {
		GSETERROR(error, "Failed to stat file [%s] : %s", file_name,
			strerror(errno));
		return (0);
	}
	...
	if (rc < 0) {
		...
		if (!save_known_service_to_file(known_services, file_name, error)) {
			GSETERROR(error, "Failed to save known services to file [%s]",
				file_name);
		...
		for (i = 0; i < known_services->len; i++) {
			known_service =
				&g_array_index(known_services, struct grid_service_data, i);
		...
		if (!idx_found) {
			...
			if (!save_known_service_to_file(known_services, file_name, error)) {
				GSETERROR(error, "Failed to save known sevices to file [%s]",
					file_name);
				g_array_free(known_services, TRUE);
				return (0);
			}
