...
free_key(gpointer key)
{
	if (!key)
		return;
	g_slist_foreach(((const struct broken_event_s *) key)->broken_elements,
		broken_element_gfree, NULL);
	g_free(key);
}
...
gboolean
notify_broken_events_processing(const struct broken_event_s * event,
	GError ** error)
{
	(void) event;
	(void) error;
 ...
 */
gboolean
generate_and_push_broken_event(container_id_t container_id,
	service_info_t * srv_info)
{
	GError *error;
	...
	broken_element =
		broken_element_alloc(container_id, test_char, test_char, L_META2,
		P_CONTAINER_ID, R_MISMATCH, NULL);
	NOTICE("alloc broken element ok");
	broken_event->broken_elements =
		g_slist_prepend(broken_event->broken_elements, broken_element);
	NOTICE("hash_table content: ");
	g_hash_table_foreach(knowable_broken_events, display_hash_table_content,
		NULL);
	NOTICE("\n");
	if (!g_hash_table_lookup(knowable_broken_events, broken_event)) {
		if (!record_broken_event(broken_event, error)) {
			...
			NOTICE("knowable_broken_events size =%d",
				g_hash_table_size(knowable_broken_events));
			g_hash_table_insert(knowable_broken_events, g_memdup(broken_event,
					sizeof(struct broken_event_s)), FALSE);
			NOTICE
				("event saved in the hash map, knowable_broken_events size = %d",
				g_hash_table_size(knowable_broken_events));
		}
	}
	else {
...
puller_main_func(gpointer data, gpointer user_data)
{
	...
	service_info_set_address(service_info, g_strdup(split[0]),
		g_strdup(split[1]), err);
	g_strlcpy(service_info->ns_name, ns_name,
		sizeof(service_info->ns_name) - 1);
	g_strlcpy(service_info->type, NAME_SRVTYPE_META2,
		sizeof(service_info->type) - 1);
...
pull_events_from_agent(GSList ** events, GError ** error)
{
	...
	knowable_broken_events =
		g_hash_table_new_full(g_str_hash, g_str_equal, free_key, free_value);
	while (1) {
...
gboolean
start_event_puller_thread(GError ** error)
{
