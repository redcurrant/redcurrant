...
_exec_filters(gpointer data, gpointer user_data)
{
	...
	while (g_hash_table_iter_next(&iterator, &key, &value)) {
		...
		NOTICE("broken_element->location: %d, filter->location = %d",
			broken_element->location, filter->location);
		NOTICE("broken_element->property: %d, filter->property = %d",
			broken_element->property, filter->property);
		NOTICE("broken_element->reason: %d, filter->reason = %d",
			broken_element->reason, filter->reason);

		/* really up???  */

		if (broken_element->location & filter->location
			&& broken_element->property & filter->property
			&& broken_element->reason & filter->reason) {
		...
		/* debug trace */
			...
			NOTICE
				("Event filtered but some property missing for filter execution...");
		}
	}
}
...
_thread_func(gpointer data)
{
	...
	while ((broken_event = g_async_queue_pop(broken_events_queue))) {
		...
		g_slist_foreach(broken_event->broken_elements, _exec_filters,
			broken_event);
	}
...
gboolean
start_event_filter(const struct integrity_loop_config_s * config,
	GError ** error)
{
	(void) error;
	(void) config;
...
gboolean
record_broken_event(struct broken_event_s * broken_event, GError ** error)
{
	(void) error;
...
gboolean
register_event_filter(const struct broken_event_filter_s * filter,
	broken_event_filter_f filter_func, void *data, GError ** error)
{
	...
	g_hash_table_insert(filter_mappings, g_memdup(filter,
			sizeof(struct broken_event_filter_s)), filter_exec);
