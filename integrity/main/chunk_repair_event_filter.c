...
gboolean
repair_chunk_attr(const struct broken_event_s *broken_event, void *data,
	GError ** error)
{
	...
	for (l1 = broken_event->broken_elements; l1 && l1->data; l1 = l1->next) {
		struct broken_element_s *broken_element =
			(struct broken_element_s *) l1->data;
		struct info_s *infos =
			g_hash_table_lookup(chunks, broken_element->chunk_id);
		...
		switch (broken_element->property) {
			case P_CONTAINER_ID:
				...
				buffer2str(broken_element->reference_value,
					sizeof(container_id_t), str_container_id,
					sizeof(STRLEN_CONTAINERID));
			...
			case P_CONTENT_NAME:
				...
				break;
			case P_CONTENT_SIZE:
				infos->content_info.size =
					g_strdup_printf("%" G_GINT64_FORMAT,
					*(gint64 *) (broken_element->reference_value));
				break;
			case P_CONTENT_CHUNK_NB:
				infos->content_info.chunk_nb =
					g_strdup_printf("%" G_GUINT32_FORMAT,
					*(guint32 *) (broken_element->reference_value));
				break;
			case P_CONTENT_METADATA:
				infos->content_info.metadata =
					g_strdup(((GByteArray *) broken_element->reference_value)->
					data);
				break;
			case P_CONTENT_SYSMETADATA:
				infos->content_info.system_metadata =
					g_strdup(((GByteArray *) broken_element->reference_value)->
					data);
				break;
			case P_CHUNK_ID:
				...
				buffer2str(broken_element->reference_value,
					sizeof(hash_sha256_t), str_chunk_id,
					sizeof(STRLEN_CHUNKID));
				infos->chunk_info.id = str_chunk_id;
				break;
			case P_CHUNK_SIZE:
				NOTICE(" broken property identified as chunk size");
				NOTICE("broken_element->reference_value = %s",
					broken_element->reference_value);
				infos->chunk_info.size =
					g_strdup_printf("%" G_GINT64_FORMAT,
					*(gint64 *) (broken_element->reference_value));
				break;
			case P_CHUNK_HASH:
				str_chunk_hash = g_malloc0(sizeof(hash_md5_t));
				buffer2str(broken_element->reference_value, sizeof(hash_md5_t),
					str_chunk_hash, sizeof(STRLEN_CHUNKHASH));
				infos->chunk_info.hash = str_chunk_hash;
				break;
			case P_CHUNK_POS:
				infos->chunk_info.position =
					g_strdup_printf("%" G_GUINT32_FORMAT,
					*(guint32 *) (broken_element->reference_value));
				break;
			case P_CHUNK_METADATA:
				infos->chunk_info.metadata =
					g_strdup(((GByteArray *) broken_element->reference_value)->
					data);
				break;
		}
	...
	while (g_hash_table_iter_next(&chunk_iterator, &key, &value)) {
		...
		buffer2str(key, sizeof(hash_sha256_t), str_chunk_id,
			sizeof(str_chunk_id));
		...
		/* Create http session */
		session =
			rawx_client_create_session(&(broken_event->service_info.addr),
			error);
		if (session == NULL) {
		...
		rawx_client_session_set_timeout(session, RAWX_CONN_TIMEOUT,
			RAWX_REQ_TIMEOUT);

		if (!rawx_client_set_directory_data(session, key, &(info->content_info),
				&(info->chunk_info), error)) {
