...
{
	struct slice_s longest = { 0, 0 }, tmp = {
	0, 0};
...
static GError *
_request_prepare(const gchar * m, const gchar * u, ne_session ** ps,
	ne_request ** pr)
{
	GError *err = NULL;
	ne_uri uri;
...
_request_dispatch(ne_session * session, ne_request * request)
{
	...
	switch (ne_rc) {
		...
		case NE_CONNECT:
			return NEWERROR(10061, "Connection error: %s",
				ne_get_error(session));
		case NE_AUTH:
			return NEWERROR(500, "Authentication error: %s",
				ne_get_error(session));
		default:
	...
	GSList * used_loc, gint count, guint dist)
{
	...
	struct grid_lb_iterator_s *iter =
		grid_lbpool_get_iterator(pc->lbpool, "rawx");
...
static void
_unref_outof_quorum(struct policy_check_s *pc, GArray * pairs, GCompareFunc cmp)
{
	GArray *quorum, *out;
...
static void
_tell_content_is_lost(struct policy_check_s *pc,
...
_find_unwanted_location(GArray * locations, guint distance_requested)
{
	...
	for (guint i = 1; i < locations->len; ++i) {
		for (guint j = 0; j < i; ++j) {
			char *l0 =
				(&((struct chunk_location_s *) locations->data)[j])->location;
			char *l1 =
				(&((struct chunk_location_s *) locations->data)[i])->location;
	...
	GSList * chunks_to_move, GSList * chunks_to_keep)
{
	...
	err = m2v2_remote_execute_RAW_DEL(m1u->host, NULL, pc->url, old_contents);
	if (err != NULL) {
...
download_and_check_chunk_bean(struct bean_CHUNKS_s *chunk)
{
	...
	err =
		_request_prepare("GET", CHUNKS_get_id(chunk)->str, &session, &request);
	if (NULL != err)
		return err;
	checksum = g_checksum_new(G_CHECKSUM_MD5);

	ne_add_response_body_reader(request, ne_accept_2xx, md5_data_handler,
		checksum);
...
policy_load_beans(struct policy_check_s *pc)
{
	...
	err =
		hc_resolve_reference_service(pc->resolver, pc->url, "meta2",
		&pc->m2urlv);
	...
	for (gchar ** u = pc->m2urlv; !done && !err && *u; ++u) {
		...
		if (NULL != err) {
			...
			GRID_ERROR("META2 request error : (%d) %s", err->code,
				err->message);
