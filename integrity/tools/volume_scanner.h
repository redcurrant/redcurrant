 ...
 */
typedef enum scanner_traversal_e (*scanner_dir_notifier_f) (const gchar * path,
	guint depth, void *data);

typedef enum scanner_traversal_e (*scanner_file_notifier_f) (const gchar *
	file_path, void *data, struct rules_motor_env_s ** motor);

typedef gboolean(*scanner_file_matcher_f) (const gchar * dirname,
	const gchar * basename, void *data);

typedef enum scanner_traversal_e (*scanner_error_notifier_f) (const gchar * dir,
 ...
 */
void scan_volume(struct volume_scanning_info_s *scanning_info,
