...
polcheck_action(void)
{
	...
	if (err) {
		GRID_ERROR("Policy check repair error : (%d) %s", err->code,
			err->message);
		g_clear_error(&err);
...
polcheck_configure(int argc, char **argv)
{
	...
	policy_check.nsinfo =
		get_namespace_info(hc_url_get(policy_check.url, HCURL_NSPHYS), &err);
	if (!policy_check.nsinfo) {
	...
	policy_check.lbpool =
		grid_lbpool_create(hc_url_get(policy_check.url, HCURL_NSPHYS));
	if (!policy_check.lbpool) {
		GRID_ERROR("LB pool init failure : (%d) %s", 0,
