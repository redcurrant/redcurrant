...
polcheck_action(void)
{
	if (url) {
		...
		if (!check_content_storage_policy(hc_url_get(url, HCURL_NS),
				hc_url_get(url, HCURL_HEXID), hc_url_get(url, HCURL_PATH),
				check_only, &local_error)) {
			GRID_ERROR("Storage policy check failure : %s",
