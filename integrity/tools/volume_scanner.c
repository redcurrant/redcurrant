...
_recursive_scan_dir(const gchar * str_dir, guint depth,
	...
	struct rules_motor_env_s **motor)
{
	...
	enum scanner_traversal_e notify_error(void)
	{
		...
		err = NULL;
		GSETCODE(&err, errno, "Failed to scan dir [%s] : %s", str_dir,
			strerror(errno));
		local_rc =
			scanning_info->error(str_dir, err, scanning_info->callback_data);
		g_error_free(err);
	...
	int select_file(const struct dirent *file)
	{
		...
		do {
			gchar *fullpath =
				g_strconcat(str_dir, G_DIR_SEPARATOR_S, file->d_name, NULL);
	...
	switch (scanning_info->dir_enter(str_dir, depth,
			scanning_info->callback_data)) {
		case SCAN_ABORT:
			return SCAN_ABORT;
		case SCAN_STOP_BRANCH:
	...
	for (i = 0; i < nb_names; i++) {
		...
		path =
			g_strconcat(str_dir, G_DIR_SEPARATOR_S, namelist[i]->d_name, NULL);
		...
		if (S_ISDIR(path_stat.st_mode)) {
			local_rc =
				_recursive_scan_dir(path, depth + 1, scanning_info, motor);
		}
		else {
			DEBUG("Lauching callback on file [%s]", path);
			local_rc =
				scanning_info->file_action(path, scanning_info->callback_data,
				motor);
		}
		g_free(path);
	...
	if (scanning_info->dir_exit) {
		if (SCAN_ABORT == scanning_info->dir_exit(str_dir, depth,
				scanning_info->callback_data))
			rc = SCAN_ABORT;
	}
...
void
scan_volume(struct volume_scanning_info_s *scanning_info,
	struct rules_motor_env_s **motor)
{
	...
	rc = _recursive_scan_dir(scanning_info->volume_path, 0, scanning_info,
		motor);
