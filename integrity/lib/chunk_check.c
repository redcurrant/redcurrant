 ...
 */
static gboolean
_compute_chunk_md5_hash(const gchar * chunk_path, chunk_hash_t chunk_hash,
	GError ** error)
{
	int fd;
	size_t len;
	...
	if (fd == -1) {
		GSETERROR(error, "Failed to open chunk file [%s] for reading : %s",
	...
	if (size_read == -1) {
		GSETERROR(error, "Failed while reading chunk file [%s] : %s",
			chunk_path, strerror(errno));
	...
	if (len != sizeof(chunk_hash_t)) {
		GSETERROR(error,
			"Chunk file content computed hash hash wrong size : %d instead of %d",
			len, sizeof(chunk_hash_t));
		memset(chunk_hash, 0, sizeof(chunk_hash_t));
		return FALSE;
	}
...
gboolean
check_chunk_integrity(const char *chunk_path,
	const struct chunk_textinfo_s * text_chunk, GSList ** mismatch,
	GError ** error)
{
...
do {\
	...
	if (text_chunk->hash != NULL) {
		...
		if (!_compute_chunk_md5_hash(chunk_path, chunk_file_hash, error)) {
			GSETERROR(error, "Failed to compute MD5 hash of chunk file [%s]",
				chunk_path);
			goto error;
		}

		/* Convert attr chunk hash to bin */
		if (!hex2bin(text_chunk->hash, chunk_attr_hash, sizeof(chunk_hash_t),
				&local_error)) {
			WARN("Failed to convert chunk hash from hex [%s] to bin format : %s", text_chunk->hash, local_error->message);
			g_clear_error(&local_error);
		}
		else if (0 != memcmp(chunk_file_hash, chunk_attr_hash,
				sizeof(chunk_hash_t)))
			ADD_BRK_EL_TXT(L_CHUNK, P_CHUNK_HASH, R_MISMATCH,
				g_memdup(chunk_attr_hash, sizeof(chunk_hash_t)));
	}
	...
	if (text_chunk->size != NULL) {
		...
		if (-1 == stat(chunk_path, &file_stat)) {
			GSETERROR(error, "Failed to stat chunk file [%s] : %s", chunk_path,
				strerror(errno));
			goto error;
		}
		...
		if (file_stat.st_size != chunk_attr_size) {
			...
			ADD_BRK_EL_TXT(L_CHUNK, P_CHUNK_SIZE, R_MISMATCH,
				g_memdup(&chunk_attr_size, sizeof(chunk_attr_size)));
		}
	}
	else {
...
{
	guint32 position =
		g_ascii_strtoull((const gchar *) _str_position, NULL, 10);
...
_chunk_hash_comparator(gconstpointer _chunk, gconstpointer _str_hash)
{
	...
	buffer2str(chunk->hash, sizeof(chunk->hash), hash_from_chunk,
		STRLEN_CHUNKHASH);
...
static gboolean
check_chunk_referencing_full(const struct content_textinfo_s
	*content_from_chunk, const struct chunk_textinfo_s *chunk_from_chunk,
	const struct meta2_raw_content_s *content_from_meta2,
...
do {\
	...
	/* Check chunk hash */
	if (chunk_from_chunk->hash == NULL
		&& !data_is_zeroed(chunk_from_meta2->hash, sizeof(chunk_hash_t))) {
		ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_HASH, R_MISSING,
			g_memdup(chunk_from_meta2->hash, sizeof(chunk_hash_t)));
	}
	else if (data_is_zeroed(chunk_from_meta2->hash, sizeof(chunk_hash_t))
		&& chunk_from_chunk->hash != NULL) {
		ADD_BRK_EL_BIN(L_META2, P_CHUNK_HASH, R_MISSING, NULL);
	}
	else {
		chunk_hash_t hash;

		if (!hex2bin(chunk_from_chunk->hash, hash, sizeof(chunk_hash_t),
				&local_error)) {
			WARN("Failed to convert chunk_hash from hex [%s] to bin : %s",
				chunk_from_chunk->hash, local_error->message);
			g_clear_error(&local_error);
			ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_HASH, R_FORMAT,
				g_memdup(chunk_from_meta2->hash, sizeof(chunk_hash_t)));
		}
		else if (0 != memcmp(chunk_from_meta2->hash, hash,
				sizeof(chunk_hash_t)))
			ADD_BRK_EL_BIN(L_ALL, P_CHUNK_HASH, R_MISMATCH,
				g_memdup(chunk_from_meta2->hash, sizeof(chunk_hash_t)));
		else
			check_chunk_hash_same = TRUE;

	}

	/* Check chunk id */
	if (chunk_from_chunk->id == NULL
		&& !data_is_zeroed(chunk_from_meta2->id.id, sizeof(hash_sha256_t))) {
		ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_ID, R_MISSING,
			g_memdup(chunk_from_meta2->id.id, sizeof(hash_sha256_t)));
	}
	else if (data_is_zeroed(chunk_from_meta2->id.id, sizeof(hash_sha256_t))
		&& chunk_from_chunk->id != NULL) {
		ADD_BRK_EL_BIN(L_META2, P_CHUNK_ID, R_MISSING, NULL);
	}
	else {
		hash_sha256_t id;

		if (!hex2bin(chunk_from_chunk->id, id, sizeof(hash_sha256_t),
				&local_error)) {
			WARN("Failed to convert chunk_id from hex [%s] to bin : %s",
				chunk_from_chunk->id, local_error->message);
			g_clear_error(&local_error);
			ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_ID, R_FORMAT,
				g_memdup(chunk_from_meta2->id.id, sizeof(hash_sha256_t)));
		}
		else if (0 != memcmp(chunk_from_meta2->id.id, id,
	...
	if (chunk_from_chunk->size == NULL) {
		ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_SIZE, R_MISSING,
			g_memdup(&(chunk_from_meta2->size),
		...
		if (size != chunk_from_meta2->size)
			ADD_BRK_EL_BIN(L_ALL, P_CHUNK_SIZE, R_MISMATCH,
				g_memdup(&(chunk_from_meta2->size),
					sizeof(chunk_from_meta2->size)));
	}
	...
	if (chunk_from_chunk->position == NULL) {
		ADD_BRK_EL_BIN(L_CHUNK, P_CHUNK_POS, R_MISSING,
			g_memdup(&(chunk_from_meta2->position),
		...
		if (position != chunk_from_meta2->position)
			ADD_BRK_EL_BIN(L_ALL, P_CHUNK_POS, R_MISMATCH,
				g_memdup(&(chunk_from_meta2->position),
					sizeof(chunk_from_meta2->position)));
	}
	...
	if (content_from_chunk->path == NULL
		&& !data_is_zeroed(content_from_meta2->path,
			sizeof(content_from_meta2->path))) {
		NOTICE("chunk_check: path null..\n");
		ADD_BRK_EL_BIN(L_CHUNK, P_CONTENT_NAME, R_MISSING,
			g_strdup(content_from_meta2->path));
	}
	else if (data_is_zeroed(content_from_meta2->path,
			sizeof(content_from_meta2->path))
		&& content_from_chunk->path != NULL) {
		ADD_BRK_EL_BIN(L_META2, P_CONTENT_NAME, R_MISSING,
			g_strdup(content_from_chunk->path));
	}
	else if (strncmp(content_from_chunk->path, content_from_meta2->path,
			LIMIT_LENGTH_CONTENTPATH)) {
		ADD_BRK_EL_BIN(L_ALL, P_CONTENT_NAME, R_MISMATCH,
			g_strdup(content_from_meta2->path));
	}

	/* Check content system metadata */
	if (content_from_chunk->system_metadata == NULL
		&& content_from_meta2->system_metadata != NULL) {
	...
	else if ((content_from_meta2->system_metadata == NULL
			|| content_from_meta2->system_metadata->len == 0)
		&& content_from_chunk->system_metadata != NULL) {
		ADD_BRK_EL_BIN(L_META2, P_CONTENT_SYSMETADATA, R_MISSING,
			g_strdup(content_from_chunk->system_metadata));
	}
	else {
		gchar metadata[content_from_meta2->system_metadata->len + 1];

		memset(metadata, '\0', sizeof(metadata));
		memcpy(metadata, content_from_meta2->system_metadata->data,
			sizeof(metadata) - 1);

		if (!metadata_equal(content_from_chunk->system_metadata, metadata,
				NULL))
	...
	if (chunk_from_chunk->position != NULL
		...
		if (content_from_chunk->size == NULL) {
			ADD_BRK_EL_BIN(L_CHUNK, P_CONTENT_SIZE, R_MISSING,
				g_memdup(&(content_from_meta2->size),
			...
			if (size != content_from_meta2->size)
				ADD_BRK_EL_BIN(L_ALL, P_CONTENT_SIZE, R_MISMATCH,
					g_memdup(&(content_from_meta2->size),
						sizeof(content_from_meta2->size)));
		}
	}
	...
	if ((*broken != NULL) && (g_slist_length(*broken) > 0)) {
		...
		if (check_chunk_id_notsame == TRUE)
			ADD_BRK_EL_BIN(L_ALL, P_CHUNK_ID, R_MISMATCH,
				g_memdup(chunk_from_meta2->id.id, sizeof(hash_sha256_t)));
...
check_chunk_referencing(const struct content_textinfo_s * content_from_chunk,
	const struct chunk_textinfo_s * chunk_from_chunk,
	const struct meta2_raw_content_s * content_from_meta2, GSList ** broken,
	GError ** error)
{
...
static gboolean
_create_container(struct meta2_ctx_s *ctx, container_id_t * p_cid,
	GError ** p_err)
{
	...
	if (!p_m1_addr) {
		GSETERROR(p_err, "META1 resolution error for [%s]",
			ctx->loc->container_name);
		goto clean_up;
	}
	...
	gchar * stgpol, check_result_t * cres)
{
	...
	ret = meta2_remote_container_create_v3(&m2, META2_TIMEOUT,
		ctx->ns, ctx->loc->container_name, *p_cid, stgpol, &local_error);
	...
	if (!ret && local_error) {
		if (local_error->code == CODE_CONTAINER_EXISTS) {
			...
			GRID_DEBUG("Container [%s/%s] already exist on meta2 [%s]",
				ctx->ns, ctx->loc->container_name, ctx->loc->m2_url[0]);
			ret = TRUE;
		}
	}
	...
	check_result_t * cres, GError ** p_err)
{
	...
	GRID_DEBUG
		("Check/Create container with...storage_policy=%s, version=%s, container_id=%s, container_name=%s/%s,",
		ct_info->storage_policy, ct_info->version, hexid, ctx->ns,
		ctx->loc->container_name);
...
compute_file_md5(const gchar * filepath)
{
	...
	if (NULL != (fp = fopen(filepath, "rb"))) {
		...
		GRID_DEBUG("Could not open file [%s] (%s)", filepath, strerror(errno));
	}
	return strmd5;
}
...
_check_chunk_md5(check_info_t * check_info)
{
	...
	ret =
		(0 == g_ascii_strncasecmp(filemd5, check_info->ck_info->hash,
			STRLEN_CHUNKHASH));
	g_free(filemd5);
	return ret;
}
	...
	check_result_t * cres, GError ** p_err)
{
	...
	if (!ctx->content) {
		GRID_DEBUG("Cannot add chunk [%s]: no content.",
	...
	ctx->content->raw_chunks =
		g_slist_prepend(ctx->content->raw_chunks, newchunk);

	if (is_dryrun) {
		check_result_append_msg(cres,
			"dryrun prevented chunk update in [%s/%s]",
			check_info->ct_info->container_id, check_info->ct_info->path);
	}
	else {
		if (!meta2raw_remote_update_content(ctx->m2_cnx, p_err, ctx->content,
				TRUE)) {
...
_find_content_from_chunkid(struct meta2_ctx_s *ctx, check_info_t * check_info)
{
	...
	if (ctx->m2_cnx) {
		addr_info_to_string(&(ctx->m2_cnx->addr), m2addr_str,
			sizeof(m2addr_str));
		local_error =
			find_storage_policy_and_friend_chunks_full(m2addr_str, url,
			check_info, &chunk_ids, &(ctx->content));
	}
	...
	check_result_t * cres)
{
	...
	//remove content
	if (!meta2_remote_content_remove(&m2, META2_TIMEOUT, &local_error, cid,
			content_path)) {
		if (!local_error)
	...
	// content commit ?
	if (!meta2_remote_content_commit(&m2, META2_TIMEOUT, &local_error, cid,
			content_path)) {
		if (!local_error)
...
_check_chunk_pending(check_info_t * check_info, check_result_t * cres)
{
	...
	if (_check_chunk_md5(check_info)) {
		if (is_dryrun) {
			check_result_append_msg(cres,
				"dryrun prevented from removing extension [%s]",
				check_info->source_path);
		}
		else {
			if (remove_file_extension(check_info->source_path))
				check_result_append_msg(cres,
					"Removed extension of filename [%s]",
					check_info->source_path);
			else
	...
	struct content_textinfo_s *content_info, GError ** p_err)
{
	...
	*p_ctx = NULL;
	...
	if (!new_ctx) {
		GRID_DEBUG
			("Failed to get meta2 context (reloaded), check your NS is started. (NS:%s)",
			ns);
		goto clean_up;
	}
	if (new_ctx->loc == NULL || new_ctx->loc->m2_url == NULL) {
		GRID_DEBUG
			("Failed to locate container on a meta2 (reloaded) [%s][%s/%s]",
			content_info->container_id, new_ctx->ns,
			new_ctx->loc->container_name);
		goto clean_up;
	}
	...
	check_info_t * check_info, gboolean * p_same_age)
{
	...
	if (content->system_metadata)
		date_in_m2 = _extract_date_from_sysmd(content->system_metadata->data,
...
gboolean
check_chunk_orphan(check_info_t * check_info, check_result_t * cres,
	GError ** p_err)
{
	const gboolean is_dryrun = check_option_get_bool(check_info->options,
		CHECK_OPTION_DRYRUN);
	...
	if (!ctx) {
		GRID_DEBUG
			("Failed to get meta2 context, check your NS is started. (NS:%s)",
			check_info->ns_name);
		return FALSE;
	}
	...
	is_chunk_newer =
		_is_newer_than_content(ctx->content, check_info, &same_age);
	if (is_chunk_newer) {
		...
		raw_chunk =
			_get_chunk(ctx->content->raw_chunks, check_info->ck_info->position);
		is_referenced =
			check_chunk_referencing_full(content_info, check_info->ck_info,
	...
	if (!is_referenced) {
		...
		if (broken_elements) {
			...
			if (cres && cres->msg == NULL) {
				cres->check_ok = TRUE;
				GRID_DEBUG("Chunk [%s] is well referenced.",
					check_info->ck_info->id);
			}
		}
	}
