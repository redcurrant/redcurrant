...
meta2_sqlite_maintenance(const gchar * meta2_db_path, GError ** error)
{
	...
	if (rc) {
		GSETERROR(error, "Failed to open database [%s] : %s", meta2_db_path,
			sqlite3_errmsg(db));
		sqlite3_close(db);
		return FALSE;
	}
	...
	if (rc) {
		GSETERROR(error, "Failed to vacuum databse [%s] : %s", meta2_db_path,
			sqlite_error);
...
gboolean
check_meta2_chunk(const struct meta2_raw_content_s * raw_content,
	GSList ** broken, GError ** error)
{
	...
	buffer2str(raw_chunk->id.id, sizeof(hash_sha256_t), str_chunk_id,
		sizeof(str_chunk_id) - 1);

	/* Create http session */
	session = rawx_client_create_session(&(raw_chunk->id.addr), error);
	if (session == NULL) {
		GSETERROR(error, "Failed to create HTTP session to access rawx [%s]",
			str_url);
		return FALSE;
	}

	rawx_client_session_set_timeout(session, RAWX_CONN_TMEOUT,
		RAWX_REQ_TIMEOUT);

	if (!rawx_client_get_directory_data(session, raw_chunk->id.id,
			&text_content, &text_chunk, error)) {
		GSETERROR(error,
			"Failed to get directory data from rawx [%s] for chunk [%s]",
			str_url, str_chunk_id);
	...
	if (!check_chunk_referencing(&text_content, &text_chunk, raw_content,
			broken, error)) {
		GSETERROR(error,
			"Failed to compare chunk from META2 and chunk from RAWX");
		return FALSE;
	}
