...
teardown()
{
	...
	container_db_path =
		g_strdup_printf("%s/%s", VOLUME_ROOT, CONTAINER_DB_NAME);
...
void
test_add_chunk_to_db_args_null()
{
	GError *error = NULL;

	test_dept_assert_false(add_chunk_to_db(NULL, CHUNK_PATH, CONTENT_NAME,
			CONTAINER_ID, &error));
	...
	test_dept_assert_false(add_chunk_to_db(VOLUME_ROOT, NULL, CONTENT_NAME,
			CONTAINER_ID, &error));
	test_dept_assert_true(error);
	...
	test_dept_assert_false(add_chunk_to_db(VOLUME_ROOT, CHUNK_PATH, NULL,
			CONTAINER_ID, &error));
	test_dept_assert_true(error);
	...
	test_dept_assert_false(add_chunk_to_db(VOLUME_ROOT, CHUNK_PATH,
			CONTENT_NAME, NULL, &error));
	test_dept_assert_true(error);
}

void
test_add_chunk_to_db()
{
	GError *error = NULL;
	...
	test_dept_assert_true(add_chunk_to_db(VOLUME_ROOT, CHUNK_PATH, CONTENT_NAME,
			CONTAINER_ID, &error));
	...
	/* Get on content db */
	test_dept_assert_equals_int(0, content_db->open(content_db, NULL,
			content_db_path, NULL, DB_BTREE, 0, 0));
	test_dept_assert_equals_int(0, content_db->cursor(content_db, NULL,
			&db_cursor, 0));
	...
	/* Get on container db */
	test_dept_assert_equals_int(0, container_db->open(container_db, NULL,
			container_db_path, NULL, DB_BTREE, 0, 0));
	test_dept_assert_equals_int(0, container_db->cursor(container_db, NULL,
			&db_cursor, 0));
...
test_get_content_chunks_args_null()
{
	...
	test_dept_assert_false(get_content_chunks(NULL, CONTENT_NAME, &list_chunk,
			&error));
	test_dept_assert_true(error);

	test_dept_assert_false(get_content_chunks(VOLUME_ROOT, NULL, &list_chunk,
			&error));
	test_dept_assert_true(error);

	test_dept_assert_false(get_content_chunks(VOLUME_ROOT, CONTENT_NAME, NULL,
			&error));
	test_dept_assert_true(error);
}
...
test_get_content_chunks()
{
	...
	test_dept_assert_true(add_chunk_to_db(VOLUME_ROOT, CHUNK_PATH, CONTENT_NAME,
			CONTAINER_ID, &error));
	test_dept_assert_true(add_chunk_to_db(VOLUME_ROOT, CHUNK_PATH2,
			CONTENT_NAME, CONTAINER_ID, &error));
	...
	test_dept_assert_not_equals_string(((char *) g_slist_nth(list_chunk,
				0)->data), ((char *) g_slist_nth(list_chunk, 1)->data));
}

void
test_get_container_chunks_args_null()
{
	GError *error = NULL;
	GSList *list_chunk = NULL;

	test_dept_assert_false(get_container_chunks(NULL, CONTAINER_ID, &list_chunk,
			&error));
	test_dept_assert_true(error);

	test_dept_assert_false(get_container_chunks(VOLUME_ROOT, NULL, &list_chunk,
			&error));
	test_dept_assert_true(error);

	test_dept_assert_false(get_container_chunks(VOLUME_ROOT, CONTAINER_ID, NULL,
			&error));
	test_dept_assert_true(error);
}
