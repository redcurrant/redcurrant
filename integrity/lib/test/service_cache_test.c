...
list_local_services_fake(GError ** error)
{
	...
	memcpy(&(si1->addr), build_addr_info("127.0.0.1", 6000, error),
		sizeof(addr_info_t));

	si2 = g_new0(service_info_t, 1);
	strcpy(si2->ns_name, NS_NAME);
	strcpy(si2->type, NAME_SRVTYPE_RAWX);
	memcpy(&(si2->addr), build_addr_info("127.0.0.1", 8000, error),
		sizeof(addr_info_t));
...
void
test_update_service_cache_args_null()
{
	GError *error = NULL;
...
test_update_service_cache()
{
	...
	strcpy(si1->ns_name, NS_NAME);
	strcpy(si1->type, NAME_SRVTYPE_RAWX);
	memcpy(&(si1->addr), build_addr_info("127.0.0.1", 7000, &error),
		sizeof(addr_info_t));
	...
	service_cache.service_list =
		g_slist_prepend(service_cache.service_list, ps1);
	service_cache.service_list =
		g_slist_prepend(service_cache.service_list, ps2);
