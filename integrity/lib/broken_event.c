...
broken_element_alloc(const container_id_t container_id,
	...
	enum broken_reason_e reason, void *reference_value)
{
	...
	if (element != NULL) {
		...
		if (content_name != NULL)
			strncpy(element->content_name, content_name,
				sizeof(element->content_name) - 1);
...
struct broken_element_s *
broken_element_alloc2(const gchar * container_id, const gchar * content_name,
	const gchar * chunk_id, enum broken_location_e location,
	enum broken_property_e property, enum broken_reason_e reason,
	void *reference_value)
{
	...
	if (!hex2bin(container_id, raw_container_id, sizeof(container_id_t),
			&local_error)) {
		ERROR("Failed to convert container_id from hex [%s] to bin format : %s",
			container_id, local_error->message);
		g_clear_error(&local_error);
		return NULL;
	}

	if (!hex2bin(chunk_id, raw_chunk_id, sizeof(hash_sha256_t), &local_error)) {
		ERROR("Failed to convert chunk_id from hex [%s] to bin format : %s",
			chunk_id, local_error->message);
		g_clear_error(&local_error);
		return NULL;
	}

	return broken_element_alloc(raw_container_id, content_name, raw_chunk_id,
		location, property, reason, reference_value);
}
