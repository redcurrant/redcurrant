...
_extract_location(gpointer data, gpointer udata)
{
	...
	if (data)
		loc_tag =
			service_info_get_tag(((service_info_t *) data)->tags,
			NAME_TAGNAME_RAWX_LOC);
...
_is_rawx_in_garbage(service_info_t * rawx, GSList * garbage)
{
	...
	for (r = garbage; r && r->data; r = r->next) {
		if (addr_info_equal(&(rawx->addr),
				&(((service_info_t *) r->data)->addr))) {
			return TRUE;
		}
	}
	return FALSE;
	...
	const gchar * stg_class, GSList ** rawx_garbage)
{
	...
	for (l = rawx; l && l->data; l = l->next) {
		...
		bzero(loc, sizeof(loc));
		loc_tag =
			service_info_get_tag(((service_info_t *) l->data)->tags,
			NAME_TAGNAME_RAWX_LOC);
		GRID_DEBUG("service tag extracted");
		if (!loc_tag) {
			if (distance > 1) {
...
__dst_cb_in(struct bufferevent *bev, void *ctx)
{
	...
	for (;;) {
		if (ct->reply_parsing == PARSE_STATUS) {
			...
			status_line =
				evbuffer_readln(dst_in, &status_line_length, EVBUFFER_EOL_CRLF);
			if (!status_line) {
				GRID_DEBUG("Output status line not ready, %" G_GSIZE_FORMAT
					" bytes remaining", evbuffer_get_length(dst_in));
				return;
			}
			else {
		...
		if (ct->reply_parsing == PARSE_HEADER) {
			...
			header_line =
				evbuffer_readln(dst_in, &header_line_length, EVBUFFER_EOL_CRLF);
			if (!header_line) {
				GRID_DEBUG("Output header line not ready, %" G_GSIZE_FORMAT
					" bytes remaining", evbuffer_get_length(dst_in));
				return;
			}
			else {
...
__out_start(struct chunk_transfer_s *ct)
{
	...
	const char *content_length =
		chunk_transfer_find_req_header(ct, "Content-Length");

	ct->dst_size = ct->dst_size_remaining =
		g_ascii_strtoll(content_length, NULL, 10);
	...
	for (l = ct->dst_rawx; l && l->data; l = l->next) {
		...
		addr_info_get_addr(&(((service_info_t *) l->data)->addr), dst,
			sizeof(dst), &port);
		...
		bufferevent_setcb(bevent, __dst_cb_in, __dst_cb_out, __dst_cb_error,
			ct);
	...
	for (lc = ct->dst_chunks, lb = dsts_out; lc && lc->data && lb && lb->data;
		lc = lc->next, lb = lb->next) {
		gchar idstr[65];
...
__transfer(struct chunk_transfer_s *ct, gboolean finished)
{
	...
	GRID_DEBUG("Input buffer has %" G_GSIZE_FORMAT " bytes",
		evbuffer_get_length(in_buffer));
...
__cb_req_in_final(struct evhttp_request *req, void *udata)
{
	...
	if (!req || req->response_code < 200 || req->response_code > 299) {
		GRID_ERROR("input request error : %d (%s)",
			(req) ? req->response_code : -1,
			(req) ? req->response_code_line : NULL);
		ct->src_status = CNX_FAILED;
	}
	else {
		const char *metadata_sys =
			chunk_transfer_find_req_header(ct, "content_path");
...
static GError *
_copy_chunk_data(meta2_raw_content_t * content, meta2_raw_chunk_t * src_chunk,
	GSList * dest)
{
	...
	if (req_status == 0 && ct->src_status == CNX_SUCCEEDED) {
		...
		g_set_error(&local_error, gquark_log, 500,
			"Destination rawx connection error");
	}
	...
	meta2_raw_chunk_t * src_chunk)
{
	...
	const struct data_security_s *ds =
		storage_policy_get_data_security(ctx->sp);
	const char *tmp = data_security_get_param(ds, DS_KEY_DISTANCE);
	gint64 distance = (NULL != tmp) ? g_ascii_strtoll(tmp, NULL, 10) : 1;
	const gchar *stgclass =
		storage_class_get_name(storage_policy_get_storage_class(ctx->sp));
	GSList *dest = NULL;
	...
	for (i = 0; i < nb_dup; i++) {
		...
		if (!(si =
				_find_matching_rawx(rawx, used_loc, distance, stgclass,
					rawx_garbage))) {
			GRID_DEBUG
				("Failed to find a rawx which matches with duplication criteria");
			ctx->fail = TRUE;
	...
	GSList ** almost_good)
{
	...
	GSList *tmp = NULL;
	const char *str =
		data_security_get_param(storage_policy_get_data_security(ctx->sp),
		DS_KEY_COPY_COUNT);
	guint32 nb_copy = (NULL != str) ? atoi(str) : 1;

	GRID_DEBUG("adjust chunk copies, we have %d positions to scan",
		g_strv_length((gchar **) chunks));
	...
	for (i = 0; i < g_strv_length((gchar **) chunks); i++) {
		...
		if (size == 0 && size2 == 0) {
			...
			GRID_ERROR
				("No valid chunk found at position %d, cannot repair anything",
				i);
			if (!ctx->check_only) {
			...
			if (size > nb_copy) {
				GRID_DEBUG("Too many copies of chunk at position %"
					G_GUINT32_FORMAT " (%" G_GUINT32_FORMAT " copies, %"
					G_GUINT32_FORMAT " expected)", i, size, nb_copy);
				if (!ctx->check_only) {
					/* add chunks to the unwanted list */
					guint j = 0;

					tmp =
						dup_chunk_info_get_chunks((struct dup_chunk_info_s *)
						chunks[i]);
					for (j = 0; j < size - nb_copy; j++) {
						if (tmp && tmp->data) {
							gchar *chunk_str =
								meta2_raw_chunk_to_string((meta2_raw_chunk_t *)
								tmp->data);
							GRID_INFO("%s marked for delete, too many copies.",
								chunk_str);
				...
				GRID_INFO("Not enough copies of chunk at position %"
					G_GUINT32_FORMAT " (%" G_GUINT32_FORMAT " copies, %"
					G_GUINT32_FORMAT " expected)", i, size, nb_copy);
				...
				if (!ctx->check_only) {
					...
					meta2_raw_chunk_t *src_chunk =
						(size > 0) ? NULL : almost_good[i]->data;
					local_error =
						_upload_new_copy(ctx, rawx, rawx_garbage, chunks[i],
						nb_copy - size, src_chunk);
					if (local_error) {
						GRID_ERROR("Error while creating new chunk copy: %s",
							local_error->message);
...
_delete_chunk(gpointer data, gpointer udata)
{
	...
	if (local_error) {
		GRID_INFO("Failed to delete chunk from disk : %s",
			local_error->message);
		g_clear_error(&local_error);
	}
}
...
_check_duplicated_content_is_valid(struct meta2_ctx_s *ctx)
{
	...
	GRID_DEBUG("Creating array of %d list, (nb_chunks)",
		ctx->content->nb_chunks);
	...
	GSList *l = NULL;
	const struct storage_class_s *stg_class =
		storage_policy_get_storage_class(ctx->sp);
	const char *str =
		data_security_get_param(storage_policy_get_data_security(ctx->sp),
		DS_KEY_DISTANCE);
	...
	if (!rawx) {
		if (local_error) {
			GRID_ERROR("Cannot get rawx list from namespace : %s",
				local_error->message);
		}
		else {
	...
	for (l = ctx->content->raw_chunks; l && l->data; l = l->next) {
		...
		if (local_error) {
			...
			GRID_INFO("Chunk %s marked for suppression: %s", chunk_str,
				local_error->message);
		...
		if (!grid_lb_check_storage_class(storage_class_get_name(stg_class), si)) {
			...
			almost_good[c->position] =
				g_slist_prepend(almost_good[c->position], c);
			if (!ctx->check_only) {
				garbage = g_slist_prepend(garbage, c);
			}
			g_free(chunk_str);
		...
		if (__test_location(loc,
				dup_chunk_info_get_used_locations(chunks[c->position]),
				req_dst)) {
			GRID_DEBUG
				("Chunk copy is ok and could be added to the valid chunk list.");
			dup_chunk_info_add_chunk(chunks[c->position], c, loc);
		}
		else {
			/* unwanted chunk */
			GRID_INFO
				("Chunk %s not compatible with configured duplication, and will be deleted",
				chunk_str);
			almost_good[c->position] =
				g_slist_prepend(almost_good[c->position], c);
			if (!ctx->check_only) {
	...
	_adjust_chunk_copies(ctx, rawx, chunks, &garbage, &rawx_garbage,
		almost_good);

	if (ctx->modified) {
		GRID_DEBUG("Chunks modified, we need to update the content");
		/* send chunk modification to the meta2 */
		if (!meta2raw_remote_update_content(ctx->m2_cnx, &local_error,
				ctx->content, TRUE)) {
			if (NULL != local_error)
				GRID_DEBUG("Update content request failed (%d): %s",
	...
	if (garbage) {
		...
		GRID_DEBUG("Garbage of %d chunks to be deleted",
			g_slist_length(garbage));
		ctx->content->raw_chunks = garbage;

		if (!meta2raw_remote_delete_chunks(ctx->m2_cnx, &local_error,
				ctx->content)) {
...
gboolean
check_content_storage_policy(const gchar * namespace,
	const gchar * container_id, const gchar * content_name, gboolean check_only,
	GError ** error)
{
	struct meta2_ctx_s *ctx =
		get_meta2_ctx(namespace, container_id, content_name, check_only, error);
	...
	switch (data_security_get_type(storage_policy_get_data_security(ctx->sp))) {
		case DUPLI:
			if (!_check_duplicated_content_is_valid(ctx)) {
				GSETERROR(error,
					"Duplicated content check return in error, content not completly valid");
				goto clean_up;
			}
			break;
		case RAIN:
...
check_content_info(struct content_textinfo_s * content, GError ** p_error)
{
	...
	CHECK_INFO(content->container_id, p_error,
		"Missing mandatory container identifier");
	return TRUE;
}
