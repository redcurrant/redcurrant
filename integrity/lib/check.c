	...
	const gchar * content_name, gboolean check_only, GError ** error)
{
	...
	ctx->loc =
		gs_locate_container_by_hexid_v2(ctx->hc, container_hexid, &ct_ns_name,
		&gs_error);
	if (ctx->loc == NULL || ctx->loc->m2_url == NULL) {
		if (gs_error) {
			...
			GSETERROR(error,
				"Failed to locate container [%s] in namespace: No error",
				container_hexid);
		}
	}
	...
	if ((local_error = _init_meta2_connection(ctx)) != NULL) {
		GSETERROR(error, "Failed to init meta2 connection :%s",
			local_error->message);
		goto clean_up;
	}

	/* Try to find content */
	if (!container_id_hex2bin(container_hexid, strlen(container_hexid), &cid,
			&local_error))
		goto clean_up;
	ctx->content =
		meta2_remote_stat_content(ctx->m2_cnx, cid, content_name,
		strlen(content_name), &local_error);

	if (!ctx->content) {
		GRID_DEBUG("Content %s/%s/%s doesn't exist", ctx->ns /*ns_name */ ,
			container_hexid, content_name);
		if (local_error) {
			GSETERROR(error,
				"Cannot check content state, content not found %s/%s/%s : %s",
				ctx->ns /*ns_name */ , container_hexid, content_name,
				local_error->message);
		}
		goto clean_up;
	}
	...
	if ((local_error =
			storage_policy_from_metadata(ctx->content->system_metadata,
				&storage_policy)) != NULL) {
		GSETERROR(error,
			"Failed to extract storage policy from content metadata : %s",
			local_error->message);
		goto clean_up;
	}
	...
	if (!storage_policy) {
		GSETERROR(error,
			"Failed to get content storage policy, cannot check it");
		goto clean_up;
	}
	...
	struct content_textinfo_s *content_info)
{
	...
	if (!content_info->storage_policy) {
		if (p_raw_content->system_metadata) {
			sysmd_ht =
				metadata_unpack_gba(p_raw_content->system_metadata, NULL);
			content_info->storage_policy =
				g_strdup(g_hash_table_lookup(sysmd_ht, "storage-policy"));
	...
	content_info->version =
		g_strdup_printf("%" G_GINT64_FORMAT, p_raw_content->version);
}
	...
	struct chunk_textinfo_s *chunk_info, GSList ** chunk_ids)
{
	...
	void _find_chunks_at_given_pos(gpointer _raw_chunk, gpointer _unused)
	{
		...
		if (current_chunk_pos == given_chunk_pos) {
			...
			addr_info_to_string(&(raw_chunk->id.addr), straddr,
				sizeof(straddr));
			*chunk_ids =
				g_slist_prepend(*chunk_ids, assemble_chunk_id(straddr,
					raw_chunk->id.vol, strid));
		}
	}
...
GError *
generate_raw_chunk(check_info_t * info, struct meta2_raw_chunk_s *p_raw_chunk)
{
	GError *err = NULL;
	gchar *id_first_char;
	...
	if (!convert_chunk_text_to_raw(info->ck_info, p_raw_chunk, &err)) {
		...
		id_first_char++;
		GRID_INFO
			("Try again conversion with chunk id regenerated from chunk path: [%s].",
			id_first_char);
		...
		if (!convert_chunk_text_to_raw(info->ck_info, p_raw_chunk, &err)) {
			g_prefix_error(&err,
				"Could not convert text chunk_info to raw: [%s]",
				info->ck_info->id);
			return err;
		}
...
void
check_result_append_msg(check_result_t * res, const gchar * format, ...)
{
	va_list args;
...
void
check_result_clear(check_result_t ** p_res, void (*free_udata(gpointer)))
{
	if (p_res && *p_res) {
		if ((*p_res)->msg)
	...
	GSList ** chunk_ids, struct meta2_raw_content_s **p_raw_content)
{
	...
	g_strlcpy(raw_chunk.id.vol, check_info->rawx_vol,
		sizeof(raw_chunk.id.vol) - 1);
	grid_string_to_addrinfo(check_info->rawx_str_addr, NULL,
		&(raw_chunk.id.addr));
	...
	raw_content =
		meta2raw_remote_get_content_from_chunkid(&ctx, &err, cid,
		&(raw_chunk.id));
...
{
	return NULL != g_slist_find_custom(m2v1_list, meta2,
	...
	GSList ** chunk_ids, struct meta2_raw_content_s **p_raw_content)
{
	...
	if (!is_v1) {
		...
		if (!err) {
			if (p_raw_content)
				convert_content_text_to_raw(check_info->ct_info, *p_raw_content,
					&err);
			return err;
		}
	}
	if (is_v1 || (err != NULL && err->code == 404)) {
		...
		if (err2) {
			g_prefix_error(&err, "M2V1 request also failed: [%s] ",
				err2->message);
			g_clear_error(&err2);
		}
		else {
...
find_storage_policy_and_friend_chunks(const gchar * meta2,
	struct hc_url_s * url, check_info_t * check_info, GSList ** chunk_ids)
{
	return find_storage_policy_and_friend_chunks_full(meta2, url, check_info,
		chunk_ids, NULL);
}
...
static void
_set_option(GHashTable * options, const gchar * oname, gpointer ovalue)
{
	if (options && oname)
...
void
check_option_set_int(GHashTable * options, const gchar * oname, gint ovalue)
{
