...
gboolean check_content_storage_policy(const gchar * namespace,
...
void chunk_transfer_set_source_chunk(struct chunk_transfer_s *ct,
	meta2_raw_chunk_t * source_chunk);

void chunk_transfer_set_source_chunk_attrinfo(struct chunk_transfer_s *ct,
	chunk_attrinfo_t * info);

void chunk_transfer_set_target_rawx(struct chunk_transfer_s *ct, GSList * rawx);

void chunk_transfer_init_base_conn(struct chunk_transfer_s *ct,
	const gchar * host, short port);

void chunk_transfer_set_base_conn_close_cb(struct chunk_transfer_s *ct,
	void (*cb) (struct evhttp_connection *, void *), void *arg);

void chunk_transfer_init_req(struct chunk_transfer_s *ct,
	void (*cb) (struct evhttp_request *, void *), void *arg);

void chunk_transfer_set_req_chunked_cb(struct chunk_transfer_s *ct,
...
void chunk_transfer_write_to_dst(struct chunk_transfer_s *ct, gchar * buf,
	size_t size);

void chunk_transfer_set_dst_remaining_size(struct chunk_transfer_s *ct,
	gint64 size);

void chunk_transfer_flush_dst(struct chunk_transfer_s *ct);

const char *chunk_transfer_find_req_header(struct chunk_transfer_s *ct,
	const gchar * header);
...
void chunk_transfer_get_container_id(struct chunk_transfer_s *ct,
	container_id_t cid);
...
void dup_chunk_info_add_chunk(struct dup_chunk_info_s *dup_chunk,
...
chunk_attrinfo_t *build_chunk_attrinfo_from_content(meta2_raw_content_t *
	content);

GError *download_and_check_chunk(const meta2_raw_chunk_t * rc,
 ...
 */
gboolean check_content_info(struct content_textinfo_s *content,
	GError ** p_error);
