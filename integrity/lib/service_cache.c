...
compare_pooled_service(gconstpointer a, gconstpointer b)
{
	...
	return service_info_equal(pooled_service->service_info,
		new_service) ? 0 : -1;
}
...
update_service_cache(struct service_cache_s *service_cache, GError ** error)
{
	...
	for (list = list_services; list && list->data; list = list->next) {
		...
		if (service_cache->service_type == NULL
			|| 0 == strcmp(service->type, service_cache->service_type)) {
			struct pooled_service_s *pooled_service = NULL;
			GSList *result = NULL;

			/* Check if we have already that service in list */
			result =
				g_slist_find_custom(service_cache->service_list, service,
				compare_pooled_service);
			if (result == NULL) {
				...
				service_cache->service_list =
					g_slist_remove(service_cache->service_list, pooled_service);
				service_info_clean(service);
			}

			/* Add pooled service in the new list */
			list_pooled_services =
				g_slist_prepend(list_pooled_services, pooled_service);
		}
		else
			service_info_clean(service);
	}
