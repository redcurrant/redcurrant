...
static GError *_get_container_policy(struct sqlx_sqlite3_s *sq3,
...
_list_params_to_sql_clause(struct list_params_s *lp)
{
	...
	switch (lp->type) {
		case S3:
			...
			if (clause->len > 0 && NULL != lp->params.s3.marker)
...
gboolean
m2v2_parse_chunk_position(const gchar * s, gint * pos, gboolean * par,
	gint * sub)
{
	gchar *end = NULL;
...
m2db_get_container_size(sqlite3 * db, gboolean check_alias)
{
	...
	g_snprintf(tmp, 512, "%s%s", "SELECT SUM(size) FROM content_header_v2",
		!check_alias ? "" :
		" WHERE id IN (SELECT DISTINCT content_id FROM alias_v2)");
	const gchar *sql = tmp;
...
{
	return sqlx_admin_get_i64(sq3, M2V2_PROP_PREFIX_SYS M2V2_KEY_VERSION, 1);
}
...
{
	sqlx_admin_set_i64(sq3, M2V2_PROP_PREFIX_SYS M2V2_KEY_SIZE, size);
}

gint64
m2db_get_quota(struct sqlx_sqlite3_s *sq3, gint64 def)
{
	return sqlx_admin_get_i64(sq3, M2V2_PROP_PREFIX_SYS M2V2_KEY_QUOTA, def);
}

void
m2db_set_quota(struct sqlx_sqlite3_s *sq3, gint64 quota)
{
	sqlx_admin_set_i64(sq3, M2V2_PROP_PREFIX_SYS M2V2_KEY_QUOTA, quota);
}
	...
	m2_onbean_cb cb, gpointer u)
{
	...
	if (lp->flags & M2V2_FLAG_HEADERS) {
		...
		if (NULL != (err = CONTENTS_HEADERS_load(sq3->db, "1", params, cb, u))) {
	...
	void wrapper(gpointer u0, gpointer bean)
	{
		...
		found =
			g_slist_insert_sorted(found, alias,
			(GCompareFunc) _compare_version);
	...
	gboolean runner(gpointer k, gpointer v, gpointer u0)
	{
		...
		if (!((lp->flags & M2V2_FLAG_NODELETED)
	...
	gpointer * result)
{
	...
	err = _db_get_bean(&descr_struct_ALIASES, sq3->db,
		"alias=? AND version=? LIMIT 1", params, _bean_buffer_cb, tmp);
	gvariant_unrefv(params);
	...
	gpointer * result)
{
	...
	params[0] = g_variant_new_string(hc_url_get(url, HCURL_PATH));
	err = _db_get_bean(&descr_struct_ALIASES, sq3->db,
		"alias=? ORDER BY version DESC LIMIT 1", params, _bean_buffer_cb, tmp);
	g_variant_unref(params[0]);
...
m2db_delete_content(struct sqlx_sqlite3_s * sq3, gpointer content)
{
	...
	while (SQLITE_ROW == (rc = sqlite3_step(stmt))) {
		if (rc != SQLITE_OK && rc != SQLITE_DONE) {
			e = NEWERROR(500, "SQLite error: (%d) %s", rc,
...
_filter_properties(gpointer u, gpointer bean)
{
	...
	if (!found) {
		g_tree_replace(ctx->done, g_strdup(PROPERTIES_get_key(bean)->str),
			bean);
		return;
	}
	...
	const gchar * prop_name, guint32 flags, m2_onbean_cb cb, gpointer u0)
{
	...
	gchar *clause =
		g_strconcat(cl_alias, cl_version, cl_propname, cl_del, NULL);

	// Now perform the SELECT
	if (!err) {
		if (flags & M2V2_FLAG_ALLVERSION) {
			err =
				_db_get_bean(&descr_struct_PROPERTIES, sq3->db, clause, params,
				cb, u0);
		}
		else {
	...
	if (!err) {
		...
		for (; !err && beans; beans = beans->next) {
			...
			if (value == NULL || value->len == 0
				|| PROPERTIES_get_deleted(prop)) {
				GRID_DEBUG("Deleting property %s(%ld) of %s",
					PROPERTIES_get_key(prop)->str,
					PROPERTIES_get_alias_version(prop), hc_url_get(url,
						HCURL_PATH));
				GVariant *params[3] = { NULL, NULL, NULL };
				params[0] =
					g_variant_new_string(PROPERTIES_get_alias(prop)->str);
				params[1] = g_variant_new_string(PROPERTIES_get_key(prop)->str);
				err =
					PROPERTIES_delete(sq3->db, "alias = ? AND key = ?", params);
				_bean_clean(prop);
	...
	gpointer cb_data, m2_onprop_cb cb)
{
	gboolean runner(gchar * k, GByteArray * v, gpointer ignored)
	{
		...
		if (!(flags & M2V2_FLAG_ALLPROPS)
			&& !g_str_has_prefix(k, M2V2_PROP_PREFIX_SYS)
...
GError *
m2db_set_container_properties(struct sqlx_sqlite3_s * sq3, guint32 flags,
	GSList * props)
{
	int delete = 0;
	GError *err = NULL;
...
GError *
m2db_get_all_properties(struct sqlx_sqlite3_s * sq3, const gchar * k,
	guint32 flags, m2_onbean_cb cb, gpointer u0)
{
	GVariant *params[2] = { NULL, NULL };
	GError *err;
...
GError *
m2db_del_property(struct sqlx_sqlite3_s * sq3, struct hc_url_s * url,
	const gchar * k)
{
	guint i;
	GError *err;
	...
	tmp = g_ptr_array_new();
	err =
		m2db_get_property(sq3, url, k, M2V2_FLAG_NODELETED, _bean_buffer_cb,
		tmp);
	if (!err) {
...
static GError *
_real_delete(struct sqlx_sqlite3_s *sq3, GSList * beans,
	GSList ** deleted_chunks)
{
...
static GError *
m2db_merge_alias(struct m2db_put_args_s *m2db_args,
	struct bean_ALIASES_s *latest, struct put_args_s *args)
{
	...
	if (NULL != (err =
			_db_get_FK_by_name(latest, "image", m2db_args->sq3->db, cb, &gba)))
...
static gint64
m2db_patch_alias_beans_list(struct m2db_put_args_s *args, GSList * beans)
{
	gchar policy[256];
	GSList *l;
	gint64 size = 0;
...
m2db_force_alias(struct m2db_put_args_s * args, GSList * beans)
{
	...
	if (hc_url_has(args->url, HCURL_VERSION)) {
		...
		err =
			m2db_get_versioned_alias(args->sq3, args->url,
			(gpointer *) & latest);
	}
	else {
	...
	m2_onbean_cb cb, gpointer u0)
{
	...
	if (NULL != (err =
			m2db_latest_alias(args->sq3, args->url, (gpointer *) & latest))) {
		if (err->code == CODE_CONTENT_NOTFOUND)
			g_clear_error(&err);
		else
			g_prefix_error(&err, "Version error: ");
		...
		gint64 count_versions =
			_m2db_count_alias_versions(args->sq3, args->url);
...
m2db_copy_alias(struct m2db_put_args_s * args, const char *source)
{
	...
	GRID_TRACE("M2 COPY(%s FROM %s)", hc_url_get(args->url, HCURL_WHOLE),
		source);
	g_assert(args != NULL);
	g_assert(args->sq3 != NULL);
	g_assert(args->url != NULL);
	...
	if (!err) {
		if (latest == NULL) {
			...
			/* source ok */
			if (!(err =
					m2db_latest_alias(args->sq3, args->url,
						(gpointer *) & dst_latest))) {
				if (dst_latest) {
					if (VERSIONS_DISABLED(args->max_versions)) {
						err =
							NEWERROR(CODE_CONTENT_EXISTS,
							"Cannot copy content,"
							"destination already exists (and versioning disabled)");
					}
					else if (VERSIONS_ENABLED(args->max_versions)) {
						ALIASES_set_version(latest,
							ALIASES_get_version(dst_latest) + 1);
...
_increment_position(GString * encoded_position, gint64 inc)
{
	...
	g_string_printf(encoded_position, "%" G_GINT64_FORMAT "%s", pos + inc,
		tail);
}
...
_mangle_new_bean(struct append_context_s *ctx, gpointer bean)
{
	if (DESCR(bean) == &descr_struct_ALIASES) {
		...
		ALIASES_set_version(bean,
			((ctx->versioning) ? ctx->old_version + 1 : ctx->old_version));
	...
	if (DESCR(bean) == &descr_struct_CONTENTS_HEADERS) {
		bean = _bean_dup(bean);
		CONTENTS_HEADERS_set_size(bean,
			CONTENTS_HEADERS_get_size(bean) + ctx->old_size);
...
static void
_delete_contents_by_id(struct sqlx_sqlite3_s *sq3, GByteArray * cid,
	GError ** err)
{
		...
		if (ctx.fresh) {
			...
			if (!err && VERSIONS_DISABLED(max_versions)) {
				...
				if (err) {
					...
					GRID_WARN
						("Some chunks may be referenced twice, purge recommended");
					g_clear_error(&err);
				}
			}
		}
	...
	guint pos, gint64 cs, gint subpos, gboolean parity)
{
	...
	if (subpos >= 0)
		g_snprintf(strpos, sizeof(strpos), (parity ? "%u.p%d" : "%u.%d"), pos,
			subpos);
	else
	...
	struct namespace_info_s *nsinfo, struct storage_policy_s **result)
{
	...
	if (!(err = m2db_latest_alias(sq3, url, (gpointer *) & latest))) {
		if (latest != NULL) {
			err =
				_db_get_FK_by_name(latest, "image", sq3->db, _bean_buffer_cb,
				tmp);
			if (!err && tmp->len > 0) {
				struct bean_CONTENTS_HEADERS_s *header = tmp->pdata[0];
...
static GError *
_get_container_policy(struct sqlx_sqlite3_s *sq3,
	struct namespace_info_s *nsinfo, struct storage_policy_s **result)
{
	gchar *pname;
...
GError *
m2db_set_storage_policy(struct sqlx_sqlite3_s * sq3, const gchar * polname,
	int replace)
{
...
{
	*status =
		sqlx_admin_get_i64(sq3, "flags", (gint64) CONTAINER_STATUS_ENABLED);
	return NULL;
}
...
_purge_exceeding_aliases(struct sqlx_sqlite3_s *sq3, gint64 max_versions)
{
	...
	GRID_TRACE("%s, max_versions = %" G_GINT64_FORMAT, __FUNCTION__,
		max_versions);

	const gchar *sql_lookup = "SELECT alias, count(*)" "FROM alias_v2 " "WHERE NOT deleted "	// Do not count last extra deleted version
		"AND container_version NOT IN (SELECT version FROM snapshot_v2) "
		"GROUP BY alias " "HAVING COUNT(*) > ?";
...
_purge_deleted_aliases(struct sqlx_sqlite3_s *sq3, gint64 delay)
{
	...
	// Do the purge.
	GRID_DEBUG
		("Purging deleted aliases older than %ld seconds (timestamp < %ld)",
		delay, time_limit);
...
_get_chunks_to_drop(sqlite3 * db, m2_onbean_cb cb, gpointer u0)
{
	...
	CHUNKS_load(db,
		"id NOT IN (select distinct chunk_id from content_v2)",
	...
	gint64 retention_delay, guint32 flags, m2_onbean_cb cb, gpointer u0)
{
	...
	if (!dry_run) {
		...
		sqlx_exec(sq3->db, "delete from properties_v2 "
			"WHERE alias NOT IN (select distinct alias from alias_v2)");

		/* purge unreferenced content_headers, cascading to contents */
		sqlx_exec(sq3->db, "delete from content_header_v2 "
			"WHERE id NOT IN (select distinct content_id from alias_v2)");

		sqlx_exec(sq3->db, "delete from content_v2 "
	...
	if (NULL != cb) {
		...
		if (!dry_run) {
			...
			sqlx_exec(sq3->db, "delete from chunk_v2 "
...
GError *
m2db_deduplicate_alias_chunks(struct sqlx_sqlite3_s * sq3,
	...
	guint32 flags, GString ** status_message)
{
	...
	guint64 saved_space =
		dedup_aliases(sq3->db, url, dry_run, &impacted_aliases, &err);
...
GError *
m2db_content_urls_from_chunk_id(struct sqlx_sqlite3_s * sq3,
	struct hc_url_s * url, const gchar * chunk_id, gint64 limit, GSList ** urls)
{
	...
	void alias_bean_cb(gpointer user_data, struct bean_ALIASES_s *alias)
	{
		...
		g_snprintf(version, 16, "%" G_GINT64_FORMAT,
	...
	// FIXME: get full path instead of matching with glob
	gchar *sql = (" content_id IN"
		" (SELECT DISTINCT content_id"
		"  FROM content_v2 " "  WHERE chunk_id LIKE ?) LIMIT ?");

	GVariant *params[3] = { NULL, NULL, NULL };
	gchar *glob = g_strdup_printf("%%%s", chunk_id);

	params[0] = g_variant_new_string(glob);
	params[1] = g_variant_new_int64(limit);
	err =
		ALIASES_load(sq3->db, sql, params, (m2_onbean_cb) alias_bean_cb, NULL);
	g_variant_unref(params[0]);
	g_variant_unref(params[1]);
	g_free(glob);
	return err;
...
m2v2_dup_alias(struct dup_alias_params_s *params, gpointer bean)
{
	...
	if (DESCR(bean) == &descr_struct_ALIASES) {
		...
		if (params->src_c_version >= 1) {
			...
			local_err =
				m2db_get_alias_version(params->sq3, url, 0, &latest_version);
			if (local_err != NULL) {
