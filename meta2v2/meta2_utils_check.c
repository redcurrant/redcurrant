...
_alert_for_invalid_rain_stgpol(header_check_t *hc, struct ctx_rain_stgpol_s *ctx)
{
	...
	if (ctx->size_metachunk != ns_chunk_size && actual_k != lend) {
		// TODO: refine the test by looking for gaps in positions
		// gaps-> something missing, no gaps -> unsure
		if (ctx->last->position.meta != (size_real / ns_chunk_size)) {
			GRID_WARN("Found a metachunk smaller than the namespace chunk "
					"size at an intermediate position (%d), cannot determine "
					"if it's normal or if we miss a data chunk (%u/%u found).",
					ctx->last->position.meta, lend, actual_k);
		}
