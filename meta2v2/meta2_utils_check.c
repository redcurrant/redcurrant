...
static inline void
_check_one_chunk(header_check_t * hc, single_check_t * pcheck,
	chunk_pair_t * pair)
{
...
m2v2_check_append_flaw(header_check_t * hc, int type, GError * error)
{
	...
	struct m2v2_check_error_s *flaw =
		g_malloc0(sizeof(struct m2v2_check_error_s));
	flaw->type = type;
...
_pairs_copy(GPtrArray * src)
{
	...
	if (src && src->len) {
		for (guint i = 0; i < src->len; ++i) {
			...
			pair.content =
				((chunk_pair_t *) g_ptr_array_index(src, i))->content;
...
static void
_extract_location_from_services(header_check_t * hc, GPtrArray * dst,
	GPtrArray * gpa)
{
	...
	for (guint i = 0; i < gpa->len; ++i) {
		...
		if (!loc) {
			flaw = m2v2_check_append_flaw(hc, M2CHK_RAWX_UNKNOWN,
				NEWERROR(0, "RAWX [%s] not declared",
...
_check_distances(GPtrArray * locations, guint distance_requested)
{
	for (guint i = 1; i < locations->len; ++i) {
		for (guint j = 0; j < i; ++j) {
			guint distance =
				distance_between_location(locations->pdata[j],
				locations->pdata[i]);
...
hook_CHUNK_stgclass(header_check_t * hc, chunk_pair_t * pair, void *u)
{
	...
	struct service_info_s *rawx =
		grid_lbpool_get_service_from_url(hc->check->lbpool, "rawx", url);
	if (rawx != NULL) {
		...
		if (ctx->bad_pairs->len > 0) {
			flaw = m2v2_check_append_flaw(hc, M2CHK_CONTENT_STGCLASS,
				NEWERROR(0,
					...
					storage_class_get_name(ctx->stgclass),
					ctx->bad_pairs->len));
	...
	struct ctx_dupli_stgpol_s *ctx)
{
	...
	if (ctx->pairs->len && 0 < CONTENTS_HEADERS_get_size(hc->header)) {
		...
		for (guint i = 1; !match && i < ctx->pairs->len; ++i) {
			...
			s0 = CHUNKS_get_size(((chunk_pair_t *) (ctx->pairs->pdata[0]))->
				chunk);
			s1 = CHUNKS_get_size(((chunk_pair_t *) (ctx->pairs->pdata[i]))->
				chunk);
			if (s0 != s1)
				match = TRUE;
		}
	...
	if (ctx->pairs->len && 0 < CONTENTS_HEADERS_get_size(hc->header)) {
		...
		for (guint i = 1; !match && i < ctx->pairs->len; ++i) {
			h0 = CHUNKS_get_hash(((chunk_pair_t *) (ctx->pairs->pdata[0]))->
				chunk);
			h1 = CHUNKS_get_hash(((chunk_pair_t *) (ctx->pairs->pdata[i]))->
				chunk);
	...
	if (ctx->pairs->len < ctx->replicas
		&& 0 < CONTENTS_HEADERS_get_size(hc->header)) {
		flaw =
			m2v2_check_append_flaw(hc, M2CHK_CHUNK_DUPLI_TOOFEW, NEWERROR(0,
				"Too few chunks [%u] at position [%u]", ctx->pairs->len,
				ctx->last->position.meta));
...
_check_dupli_alias(header_check_t * hc, guint replicas, guint distance)
{
	...
	if (CONTENTS_HEADERS_get_size(hc->header) > 0) {
		single_check_t checks[] = {
			...
			,
			{&ctx_total_size, hook_CHUNK_dupli_total_size,
					hook_END_dupli_total_size}
			,
...
static inline void
_alert_for_invalid_rain_stgpol(header_check_t * hc,
	struct ctx_rain_stgpol_s *ctx)
{
	struct m2v2_check_error_s *flaw;
	...
	if (ctx->last->position.meta == (size_real / ns_chunk_size)) {
		...
		GRID_TRACE
			("metachunk size: %ld, rain chunk size: %ld, k: %u, actual_k: %u",
	...
	// but fixing one of them will fix both.
		...
		flaw = m2v2_check_append_flaw(hc, M2CHK_CHUNK_RAIN_TOOFEW, NEWERROR(0,
				"Missing [%d] parity chunks at [%u]", delta,
				ctx->last->position.meta));
		...
		flaw->param.rain_toofew.pairs_unavailable =
			_pairs_copy2(hc->unavail_chunks);
...
_get_header(struct m2v2_check_s *check, GByteArray * id0)
{
	for (guint i = 0; i < check->headers->len; ++i) {
		struct bean_CONTENTS_HEADERS_s *hdr =
			g_ptr_array_index(check->headers, i);
...
void
init_chunk_pair(GPtrArray * chunks, chunk_pair_t * pair,
...
_load_storage_policy(header_check_t * hc)
{
	...
	if (!gpol)
		return NEWERROR(CODE_POLICY_NOT_SATISFIABLE,
			"Storage Policy undefined");
	DEBUG("Checking existence of stgpol : [%s]", gpol->str);

	hc->stgpol = storage_policy_init(hc->check->ns_info, gpol->str);
	if (!hc->stgpol)
		return NEWERROR(CODE_POLICY_NOT_SATISFIABLE,
			"Storage Policy unknown [%s]", gpol->str);
	return NULL;
}
	...
	struct check_args_s *args)
{
	...
	if (args && args->lbpool)
		...
		check->lbpool =
			grid_lbpool_create(hc_url_get(check->url, HCURL_NSPHYS));
...
_clean_m2v2_check(struct m2v2_check_s *check)
{
	if (check->flaws) {
		while (check->flaws->len > 0) {
			struct m2v2_check_error_s *flaw =
				g_ptr_array_index(check->flaws, 0);
	...
	for (guint i = 0; i < check->aliases->len; ++i) {
		...
		if (NULL != err) {
			...
			GRID_WARN("Erroneous ALIAS [%s] : (%d) %s",
				hc_url_get(check->url, HCURL_WHOLE), err->code, err->message);
			g_clear_error(&err);
		}
	}
...
{
	guint32 result = M2V2_CHECK_GAPS | M2V2_CHECK_DIST | M2V2_CHECK_STGCLS;
