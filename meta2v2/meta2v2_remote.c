	...
	struct hc_url_s *url, GByteArray * body)
{
	...
	msg = message_create_request(NULL, sid, name,
		...
		"CONTAINER_ID", gba_poolify(&pool, _url_2_gba(url)), NULL);
	GBA_POOL_CLEAN(pool);
...
_m2v2_pack_request(const gchar * name, GByteArray * sid,
	struct hc_url_s *url, GByteArray * body)
{
	return message_marshall_gba_and_clean(_m2v2_build_request(name, sid, url,
			body));
}
	...
	struct m2v2_create_params_s * pols)
{
	...
	if (pols && pols->storage_policy) {
		(void) message_add_field(msg,
			...
			pols->storage_policy, strlen(pols->storage_policy), NULL);
	}
	if (pols && pols->version_policy) {
		(void) message_add_field(msg,
			M2_KEY_VERSION_POLICY, sizeof(M2_KEY_VERSION_POLICY) - 1,
			pols->version_policy, strlen(pols->version_policy), NULL);
	}
	return message_marshall_gba_and_clean(msg);
}
...
GByteArray *
m2v2_remote_pack_OVERWRITE(GByteArray * sid, struct hc_url_s * url,
	GSList * beans)
{
	struct message_s *msg;
	GByteArray *body = bean_sequence_marshall(beans);
	...
	(void) message_add_field(msg,
		M2_KEY_OVERWRITE, sizeof(M2_KEY_OVERWRITE) - 1, "1", 1, NULL);
	return message_marshall_gba_and_clean(msg);
}
...
GByteArray *
m2v2_remote_pack_COPY(GByteArray * sid, struct hc_url_s * url,
	const gchar * src)
{
	struct message_s *msg;
...
GByteArray *
m2v2_remote_pack_RAW_DEL(GByteArray * sid, struct hc_url_s * url,
	GSList * beans)
{
	GByteArray *body = bean_sequence_marshall(beans);
	...
	const gchar * chunk_id, gint64 limit)
{
	...
	message_add_fields_str(msg,
		M2_KEY_CHUNK_ID, chunk_id, M2_KEY_MAX_KEYS, limit_str, NULL);
	return message_marshall_gba_and_clean(msg);
}
...
GByteArray *
m2v2_remote_pack_PROP_SET(GByteArray * sid, struct hc_url_s * url,
	guint32 flags, GSList * beans)
{
	GByteArray *body = bean_sequence_marshall(beans);

	return _m2v2_pack_request_with_flags("M2V2_PROP_SET", sid, url, body,
		flags);
}

GByteArray *
m2v2_remote_pack_PROP_GET(GByteArray * sid, struct hc_url_s * url,
	guint32 flags)
{
	return _m2v2_pack_request_with_flags("M2V2_PROP_GET", sid, url, NULL,
		flags);
}
	...
	const gchar * pol, gint64 size, gboolean append)
{
	...
	if (!append) {
		message_add_fields_str(msg,
			NAME_MSGKEY_CONTENTLENGTH, strsize, "STORAGE_POLICY", pol, NULL);
	}
	else {
		message_add_fields_str(msg,
	...
	const gchar * pol, GSList * notin_list, GSList * broken_list)
{
	...
	message_add_fields_str(msg,
		M2_KEY_STORAGE_POLICY, pol, M2_KEY_SPARE, spare_type, NULL);
...
GByteArray *
m2v2_remote_pack_TOUCH_container(GByteArray * sid, struct hc_url_s * url,
	guint32 flags)
{
	return _m2v2_pack_request_with_flags("REQ_M2RAW_TOUCH_CONTAINER", sid, url,
		NULL, flags);
}
	...
	gdouble timeout_to_step, gdouble timeout_to_overall, GSList ** out)
{
	...
	if (!client)
		...
		if ((timeout_to_step >= 0) && (timeout_to_overall >= 0))
			gridd_client_set_timeout(client, timeout_to_step,
				timeout_to_overall);
...
{
	return _m2v2_request(target, m2v2_remote_pack_DESTROY(sid, url, flags),
		NULL);
}
...
{
	return _m2v2_request(target, m2v2_remote_pack_BEANS(sid, url, pol, size,
			append), out);
}
...
m2v2_remote_execute_SPARE(const gchar * target, GByteArray * sid,
	...
	GSList * notin_list, GSList * broken_list, GSList ** out)
{
...
{
	return _m2v2_request(target, m2v2_remote_pack_OVERWRITE(sid, url, in),
		NULL);
}
...
{
	return _m2v2_request(target, m2v2_remote_pack_RAW_DEL(sid, url, beans),
		NULL);
}
...
{
	return _m2v2_request(target, m2v2_remote_pack_PROP_SET(sid, url, flags, in),
		NULL);
}
...
m2v2_remote_execute_PROP_GET(const gchar * target, GByteArray * sid,
	struct hc_url_s * url, guint32 flags, GSList ** out)
{
	return _m2v2_request(target, m2v2_remote_pack_PROP_GET(sid, url, flags),
		out);
}
...
m2v2_remote_touch_content(const gchar * target, GByteArray * sid,
	struct hc_url_s * url)
{
	return _m2v2_request(target, m2v2_remote_pack_TOUCH_content(sid, url),
		NULL);
}
...
m2v2_remote_touch_container_ex(const gchar * target, GByteArray * sid,
	struct hc_url_s * url, guint32 flags)
{
	return _m2v2_request(target, m2v2_remote_pack_TOUCH_container(sid, url,
			flags), NULL);
}
