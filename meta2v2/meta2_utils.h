 ...
 * @return
 */
GError *m2db_set_storage_policy(struct sqlx_sqlite3_s *sq3,
	const gchar * polname, int repl);
 ...
 * @return
 */
GError *m2db_get_versioned_alias(struct sqlx_sqlite3_s *sq3,
	struct hc_url_s *url, gpointer * result);
...
GError *m2_generate_beans_v1(struct hc_url_s *url, gint64 size,
	gint64 chunk_size, struct storage_policy_s *pol, const char *mdsys,
	const char *mdusr, struct grid_lb_iterator_s *iter, m2_onbean_cb cb,
	gpointer cb_data);
 ...
 * @return
 */
GError *m2_generate_spare_chunks(struct hc_url_s *url,
	struct storage_policy_s *pol, struct grid_lb_iterator_s *iter,
	GSList ** result);
 ...
 * @return
 */
GError *m2_generate_conditionned_spare_chunks_beans(struct grid_lb_iterator_s
	*iter, struct lb_next_opt_s *opt, service_filter filter, GSList ** result);
 ...
 * @return
 */
GError *m2db_get_storage_policy(struct sqlx_sqlite3_s *sq3,
	struct hc_url_s *url, struct namespace_info_s *nsinfo,
	gboolean from_previous, struct storage_policy_s **result);
 ...
 * @return
 */
GError *m2db_update_alias_header(struct sqlx_sqlite3_s *sq3,
	gint64 max_versions, struct hc_url_s *url, GSList * beans,
...
GError *m2db_purge_alias_being_deleted(struct sqlx_sqlite3_s *sq3,
	GSList * beans, GSList ** deleted);
...
GError *m2db_dup_all_aliases(struct sqlx_sqlite3_s *sq3,
	gint64 container_version, gboolean set_deleted, gboolean overwrite_latest);
 ...
 */
GError *m2db_get_snapshot_by_name(struct sqlx_sqlite3_s *sq3,
	const gchar * name, struct bean_SNAPSHOTS_s **snapshot);
 ...
 * @return
 */
GSList *m2v2_beans_from_raw_content_custom(const char *id,
	meta2_raw_content_t * rc, char *(*make_pos) (guint32, void *), void *udata);

/*!
 * @param beans
 * @return
 */
meta2_raw_content_t *raw_content_from_m2v2_beans(const container_id_t cid,
	GSList * beans);
 ...
 * @return
 */
GSList *m2v2_beans_from_raw_content_v2(const char *id,
...
/* chunk_pair */
void init_chunk_pair(GPtrArray * chunks, chunk_pair_t * pair,
...
char *location_from_chunk(struct bean_CHUNKS_s *chunk,
