...
static void
check_list_count(struct meta2_backend_s *m2, struct hc_url_s *url,
	guint expected)
{
	GError *err;
	guint counter = 0;
...
test_content_put_prop_get(void)
{
	void test(struct meta2_backend_s *m2, struct hc_url_s *u)
	{
		...
		tmp = g_ptr_array_new();
		err =
			meta2_backend_get_alias(m2, u, M2V2_FLAG_ALLVERSION, _appender,
			tmp);
		g_assert_no_error(err);
		...
		tmp = g_ptr_array_new();
		err =
			meta2_backend_get_alias(m2, u, M2V2_FLAG_NODELETED, _appender, tmp);
		g_assert_error(err, GQ(), CODE_CONTENT_NOTFOUND);
		expected = 0;
		GRID_DEBUG("TEST count=%u expected=%u", tmp->len, expected);
		g_assert(tmp->len == expected);
...
test_content_put_get_delete(void)
{
	void test(struct meta2_backend_s *m2, struct hc_url_s *u)
	{
		...
		GRID_DEBUG
			("TEST Got %u beans for all versions, expected %u (chunks count: %"
			G_GINT64_FORMAT ")", tmp->len, expected, chunks_count);
		g_assert(tmp->len == expected);
		_bean_cleanv2(tmp);
...
test_content_append(void)
{
	void test(struct meta2_backend_s *m2, struct hc_url_s *u)
	{
		...
		GRID_DEBUG("TEST After the append, got %u, expected %u", tmp->len,
			expected);
		g_assert(tmp->len == expected);
		_bean_cleanv2(tmp);
...
main(int argc, char **argv)
{
	...
	g_test_add_func("/meta2v2/backend/content/append", test_content_append);
	g_test_add_func("/meta2v2/backend/content/append_notfound",
		test_content_append_not_found);
	g_test_add_func("/meta2v2/backend/props/set_simple", test_props_set_simple);
	g_test_add_func("/meta2v2/backend/props/set_deleted",
		test_props_set_deleted);
	g_test_add_func("/meta2v2/backend/props/gotchas", test_props_gotchas);
