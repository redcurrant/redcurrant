...
static GError *
_meta2_event_add_raw_v2(gridcluster_event_t * event,
...
static GError *
_meta2_event_add_bean(gridcluster_event_t * event, const gchar * key,
	GSList * bean_chunk)
{
	GByteArray *gba = NULL;
...
static gchar *
ueid_generate(struct event_config_s *evt_config, gchar * d, gsize dsize)
{
	struct timeval tv;
	gchar hostname[256];
...
_notify_content(struct gridd_filter_ctx_s *ctx, struct gridd_reply_ctx_s *reply,
	gboolean bChunk_purged_only, struct on_bean_ctx_s *purged_chunk,
	const char *evt_type)
{
	TRACE_FILTER();
	(void) reply;
	...
	// add 'purged chunk' field
		...
		if (purged_chunk == NULL) {
			...
			err =
				meta2_backend_purge_container(m2b, url, flags, _get_beans, obc);
		}
		else
			obc = purged_chunk;

		GRID_ERROR("%s: [%d beans to purged]", __FUNCTION__,
			g_slist_length(obc->l));
...
{
	return _notify_content(ctx, reply, TRUE, purged_chunk,
		META2_EVTTYPE_DELETE);
}
