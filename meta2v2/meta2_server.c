...
_task_reload_event_config(gpointer p)
{
	...
	void _update_each(gpointer k, gpointer v, gpointer ignored)
	{
		...
		if (!err) {
			err =
				event_config_reconfigure(meta2_backend_get_event_config(m2,
					(char *) k), (char *) v);
		}
	}

	GHashTable *ht = gridcluster_get_event_config(&(PSRV(p)->nsinfo), "meta2");
...
meta2_on_close(struct sqlx_sqlite3_s *sq3, gboolean deleted, gpointer cb_data)
{
	...
	hc_decache_reference_service(PSRV(cb_data)->resolver, u, "meta2");
	hc_url_clean(u);
	u = NULL;
}
...
_post_config(struct sqlx_service_s *ss)
{
	...
	if (err) {
		GRID_WARN("META2 backend init failure : (%d) %s", err->code,
			err->message);
		g_clear_error(&err);
		return FALSE;
	}
	...
	sqlx_repository_configure_close_callback(ss->repository, meta2_on_close,
		ss);
	...
	grid_task_queue_register(ss->gtq_reload, 5, _task_reconfigure_m2, NULL, ss);
	grid_task_queue_register(ss->gtq_reload, 10, _task_reload_lb, NULL, ss);
...
main(int argc, char **argv)
{
	struct sqlx_service_config_s cfg = {
		"meta2", "m2v2", "el/meta2", 2, 2, schema,
