...
_create_container(struct gridd_filter_ctx_s *ctx)
{
	...
	params.version_policy =
		meta2_filter_ctx_get_param(ctx, M2_KEY_VERSION_POLICY);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	GError *err =
		meta2_backend_destroy_container(meta2_filter_ctx_get_backend(ctx),
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	GError *err =
		meta2_backend_purge_container(meta2_filter_ctx_get_backend(ctx),
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	GError *err =
		meta2_backend_deduplicate_contents(meta2_filter_ctx_get_backend(ctx),
		meta2_filter_ctx_get_url(ctx),
		flags,
		&status_message);
	...
	struct list_params_s *lp)
{
	...
	GSList *common_prefixes = NULL,
		*aliases = NULL, *headers = NULL, *result = NULL;
	gint64 max = 0;
	...
	lp->params.s3.prefix = meta2_filter_ctx_get_param(ctx, M2_KEY_PREFIX);
	lp->params.s3.marker = meta2_filter_ctx_get_param(ctx, M2_KEY_MARKER);
	lp->params.s3.delimiter = meta2_filter_ctx_get_param(ctx, M2_KEY_DELIMITER);
	...
	void check_alias(gpointer alias)
	{
		...
		if (r) {
			GRID_DEBUG("Found common prefix : %.*s",
				((int) (r - alias_name)) + 1, alias_name);
			add_prefix(g_strndup(alias_name, r + 1 - alias_name));
		}
		else {
	...
	void s3_list_cb(gpointer ignored, gpointer bean)
	{
		...
		if (max > 0) {
			if (DESCR(bean) == &descr_struct_ALIASES) {
				...
				/* keep for the moment,
				   we check which header we need at the end */
	...
	gpointer search_header(GByteArray * id)
	{
		for (GSList * h = headers; h && h->data; h = h->next) {
			if (!metautils_gba_cmp(CONTENTS_HEADERS_get_id(h->data), id))
				return h->data;
		}
		return NULL;
	}
	...
	void pack_result()
	{
		for (GSList * a = aliases; a && a->data; a = a->next) {
			gpointer header = search_header(ALIASES_get_content_id(a->data));

			if (header) {
				result = g_slist_prepend(result, a->data);
				result = g_slist_prepend(result, header);
			}
		}
		GRID_DEBUG("result length = %d", g_slist_length(result));
	}
	...
	void send_result()
	{
		...
		S3_RESPONSE_HEADER(M2_KEY_MAX_KEYS,
			(maxkeys_str) ? maxkeys_str : "1000");
	...
	GRID_DEBUG("prefix = %s | marker = %s | delimiter = %s | max-keys = %"
		G_GINT64_FORMAT, lp->params.s3.prefix, lp->params.s3.marker,
		lp->params.s3.delimiter, lp->params.s3.maxkeys);
	...
	g_slist_free_full(headers, _bean_clean);
	g_slist_free(result);
	/* content freed in send result */
	...
	for (; aliases; aliases = aliases->next) {
		...
		for (l = headers; l; l = l->next) {
			...
			if (!memcmp(ch_id->data, al_id->data, al_id->len)) {
				...
				g_strlcpy(pi->path, ALIASES_get_alias(aliases->data)->str,
					sizeof(pi->path));
				pi->user_metadata = g_byte_array_new();
				pi->hasSize = TRUE;
				pi->size = CONTENTS_HEADERS_get_size(l->data);
				pi->system_metadata =
					metautils_gba_from_string(ALIASES_get_mdsys(aliases->data)->
					str);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if (GRID_TRACE_ENABLED()) {
		...
		GRID_TRACE("Prefix: %s", meta2_filter_ctx_get_param(ctx,
				M2_KEY_PREFIX));
	}
	if (NULL != ltype && !g_ascii_strcasecmp(ltype, S3_LISTING_TYPE)) {
		...
		lp.params.s3.delimiter = meta2_filter_ctx_get_param(ctx,
			M2_KEY_DELIMITER);
		const char *maxkeys_str =
			meta2_filter_ctx_get_param(ctx, M2_KEY_MAX_KEYS);
		if (NULL != maxkeys_str)
			lp.params.s3.maxkeys = g_ascii_strtoll(maxkeys_str, NULL, 10);
	}
	else {
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	(void) reply;
	TRACE_FILTER();
	err = meta2_backend_open_container(meta2_filter_ctx_get_backend(ctx),
		meta2_filter_ctx_get_url(ctx));
	if (!err)
		return FILTER_OK;
	meta2_filter_ctx_set_error(ctx, err);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	(void) reply;
	TRACE_FILTER();
	err = meta2_backend_close_container(meta2_filter_ctx_get_backend(ctx),
		meta2_filter_ctx_get_url(ctx));
	if (!err)
		return FILTER_OK;
	meta2_filter_ctx_set_error(ctx, err);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	reply->add_body(g_byte_array_append(g_byte_array_new(), (guint8 *) & status,
			4));
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	e = meta2_backend_set_container_status(m2b, url, NULL,
		CONTAINER_STATUS_ENABLED);
	if (NULL != e) {
		GRID_DEBUG("Status change failure %s", hc_url_get(url, HCURL_WHOLE));
		meta2_filter_ctx_set_error(ctx, e);
		return FILTER_KO;
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	e = meta2_backend_set_container_status(m2b, url, &flags,
		CONTAINER_STATUS_DISABLED);
	if (NULL != e) {
		GRID_DEBUG("Status change failure %s", hc_url_get(url, HCURL_WHOLE));
		meta2_filter_ctx_set_error(ctx, e);
		return FILTER_KO;
...
int
meta2_filter_action_list_usr_container_properties(struct gridd_filter_ctx_s
...
int
meta2_filter_action_list_all_container_properties(struct gridd_filter_ctx_s
	*ctx, struct gridd_reply_ctx_s *reply)
{
	int rc;
	GError *err;
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	void cb(gpointer u, gpointer bean)
	{
		...
		*((GSList **) u) =
			g_slist_prepend(*((GSList **) u), g_strdup(path->str));
	}
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if (!err)
		err =
			message_extract_header_gba(reply->request, "ADMIN_VALUE", TRUE,
			&value);
	if (!err) {
...
restorev1_content_cb(gpointer u, const meta2_raw_content_v2_t * p)
{
	...
	if ((!p && ctx->count_content) || ctx->count_content >= 50) {
		g_snprintf(str, sizeof(str), "%" G_GUINT32_FORMAT " contents",
			ctx->count_content);
...
restorev1_admin_cb(gpointer u, const key_value_pair_t * p)
{
	...
	if ((!p && ctx->count_admin) || ctx->count_admin >= 50) {
		g_snprintf(str, sizeof(str), "%" G_GUINT32_FORMAT " admin",
			ctx->count_admin);
		ctx->reply->send_reply(201, str);
		ctx->count_admin = 0;
	}
...
restorev1_event_cb(gpointer u, const container_event_t * p)
{
	...
	if ((!p && ctx->count_event) || ctx->count_event >= 50) {
		g_snprintf(str, sizeof(str), "%" G_GUINT32_FORMAT " events",
			ctx->count_event);
		ctx->reply->send_reply(201, str);
		ctx->count_event = 0;
	}
...
restorev1_property_cb(gpointer u, const meta2_property_t * p)
{
	...
	if ((!p && ctx->count_property) || ctx->count_property >= 50) {
		g_snprintf(str, sizeof(str), "%" G_GUINT32_FORMAT " properties",
			ctx->count_property);
		ctx->reply->send_reply(201, str);
