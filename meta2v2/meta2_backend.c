...
{
	return m2db_get_quota(sq3, m2b_quota(m2b));
}
...
void
transient_put(GTree * tree, const gchar * key, gpointer what,
...
GError *
m2b_transient_put(struct meta2_backend_s *m2b, const gchar * key,
	const gchar * hexID, gpointer what, GDestroyNotify cleanup)
{
	g_assert(m2b != NULL);
	g_assert(key != NULL);
	g_assert(what != NULL);
...
gpointer
m2b_transient_get(struct meta2_backend_s * m2b, const gchar * key,
	const gchar * hexID, GError ** err)
{
	struct transient_s *trans = NULL;
	...
	if (hexID != NULL)
		local_err =
			sqlx_repository_status_base(m2b->repo, META2_TYPE_NAME, hexID);
...
GError *
m2b_transient_del(struct meta2_backend_s * m2b, const gchar * key,
	const gchar * hexID)
{
	g_assert(m2b != NULL);
	g_assert(key != NULL);
...
_check_policy(struct meta2_backend_s *m2, const gchar * polname)
{
	...
	if (!(policy = storage_policy_init(&nsinfo, polname)))
		err =
			NEWERROR(CODE_POLICY_NOT_SUPPORTED, "Invalid policy: %s",
			"not found");
	else
	...
	enum m2v2_open_type_e how, struct sqlx_sqlite3_s **result)
{
	...
	err = sqlx_repository_open_and_lock(m2->repo, META2_TYPE_NAME,
		hc_url_get(url, HCURL_HEXID), how & 0x00FF, &sq3, NULL);
	if (NULL != err) {
...
GError *
meta2_backend_has_container(struct meta2_backend_s * m2, struct hc_url_s * url)
{
	GError *err = NULL;
	...
	struct hc_url_s * url, struct m2v2_create_params_s * params)
{
	...
	if (sq3 && !err) {
		if (sqlx_admin_has(sq3, META2_INIT_FLAG))
			err =
				NEWERROR(CODE_CONTAINER_EXISTS, "Container already initiated");
		else
	...
	struct hc_url_s * url, guint32 flags)
{
	...
	if (!err) {
		...
		if (!err) {
			...
			m2b_destroy(sq3);
			err = election_exit(sqlx_repository_get_elections_manager(m2->repo),
	...
	struct hc_url_s * url)
{
	...
	err =
		sqlx_config_has_peers(election_manager_get_config
		(sqlx_repository_get_elections_manager(m2->repo)), hc_url_get(url,
			HCURL_HEXID), META2_TYPE_NAME, &has);
	if (NULL != err) {
	...
	const gchar * strid, gint64 size)
{
	if (m2b && strid) {
		...
		g_hash_table_replace(m2b->modified_containers, g_strdup(strid),
			g_memdup(&size, sizeof(gint64)));
...
meta2_backend_delete_alias(struct meta2_backend_s * m2b,
	struct hc_url_s * url, gboolean sync_delete, m2_onbean_cb cb, gpointer u0)
{
	GError *err = NULL;
	struct sqlx_sqlite3_s *sq3 = NULL;
	gint64 max_versions;
	...
	if (!err) {
		...
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			if (!(err =
					m2db_delete_alias(sq3, max_versions, url, sync_delete, cb,
						u0))) {
				m2db_increment_version(sq3);
				meta2_backend_add_modified_container(m2b, hc_url_get(url,
						HCURL_HEXID), m2db_get_size(sq3));
			}
			err = sqlx_transaction_end(repctx, err);
		}
		m2b_close(sq3);
	...
	struct hc_url_s * url, GSList * beans, m2_onbean_cb cb, gpointer u0)
{
	...
	if (!err) {
		...
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			if (!(err =
					m2db_set_properties(sq3, max_versions, url, beans, cb, u0)))
				m2db_increment_version(sq3);
			err = sqlx_transaction_end(repctx, err);
		}
		m2b_close(sq3);
	...
	m2_onbean_cb cb, gpointer cb_data)
{
	...
	GRID_TRACE("BEANS(%s,%" G_GINT64_FORMAT ",%s)", hc_url_get(url,
			HCURL_WHOLE), size, polname);
	g_assert(m2b != NULL);
	g_assert(url != NULL);
	g_assert(cb != NULL);
		...
		if (!err) {
			if (polname) {
				...
				if (NULL != polstr) {
					...
					err =
						m2db_get_storage_policy(sq3, url, &nsinfo, append,
						&policy);
					if (err || !policy) {
						gchar *default_ns_policy_name =
							namespace_storage_policy(&nsinfo, hc_url_get(url,
								HCURL_NS));
	...
	if (!err) {
		...
		if (!iter)
			...
			err =
				m2_generate_beans_v1(url, size, nsinfo.chunk_size, policy,
				mdsys, mdusr, iter, cb, cb_data);
	}
	...
	struct hc_url_s * url, GSList * beans, gboolean skip_checks)
{
	...
	if (!err) {
		...
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			err =
				m2db_update_alias_header(sq3, max_versions, url, beans,
				skip_checks);
			err = sqlx_transaction_end(repctx, err);
		}
		m2b_close(sq3);
	...
	struct hc_url_s * url)
{
	...
	if (!err) {
		GRID_INFO("Starting chunk deduplication on %s (%s)",
			hc_url_get(url, HCURL_WHOLE), hc_url_get(url, HCURL_PATH));
		m2b_close(sq3);
	...
	const char *broken, GSList ** result, gboolean answer_beans)
{
	...
	(void) url;
	GRID_TRACE("CONDITIONNED SPARE(%s, %" G_GINT64_FORMAT ", %" G_GINT64_FORMAT
		", %s, %s)", hc_url_get(url, HCURL_WHOLE), count, dist, notin, broken);
...
GError *
meta2_backend_get_spare_chunks(struct meta2_backend_s * m2b,
	...
	const char *snapshot_name)
{
	...
	if (!err) {
		...
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			...
			if (err == NULL) {
				err =
					NEWERROR(CODE_SNAPSHOT_EXISTS,
					"Snapshot '%s' already exists", snapshot_name);
	...
	gpointer dump_hooks_data, struct meta2_dumpv1_hooks_s *dh)
{
	...
	rc = meta2_remote_dumpv1_container(&(wd->peer_cnx), wd->peer_cid,
		(struct meta2_dumpv1_hooks_remote_s *) dh, dump_hooks_data, &e);
	if (!rc)
...
_restore_content(gpointer u, meta2_raw_content_v2_t * p)
{
	...
	GRID_DEBUG("Content converted to %d beans",
		(NULL != beans) ? g_slist_length(beans) : 0);
	...
	if (!(err = m2db_put_alias(&args, beans, NULL, NULL))) {
		...
		meta2_backend_add_modified_container(cb_data->m2b,
			hc_url_get(cb_data->url, HCURL_HEXID), m2db_get_size(cb_data->sq3));
		rc = TRUE;
	}
	*(cb_data->perr) = err;
...
_restore_admin(gpointer u, key_value_pair_t * p)
{
	...
	if (p) {
		if (!skip(p->key)) {
			sqlx_admin_set_gba(cb_data->sq3, p->key,
				metautils_gba_dup(p->value));
		}
...
_restore_property(gpointer u, meta2_property_t * p)
{
	...
	if (p) {
		if (!skip(p->name)) {
			sqlx_admin_set_gba(cb_data->sq3, p->name,
				metautils_gba_dup(p->value));
		}
	...
	gpointer notify_data, struct meta2_restorev1_hooks_s (*notify_hooks))
{
	...
	if (!e) {
		if (sqlx_admin_has(sq3, META2_INIT_FLAG))
			...
			memcpy(&(cb_data.notify_hooks), notify_hooks,
				sizeof(cb_data.notify_hooks));
	...
	gpointer notify_udata, struct meta2_restorev1_hooks_s(*notify_hooks))
{
	...
	addr_info_to_string(peer_addr, wd.peer_addr_str,
		sizeof(wd.peer_addr_str) - 1);
	...
	e = _restore_container(m2b, url, &wd, _dumpv1_wrapper, notify_udata,
		notify_hooks);
...
static void
_meta2_backend_send_container_list_to_m1(gchar * strm1, GSList * cid_list,
	GError ** err)
{
	...
	if (!meta1_remote_update_containers(strm1, cid_list, 4000, err)) {
		if (err && *err) {
			WARN("Request containers update to meta1 [%s] failed :%s", strm1,
				(*err)->message);
		}
		else {
			WARN("Request containers update to meta1 [%s] failed: no error",
				strm1);
		}
	}
}
...
_add_cinfo_to_modified_containers(gpointer _cinfo, gpointer _m2b)
{
	...
	if (g_hash_table_lookup(m2b->modified_containers, strid)) {
		...
		DEBUG
			("Container [%s] is back in modified containers list with a size of %"
			G_GINT64_FORMAT, strid, cinfo->size);
	}
	g_mutex_unlock(m2b->modified_containers_lock);
}

static void
_meta2_backend_process_containers_to_update(gpointer _p_prefix,
	gpointer _cinfo_list, gpointer _m2b)
{
	...
	struct meta2_backend_s *m2b = _m2b;
	const guint nb_meta1 =
		g_slist_length(g_ptr_array_index(m2b->m0_mapping, 0));
	...
	do {
		g_clear_error(&err);
		memcpy(&m1_addr, g_slist_nth_data(m1_list, m1_index_to_pick),
			sizeof(addr_info_t));
		addr_info_to_string(&m1_addr, strm1, sizeof(strm1));
		nb_tries++;
		DEBUG
			("Processing update of container sizes for prefix [%02X%02X] (try %u with meta1 [%s]).",
	...
	container_info_t * p_ci, guint16 prefix)
{
	...
	g_hash_table_insert(containers_ht, g_memdup(&prefix_int,
			sizeof(prefix_int)), cid_list);
}
...
meta2_backend_notify_modified_containers(struct meta2_backend_s *m2b)
{
	...
	void _add_container_to_update(gpointer _strci, gpointer _p_csize,
		gpointer _udata)
	{
		gchar *strci = _strci;
		...
		TRACE("Add container to update id=%s size=%" G_GINT64_FORMAT, strci,
			*p_csize);
	...
	g_hash_table_foreach(containers_to_update,
		_meta2_backend_process_containers_to_update, m2b);
...
meta2_backend_build_meta0_prefix_mapping(struct meta2_backend_s *m2b)
{
	...
	void _meta0_mapping_fill(gpointer d, gpointer u)
	{
		...
		for (j = 0, max = (mapping->prefixes_size - 1); j < max;
			j += sizeof(guint16)) {
	...
	do {
		...
		if (!m0) {
			...
			if (err_local) {
				GRID_WARN
					("Cannot update containers in meta1, failed to get meta0 info from cluster: %s",
					err_local->message);
				g_clear_error(&err_local);
			}
			else {
				GRID_WARN
					("Cannot update containers in meta1, failed to get meta0 info from cluster: no_error");
			}
		}
		else {
			...
			if (!m0_list) {
				...
				if (err_local) {
					GRID_WARN("Cannot get meta1 informations from meta0: %s",
						err_local->message);
					g_clear_error(&err_local);
				}
				else {
					GRID_WARN
						("Cannot get meta1 informations from meta0: no_error");
				}
	...
	if (nb_updates % 65536) {
		ERROR
			("The number of META0 mappings should be a multiple of 65536: %u mappings set.",
			nb_updates);
	}
	else {
...
meta2_backend_get_local_addr(struct meta2_backend_s *m2)
{
	...
	if (em) {
		const struct replication_config_s *emrc =
			election_manager_get_config(em);
		if (emrc) {
