...
_generate_api_alias(const M2V2Bean_t * asn)
{
	...
	ALIASES_set2_content_id(result, asn->alias->contentId.buf,
		asn->alias->contentId.size);
...
_generate_api_header(const M2V2Bean_t * asn)
{
	...
	if (asn->header->policy && asn->header->policy->buf
		&& asn->header->policy->size > 0)
		CONTENTS_HEADERS_set2_policy(result,
			(const char *) asn->header->policy->buf);

	/* hash */
	if (asn->header->hash && asn->header->hash->buf
		&& asn->header->hash->size > 0)
		CONTENTS_HEADERS_set2_hash(result, asn->header->hash->buf,
			asn->header->hash->size);
...
_generate_api_content(const M2V2Bean_t * asn)
{
	...
	CONTENTS_set2_content_id(result, asn->content->contentId.buf,
		asn->content->contentId.size);
...
_header_to_asn(gpointer api, M2V2Bean_t * asn)
{
	struct bean_CONTENTS_HEADERS_s *header =
		(struct bean_CONTENTS_HEADERS_s *) api;
	...
	asn_int64_to_INTEGER(&(asn->header->size),
		CONTENTS_HEADERS_get_size(header));
...
_content_to_asn(gpointer api, M2V2Bean_t * asn)
{
	...
	OCTET_STRING_fromBuf(&(asn->content->contentId),
		(const char *) content_id->data, content_id->len);
	OCTET_STRING_fromBuf(&(asn->content->chunkId), chunk_id->str,
		chunk_id->len);
	OCTET_STRING_fromBuf(&(asn->content->position), position->str,
		position->len);
...
_chunk_to_asn(gpointer api, M2V2Bean_t * asn)
{
	...
	OCTET_STRING_fromBuf(&(asn->chunk->hash), (const char *) hash->data,
		hash->len);
...
_property_to_asn(gpointer api, M2V2Bean_t * asn)
{
	...
	OCTET_STRING_fromBuf(&(asn->prop->aliasName), alias_name->str,
		alias_name->len);

	asn_int64_to_INTEGER(&(asn->prop->aliasVersion),
	...
	OCTET_STRING_fromBuf(&(asn->prop->value), (const char *) val->data,
		val->len);
...
_alias_to_asn(gpointer api, M2V2Bean_t * asn)
{
	...
	asn_int64_to_INTEGER(&(asn->alias->containerVersion),
		ALIASES_get_container_version(alias));

	GByteArray *id = ALIASES_get_content_id(alias);

	OCTET_STRING_fromBuf(&(asn->alias->contentId), (const char *) id->data,
		id->len);
...
_snapshot_to_asn(gpointer api, M2V2Bean_t * asn)
{
	...
	asn_int64_to_INTEGER(&(asn->snapshot->version),
		SNAPSHOTS_get_version(snap));
