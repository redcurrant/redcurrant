	...
	for (l = props; l && l->data; l = l->next) {
		if (!g_ascii_strcasecmp(PROPERTIES_get_key(l->data)->str,
			...
			result = g_byte_array_append(g_byte_array_new(),
				PROPERTIES_get_value(l->data)->data,
				PROPERTIES_get_value(l->data)->len);
...
_fill_chunk_id(chunk_id_t * cid, gpointer chunk)
{
	...
	if (g_strv_length(url_tok) >= 5) {
		...
		if (g_strv_length(tok_addr) == 2) {
			addr_info_t *pa =
				build_addr_info(tok_addr[0], atoi(tok_addr[1]), &e);
			if (NULL == e) {
				memcpy(&(cid->addr), pa, sizeof(addr_info_t));
				hex2bin(url_tok[g_strv_length(url_tok) - 1], cid->id,
					sizeof(cid->id), &e);
			}
			if (NULL != pa)
				g_free(pa);
...
_build_chunk_info_list(GSList * chunks, GSList * contents)
{
	...
	for (l1 = chunks; l1 && l1->data; l1 = l1->next) {
		...
		if (NULL != e) {
			GRID_WARN("Error while converting beans in raw_chunk : %s",
				e->message);
			g_clear_error(&e);
			g_free(ci);
			continue;
		...
		for (l2 = contents; l2 && l2->data; l2 = l2->next) {
			if (0 == g_ascii_strcasecmp(CHUNKS_get_id(l1->data)->str,
					CONTENTS_get_chunk_id(l2->data)->str)) {
				char **pos_tok =
					g_strsplit(CONTENTS_get_position(l2->data)->str, ".", 0);
...
static guint32
_build_raw_chunks_list(GSList * chunks, GSList * contents,
	GSList ** meta2_raw_chunk_lst)
{
	GError *e = NULL;
	guint32 nbChunks = 0;
...
_fill_raw_content_v2_with_alias(meta2_raw_content_v2_t * rc, gpointer alias)
{
	...
	g_byte_array_append(rc->header.system_metadata, (guint8 *) md->str,
		md->len);
}
...
static gpointer
_generate_bean_from_meta2_prop(const char *alias, gint64 alias_version,
...
static gpointer
_generate_m2v2_alias_from_raw_content_v2(GByteArray * id,
	meta2_raw_content_v2_t * rc)
{
	...
	if (!rc->header.system_metadata)
		...
		tmp =
			g_strndup((const char *) rc->header.system_metadata->data,
			rc->header.system_metadata->len);
	ALIASES_set2_mdsys(alias, tmp);
	g_free(tmp);
...
_generate_m2v2_headers(GByteArray * id, gint64 size, GByteArray * sysmd)
{
	...
	if (NULL != (err = storage_policy_from_metadata(sysmd, &polname))) {
		WARN("Possible bad content state, cannot extract correctly storage policy from mdsys (%d) : %s", err->code, err->message);
		g_clear_error(&err);
	}
	if (NULL != polname)
	...
	char *(*make_pos) (guint32, void *), void *udata)
{
	...
	for (l = rc->raw_chunks; l && l->data; l = l->next) {
		beans =
			g_slist_concat(beans, _generate_beans_from_raw_chunk_custom(id_gba,
				(meta2_raw_chunk_t *) l->data, make_pos, udata));
	}

	/*alias */
	beans =
		g_slist_prepend(beans, _generate_m2v2_alias_from_raw_content(id_gba,
			rc));
...
raw_content_from_m2v2_beans(const container_id_t cid, GSList * l)
{
	...
	rc->nb_chunks =
		_build_raw_chunks_list(chunk_beans, content_beans, &(rc->raw_chunks));
...
m2v2_beans_from_raw_content_v2(const char *id, meta2_raw_content_v2_t * rc)
{
	...
	/* headers */
	beans = g_slist_prepend(beans,
		_generate_m2v2_headers(id_gba, rc->header.size,
			rc->header.system_metadata));
	...
	/*alias */
	beans =
		g_slist_prepend(beans, _generate_m2v2_alias_from_raw_content_v2(id_gba,
			rc));

	/* mdusr to props */
	gpointer prop =
		_generate_property_from_mdusr(rc->header.path, rc->header.metadata);
