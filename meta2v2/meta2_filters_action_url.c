...
static int
_update_container_storage_policy(struct gridd_filter_ctx_s *ctx,
	struct meta2_backend_s *m2b, struct hc_url_s *url, const char *stgpol)
{
	GError *e = NULL;
	GByteArray *val =
		g_byte_array_append(g_byte_array_new(), (const guint8 *) stgpol,
		strlen(stgpol));
...
static int
_update_content_storage_policy(struct gridd_filter_ctx_s *ctx,
	struct meta2_backend_s *m2b, struct hc_url_s *url, const char *stgpol)
{
	GError *e = NULL;
	GSList *beans = NULL;
	...
	if (0 == g_ascii_strcasecmp(CONTENTS_HEADERS_get_policy(header)->str,
			stgpol)) {
		_bean_clean(header);
		_bean_clean(alias);
		_bean_cleanl2(beans);
	...
	GHashTable *unpacked =
		metadata_unpack_string(ALIASES_get_mdsys(alias)->str, &e);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if ((!stgpol) || (NULL == (sp = storage_policy_init(&ni, stgpol)))) {
		meta2_filter_ctx_set_error(ctx, NEWERROR(400,
				"Invalid storage policy [%s]", stgpol));
		return FILTER_KO;
	}
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if (hc_url_has(url, HCURL_HEXID)) {
		election_exit(sqlx_repository_get_elections_manager(m2b->repo),
			hc_url_get(url, HCURL_HEXID), "meta2");
	}
	else {
		election_manager_exit_all(sqlx_repository_get_elections_manager(m2b->
				repo), NULL, FALSE);
	}
	return FILTER_OK;
}
