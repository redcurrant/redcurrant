...
_poll_services(struct grid_lbpool_s *lbp, const gchar * srvtype,
	struct lb_next_opt_ext_s *opt_ext, GSList ** result, gboolean use_beans)
{
	...
	GSList ** result, gboolean use_beans)
{
	...
	switch (data_security_get_type(ds)) {
		case RAIN:
			...
			if (!k || !m)
				return NEWERROR(400,
					"Invalid RAIN policy (missing K and/or M)");
...
get_conditioned_spare_chunks(struct grid_lbpool_s * lbp,
	...
	GSList * notin, GSList * broken, GSList ** result, gboolean answer_beans)
{
	struct lb_next_opt_ext_s opt_ext;
...
convert_chunks_to_srvinfo(struct grid_lbpool_s *lbp, GSList * src)
{
	...
	for (GSList * l = src; l; l = l->next) {
		...
		struct service_info_s *si = NULL;
		GError *e =
			service_info_from_chunk_id(lbp, CHUNKS_get_id(l->data)->str, &si);
		if (NULL != e) {
...
get_conditioned_spare_chunks2(struct grid_lbpool_s * lbp,
	...
	GSList * already, GSList * broken, GSList ** result, gboolean answer_beans)
{
	const struct data_security_s *ds = storage_policy_get_data_security(stgpol);
	...
	memset(&opt_ext, 0, sizeof(opt_ext));
	opt_ext.req.max = 0;
	opt_ext.req.distance =
		data_security_get_int64_param(ds, DS_KEY_DISTANCE, 0);
	...
	switch (data_security_get_type(ds)) {
		case DUPLI:
			opt_ext.req.max =
				data_security_get_int64_param(ds, DS_KEY_COPY_COUNT, 1);
			break;
		case RAIN:
	...
	g_slist_free_full(opt_ext.srv_forbidden,
