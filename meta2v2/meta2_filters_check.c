	...
	struct gridd_reply_ctx_s *reply, int optional)
{
	...
	const char *req_ns =
		hc_url_get(meta2_filter_ctx_get_url(ctx), HCURL_NSPHYS);
	...
	struct gridd_reply_ctx_s *reply)
{
	...
	if (!found) {
		meta2_filter_ctx_set_error(ctx, NEWERROR(CODE_NAMESPACE_FULL,
				"VNS full"));
		return FILTER_KO;
	}
	...
	if (strlen(snapshot_name) <= 0) {
		...
		meta2_filter_ctx_set_error(ctx,
			NEWERROR(CODE_BAD_REQUEST, "Invalid snapshot name: '%s' (%s)",
				hc_url_get(url, HCURL_SNAPSHOT), "must not start with digits"));
		return FILTER_KO;
	}
