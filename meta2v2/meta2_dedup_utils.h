...
guint64 substitute_content_header(sqlite3 * db,
	struct bean_CONTENTS_HEADERS_s *new_ch, GSList * old_ch, gboolean dry_run,
	GSList ** impacted_aliases, GError ** err);

void dedup_chunks_of_alias(sqlite3 * db, GString * alias, guint nb_copy,
	GError ** err);
