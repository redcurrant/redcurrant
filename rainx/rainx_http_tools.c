...
rainx_http_req(struct req_params_store * rps)
{
	...
	char *content_hexid =
		apr_pstrdup(local_pool, remote_uri + strlen(full_remote_url));

	char *remote_ip = NULL;
	char *scope_id = NULL;
	apr_port_t remote_port;

	apr_parse_addr_port(&remote_ip, &scope_id, &remote_port, full_remote_url,
		local_pool);
	/* ------- */
	...
	if ((status =
			apr_sockaddr_info_get(&sockaddr, remote_ip, APR_INET, remote_port,
				0, local_pool)) != APR_SUCCESS) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"unable to connect to the rawx %s", full_remote_url);
		return status;
	}

	if ((status =
			apr_socket_create(&sock, sockaddr->family, SOCK_STREAM,
				APR_PROTO_TCP, local_pool)) != APR_SUCCESS) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"unable to create a socket to the rawx %s", full_remote_url);
		return status;
	}

	if ((status = apr_socket_timeout_set(sock, timeout)) != APR_SUCCESS) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"unable to set timeout for the socket to the rawx %s",
			full_remote_url);
		return status;
	}
	...
	if ((status = apr_socket_connect(sock, sockaddr)) != APR_SUCCESS) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"unable to establish the connection to the rawx %s",
			full_remote_url);
		return status;
	}
	/* ------- */
	...
	/* Forging the message */
	char *forged_header =
		apr_psprintf(local_pool, "%s %s HTTP/1.1\nHost: %s", req_type,
		content_hexid, full_remote_url);
	if (header)
		forged_header =
			apr_psprintf(local_pool, "%s\n%s", forged_header, header);
	if (data)
		forged_header =
			apr_psprintf(local_pool, "%s\nContent-Length: %d\n\n",
	...
	while (remaining_to_send > 0) {
		...
		if ((status =
				apr_socket_send(sock, ptr_start,
					&send_buffer_size)) != APR_SUCCESS) {
			DAV_DEBUG_REQ(resource->info->request, 0,
				"failed to send the %s request to the rawx %s", req_type,
				full_remote_url);
	...
	DAV_DEBUG_REQ(resource->info->request, 0,
		"%s request to the rawx %s sent for the content %s", req_type,
		full_remote_url, content_hexid);
	/* ------ */
	...
	do {
		...
		if (status == APR_EOF || (status == APR_SUCCESS && !is_get) ||
			(reply_size == 0) || total_replied_size >= total_size) {
			break;
		}
