...
rainx_stats_rrd_create(apr_pool_t * pool, time_t period)
{
	...
	result =
		apr_palloc(pool,
		sizeof(struct rainx_stats_rrd_s) + (period * (sizeof(apr_uint64_t))));
	result->last = time(0);
...
rainx_stats_rrd_dup(apr_pool_t * pool, struct rainx_stats_rrd_s *rrd)
{
	...
	result =
		apr_palloc(pool,
		sizeof(struct rainx_stats_rrd_s) +
		(rrd->period * (sizeof(apr_uint64_t))));
...
static void
_rsr_blank_empty_slots(struct rainx_stats_rrd_s *rsr, apr_uint64_t v,
	time_t now)
{
	time_t i;
...
_dump_rrd(struct rainx_stats_rrd_s *rsr, apr_pool_t * p)
{
	return apr_psprintf(p,
		"[\"last\": \"%" G_GUINT64_FORMAT "\", period\": \"%" G_GUINT64_FORMAT
		"\", ten\": %s] ", rsr->last, rsr->period,
		rainx_stats_rrd_dump_values(rsr, p));
}

struct delta_debug_s *
rainx_stats_rrd_debug_get_delta(struct rainx_stats_rrd_s *rsr, apr_pool_t * p,
	time_t period)
{
	time_t now;
	...
	result =
		apr_psprintf(p, "%s | period : %" G_GUINT64_FORMAT " ", _dump_rrd(rsr,
			p), period);
	...
	apr_uint64_t op =
		rsr->ten[now % rsr->period] - rsr->ten[(now - period) % rsr->period];
	...
	result = apr_psprintf(p, "%s| now : %" G_GUINT64_FORMAT " after bs: %s"
		...
		"] - " "rsr->ten[(%" G_GUINT64_FORMAT "-%" G_GUINT64_FORMAT ") mod %"
		G_GUINT64_FORMAT "] " "(rsr->ten[%" G_GUINT64_FORMAT "] - rsr->ten[%"
		G_GUINT64_FORMAT "] " "(%" G_GUINT64_FORMAT " - %" G_GUINT64_FORMAT
		" (%" G_GUINT64_FORMAT ")))", result, now, _dump_rrd(rsr, p), now,
		rsr->period, now, period, rsr->period, nrp, nprp, rsr->ten[nrp],
		rsr->ten[nprp], op);
