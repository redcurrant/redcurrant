...
_get_compression_block_size(apr_pool_t * p, namespace_info_t * ns_info)
{
	...
	apr_snprintf(bsize_buff, sizeof(bsize_buff), "%.*s", i_len,
		((char *) bsize->data));
...
_get_acl(apr_pool_t * p, namespace_info_t * ns_info)
{
	...
	acl =
		g_slist_concat(parse_acl(acl_allow, TRUE), parse_acl(acl_deny, FALSE));
	if (!acl)
		return NULL;
...
gboolean
update_rainx_conf(apr_pool_t * p, rawx_conf_t ** rainx_conf,
	const gchar * ns_name)
{
