...
__gen_stats(const dav_resource * resource, apr_pool_t * pool)
{
	...
	apr_uint64_t reqavgtime =
		rainx_stats_rrd_get_delta(&(stats.rrd_duration), 4);
	apr_uint64_t req_put =
		rainx_stats_rrd_get_delta(&(stats.rrd_req_put_sec), 4);
	apr_uint64_t reqavgtime_put =
		rainx_stats_rrd_get_delta(&(stats.rrd_put_duration), 4);
	apr_uint64_t req_get =
		rainx_stats_rrd_get_delta(&(stats.rrd_req_get_sec), 4);
	apr_uint64_t reqavgtime_get =
		rainx_stats_rrd_get_delta(&(stats.rrd_get_duration), 4);
	apr_uint64_t req_del =
		rainx_stats_rrd_get_delta(&(stats.rrd_req_del_sec), 4);
	apr_uint64_t reqavgtime_del =
		rainx_stats_rrd_get_delta(&(stats.rrd_del_duration), 4);
	...
	return apr_pstrcat(pool,
		...
		apr_psprintf(pool, "rainx.avputreqtime %" APR_UINT64_T_FMT "\n",
			...
			"rainx.avgetreqtime %" APR_UINT64_T_FMT "\n", r_get_time),
...
dav_error *
dav_rainx_stat_get_resource(request_rec * r, const char *root_dir,
	const char *label, int use_checked_in, dav_resource ** result_resource)
{
	...
	*result_resource =
		__build_req_resource(r, &dav_hooks_repository_rainxstat, __gen_stats);
	(*result_resource)->info->type = STAT;
	return NULL;
}

dav_error *
dav_rainx_info_get_resource(request_rec * r, const char *root_dir,
	const char *label, int use_checked_in, dav_resource ** result_resource)
{
	(void) root_dir;
	(void) label;
	...
	*result_resource =
		__build_req_resource(r, &dav_hooks_repository_rainxinfo, __gen_info);
...
static dav_error *
dav_rainx_get_parent_resource_SPECIAL(const dav_resource * resource,
...
static int
dav_rainx_is_same_resource_SPECIAL(const dav_resource * res1,
...
static int
dav_rainx_is_parent_resource_SPECIAL(const dav_resource * res1,
	const dav_resource * res2)
{
	DAV_XDEBUG_RES(res1, 0, "%s(...)", __FUNCTION__);
