...
dav_rainx_create_server_config(apr_pool_t * p, server_rec * s)
{
	...
	apr_snprintf(conf->lock.path, sizeof(conf->lock.path),
		"/var/run/httpd-lock.%d", getpid());
	apr_snprintf(conf->shm.path, sizeof(conf->shm.path),
		"/var/run/httpd-shm.%d", getpid());
...
static const char *
dav_rainx_cmd_gridconfig_hash_width(cmd_parms * cmd, void *config,
	const char *arg1)
{
...
static const char *
dav_rainx_cmd_gridconfig_hash_depth(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rainx_server_conf *conf;
...
static const char *
dav_rainx_cmd_gridconfig_docroot(cmd_parms * cmd, void *config,
	const char *arg1)
{
	apr_finfo_t finfo;
	dav_rainx_server_conf *conf;
	...
	do {
		...
		if (finfo.filetype != APR_DIR) {
			DAV_DEBUG_POOL(cmd->temp_pool, 0,
				"Docroot for GridStorage chunks must be a directory : %s",
				arg1);
			return apr_pstrcat(cmd->temp_pool,
...
static const char *
dav_rainx_cmd_gridconfig_namespace(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rainx_server_conf *conf;
...
static const char *
dav_rainx_cmd_gridconfig_headers(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rainx_server_conf *conf;
...
static const char *
dav_rainx_cmd_gridconfig_upblock(cmd_parms * cmd, void *config,
	const char *arg1)
{
	dav_rainx_server_conf *conf;
...
static int
rainx_hook_post_config(apr_pool_t * pconf, apr_pool_t * plog,
	apr_pool_t * ptemp, server_rec * server)
{
	...
	for (s = server; s; s = s->next) {
		...
		for (a = s->addrs; a; a = a->next) {
			...
			host = NULL;
			status =
				apr_getnameinfo(&host, a->host_addr,
	...
	if (APR_SUCCESS != status) {
		...
		apr_pool_userdata_set(conf, apr_psprintf(pconf,
				"RAINX-config-to-be-cleaned-%d", i++),
...
static const command_rec dav_rainx_cmds[] = {
	AP_INIT_TAKE1("grid_hash_width", dav_rainx_cmd_gridconfig_hash_width, NULL,
		RSRC_CONF, "hash width on a chunk's name"),
	AP_INIT_TAKE1("grid_hash_depth", dav_rainx_cmd_gridconfig_hash_depth, NULL,
		RSRC_CONF, "hash depth on a chunk's name"),
	AP_INIT_TAKE1("grid_docroot", dav_rainx_cmd_gridconfig_docroot, NULL,
		RSRC_CONF, "chunks docroot"),
	AP_INIT_TAKE1("grid_namespace", dav_rainx_cmd_gridconfig_namespace, NULL,
		RSRC_CONF, "namespace name"),
	AP_INIT_TAKE1("grid_dir_run", dav_rainx_cmd_gridconfig_dirrun, NULL,
		RSRC_CONF, "run directory"),
	AP_INIT_TAKE1("grid_fsync", dav_rainx_cmd_gridconfig_fsync, NULL, RSRC_CONF,
		"do fsync on file close"),
	AP_INIT_TAKE1("grid_headers", dav_rainx_cmd_gridconfig_headers, NULL,
		RSRC_CONF, "which header scheme to adopt (1, 2, both)"),
	AP_INIT_TAKE1("grid_acl", dav_rainx_cmd_gridconfig_acl, NULL, RSRC_CONF,
		"enabled acl"),
	AP_INIT_TAKE1("grid_upload_blocksize", dav_rainx_cmd_gridconfig_upblock,
