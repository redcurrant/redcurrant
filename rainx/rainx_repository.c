	...
	dav_resource ** result_resource)
{
	...
	if (0 == apr_strnatcasecmp(src, "info")) {
		return dav_rainx_info_get_resource(req, root_dir, label, use_checked_in,
			result_resource);
	}

	if (0 == apr_strnatcasecmp(src, "stat")) {
		return dav_rainx_stat_get_resource(req, root_dir, label, use_checked_in,
			result_resource);
	}
...
static void
__load_one_header(request_rec * request, apr_uint32_t headers, const char *name,
	char **dst)
{
	const char *value;
	...
	if (headers & HEADER_SCHEME_V2) {
		...
		if (NULL != (value = apr_table_get(request->headers_in, new_name))) {
			*dst = apr_pstrdup(request->pool, value);
			DAV_XDEBUG_REQ(request, 0, "Header found [%s]:[%s]", new_name,
				*dst);
		}
	}
...
request_load_chunk_info(request_rec * request, dav_resource * resource)
{
	dav_rainx_server_conf *conf =
		ap_get_module_config(resource->info->request->server->module_config,
		&dav_rainx_module);
	...
	int use_checked_in, dav_resource ** result_resource)
{
	...
	*result_resource = NULL;
	...
	e = rainx_repo_check_request(r, root_dir, label, use_checked_in, &ctx,
		result_resource);
...
static int
dav_rainx_is_parent_resource(const dav_resource * res1,
...
rainx_repo_stream_create(const dav_resource * resource, dav_stream ** result)
{
	...
	if (str == NULL
		|| NULL == (sp = storage_policy_init(conf->rainx_conf->ni, str))) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"\"%s\" policy init failed for namespace \"%s\"", str,
			conf->rainx_conf->ni->name);
	...
	DAV_DEBUG_REQ(resource->info->request, 0,
		"calculated chunk size is %d bytes", resource->info->metachunk_size);
	/* ------- */
	...
	for (i = 0; temp_tok != NULL && i < resource->info->k + resource->info->m;
		i++) {
	...
	dav_stream ** stream)
{
	...
	*stream = ds;
	...
	data_put_params =
		(struct req_params_store **) apr_pcalloc(resource->info->request->pool,
...
{
	char *reply =
		apr_pcalloc(stream->r->info->request->pool,
...
static void
update_response_list(dav_stream * stream, char *rawx_entry, int stored_size,
	char *md5_digest)
{
	char *response_entry =
		apr_psprintf(stream->r->info->request->pool, "%s%s%d%s%s", rawx_entry,
	...
	if (NULL == stream->r->info->response_chunk_list)
		...
		stream->r->info->response_chunk_list =
			apr_psprintf(stream->r->info->request->pool, "%s%s%s",
			stream->r->info->response_chunk_list, RAWXLIST_SEPARATOR2,
			response_entry);
}
...
dav_rainx_close_stream(dav_stream * stream, int commit)
{
	...
	dav_rainx_server_conf *conf =
		ap_get_module_config(stream->r->info->request->server->module_config,
		&dav_rainx_module);
	...
	if (!commit) {
		...
		char *custom_header =
			apr_psprintf(stream->r->info->request->pool,
			"containerid: %s\nchunknb: %s\ncontentpath: %s\ncontentsize: %s",
			temp_content.container_id, temp_content.chunk_nb, temp_content.path,
			temp_content.size);
		...
		int startid =
			strlen(stream->r->info->rawx_list[stream->r->info->current_rawx]) -
			64;
		custom_chunkid =
			apr_pstrdup(stream->r->info->request->pool,
			stream->r->info->rawx_list[stream->r->info->current_rawx] +
			startid);
		custom_chunkpos =
			apr_psprintf(stream->r->info->request->pool, "%s.%d",
			temp_chunk.position, stream->r->info->current_rawx);
		custom_chunksize =
			apr_itoa(stream->r->info->request->pool,
			metachunk_size - stream->r->info->current_chunk_remaining);
		custom_chunkhash =
		...
		if (stream->original_data_stored > stream->original_data_size) {
			DAV_DEBUG_REQ(stream->r->info->request, 0,
				"request entity too large");
			e = server_create_and_stat_error(conf, stream->p, HTTP_BAD_REQUEST,
		...
		if (metachunk_size - stream->r->info->current_chunk_remaining > 0) {
			...
			data_put_params[i] =
				(struct req_params_store *) apr_pcalloc(subpool,
				sizeof(struct req_params_store));
			...
			data_put_params[i]->reply =
				apr_pcalloc(subpool, MAX_REPLY_HEADER_SIZE + REPLY_BUFFER_SIZE);
		...
		for (i = 0; i < stream->r->info->k; i++) {
			if (data_put_params[i] == NULL) {
				...
				if (!extract_code_message_reply(stream->r,
					...
					DAV_DEBUG_REQ(stream->r->info->request, 0, "error while putting the data to the rawx %d: (%d) %s", i, data_put_params[i]->req_status, data_put_params[i]->reply);	// FIXME: call apr_strerror
					e = server_create_and_stat_error(conf, stream->p,
						HTTP_INTERNAL_SERVER_ERROR, 0,
		...
		if (NULL == (coding_metachunks =
				rain_get_coding_chunks(stream->original_data,
			...
			coding_put_params =
				(struct req_params_store **) apr_pcalloc(stream->r->info->
				request->pool,
				stream->r->info->m * sizeof(struct req_params_store *));
			coding_subpools =
				(apr_pool_t **) apr_pcalloc(stream->r->info->request->pool,
			...
			for (i = 0; i < stream->r->info->m; i++) {
				...
				startid =
					strlen(stream->r->info->rawx_list[stream->r->info->
						current_rawx]) - 64;
				custom_chunkid =
					apr_pstrdup(stream->r->info->request->pool,
					stream->r->info->rawx_list[stream->r->info->current_rawx] +
					startid);
				custom_chunkpos =
					apr_psprintf(stream->r->info->request->pool, "%s.p%d",
					temp_chunk.position,
					stream->r->info->current_rawx - stream->r->info->k);
				custom_chunksize =
					apr_itoa(stream->r->info->request->pool, metachunk_size);
				custom_chunkhash =
					g_compute_checksum_for_string(G_CHECKSUM_MD5,
					coding_metachunks[i], metachunk_size);
				apr_pool_create(&(coding_subpools[i]), mp);
				/* ------- */

				/* Initializing the PUT params structure */
				coding_put_params[i] =
					(struct req_params_store *) apr_pcalloc(coding_subpools[i],
					sizeof(struct req_params_store));
				...
				coding_put_params[i]->header =
					apr_psprintf(coding_subpools[i],
					"%s\nchunkid: %s\nchunkpos: %s\nchunksize: %s\nchunkhash: %s",
					custom_header, custom_chunkid, custom_chunkpos,
					custom_chunksize, custom_chunkhash);
				coding_put_params[i]->req_type = "PUT";
				coding_put_params[i]->reply =
					apr_pcalloc(coding_subpools[i],
					MAX_REPLY_HEADER_SIZE + REPLY_BUFFER_SIZE);
				...
				update_response_list(stream,
					stream->r->info->rawx_list[stream->r->info->current_rawx],
					metachunk_size, custom_chunkhash);
			...
			for (i = 0; i < stream->r->info->m; i++) {
				if (coding_put_params[i]->req_status != APR_SUCCESS) {
					if (FALSE == extract_code_message_reply(stream->r,
							coding_put_params[i]->reply, &reply_code,
							&reply_message)) {
						DAV_DEBUG_REQ(stream->r->info->request, 0,
...
dav_rainx_write_stream(dav_stream * stream, const void *buf, apr_size_t bufsize)
{
	...
	if (stream->original_data_stored + (int) bufsize >
		stream->original_data_size) {
		/* Rollback */
		DAV_DEBUG_REQ(stream->r->info->request, 0, "request entity too large");
		return server_create_and_stat_error(conf, stream->p, HTTP_BAD_REQUEST,
			0, "Request entity too large");
	}
	...
	while (to_read > 0) {
		if (stream->r->info->current_chunk_remaining < to_read) {
			...
			char *custom_chunkid =
				...
				stream->r->info->rawx_list[stream->r->info->current_rawx] +
				startid);
			char *custom_chunkpos =
				apr_psprintf(stream->r->info->request->pool, "%s.%d",
				temp_chunk.position, stream->r->info->current_rawx);
			char *custom_chunksize =
				apr_itoa(stream->r->info->request->pool, metachunk_size);
			char *custom_chunkhash =
				g_compute_checksum_for_string(G_CHECKSUM_MD5,
				stream->original_data_chunk_start_ptr, metachunk_size);
			/* ------- */
			...
			data_put_params[i] =
				(struct req_params_store *) apr_pcalloc(data_subpools[i],
				sizeof(struct req_params_store));
			data_put_params[i]->service_address =
				stream->r->info->rawx_list[stream->r->info->current_rawx];
			data_put_params[i]->data_to_send =
			...
			data_put_params[i]->header =
				apr_psprintf(data_subpools[i],
				"%s\nchunkid: %s\nchunkpos: %s\nchunksize: %s\nchunkhash: %s",
				custom_header, custom_chunkid, custom_chunkpos,
				custom_chunksize, custom_chunkhash);
			data_put_params[i]->req_type = "PUT";
			data_put_params[i]->reply =
				apr_pcalloc(data_subpools[i],
				MAX_REPLY_HEADER_SIZE + REPLY_BUFFER_SIZE);
			data_put_params[i]->resource = stream->r;
			data_put_params[i]->req_status = INIT_REQ_STATUS;	/* APR_SUCCESS will set it to 0 */
	...
	const dav_resource * resource)
{
	...
	char *content_length_str =
		apr_pstrndup(resource->info->request->pool, ptr_start,
		ptr_end - ptr_start);
...
dav_rainx_deliver(const dav_resource * resource, ap_filter_t * output)
{
	...
	if (str == NULL
		|| (sp = storage_policy_init(conf->rainx_conf->ni, str)) == NULL) {
		DAV_DEBUG_REQ(resource->info->request, 0,
			"\"%s\" policy init failed for namespace \"%s\"", str,
			conf->rainx_conf->ni->name);
	...
	apr_pool_cleanup_register(pool, sp, apr_storage_policy_clean,
		apr_pool_cleanup_null);
	...
	if ((NULL == (orig_k = data_security_get_param(datasec, "k")))
		...
		DAV_DEBUG_REQ(resource->info->request, 0,
			"failed to get all the \"%s\" policy parameters", str);
		e = server_create_and_stat_error(conf, pool, HTTP_INTERNAL_SERVER_ERROR,
			0, "Rain operation failed on loading policy");
		goto end_deliver;
	}
	...
	if (NULL == resource->info->content.spare_rawx_list) {
		...
		e = server_create_and_stat_error(conf, pool, HTTP_BAD_REQUEST, 0,
			"Bad spare rawx list parameter");
		goto end_deliver;
	}
	/* The array showing if a rawx is a failure */
	gboolean *failure_array =
		(gboolean *) apr_pcalloc(resource->info->request->pool,
		(k + m) * sizeof(gboolean));

	/* The array containing the spared rawx addresses */
	char **spare_rawx_list =
		(char **) apr_pcalloc(resource->info->request->pool, (k + m)
		* sizeof(char *));
	...
	temp_tok =
		apr_strtok(resource->info->content.spare_rawx_list, RAWXLIST_SEPARATOR2,
		&last);
	...
	while (temp_tok) {
		...
		if (!temp_tok2) {
			DAV_DEBUG_REQ(resource->info->request, 0,
				"bad spare rawx address(es) format");
			e = server_create_and_stat_error(conf, pool, HTTP_BAD_REQUEST, 0,
				"Bad spare rawx address(es) format");
			goto end_deliver;
		}
	...
	DAV_DEBUG_REQ(resource->info->request, 0,
		"calculated metachunk size is %d bytes", metachunk_size);
	/* ------- */

	/* Creating data strips */
	char **datachunks =
		(char **) apr_pcalloc(resource->info->request->pool,
		k * sizeof(char *));
	char *reply_code = apr_pcalloc(resource->info->request->pool, 4);
	char *reply_message =
		apr_pcalloc(resource->info->request->pool, MAX_REPLY_MESSAGE_SIZE);
	reply =
		apr_palloc(resource->info->request->pool,
		MAX_REPLY_HEADER_SIZE + metachunk_size);
	struct req_params_store rps;
	...
	for (i = 0; i < data_rawx_list_size; i++) {
		if (failure_array[i] == FALSE) {
			...
			if (FALSE == extract_code_message_reply(resource, reply,
					&reply_code, &reply_message)
				|| FALSE == g_str_has_prefix(reply_code, "20")) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"unexpected failure on data rawx %d : %s", i, reply);
				e = server_create_and_stat_error(conf, pool, HTTP_BAD_REQUEST,
					0, "Unexpected failure on data rawx");
				goto end_deliver;
			}
			DAV_DEBUG_REQ(resource->info->request, 0,
				"got the data chunk from the rawx %d", i);
			/* ------- */

			int lcont =
				extract_content_from_reply(datachunks + i, reply, resource);
			if (-1 == lcont) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"problem occured while extracting the content got from the data rawx %d",
					i);
	...
	for (i = 0; i < m; i++) {
		if (failure_array[k + i] == FALSE) {
			...
			DAV_DEBUG_REQ(resource->info->request, 0,
				"got the coding chunk from the rawx %d", i);
			/* ------- */

			int lcont =
				extract_content_from_reply(codingchunks + i, reply, resource);
			if (-1 == lcont) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"problem occured while extracting the content got from the coding rawx %d",
					i);
				e = server_create_and_stat_error(conf, pool,
					HTTP_INTERNAL_SERVER_ERROR, 0,
					"Problem occured while extracting the content got from a coding rawx");
				goto end_deliver;
			}
		}
		else
	...
	reconstructed_data =
		rain_repair_and_get_raw_data(datachunks, codingchunks, chunk_size, k, m,
		algo);
	...
	for (i = 0; i < k; i++) {
		if (failure_array[i] == TRUE) {
			...
			char *custom_chunkhash =
				g_compute_checksum_for_string(G_CHECKSUM_MD5, datachunks[i],
				data_to_hash_size);
	...
	for (i = 0; i < m; i++) {
		if (failure_array[k + i] == TRUE) {
			char *custom_chunkhash =
				g_compute_checksum_for_string(G_CHECKSUM_MD5, codingchunks[i],
				metachunk_size);
	...
	for (i = 0; i < data_rawx_list_size; i++) {
		...
		if (failure_array[i] == TRUE) {
			...
			char *custom_chunkid =
				apr_pstrdup(resource->info->request->pool,
				spare_rawx_list[cur_spare_rawx] + startid);
			char *custom_chunkpos =
				apr_psprintf(resource->info->request->pool, "%s.%d",
				temp_chunk.position, i);
			char *custom_chunksize =
				apr_itoa(resource->info->request->pool, byte_count_to_send);
			char *custom_chunkhash =
				g_compute_checksum_for_string(G_CHECKSUM_MD5, datachunks[i],
				byte_count_to_send);
			char *custom_header2 =
				apr_psprintf(resource->info->request->pool,
				"%s\nchunkid: %s\nchunkpos: %s\nchunksize: %s\nchunkhash: %s",
				custom_header, custom_chunkid, custom_chunkpos,
				custom_chunksize, custom_chunkhash);
			/* ------- */
			...
			if (FALSE == extract_code_message_reply(resource, reply,
					&reply_code, &reply_message)) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"error while putting the reconstructed data to the rawx %d : %s",
					i, reply);
				e = server_create_and_stat_error(conf, pool,
					HTTP_INTERNAL_SERVER_ERROR, 0,
					"Rain operation failed on put");
			...
			if (FALSE == g_str_has_prefix(reply_code, "20")) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"error while putting the reconstructed data to the rawx %d : %s",
					i, reply);
				e = server_create_and_stat_error(conf, pool, atoi(reply_code),
					0, reply_message);
				if (custom_chunkhash) {
					g_free(custom_chunkhash);
					custom_chunkhash = NULL;
	...
	for (i = 0; i < m; i++) {
		if (failure_array[k + i] == TRUE) {
			...
			char *custom_chunkid =
				...
				spare_rawx_list[cur_spare_rawx] + startid);
			char *custom_chunkpos =
				apr_psprintf(resource->info->request->pool, "%s.p%d",
				temp_chunk.position, i);
			char *custom_chunksize =
				apr_itoa(resource->info->request->pool, metachunk_size);
			char *custom_chunkhash =
				g_compute_checksum_for_string(G_CHECKSUM_MD5, codingchunks[i],
				metachunk_size);
			char *custom_header2 =
			...
			if (FALSE == extract_code_message_reply(resource, reply,
					&reply_code, &reply_message)) {
				DAV_DEBUG_REQ(resource->info->request, 0, "error while putting the reconstructed coding to the rawx %d: (%d) %s", i, req_status, reply);	// FIXME: use apr_strerror() to get a clear message
				e = server_create_and_stat_error(conf, pool,
					HTTP_INTERNAL_SERVER_ERROR, 0,
					"Rain operation failed on put");
				if (custom_chunkhash) {
			...
			if (FALSE == g_str_has_prefix(reply_code, "20")) {
				DAV_DEBUG_REQ(resource->info->request, 0,
					"error while putting the reconstructed coding to the rawx %d : %s",
					i, reply);
				e = server_create_and_stat_error(conf, pool, atoi(reply_code),
					0, reply_message);
				if (custom_chunkhash) {
	...
	apr_bucket_brigade *bb =
		apr_brigade_create(resource->info->request->pool,
...
dav_rainx_getetag(const dav_resource * resource)
{
	...
	if (!resource->exists) {
		DAV_DEBUG_RES(resource, 0, "%s : resource not found", __FUNCTION__);
...
static dav_error *
dav_rainx_walk(const dav_walk_params * params, int depth,
