...
_is_meta2v2(const gchar * ns_name, GSList * meta2, const char *url)
{
	...
	if (url_split && g_strv_length(url_split) == 2 &&
		...
		if (si_found && si_found->data) {
			...
			result =
				(0 ==
				g_ascii_strcasecmp(service_info_get_tag_value(si_found->data,
						"tag.type", "m2v1"), "m2v2"));
		}
	}
...
_disable_container(struct xcid_s *scid, struct xaddr_s *addr)
{
	...
	for (attempts = 5;; --attempts) {
		...
		if (meta2_remote_container_disable_disabled(&(addr->cnx), scid->cid,
				&err)) {
			DEBUG("Disabled the source container, (%d attempts remaining) ...",
				attempts);
			break;
		}
		...
		ERROR("Failed to disable the source container: (%d) %s", code,
...
_enable_container(struct xcid_s *scid, struct xaddr_s *addr)
{
	...
	for (attempts = 5;; --attempts) {
		...
		code = gerror_get_code(err);
		WARN("Enabling error on %s: (%d) %s", addr->str, code,
			gerror_get_message(err));
		if (err)
			g_clear_error(&err);
	}
	...
	const gchar * meta2_addr, gboolean forcing_ip)
{
	...
	gboolean _poll(void)
	{
		...
		DEBUG("Polling the meta2 with specified tag \"%s\" selection "
			"succeeded with %u results", meta2_addr, g_strv_length(result));
	...
	GSList * meta2, struct xcid_s *scid, struct cid_move_s *move)
{
	...
	if (!scid->location->m2_url || !scid->location->m2_url[0]) {
		return GS_ERROR_NEW(CODE_CONTAINER_NOTFOUND,
			"No meta2 for this container");
	}
	...
	do {
		...
		result = meta1v2_remote_list_reference_services(&(move->m1.addr), &err,
			gs_get_namespace(ns_client), scid->cid, "meta2", to_step, to_all);
		if (!result) {
			if (!err)
			...
			if (!result[0]) {
				...
				move->v2 =
					_is_meta2v2(gs_get_namespace(ns_client), meta2,
					move->src_url->host);
				if (err != NULL)
					GS_ERROR_STACK(&err);
			}
	...
	struct xcid_s *scid, struct cid_move_s *move)
{
	...
	for (attempts = 5;; --attempts) {
		...
		if (code < 100) {		/* network error */
			...
			DEBUG("Rollback, Destroy created destination container on [%s]",
				move->dst.str);
			addr_info_t *m2addr = addr_info_from_service_str(move->dst.str);

			if (m2addr) {
				meta2_remote_container_destroy(m2addr, 3000, &local_error,
					scid->cid);
				if (local_error) {
					ERROR
						("Failed to destroy created container during rollback [%s] on [%s]",
	...
	const gchar * meta2_addr)
{
	...
	if (!(err = meta2_mover_locate_source(ns_client, meta2, scid, &move))) {
		...
		if (NULL != meta2_addr && move.v2 && full_url
			...
			err =
				GS_ERROR_NEW(0,
				"Source meta2 is identified as v2, but target"
