	...
	if (err) {
		...
		if ((strstr(err->message, strerror(ENOENT)) != NULL)) {
			...
			if (!err) {
				...
				err =
					sqlx_repository_open_and_lock(m0->repository,
					META0_TYPE_NAME, m0->id, SQLX_OPEN_LOCAL, &oldhandle, NULL);
				if (!err) {
					GRID_INFO("Start Migrate meta0 database");
					err = sqlx_repository_backup_base(oldhandle, handle);
					if (err) {
						GRID_ERROR("Failed to migrate meta0 database : (%d) %s",
						...
						rc = sqlite3_exec(handle->db, META0_SCHEMA, NULL, NULL,
							&errmsg);
						if (rc != SQLITE_OK && rc != SQLITE_DONE) {
							GRID_WARN("Failed to apply schema (%d) %s %s", rc,
								sqlite3_errmsg(handle->db), errmsg);
						}
						if (errmsg != NULL)
							g_free(errmsg);
				...
				GRID_ERROR("Failed to create meta0 database :(%d) %s",
					err->code, err->message);
			}
		}
	}
...
_load_meta1ref_from_base(struct sqlx_sqlite3_s *sq3, GPtrArray ** result)
{
	...
	sqlite3_prepare_debug(rc, sq3->db,
		"SELECT addr,state,prefixes FROM meta1_ref", -1, &stmt, NULL);
	if (rc != SQLITE_OK && rc != SQLITE_DONE) {
		if (rc == SQLITE_ERROR) {
	...
	for (;;) {
		...
		if (rc == SQLITE_ROW) {
			...
			g_ptr_array_add(array, meta0_utils_pack_meta1ref((gchar *) url,
					(gchar *) ref, (gchar *) prefix_nb));
			count++;
		}
		else if (rc == SQLITE_DONE || rc == SQLITE_OK)
			break;
	...
	struct sqlx_sqlite3_s **handle)
{
	...
	if (how & M0V2_OPENBASE_MASTERSLAVE || how & M0V2_OPENBASE_MASTERONLY) {
		err =
			sqlx_repository_status_base(m0->repository, META0_TYPE_NAME,
			m0->ns);
		if (NULL != err) {
			if (err->code == CODE_REDIRECT) {
				if (how & M0V2_OPENBASE_MASTERONLY)
					return err;
				g_clear_error(&err);
			}
			else {
				GRID_TRACE("STATUS error [%s.%s]: (%d) %s",
					m0->id, META0_TYPE_NAME, err->code, err->message);
				return err;
			}
		}
	}
	...
	gchar *** u)
{
	...
	GRID_TRACE("%s(%p,%02X%02X,%p)", __FUNCTION__, m0, prefix[0], prefix[1], u);
...
GError *
meta0_backend_get_all_meta1_ref(struct meta0_backend_s * m0,
	GPtrArray ** result)
{
	GError *err;
		...
		if (cmpaddr == 0) {
			if (cmpstate != 0)
				return NEWERROR(EINVAL,
					"meta1 always available to prefix allocation");
			err = _open_and_lock(m0, M0V2_OPENBASE_MASTERONLY, &sq3);
			if (NULL != err)
				return err;
