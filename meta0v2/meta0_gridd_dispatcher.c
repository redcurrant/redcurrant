...
extract_nocheck(struct message_s *msg)
{
	...
	if (0 >= message_get_field(msg, "NOCHECK", sizeof("NOCHECK") - 1, &f,
			&f_size, NULL)) {
		return TRUE;
	}
	...
	struct meta0_disp_s *m0disp, gpointer ignored)
{
	...
	err =
		message_extract_string(reply->request, "METAURL", meta1url,
		sizeof(meta1url));
	if (err != NULL) {
		reply->send_error(400, err);
		return TRUE;
	...
	struct meta0_disp_s *m0disp, gpointer ignored)
{
	...
	err =
		message_extract_string(reply->request, "METAURL", meta0url,
		sizeof(meta0url));
	if (err != NULL) {
		reply->send_error(400, err);
		return TRUE;
...
meta0_gridd_get_requests(void)
{
	static struct gridd_request_descr_s descriptions[] = {
		...
		{"REQ_M0_V2_ASSIGN_PREFIX", (hook) meta0_dispatch_v2_ASSIGN_PREFIX,
				NULL},
		{"REQ_M0_V2_DISABLE_META1", (hook) meta0_dispatch_v2_DISABLE_META1,
				NULL},
		{"REQ_M0_V2_FILL", (hook) meta0_dispatch_v2_FILL, NULL},
		{"REQ_M0_V2_GETMETA1INFO", (hook) meta0_dispatch_v2_META1_INFO, NULL},
		{"REQ_M0_V2_DESTROY_META1REF",
				(hook) meta0_dispatch_v2_DESTROY_META1REF, NULL},
		{"REQ_M0_V2_DESTROY_META0ZKNODE",
				(hook) meta0_dispatch_v2_DESTROY_ZKNODE, NULL},
		{NULL, NULL, NULL}
	};
