...
_unpack_meta1ref(gchar * s_meta1ref)
{
	...
	aM1->used =
		(g_ascii_strtoll(split_result[1], NULL, 10) == 0) ? FALSE : TRUE;
...
_pack_meta1ref(struct meta0_assign_meta1_s *m1ref)
{
	...
	gchar *result =
		meta0_utils_pack_meta1ref(m1ref->addr, (m1ref->used ? "1" : "0"), nb);
	g_free(nb);
	return result;
}
...
static struct meta0_assign_meta1_s *
_select_source_assign_m1(GList * lst, guint8 * treat_prefixes,
	const guint avgscore)
{
	if (lst == NULL)
		return NULL;
...
static struct meta0_assign_meta1_s *
_select_dest_assign_m1(GList * lst, const struct meta0_assign_meta1_s *s_aM1,
	guint8 * prefixe, gboolean unref, gboolean force)
{
	...
	gboolean loop = TRUE;
	gchar *shost = NULL, *dhost = NULL, *sport = NULL, *dport = NULL, *host =
		NULL, *port = NULL;
	guint i, len;
	...
	do {
		if (d_aM1 == NULL || (d_aM1->score >= avgscore && !unref)) {
			...
			gchar **urls =
				meta0_utils_array_get_urlv(context->array_meta1_by_prefix,
				prefix);
			if (urls) {
				...
				for (i = 0; i < len; i++) {
					...
					if (g_ascii_strncasecmp(urls[i], d_aM1->addr,
							strlen(d_aM1->addr)) == 0) {
						loop = TRUE;
					...
					if (l4_address_split(urls[i], &host, &port)) {
						if (g_ascii_strncasecmp(host, dhost, strlen(dhost)) == 0
							&& (shost == NULL
								|| g_ascii_strncasecmp(host, shost,
									strlen(shost)) != 0)) {
							if (!force) {
...
_replace(struct meta0_assign_meta1_s *s_aM1, struct meta0_assign_meta1_s *d_aM1)
{
	...
	if (meta0_utils_array_replace(context->array_meta1_by_prefix, prefix,
			s_aM1->addr, d_aM1->addr)) {
...
_assign(GList * working_m1list, GSList * unref_m1list)
{
		...
		for (; unref_m1list; unref_m1list = unref_m1list->next) {
			...
			do {
				if (_is_treat_prefix(context->treat_prefixes, prefix)) {
					GRID_ERROR("prefix [%02X%02X] already treat", prefix[0],
						prefix[1]);
					error =
						g_error_new(gquark_log, 0,
						"Failed to remove Meta1 service");
				}
				d_aM1 =
					_select_dest_assign_m1(working_m1list, s_aM1, NULL, TRUE,
					FALSE);
				if (!d_aM1) {
					d_aM1 =
						_select_dest_assign_m1(working_m1list, s_aM1, NULL,
						TRUE, TRUE);
					if (!d_aM1) {
						error =
							g_error_new(gquark_log, 0,
	...
	do {
		...
		// sort meta1 list
		working_m1list =
			g_list_sort(working_m1list, meta0_assign_sort_by_score);

		// election high meta1 and prefix
		s_aM1 =
			_select_source_assign_m1(working_m1list, context->treat_prefixes,
			context->avgscore);

		if (s_aM1) {
			d_aM1 =
				_select_dest_assign_m1(working_m1list, s_aM1, NULL, FALSE,
				FALSE);
...
_init_assign(gchar * ns_name, GList ** working_m1list, GSList ** unref_m1list)
{
	...
	if (context->replica <= 0) {
		...
		error =
			g_error_new(gquark_log, EINVAL, "Invalid replica number [%d]",
			context->replica);
		goto errorLabel;
	}

	// Duplicate the current prefix distribution and build a List
	GSList *prefixByMeta1 =
		meta0_utils_array_to_list(context->array_meta1_by_prefix);
	...
	for (; m1_list; m1_list = m1_list->next) {
		...
		struct meta0_assign_meta1_s *m1ref =
			g_hash_table_lookup(context->map_meta1_ref, aM1->addr);
		...
		g_hash_table_insert(context->working_map_meta1_ref, strdup(aM1->addr),
			aM1);
	}
			...
			if (addr_info_equal(&(m0info->addr), &addr)) {
				...
				for (; p < max; p++) {
					if (_is_treat_prefix(prefix_mask, (guint8 *) p)) {
						GRID_WARN
							("prefix %02X%02X manage by two meta1 present in the request",
...
_check(GList * working_m1list)
{
	...
	if (working_m1list) {
		...
		GRID_TRACE("check delta highscore %d ,lowscore %d", highscore,
			lowscore);
		if ((highscore - lowscore) <
			(context->avgscore * trigger_assignment) / 100) {
			GRID_WARN
				("New assign not necessary, high score %d , low score %d, average %d",
	...
	if (context->lastAssignTime) {
		...
		ltime =
			g_date_time_add_minutes(context->lastAssignTime,
			period_between_two_assign);
		GRID_TRACE("currentTime :%s , last time + %d min :%s, comp :%d",
			g_date_time_format(currentTime, "%Y-%m-%d %H:%M"),
			period_between_two_assign, g_date_time_format(ltime,
				"%Y-%m-%d %H:%M"), g_date_time_compare(ltime, currentTime));
		if (g_date_time_compare(ltime, currentTime) > 0) {
			GRID_WARN
				("delay between two meta1 assign  not respected. Try later. last date [%s]",
				g_date_time_format(context->lastAssignTime, "%Y-%m-%d %H:%M"));
			error =
				g_error_new(gquark_log, 0,
				"delay between two meta1 assign  not respected. Try later.");
			return error;
		}
	}
...
_initContext(struct meta0_backend_s *m0)
{
	...
	if (error) {
		GRID_ERROR("failed to duplicate current prefix distribution :(%d) %s",
			error->code, error->message);
		return error;
	}
	...
	if (error) {
		...
		GRID_ERROR("failed to duplicate current Meta1 reference :(%d) %s",
			error->code, error->message);
		return error;
	}
	...
	context->working_map_meta1_ref =
		g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
		_gfree_map_meta0_assign_meta1);
		...
		if (v != NULL) {
			...
			if (context->replica > 65536) {
				return g_error_new(gquark_log, EINVAL,
					"Invalid nb replica [%d]", context->replica);
			}
		}
	...
	gboolean nodist)
{
	...
	while (replicas--) {
		for (idx = 0; idx < 65536; idx++) {
			working_m1list =
				g_list_sort(working_m1list, meta0_assign_sort_by_score);
			d_aM1 =
				_select_dest_assign_m1(working_m1list, NULL, (guint8 *) (&idx),
				TRUE, nodist);
			if (!d_aM1) {
				error =
					g_error_new(gquark_log, 0,
					"Failed to assign prefix %d to meta1", idx);
				goto errorLabel;
			}

			meta0_utils_array_add(context->array_meta1_by_prefix,
	...
	error =
		meta0_backend_assign(m0, context->array_meta1_by_prefix, new_meta1ref,
		TRUE);
	if (error) {
		GRID_ERROR("failed to update BDD :(%d) %s", error->code,
			error->message);
		goto errorLabel;
	}
...
GError *
meta0_assign_prefix_to_meta1(struct meta0_backend_s * m0, gchar * ns_name,
	gboolean nocheck)
{
