...
meta0_remote_get_meta1_all(addr_info_t * meta0, gint ms, GError ** err)
{
	...
	GByteArray *gba =
		message_marshall_gba_and_clean(message_create_request(NULL, NULL,
			NAME_MSGNAME_M0_GETALL, NULL, NULL));
...
meta0_remote_cache_refresh(addr_info_t * m0a, gint ms, GError ** err)
{
	...
	GByteArray *gba =
		message_marshall_gba_and_clean(message_create_request(NULL, NULL,
			NAME_MSGNAME_M0_RELOAD, NULL, NULL));
	...
	guint nbreplicas, GError ** err)
{
	...
	message_set_NAME(request, NAME_MSGNAME_M0_FILL,
		sizeof(NAME_MSGNAME_M0_FILL) - 1, NULL);
	do {
		gchar str[32];

		g_snprintf(str, sizeof(str), "%u", nbreplicas);
		message_add_field(request, "REPLICAS", sizeof("REPLICAS"), str,
			strlen(str), NULL);
	} while (0);
	do {
	...
	guint nbreplicas, gboolean nodist, GError ** err)
{
	...
	message_create(&request, NULL);
	message_set_NAME(request, NAME_MSGNAME_M0_V2_FILL,
		sizeof(NAME_MSGNAME_M0_V2_FILL) - 1, NULL);
	do {
		gchar str[32];
	...
	do {
		...
		message_add_field(request, "NODIST", sizeof("NODIST"), str, strlen(str),
			NULL);
	} while (0);
...
meta0_remote_assign(addr_info_t * m0a, gint ms, gboolean nocheck, GError ** err)
{
	...
	message_create(&request, NULL);
	message_set_NAME(request, NAME_MSGNAME_M0_ASSIGN,
		sizeof(NAME_MSGNAME_M0_ASSIGN) - 1, NULL);
	if (nocheck) {
		gchar *str = "yes";

		message_add_field(request, "NOCHECK", sizeof("NOCHECK") - 1, str,
			strlen(str), NULL);
	}
...
gint
meta0_remote_disable_meta1(addr_info_t * m0a, gint ms, gchar ** urls,
	gboolean nocheck, GError ** err)
{
	...
	message_create(&request, NULL);
	message_set_NAME(request, NAME_MSGNAME_M0_DISABLE_META1,
		sizeof(NAME_MSGNAME_M0_DISABLE_META1) - 1, NULL);
	if (nocheck) {
		gchar *str = "yes";
...
meta0_remote_get_meta1_info(addr_info_t * m0a, gint ms, GError ** err)
{
	...
	gboolean on_reply(gpointer c1, MESSAGE reply)
	{
		...
		if (0 < message_get_BODY(reply, &body, &bsize, NULL)) {
			...
			tmpResult =
				metautils_decode_lines((gchar *) body,
				((gchar *) body) + bsize);
	...
	request =
		message_create_request(NULL, NULL, NAME_MSGNAME_M0_GET_META1_INFO, NULL,
		NULL);
	addr_info_to_string(m0a, target, sizeof(target));
	packed = message_marshall_gba(request, NULL);
	...
	GError ** err)
{
	...
	message_create(&request, NULL);
	message_set_NAME(request, NAME_MSGNAME_M0_DESTROY_META1REF,
		sizeof(NAME_MSGNAME_M0_DESTROY_META1REF) - 1, NULL);

	message_add_field(request, "METAURL", sizeof("METAURL"), urls, strlen(urls),
		NULL);
	...
	GError ** err)
{
	...
	message_create(&request, NULL);
	message_set_NAME(request, NAME_MSGNAME_M0_DESTROY_META0ZKNODE,
		sizeof(NAME_MSGNAME_M0_DESTROY_META0ZKNODE) - 1, NULL);
