...
doCmd(gchar * cmd, GError ** err)
{
	if (0 == strcasecmp("CREATE", cmd)) {
		if (!meta2_remote_container_create(&meta2addr, timeOut, err, id,
				"test de nom de conteneur"))
			goto errorLabel;
	}
		...
		if (!(list =
				meta2_remote_content_add(&meta2addr, timeOut, err, id,
					remotePath, remoteSize, NULL, NULL)))
			goto errorLabel;
		...
		if (!meta2_remote_content_commit(&meta2addr, timeOut, err, id,
				remotePath))
			goto errorLabel;
	}

	else if (0 == strcasecmp("ROLLBACK", cmd)) {
		if (!meta2_remote_content_rollback(&meta2addr, timeOut, err, id,
				remotePath))
			goto errorLabel;
	}
		...
		if (!(list =
				meta2_remote_content_retrieve(&meta2addr, timeOut, err, id,
					remotePath)))
			goto errorLabel;
		for (l = list; l; l = l->next) {
			static gchar printBuf[4096];
		...
		if (!meta2_remote_container_set_flag(&meta2addr, timeOut, err, id,
				REMOTECONTAINER_FLAG_FROZEN))
			goto errorLabel;
	}

	else if (0 == strcasecmp("UNAVAILABLE", cmd)) {
		if (!meta2_remote_container_set_flag(&meta2addr, timeOut, err, id,
				REMOTECONTAINER_FLAG_DISABLED))
			goto errorLabel;
	}

	else if (0 == strcasecmp("ENABLE", cmd)) {
		if (!meta2_remote_container_set_flag(&meta2addr, timeOut, err, id,
				REMOTECONTAINER_FLAG_OK))
			goto errorLabel;
	}
		...
		for (l = list; l; l = l->next) {
			...
			path_info_to_string((path_info_t *) l->data, printBuf,
				sizeof(printBuf));
			g_print("> %s\n", printBuf);
		}
		g_slist_foreach(list, g_free1, NULL);
		...
		if (!meta2_remote_content_remove(&meta2addr, timeOut, err, id,
				remotePath))
			goto errorLabel;
	}

	else if (0 == strcasecmp("REPAIR", cmd)) {
		if (!meta2raw_remote_mark_container_repaired(&meta2addr, timeOut, err,
				id))
			goto errorLabel;
	}
		...
		g_print("META2 information: NS=[%s] CONFIGURATION=[%s]\n",
			(char *) g_hash_table_lookup(ht, "NS"),
			(char *) g_hash_table_lookup(ht, "CFG"));
...
main(int argc, char **args)
{
	...
	DEBUG("size=%" G_GINT64_FORMAT "/%" G_GUINT64_FORMAT, remoteSize,
		(guint64) remoteSize);
