...
static gint
manage_contents(GError ** err, gpointer udata, gint code, guint8 * body,
	gsize bodySize)
{
...
static gint
manage_properties(GError ** err, gpointer udata, gint code, guint8 * body,
	gsize bodySize)
{
	struct dump_ctx_s *ctx = udata;
...
static gint
manage_admin(GError ** err, gpointer udata, gint code, guint8 * body,
	gsize bodySize)
{
	struct dump_ctx_s *ctx = udata;
	GSList *l, *result = NULL;
...
static gint
manage_events(GError ** err, gpointer udata, gint code, guint8 * body,
	gsize bodySize)
{
	struct dump_ctx_s *ctx = udata;
	GSList *l, *result = NULL;
...
status_t
meta2_remote_dumpv1_container(struct metacnx_ctx_s * ctx,
	...
	struct meta2_dumpv1_hooks_remote_s * hooks, gpointer u, GError ** err)
{
	...
	if (!ctx || !container_id) {
		GSETERROR(err, "Invalid parameter (container_id=%p)",
			(void *) container_id);
		goto error_check;
	}

	request =
		meta2_remote_build_request(err, ctx->id, "REQ_M2RAW_DUMP_CONTAINER");
	if (!request) {
