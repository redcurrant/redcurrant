...
status_t
meta2_remote_restorev1_container(struct metacnx_ctx_s * dst_cnx,
	...
	const container_id_t src_cid, GError ** err)
{
	...
	if (!dst_cnx || !src_addr || !dst_cid || !src_cid) {
		GSETCODE(err, EINVAL, "Invalid parameter (%p %p %p %p)", dst_cnx,
			dst_cid, src_addr, src_cid);
		return 0;
	}

	/*prepare the request, fill all the fields */
	request =
		meta2_remote_build_request(err, dst_cnx->id,
		"REQ_M2RAW_RESTORE_CONTAINER");
	if (!request) {
		GSETERROR(err, "message error");
		goto error_check;
	}
	if (!message_add_field(request, STATIC_STRLEN("DST_CID"),
			(guint8 *) dst_cid, sizeof(container_id_t), err)) {
		GSETERROR(err, "message error");
		goto error_label;
	}
	if (!message_add_field(request, STATIC_STRLEN("SRC_CID"),
			(guint8 *) src_cid, sizeof(container_id_t), err)) {
		GSETERROR(err, "message error");
		goto error_label;
	}
	if (!_add_singleton_address(request, STATIC_STRLEN("SRC_ADDR"), src_addr,
			err)) {
		GSETERROR(err, "message error");
		goto error_label;
	}
