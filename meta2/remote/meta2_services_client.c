...
test_flush(void)
{
	...
	error_local = NULL;
	if (!meta2_remote_container_open(&(cnx.addr), cnx.timeout.req, &error_local,
			cid)) {
	...
	services_used = NULL;
	if (!meta2_remote_service_flush(&cnx, cid, service_name, &services_used,
			&error_local)) {
	...
	remaining_services =
		meta2_remote_service_get_all_used(&cnx, cid, service_name,
		&error_local);
	if (error_local) {
...
test_add_service(int argc, char **args, int validation)
{
	...
	si = meta2_remote_service_add_contents(&cnx, cid, service_name, paths,
		&err);
	...
	if (validation) {
		...
		/* COMMIT */
		rc0 =
			meta2_remote_service_commit_contents(&cnx, cid, service_name, paths,
			&commit_failed, &err);
		...
		if (rc0) {
			...
			e = NULL;
			rc_local =
				meta2_remote_service_commit_contents(&cnx, cid, service_name,
				paths, &commit_failed, &e);
		...
		/* DELETE */
		rc1 =
			meta2_remote_service_delete_contents(&cnx, cid, service_name, paths,
			&really_removed, &services_used, &err);
		...
		/* ROLLBACK */
		rc0 =
			meta2_remote_service_rollback_contents(&cnx, cid, service_name,
			paths, &commit_failed, &err);
		_display_strlist("\tRollback wanted: ", paths);
		_display_strlist("\tRollback failed: ", commit_failed);
		_gfree_list(&commit_failed);
		g_printerr("RESULT : %s\r\n", rc0 ? "ok" : gerror_get_message(err));
		...
		if (rc0) {
			...
			_display_strlist("\tSecond Rollback failed: ", commit_failed);
			_gfree_list(&commit_failed);
...
init_static_vars(int argc, char **args)
{
	...
	if (argc < 4) {
		g_printerr
			("Failed to init the address and container_id : 4 tokens expected (IP, PORT, CONTAINER_ID, SERVICE_NAME)\r\n");
		abort();
	}
